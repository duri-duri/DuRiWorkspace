...
# 기존 시스템들 import
from .result_improver import ResultImprover, ImprovementAction
...
class UnifiedImprovementSystem:
    ...
    def execute_comprehensive_improvement(self, 
                                        conversation_context: Dict[str, Any],
                                        categories: ImprovementCategory = None) -> UnifiedImprovementResult:
        """포괄적 개선 실행"""
        ...
        try:
            ...
            overall_score = self._calculate_overall_improvement_score(
                conversation_improvements, system_improvements, 
                learning_improvements, evolution_improvements
            )
    ...
    def _execute_conversation_improvement(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not actions:
                return {"status": "no_actions", "message": "실행 가능한 개선 액션이 없습니다"}
    ...
    def _execute_system_improvement(self) -> Dict[str, Any]:
        ...
        try:
            if not self.system_improver:
                return {"status": "unavailable", "message": "시스템 개선기가 사용할 수 없습니다"}
    ...
    def _execute_learning_improvement(self) -> Dict[str, Any]:
        ...
        try:
            if not self.learning_improver:
                return {"status": "unavailable", "message": "학습 개선기가 사용할 수 없습니다"}
    ...
    def _execute_evolution_improvement(self) -> Dict[str, Any]:
        ...
        try:
            if not self.evolution_improver:
                return {"status": "unavailable", "message": "진화 개선기가 사용할 수 없습니다"}
    ...
    def _calculate_overall_improvement_score(self, 
                                           conversation_improvements: Dict[str, Any],
                                           system_improvements: Dict[str, Any],
                                           learning_improvements: Dict[str, Any],
                                           evolution_improvements: Dict[str, Any]) -> float:
        """전체 개선 점수 계산"""
        try:
            scores = []
            weights = []
            ...
            if conversation_improvements.get("status") == "success":
                conv_score = conversation_improvements.get("improvement_result", {}).get("summary", {}).get("overall_success_rate", 0.0)
    ...
    def _generate_improvement_summary(self,
                                    ...
                                    system_improvements: Dict[str, Any],
                                    learning_improvements: Dict[str, Any],
                                    evolution_improvements: Dict[str, Any]) -> Dict[str, Any]:
        """개선 요약 생성"""
        try:
            summary = {
            ...
            # 개선 수량 계산
            if conversation_improvements.get("status") == "success":
                summary["total_improvements"] += conversation_improvements.get("improvement_result", {}).get("summary", {}).get("total_count", 0)
                summary["successful_improvements"] += conversation_improvements.get("improvement_result", {}).get("summary", {}).get("success_count", 0)
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": sum(1 for result in self.improvement_history if result.overall_score > 0.5) / total_executions
        }
