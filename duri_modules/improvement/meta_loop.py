...
class MetaLoopSystem:
    ...
    def _analyze_improvement_effect(
        self, original_eval: Dict[str, Any], new_eval: Dict[str, Any]
    ) -> Dict[str, Any]:
        """개선 효과 분석"""
    ...
    def generate_improvement_feedback(self, meta_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if analysis.get("improvement_success", False):
            feedback["recommendations"].append(
                "개선이 성공적으로 이루어졌습니다. 이 패턴을 학습에 반영하세요."
            )
        ...
        if improved_dims:
            feedback["recommendations"].append(
                f"다음 영역에서 개선이 확인되었습니다: {', '.join(improved_dims)}"
            )
        ...
        if declined_dims:
            feedback["recommendations"].append(
                f"다음 영역에서 악화가 확인되었습니다: {', '.join(declined_dims)}"
            )
