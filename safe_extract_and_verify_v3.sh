...
FS_POSIX=1
case "$FSTYPE" in v9fs|fuseblk|ntfs|msdos) FS_POSIX=0 ;; esac
...
| sed 's#^\./##' | grep -v '/$' | LC_ALL=C sort > "$ORIGINAL_LIST"
...
# 6) 전수 콘텐츠 검증(일반파일: 해시, symlink: 타깃)
...
while IFS= read -r e; do
  p="$DEST/${e#./}"
  if [[ -L "$p" ]]; then LINKTARGETS["$e"]="$(readlink "$p" 2>/dev/null || echo)"; fi
done < "$EXTRACTED_LIST"
...
arc_link_target() { # $1:path → link target
  case "$ARC" in
...
while IFS= read -r p0; do
  ...
  if [[ "$t" == "l" || -n "${LINKTARGETS["${p#./}"]:-}" ]]; then
    arc_tgt="$(arc_link_target "$p")"; [[ -z "$arc_tgt" && "$p" == ./* ]] && arc_tgt="$(arc_link_target "${p#./}")"
    fs_tgt="${LINKTARGETS["${p#./}"]:-}"
