...
# Day 6: 학습 피드백 시스템
from learning_feedback_system import JudgmentMemorySystem, SelfImprovementSystem, AdaptiveLearningEngine
...
class IntegratedDuRiSystem:
    ...
    async def process_complex_situation(self, situation: str, action: str) -> Dict[str, Any]:
        ...
        print(f"  • 종합 품질: {quality_metrics.overall_quality if hasattr(quality_metrics, 'overall_quality') else 0:.2f}")
        print(f"  • 진위성 수준: {authenticity_check.authenticity_level.value if hasattr(authenticity_check, 'authenticity_level') else 'unknown'}")
