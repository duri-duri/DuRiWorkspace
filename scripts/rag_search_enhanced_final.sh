...
if [[ -z "$QUERY" ]]; then
  echo "사용법: bash scripts/rag_search_enhanced_final.sh '<검색어>' [카테고리] [patient_facing:true|false]" >&2
...
echo "🔍 RAG 검색(고급): '$QUERY'"
[[ -n "$CAT" ]] && echo "   카테고리: $CAT"
[[ -n "$PF"  ]] && echo "   환자용: $PF"
[[ "$RANK" == "1" ]] && echo "   랭킹 ON (가중치 점수)"
[[ "$MODE" == "regex" ]] && echo "   모드: 정규식"
echo "   결과 수: 최대 $K개"
echo
...
RESULTS_OUTPUT="$(if [[ -s "$TMP" ]]; then
  echo "✅ 검색 완료, 결과 ${K}개 미리보기:"
else
  echo "❌ 검색 결과 없음"
fi)"
