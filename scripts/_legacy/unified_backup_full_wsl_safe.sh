...
# ---- state.json 동기화(선택: 활성화 권장) ----
if [ -x "/home/duri/DuRiWorkspace/tools/generate_or_update_state.py" ]; then
  python3 /home/duri/DuRiWorkspace/tools/generate_or_update_state.py \
    --workspace-root "/home/duri/DuRiWorkspace" \
    --state "/home/duri/DuRiWorkspace/learning_journal/state.json" \
    --backup-root "$DEST_ROOT" \
    --backup-archive "$OUT" \
