...
# --- [패치7] 백업 설정 프리플라이트 검증 (재발 방지) ---
echo "=== 백업 설정 프리플라이트 검증 ==="
if [[ -f "backup_include_full.txt" ]]; then
  grep -qE '(^|\s)\.env' backup_include_full.txt || { echo "❌ [FAIL] include에 .env 누락"; exit 2; }
  grep -qE '(^|/)\.github/?' backup_include_full.txt || { echo "❌ [FAIL] include에 .github 누락"; exit 2; }
  grep -qE '(^|\s)\*\.md' backup_include_full.txt || { echo "❌ [FAIL] include에 *.md 누락"; exit 2; }
  echo "✅ [OK] include 설정 검증 통과"
else
  echo "⚠️  [WARN] backup_include_full.txt 파일 없음 - 기본 설정 사용"
fi

if [[ -f "backup_exclude.txt" ]]; then
  grep -qE '\.git/?' backup_exclude.txt || { echo "❌ [FAIL] exclude에 .git 누락"; exit 2; }
  echo "✅ [OK] exclude 설정 검증 통과"
else
  echo "⚠️  [WARN] backup_exclude.txt 파일 없음 - 기본 설정 사용"
fi
echo "=== 프리플라이트 검증 완료 ==="
