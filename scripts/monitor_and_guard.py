#!/usr/bin/env python3
import os, time, json, re, subprocess as sp
...
def grep_errors():
    ...
    bad = []
    for f in files:
        rc2, out2, _ = sh(fr"grep -Ein 'error|fail|traceback' {f} || true")
        if out2.strip():
...
def append_csv(row):
    ...
    with LOGCSV.open("a", encoding="utf-8") as f:
        ...
        f.write(",".join(str(row.get(k,"")) for k in ["ts","rollout","adv","gen","math","err_count","alerts"])+"\n")
...
def main():
    ...
    for i in range(ITER):
        ...
        # 콘솔 피드백
        print(f"[{i+1}/{ITER}] ts={ts} rollout={rollout} adv={bvals.get('advanced')} gen={bvals.get('general')} math={bvals.get('math')} err={len(errs)}")
        if alerts:
