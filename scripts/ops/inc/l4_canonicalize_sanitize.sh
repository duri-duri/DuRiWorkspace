...
if [[ -f "${WORK}/scripts/ops/inc/with_lock.sh" ]]; then
  bash "${WORK}/scripts/ops/inc/with_lock.sh" "$LOCK" bash -c "
    ...
    SNAP=\$(mktemp)
    cp -f -- \"${IN}\" \"\$SNAP\" 2>/dev/null || {
      echo '[WARN] Failed to snapshot input'
      rm -f \"\$SNAP\"
      exit 0
    }
    
    # 불완전 라인/널바이트 제거 + 마지막 줄 개행 보장
    SANITIZED=\$(mktemp)
    tr -d '\\000' < \"\$SNAP\" | awk 'NF{print} END{if(NR>0)print \"\"}' > \"\$SANITIZED\" 2>/dev/null || cp \"\$SNAP\" \"\$SANITIZED\"
    ...
    ' \"\$SANITIZED\" > \"${SAN}\" 2>/dev/null; then
      total=\$(wc -l < \"\$SANITIZED\" || echo 0)
  ...
  if [[ -f "${IN}" ]]; then
    SNAP=$(mktemp)
    cp -f -- "${IN}" "$SNAP" 2>/dev/null || true
    tr -d '\000' < "$SNAP" 2>/dev/null | awk 'NF{print} END{if(NR>0)print ""}' > "${SAN}" 2>/dev/null || cp "${IN}" "${SAN}"
    rm -f "$SNAP"
    chmod 0644 "${SAN}" 2>/dev/null || true
  fi
fi
