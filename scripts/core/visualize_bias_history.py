...
def setup_font_and_titles():
    try:
        plt.rcParams['font.family'] = ['DejaVu Sans', 'NanumGothic', 'Malgun Gothic', 'AppleGothic', 'sans-serif']
...
def is_interactive_backend():
    ...
    return plt.get_backend().lower() not in ["agg", "module://matplotlib_inline.backend_inline"]
...
class BiasHistoryVisualizer:
    ...
    def _create_severity_timeline(self, ax):
        ...
        ax.legend(by_label.values(), by_label.keys(), 
                 title=TITLES['severity'], loc='upper right')
    ...
    def _create_bias_type_pie(self, save_dir: str, show_plot: bool):
        ...
        wedges, texts, autotexts = ax.pie(bias_type_counts.values, 
    ...
    def print_summary_statistics(self):
        ...
        if KOREAN_FONT_AVAILABLE:
            ...
            # 가장 빈번한 감정-행동 조합
            print(f"\n가장 빈번한 감정-행동 조합:")
            emotion_action_stats = self.df.groupby(['emotion', 'action']).size().sort_values(ascending=False)
...
def create_sample_bias_history():
    ...
    bias_types = ['emotion_bias', 'action_bias', 'temporal_bias', 'intensity_bias', 'pattern_bias', 'frequency_bias']
...
def main():
    ...
    parser.add_argument('--file', '-f', default='bias_history.json',
