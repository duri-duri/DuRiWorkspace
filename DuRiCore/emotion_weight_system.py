...
class EmotionWeightSystem:
    ...
    async def update_emotion_state(self, emotion_type: EmotionType, intensity: float, 
    ...
    def _determine_secondary_emotions(self, primary_emotion: EmotionType, 
                                    context: Dict[str, Any]) -> List[EmotionType]:
        """보조 감정 결정"""
        secondary_emotions = []
    ...
    async def apply_emotion_to_judgment(self, original_judgment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.performance_metrics['average_emotion_influence'] = (
                sum(d.emotion_influence for d in self.decision_history) / 
                len(self.decision_history)
            )
            
            logger.info(f"🧠 감정 가중치 적용: {self.current_emotion_state.primary_emotion.value} "
    ...
    def _adjust_confidence(self, original_confidence: float, emotion_weight: EmotionWeight) -> float:
        """신뢰도 조정"""
    ...
    async def apply_emotion_to_action(self, original_action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            adjusted_aggressiveness = self._adjust_aggressiveness(original_aggressiveness, emotion_weight)
    ...
    def _adjust_aggressiveness(self, original_aggressiveness: str, emotion_weight: EmotionWeight) -> str:
        """공격성 조정"""
        ...
        try:
            ...
            return {
                'emotion_vector': {
                    ...
                    'intensity': self.current_emotion_state.intensity,
                    'secondary_emotions': [e.value for e in self.current_emotion_state.secondary_emotions],
                    'decision_bias': self.current_emotion_state.decision_bias.value,
                    'emotional_stability': self.current_emotion_state.emotional_stability
                },
    ...
    def _extract_emotion_from_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 긍정적 감정 키워드
        positive_keywords = ['success', 'achievement', 'opportunity', 'growth', 'improvement', 'innovation']
        ...
        # 부정적 감정 키워드
        negative_keywords = ['problem', 'risk', 'failure', 'threat', 'conflict', 'difficulty']
        ...
        # 창의적 상황
        creative_keywords = ['creative', 'innovation', 'new', 'breakthrough', 'invention']
...
async def test_emotion_weight_system():
    ...
    test_judgments = [
        ...
        {"decision": "proceed", "confidence": 0.8, "reasoning": "이미 충분한 정보가 있음"},
    ...
    logger.info(f"   평균 감정 영향: {analysis['performance_metrics']['average_emotion_influence']:.3f}")
