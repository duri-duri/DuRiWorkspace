...
class CapacityGovernance:
    ...
    def complete_work_item(self, work_item_id: str, actual_workload: int, loc_change: int=0, file_change: int=0) -> bool:
        """작업 항목 완료"""
        if work_item_id not in self.work_items:
            logger.error(f'작업 항목을 찾을 수 없습니다: {work_item_id}')
            return False
    ...
    def _calculate_priority_score(self, work_item: WorkItem) -> PriorityScore:
        ...
        priority_score = PriorityScore(work_item_id=work_item.id, base_score=base_score, slo_weight=slo_weight, dependency_weight=dependency_weight, final_score=final_score, calculated_at=datetime.now())
    ...
    def _update_average_completion_time(self, completion_time: float):
        ...
        if self.metrics.total_completed_items == 1:
            ...
            alpha = 0.1
            self.metrics.average_completion_time = alpha * completion_time + (1 - alpha) * self.metrics.average_completion_time
    ...
    def check_capacity_limits(self) -> Dict[str, bool]:
        """용량 제한 확인"""
        return {'daily_loc': self.metrics.daily_loc_change <= self.daily_loc_limit, 'weekly_loc': self.metrics.weekly_loc_change <= self.weekly_loc_limit, 'daily_file': self.metrics.daily_file_change <= self.daily_file_limit, 'weekly_file': self.metrics.weekly_file_change <= self.weekly_file_limit, 'wip': self.metrics.current_wip <= self.current_wip_limit, 'can_add_work': self.metrics.current_wip < self.current_wip_limit}
    ...
    def get_priority_queue(self) -> List[Tuple[str, float]]:
        ...
        pending_items = [(work_item_id, score.final_score) for (work_item_id, score) in latest_scores.items() if self.work_items[work_item_id].status == 'pending']
        return sorted(pending_items, key=lambda x: x[1], reverse=True)

    def get_capacity_report(self) -> Dict[str, Any]:
        """용량 보고서 생성"""
        report = {'current_status': {'workload_level': self.get_workload_level().value, 'current_wip': self.metrics.current_wip, 'wip_limit': self.current_wip_limit, 'capacity_utilization': self.metrics.capacity_utilization}, 'daily_metrics': {'loc_change': self.metrics.daily_loc_change, 'loc_limit': self.daily_loc_limit, 'file_change': self.metrics.daily_file_change, 'file_limit': self.daily_file_limit}, 'weekly_metrics': {'loc_change': self.metrics.weekly_loc_change, 'loc_limit': self.weekly_loc_limit, 'file_change': self.metrics.weekly_file_change, 'file_limit': self.weekly_file_limit}, 'capacity_limits': self.check_capacity_limits(), 'work_items_summary': {'total': len(self.work_items), 'pending': len([w for w in self.work_items.values() if w.status == 'pending']), 'in_progress': len([w for w in self.work_items.values() if w.status == 'in_progress']), 'completed': len([w for w in self.work_items.values() if w.status == 'completed']), 'blocked': len([w for w in self.work_items.values() if w.status == 'blocked'])}, 'performance_metrics': {'total_completed': self.metrics.total_completed_items, 'average_completion_time': self.metrics.average_completion_time, 'uptime_days': (datetime.now() - self.start_date).days}}
    ...
    def export_work_items(self) -> List[Dict[str, Any]]:
        """작업 항목 내보내기"""
        return [{'id': item.id, 'name': item.name, 'description': item.description, 'priority_level': item.priority_level.value, 'status': item.status, 'estimated_workload': item.estimated_workload, 'risk_score': item.risk_score, 'change_impact': item.change_impact, 'dependencies': item.dependencies, 'blockers': item.blockers, 'created_at': item.created_at.isoformat(), 'started_at': item.started_at.isoformat() if item.started_at else None, 'completed_at': item.completed_at.isoformat() if item.completed_at else None, 'actual_workload': item.actual_workload, 'metadata': item.metadata} for item in self.work_items.values()]
