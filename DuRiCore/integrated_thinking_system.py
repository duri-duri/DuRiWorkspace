...
try:
    ...
    from intuitive_thinking_system import IntuitiveThinkingSystem, IntuitivePattern
    from creative_thinking_system import CreativeThinkingSystem, CreativeIdea, CreativityLevel
...
class IntegratedThinkingSystem:
    ...
    def __init__(self):
        ...
        # DuRiThoughtFlow 초기화
        default_input_data = {"goal": "integrated_thinking", "context": "thinking_system"}
    ...
    async def think_integrated(self, context: Dict[str, Any] = None) -> IntegratedThinkingResult:
        ...
        try:
            ...
            # 4. 조화로운 사고 패턴
            harmonious_patterns = await self._discover_harmonious_patterns(integrated_thoughts, system_harmonies, context)
            
            # 5. 결과 종합
            result = await self._compile_integrated_result(
                process_id, integrated_thoughts, system_harmonies, judgments, harmonious_patterns, start_time
            )
    ...
    async def _get_system_combinations_for_mode(self, mode: IntegrationMode) -> List[List[ThinkingSystemType]]:
        """통합 모드별 시스템 조합 생성"""
        ...
        combinations = {
            ...
            IntegrationMode.PARALLEL: [
                [ThinkingSystemType.INNER, ThinkingSystemType.EMOTIONAL, ThinkingSystemType.INTUITIVE],
                [ThinkingSystemType.CREATIVE, ThinkingSystemType.META_COGNITIVE, ThinkingSystemType.SELF_DIRECTED_LEARNING]
            ],
            IntegrationMode.HARMONIOUS: [
                [ThinkingSystemType.INNER, ThinkingSystemType.EMOTIONAL, ThinkingSystemType.INTUITIVE, ThinkingSystemType.CREATIVE],
                [ThinkingSystemType.META_COGNITIVE, ThinkingSystemType.SELF_DIRECTED_LEARNING, ThinkingSystemType.CREATIVE_PROBLEM_SOLVING]
            ],
            IntegrationMode.ADAPTIVE: [
                [ThinkingSystemType.INNER, ThinkingSystemType.INTUITIVE],
                [ThinkingSystemType.EMOTIONAL, ThinkingSystemType.CREATIVE],
                [ThinkingSystemType.META_COGNITIVE, ThinkingSystemType.SELF_DIRECTED_LEARNING]
            ],
            IntegrationMode.SYNTHETIC: [
                all_systems  # 모든 시스템 통합
            ]
        }
        
        return combinations.get(mode, [all_systems])
    
    async def _create_integrated_thought(self, systems: List[ThinkingSystemType], 
                                       mode: IntegrationMode, 
                                       context: Dict[str, Any]) -> IntegratedThought:
        """통합 사고 생성"""
        # 시스템별 사고 내용 생성
        system_thoughts = []
    ...
    async def _generate_system_thought(self, system: ThinkingSystemType, context: Dict[str, Any]) -> str:
        """시스템별 사고 내용 생성"""
        thought_templates = {
    ...
    async def _calculate_harmony_level(self, systems: List[ThinkingSystemType], mode: IntegrationMode) -> ThinkingHarmony:
        """조화 수준 계산"""
        # 시스템 간 호환성 점수 계산
    ...
    async def _calculate_confidence_score(self, systems: List[ThinkingSystemType], mode: IntegrationMode) -> float:
        """신뢰도 계산"""
        # 시스템별 기본 신뢰도
        system_confidences = {
    ...
    async def _calculate_pair_harmony(self, system1: ThinkingSystemType, system2: ThinkingSystemType) -> float:
        """시스템 쌍 조화 점수 계산"""
        harmony_matrix = {
            (ThinkingSystemType.INNER, ThinkingSystemType.EMOTIONAL): 0.8,
            (ThinkingSystemType.INNER, ThinkingSystemType.INTUITIVE): 0.7,
    ...
    async def _identify_interaction_pattern(self, system1: ThinkingSystemType, system2: ThinkingSystemType) -> str:
        """상호작용 패턴 식별"""
        pattern_templates = {
            (ThinkingSystemType.INNER, ThinkingSystemType.EMOTIONAL): "내적-감정적 조화",
            (ThinkingSystemType.INNER, ThinkingSystemType.INTUITIVE): "내적-직관적 조화",
            (ThinkingSystemType.EMOTIONAL, ThinkingSystemType.INTUITIVE): "감정적-직관적 조화",
            (ThinkingSystemType.CREATIVE, ThinkingSystemType.META_COGNITIVE): "창의적-메타인식적 조화",
            (ThinkingSystemType.INTUITIVE, ThinkingSystemType.CREATIVE): "직관적-창의적 조화",
            (ThinkingSystemType.META_COGNITIVE, ThinkingSystemType.SELF_DIRECTED_LEARNING): "메타인식적-자발적학습 조화"
        }
    ...
    async def _calculate_synergy_level(self, system1: ThinkingSystemType, system2: ThinkingSystemType) -> float:
        """시너지 수준 계산"""
        synergy_matrix = {
            (ThinkingSystemType.INNER, ThinkingSystemType.EMOTIONAL): 0.8,
            (ThinkingSystemType.INNER, ThinkingSystemType.INTUITIVE): 0.7,
    ...
    async def _make_integrated_judgments(self, integrated_thoughts: List[IntegratedThought], 
                                       context: Dict[str, Any]) -> List[IntegratedJudgment]:
        """통합적 판단 능력"""
        judgments = []
        ...
        for question in judgment_questions:
            judgment = await self._create_integrated_judgment(question, integrated_thoughts, context)
            judgments.append(judgment)
        
        logger.info(f"통합적 판단 {len(judgments)}개 생성 완료")
        return judgments
    
    async def _create_integrated_judgment(self, question: str, 
                                        integrated_thoughts: List[IntegratedThought], 
                                        context: Dict[str, Any]) -> IntegratedJudgment:
        """통합적 판단 생성"""
        # 관련 통합 사고 선택
        ...
        # 신뢰도 계산
        confidence = sum(thought.confidence_score for thought in relevant_thoughts) / len(relevant_thoughts)
    ...
    async def _generate_final_decision(self, question: str, thoughts: List[IntegratedThought]) -> str:
        """최종 결정 생성"""
        decision_templates = [
    ...
    async def _generate_reasoning(self, question: str, thoughts: List[IntegratedThought]) -> str:
        ...
        for thought in thoughts:
            reasoning_parts.append(f"{', '.join([s.value for s in thought.thinking_systems])} 시스템을 통한 {thought.content}")
        
        return " | ".join(reasoning_parts)
    
    async def _discover_harmonious_patterns(self, integrated_thoughts: List[IntegratedThought],
                                         system_harmonies: List[SystemHarmony],
                                         context: Dict[str, Any]) -> List[HarmoniousPattern]:
        """조화로운 사고 패턴 발견"""
        patterns = []
        
        # 조화 수준이 높은 패턴 발견
        harmonious_thoughts = [t for t in integrated_thoughts if t.harmony_level in [ThinkingHarmony.HARMONIOUS, ThinkingHarmony.SYMPHONIC]]
    ...
    async def _compile_integrated_result(self, process_id: str,
                                       integrated_thoughts: List[IntegratedThought],
                                       system_harmonies: List[SystemHarmony],
                                       judgments: List[IntegratedJudgment],
                                       harmonious_patterns: List[HarmoniousPattern],
                                       start_time: float) -> IntegratedThinkingResult:
        """통합 결과 종합"""
        thinking_duration = time.time() - start_time
        
        # 평균 조화 점수 계산
        if integrated_thoughts:
            average_harmony_score = sum(t.confidence_score for t in integrated_thoughts) / len(integrated_thoughts)
        else:
            average_harmony_score = 0.0
    ...
    async def _learn_from_integrated_thinking(self, result: IntegratedThinkingResult):
        ...
        self.average_harmony_score = (self.average_harmony_score * (self.total_integrated_thoughts - len(result.integrated_thoughts)) + 
                                     sum(t.confidence_score for t in result.integrated_thoughts)) / self.total_integrated_thoughts
        self.average_confidence = (self.average_confidence * (len(self.judgments) - len(result.judgments)) + 
                                  sum(j.confidence for j in result.judgments)) / len(self.judgments)
...
async def test_integrated_thinking_system():
    ...
    for thought in result.integrated_thoughts[:3]:  # 처음 3개만 출력
        ...
        print(f"- {thought.integration_mode.value}: {systems_str} (조화: {thought.harmony_level.value}, 신뢰도: {thought.confidence_score:.2f})")
