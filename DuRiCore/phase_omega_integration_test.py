...
class PhaseOmegaIntegrationTest:
    ...
    async def _test_phase_omega_integration(self) -> List[TestResult]:
        ...
        try:
            ...
            success = (
                result is not None
                and result.success
                and result.survival_status is not None
            ...
            test_result = TestResult(
                ...
                metrics={
                    "survival_probability": (
                        result.survival_status.survival_probability if result and result.survival_status else 0.0
                    ),
            ...
            if result and result.survival_status:
                survival_working = (
                    result.survival_status.survival_probability > 0
                    and result.survival_status.survival_probability <= 1
                    and result.survival_status.status
                    in [
                ...
                test_result = TestResult(
                    ...
                    metrics={
                        "survival_status": (
                            result.survival_status.status.value if result.survival_status else "unknown"
                        )
                    },
                )
                test_results.append(test_result)
            ...
            if result and result.self_goals:
                goals_working = all(
                    goal.goal_id and goal.title and goal.description and goal.priority for goal in result.self_goals
                )
    ...
    async def _test_response_times(self) -> List[TestResult]:
        ...
        try:
            ...
            # 목표 생성기 응답 시간
            start_time = time.time()
            current_state = await self.goal_generator.analyze_current_state()  # noqa: F841
            response_time = time.time() - start_time
        ...
        try:
            ...
            test_result = TestResult(
                ...
                metrics={
                    ...
                    "avg_time_per_iteration": execution_time / iteration_count,
                },
            )
            test_results.append(test_result)
...
async def main():
    ...
    print("💾 테스트 보고서가 'phase_omega_test_report.json'에 저장되었습니다")
