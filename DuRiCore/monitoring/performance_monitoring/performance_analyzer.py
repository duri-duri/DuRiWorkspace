...
class PerformanceAnalyzer:
    ...
    async def generate_optimization_suggestions(self, metrics: List[Any], metric_name: str) -> List[OptimizationSuggestion]:
        """최적화 제안 생성"""
        try:
            suggestions = []
    ...
    async def _detect_periodic_pattern(self, values: List[float], timestamps: List[datetime], metric_name: str) -> Optional[PerformancePattern]:
        """주기성 패턴 감지"""
        try:
            if len(values) < 20:
                return None
    ...
    async def _detect_seasonal_pattern(self, values: List[float], timestamps: List[datetime], metric_name: str) -> Optional[PerformancePattern]:
        """계절성 패턴 감지"""
        try:
            if len(values) < 24:
                return None
            ...
            # 시간대별 변동성 계산
            hourly_std = {hour: statistics.stdev(vals) if len(vals) > 1 else 0 
    ...
    async def _detect_anomaly_pattern(self, values: List[float], timestamps: List[datetime], metric_name: str) -> Optional[PerformancePattern]:
        """이상 패턴 감지"""
        try:
            if len(values) < 10:
                return None
    ...
    async def get_analysis_report(self, metric_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 최근 예측
            recent_predictions = [pred for pred in self.predictions if not metric_name or pred.metric_name == metric_name]
            report["recent_predictions"] = [self._prediction_to_dict(pred) for pred in recent_predictions[-5:]]
            
            # 최근 제안
            recent_suggestions = [s for s in self.optimization_suggestions if not metric_name or metric_name in s.affected_metrics]
