...
class AdvancedLearningSystemTester:
    ...
    async def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        comprehensive_result = {
            ...
            'overall_success_rate': self._calculate_overall_success_rate([
                basic_test_result, continuous_learning_result, knowledge_evolution_result,
                efficiency_optimization_result, knowledge_integration_result, integrated_system_result
            ])
        }
    ...
    async def _test_continuous_learning_engine(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ 지속적 학습 엔진 테스트 성공 (지식: {len(session.knowledge_gained)}개, 통찰: {len(session.insights_discovered)}개)")
            return result
    ...
    async def _test_performance(self) -> Dict[str, Any]:
        ...
        try:
            # 성능 테스트를 위한 반복 실행
            test_contexts = [
                {'type': 'cognitive', 'content': '인지적 학습 테스트', 'difficulty': 0.5},
                {'type': 'emotional', 'content': '감정적 학습 테스트', 'difficulty': 0.6},
                {'type': 'creative', 'content': '창의적 학습 테스트', 'difficulty': 0.7}
            ]
            ...
            logger.info(f"✅ 성능 테스트 성공 (평균 실행시간: {total_time/len(test_contexts):.2f}초, 평균 점수: {average_score:.2f})")
...
async def main():
    ...
    for test_name, test_result in results.items():
        if test_name not in ['test_timestamp', 'total_test_time', 'overall_success_rate']:
