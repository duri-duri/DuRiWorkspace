...
from genetic_evolution_engine import (
    EvolutionConfig,
    EvolutionResult,
    GeneticEvolutionEngine,
    GeneticIndividual,
)
from meta_coder import CodeAnalysis, MetaCoder, RefactorProposal, RefactorResult
from phase_omega_integration import DuRiPhaseOmega

# Phase Ω 모듈들 import
from self_rewriting_module import (
    CodeAssessment,
    RewriteProposal,
    RewriteResult,
    SelfRewritingModule,
    ...
    async def _execute_self_rewriting_phase(
        ...
        try:
            ...
            assessment_step = next(
                (s for s in session.steps if s.phase == EvolutionPhase.ASSESSMENT), None
            )
            ...
            sorted_modules = sorted(
                assessments.items(), key=lambda x: x[1].complexity_score, reverse=True
            )
    ...
    async def _execute_genetic_evolution_phase(
        ...
        try:
            ...
            step.result = {
                ...
                "best_individual_id": (
                    evolution_result.best_individual.individual_id
                    if evolution_result.best_individual
                    else None
                ),
...
"""
    ...
    async def _create_evolution_result(
        self, session: EvolutionSession, evolution_time: float
    ) -> EvolutionResult:
        """진화 결과 생성"""
        try:
            # 개선사항 수집
            ...
            for step in session.steps:
                if step.result:
                    if step.phase == EvolutionPhase.ASSESSMENT:
                        maintainability_improvement = step.result.get(
    ...
    async def _create_failed_result(
        self, session: EvolutionSession, error_message: str
    ) -> EvolutionResult:
        """실패 결과 생성"""
