...
@dataclass
class EvolutionResult:  # noqa: F811
    """진화 결과 데이터 클래스"""
...
class DuRiEvolutionIntegration:
    ...
    async def _execute_assessment_phase(self, session: EvolutionSession) -> EvolutionStep:
        ...
        try:
            ...
            step.result = {
                ...
                "average_complexity": (
                    sum(a.complexity_score for a in assessments.values()) / len(assessments) if assessments else 0
                ),
                "average_maintainability": (
                    sum(a.maintainability_score for a in assessments.values()) / len(assessments) if assessments else 0
                ),
                "average_performance": (
                    sum(a.performance_score for a in assessments.values()) / len(assessments) if assessments else 0
                ),
            }
    ...
    async def _execute_self_rewriting_phase(self, session: EvolutionSession) -> EvolutionStep:
        ...
        try:
            ...
            # 복잡도가 높은 모듈부터 개선
            sorted_modules = sorted(assessments.items(), key=lambda x: x[1].complexity_score, reverse=True)
    ...
    async def _execute_genetic_evolution_phase(self, session: EvolutionSession) -> EvolutionStep:
        ...
        try:
            ...
            # 진화 실행
            evolution_result = await self.genetic_engine.evolve_capabilities(seed_code, session.target_goal)
            ...
            step.result = {
                ...
                "best_individual_id": (
                    evolution_result.best_individual.individual_id if evolution_result.best_individual else None
                ),
            ...
            for module_path in target_modules:
                if os.path.exists(module_path):
                    ...
                    if analysis.ast_tree:
                        # 리팩토링 제안
                        proposal = await self.meta_coder.refactor_code(analysis.ast_tree, session.target_goal)

                        # 검증 및 적용
                        test_suite = ["test_basic_functionality"]
                        result = await self.meta_coder.validate_and_apply(proposal.proposed_code, test_suite)
                    ...
                    if step.phase == EvolutionPhase.SELF_REWRITING:
                        ...
                        for refactoring in step.result.get("refactoring_results", []):
                            improvements_made.append(
                                f"{refactoring['module']}: 품질 개선 {refactoring['improvement']:.2f}"
                            )
