...
class MemoryOptimizer:
    ...
    async def start_optimization(self, optimization_type: OptimizationType, 
                               target_memory_ids: List[str] = None) -> str:
        """최적화 작업 시작"""
        try:
    ...
    async def perform_optimization(self, optimization_task: OptimizationTask) -> bool:
        ...
        try:
            ...
            self.performance_metrics["average_optimization_time"] = (
                (self.performance_metrics["average_optimization_time"] * (self.performance_metrics["total_optimizations"] - 1) + 
                 optimization_time) / self.performance_metrics["total_optimizations"]
            )
    ...
    async def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "in_progress_tasks": len([t for t in self.optimization_tasks.values() if t.status == OptimizationStatus.IN_PROGRESS]),
