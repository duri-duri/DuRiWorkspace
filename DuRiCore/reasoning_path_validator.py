...
class ReasoningPathValidator:
    ...
    def _initialize_validation_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "logical_consistency": {
                "weight": 0.3,
                "description": "논리적 일관성 검증"
            },
            "completeness": {
                "weight": 0.25,
                "description": "경로 완전성 검증"
            },
            "coherence": {
                "weight": 0.25,
                "description": "의미적 일관성 검증"
            },
            "strength": {
                "weight": 0.2,
                "description": "경로 강도 검증"
            }
        }
    
    async def validate_reasoning_path(self, path: ReasoningPath, 
                                    graph: 'DynamicReasoningGraph') -> PathValidationResult:
        """추론 경로 검증"""
    ...
    async def _validate_logical_consistency(self, path: ReasoningPath, 
                                          graph: 'DynamicReasoningGraph') -> float:
        """논리적 일관성 검증"""
        if len(path.nodes) < 2:
            return 0.0
    ...
    async def _validate_completeness(self, path: ReasoningPath, 
                                   graph: 'DynamicReasoningGraph') -> float:
        """완전성 검증"""
        if not path.nodes:
            return 0.0
        ...
        # 필수 노드 유형 확인
        required_node_types = {NodeType.PREMISE, NodeType.INFERENCE, NodeType.CONCLUSION}
        path_node_types = set()
        ...
        # 필수 노드 유형 포함도
        completeness_score = len(path_node_types.intersection(required_node_types)) / len(required_node_types)
    ...
    async def _validate_coherence(self, path: ReasoningPath, 
                                graph: 'DynamicReasoningGraph') -> float:
        """일관성 검증"""
        if len(path.nodes) < 2:
            return 0.0
    ...
    async def _validate_strength(self, path: ReasoningPath, 
                               graph: 'DynamicReasoningGraph') -> float:
        """강도 검증"""
        if not path.nodes:
            return 0.0
    ...
    def _generate_issues(self, path: ReasoningPath, logical_consistency: float, 
                        completeness: float, coherence: float, strength: float) -> List[str]:
        """이슈 생성"""
        issues = []
...
class ReasoningPathOptimizer:
    ...
    async def find_optimal_paths(self, graph: 'DynamicReasoningGraph', 
    ...
    async def _find_path_a_star(self, graph: 'DynamicReasoningGraph', 
                               start_node: str, end_node: str) -> Optional[ReasoningPath]:
        """A* 알고리즘을 사용한 경로 탐색"""
    ...
    def _calculate_heuristic(self, current_node: str, end_node: str, 
                           graph: 'DynamicReasoningGraph') -> float:
        """휴리스틱 함수 계산"""
...
class ReasoningPathDiversityGenerator:
    ...
    async def generate_diverse_paths(self, graph: 'DynamicReasoningGraph', 
                                   base_path: ReasoningPath, 
                                   num_variations: int = 3) -> List[ReasoningPath]:
        """다양한 경로 생성"""
    ...
    async def _generate_alternative_perspective_paths(self, graph: 'DynamicReasoningGraph', 
                                                    base_path: ReasoningPath) -> List[ReasoningPath]:
        """대안적 관점 기반 경로 생성"""
        alternative_paths = []
        
        # 반론 노드들을 포함하는 경로 생성
        counter_argument_nodes = [n for n in graph.nodes.values() 
                                if n.node_type == NodeType.COUNTER_ARGUMENT]
    ...
    async def _generate_different_reasoning_type_paths(self, graph: 'DynamicReasoningGraph', 
                                                     base_path: ReasoningPath) -> List[ReasoningPath]:
        """다양한 추론 유형 기반 경로 생성"""
        reasoning_type_paths = []
        
        # 다양한 추론 유형의 노드들 찾기
        inference_nodes = [n for n in graph.nodes.values() 
    ...
    async def _generate_varying_complexity_paths(self, graph: 'DynamicReasoningGraph', 
                                               base_path: ReasoningPath) -> List[ReasoningPath]:
        """다양한 복잡도 기반 경로 생성"""
        complexity_paths = []
        
        # 단순화된 경로 (중요도가 높은 노드만 포함)
        important_nodes = [n for n in graph.nodes.values() 
                          if n.importance_score > 0.7]
        ...
        complex_nodes = base_path.nodes + [n for n in all_node_ids 
                                          if n not in base_path.nodes][:3]
...
class ReasoningPathEvaluator:
    ...
    def _initialize_evaluation_criteria(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "quality": {
                "weight": 0.3,
                "description": "경로 품질 평가"
            },
            "efficiency": {
                "weight": 0.25,
                "description": "경로 효율성 평가"
            },
            "robustness": {
                "weight": 0.25,
                "description": "경로 견고성 평가"
            },
            "novelty": {
                "weight": 0.2,
                "description": "경로 신규성 평가"
            }
        }
    
    async def evaluate_paths(self, paths: List[ReasoningPath], 
                           graph: 'DynamicReasoningGraph') -> Dict[str, Any]:
        """경로들 평가"""
        ...
        return {
            ...
            "average_score": sum(r["overall_score"] for r in evaluation_results) / len(evaluation_results) if evaluation_results else 0.0
        }
    
    async def _evaluate_quality(self, path: ReasoningPath, 
                              graph: 'DynamicReasoningGraph') -> float:
        """품질 평가"""
        if not path.nodes:
            return 0.0
    ...
    async def _evaluate_efficiency(self, path: ReasoningPath, 
                                 graph: 'DynamicReasoningGraph') -> float:
        """효율성 평가"""
        if len(path.nodes) < 2:
            return 0.0
    ...
    async def _evaluate_robustness(self, path: ReasoningPath, 
                                 graph: 'DynamicReasoningGraph') -> float:
        """견고성 평가"""
        if not path.nodes:
            return 0.0
    ...
    async def _evaluate_novelty(self, path: ReasoningPath, 
                              graph: 'DynamicReasoningGraph') -> float:
        """신규성 평가"""
        if not path.nodes:
            return 0.0
        ...
        # 새로운 노드 유형 포함도
        novel_node_types = {NodeType.HYPOTHESIS, NodeType.ALTERNATIVE, NodeType.INTEGRATION}
        path_node_types = set()
...
async def test_reasoning_path_validator():
    ...
    # 테스트 노드들 생성
    nodes = {
        "node1": DynamicReasoningNode("node1", NodeType.PREMISE, "상황: 윤리적 딜레마", 0.8, "test"),
    ...
    for i, path in enumerate(optimal_paths):
        print(f"  • 경로 {i+1}: {path.path_id} (신뢰도: {path.confidence:.2f}, 강도: {path.strength:.2f})")
    ...
    if evaluation_results['best_path']:
        print(f"  • 최고 점수 경로: {evaluation_results['best_path']['path_id']} (점수: {evaluation_results['best_path']['overall_score']:.2f})")
