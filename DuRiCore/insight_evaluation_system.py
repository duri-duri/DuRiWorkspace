...
class JudgmentQualityMetricsEvaluator:
    ...
    async def evaluate_judgment_quality(self, judgment_content: str, 
                                      reasoning_process: Dict[str, Any]) -> JudgmentQualityMetrics:
        """판단 품질 평가"""
        logger.info("판단 품질 평가 시작")
        
        # 각 품질 지표 평가
        logical_consistency = self._evaluate_logical_consistency(judgment_content, reasoning_process)
        ...
        practical_relevance = self._evaluate_practical_relevance(judgment_content, reasoning_process)
        
        # 종합 품질 계산
        overall_quality = self._calculate_overall_quality(
            logical_consistency, evidence_support, reasoning_depth, originality, practical_relevance
        )
    ...
    def _evaluate_logical_consistency(self, content: str, reasoning_process: Dict[str, Any]) -> float:
        """논리적 일관성 평가"""
        score = 0.5  # 기본값
    ...
    def _evaluate_evidence_support(self, content: str, reasoning_process: Dict[str, Any]) -> float:
        ...
        if "premises" in reasoning_process:
            ...
            evidence_premises = [p for p in premises if any(kw in str(p) for kw in evidence_keywords)]
    ...
    def _evaluate_practical_relevance(self, content: str, reasoning_process: Dict[str, Any]) -> float:
        """실용적 관련성 평가"""
        score = 0.5  # 기본값
    ...
    def _calculate_overall_quality(self, logical_consistency: float, evidence_support: float,
                                 reasoning_depth: float, originality: float, practical_relevance: float) -> float:
        """종합 품질 계산"""
...
class InsightAuthenticityChecker:
    ...
    def _initialize_authenticity_indicators(self) -> Dict[str, Dict]:
        ...
        return {
            "evidence_quality": {
                "weight": 0.25,
                "indicators": ["구체적 사실", "검증 가능한 정보", "신뢰할 수 있는 출처"]
            },
            "reasoning_quality": {
                "weight": 0.25,
    ...
    async def check_insight_authenticity(self, insight_content: str, 
                                       reasoning_process: Dict[str, Any]) -> InsightAuthenticityCheck:
        """통찰 진위성 검사"""
        ...
        # 통찰 유형 및 진위성 수준 결정
        insight_type = self._determine_insight_type(evidence_quality, reasoning_quality, 
                                                  originality_score, practical_value)
        authenticity_level = self._determine_authenticity_level(evidence_quality, reasoning_quality,
                                                             originality_score, practical_value, red_flags, green_flags)
        
        # 신뢰도 계산
        confidence_score = self._calculate_confidence_score(evidence_quality, reasoning_quality,
                                                         originality_score, practical_value, red_flags, green_flags)
    ...
    def _evaluate_reasoning_quality(self, content: str, reasoning_process: Dict[str, Any]) -> float:
        ...
        # 논리적 키워드 검사
        logical_keywords = ["논리", "일관성", "추론", "전제", "결론", "따라서", "그러므로"]
    ...
    def _detect_red_flags(self, content: str) -> List[str]:
        ...
        for pattern in self.red_flag_patterns:
            ...
            if keyword_count > 0:
                red_flags.append(f"{pattern_name}: {keyword_count}개 키워드 발견 (심각도: {severity})")
    ...
    def _detect_green_flags(self, content: str) -> List[str]:
        ...
        for pattern in self.green_flag_patterns:
            ...
            keyword_count = sum(1 for keyword in keywords if keyword in content)
            if keyword_count > 0:
                green_flags.append(f"{pattern_name}: {keyword_count}개 키워드 발견 (강도: {strength})")
        
        return green_flags
    
    def _determine_insight_type(self, evidence_quality: float, reasoning_quality: float,
                               originality_score: float, practical_value: float) -> InsightType:
        """통찰 유형 결정"""
    ...
    def _determine_authenticity_level(self, evidence_quality: float, reasoning_quality: float,
                                    originality_score: float, practical_value: float,
                                    red_flags: List[str], green_flags: List[str]) -> AuthenticityLevel:
        """진위성 수준 결정"""
        # 기본 점수 계산
        base_score = (evidence_quality + reasoning_quality + originality_score + practical_value) / 4
    ...
    def _calculate_confidence_score(self, evidence_quality: float, reasoning_quality: float,
                                  originality_score: float, practical_value: float,
                                  red_flags: List[str], green_flags: List[str]) -> float:
        """신뢰도 계산"""
        # 기본 신뢰도
        base_confidence = (evidence_quality + reasoning_quality + originality_score + practical_value) / 4
...
class InsightEvaluationSystem:
    ...
    async def evaluate_insight(self, insight_content: str, 
                             reasoning_process: Dict[str, Any]) -> Dict[str, Any]:
        """통찰 종합 평가"""
        ...
        evaluation_result = {
            ...
            "overall_assessment": self._generate_overall_assessment(quality_metrics, authenticity_check)
        }
        
        logger.info("통찰 종합 평가 완료")
        return evaluation_result
    
    def _generate_overall_assessment(self, quality_metrics: JudgmentQualityMetrics,
                                   authenticity_check: InsightAuthenticityCheck) -> Dict[str, Any]:
        """종합 평가 생성"""
        # 품질과 진위성의 가중 평균
    ...
    def _generate_recommendations(self, quality_metrics: JudgmentQualityMetrics,
                                authenticity_check: InsightAuthenticityCheck) -> List[str]:
        """개선 권고사항 생성"""
        recommendations = []
