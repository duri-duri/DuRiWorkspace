...
class EnhancedMemorySystem:
            ...
            for existing_id, existing_memory in self.memories.items():
                ...
                if semantic_similarity > self.association_threshold:
                    await self._create_association_link(
                        memory_id, existing_id, AssociationType.SEMANTIC, semantic_similarity
                    )
                ...
                if time_diff < 3600:  # 1시간 이내
                    ...
                    if temporal_strength > self.association_threshold:
                        await self._create_association_link(
                            memory_id, existing_id, AssociationType.TEMPORAL, temporal_strength
                        )
                ...
                if new_memory.memory_type == MemoryType.EMOTION and existing_memory.memory_type == MemoryType.EMOTION:
                    ...
                    if emotion_similarity > self.association_threshold:
                        await self._create_association_link(
                            memory_id, existing_id, AssociationType.EMOTIONAL, emotion_similarity
                        )
            
        except Exception as e:
            logger.error(f"연관성 분석 오류: {e}")
    
    async def _create_association_link(self, source_id: str, target_id: str, 
    ...
    async def perform_memory_operation(self, operation_type: str, memory_ids: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(input_memories) < 2:
                return {"success": False, "error": "At least 2 memories required for operation"}
    ...
    async def _add_memories(self, memories: List[MemoryEntry]) -> Dict[str, Any]:
        ...
        return {
            "result": f"통합된 정보: {combined_content}",
    ...
    async def _subtract_memories(self, memories: List[MemoryEntry]) -> Dict[str, Any]:
        """메모리 차감 연산"""
        if len(memories) < 2:
            return {"result": "Insufficient memories for subtraction", "confidence": 0.0}
    ...
    async def _store_in_working_memory(self, operation_type: str, memory_ids: List[str], 
                                      operation_result: Dict[str, Any]) -> str:
        """작업 기억 버퍼에 저장"""
        buffer_id = f"wm_{int(time.time() * 1000)}"
        
        # 작업 기억 용량 확인
        if len(self.working_memory_buffers) >= self.working_memory_capacity:
            # 가장 오래된 버퍼 제거
            oldest_buffer = min(self.working_memory_buffers.values(), 
    ...
    async def get_working_memory_status(self) -> Dict[str, Any]:
        ...
        expired_buffers = [
            buffer_id for buffer_id, buffer in self.working_memory_buffers.items()
            if current_time > buffer.expires_at
        ]
    ...
    async def add_semantic_concept(self, name: str, concept_type: ConceptType, 
    ...
    async def add_semantic_inference(self, source_name: str, target_name: str,
    ...
    async def infer_semantic_knowledge(self, concept_name: str,
...
async def test_enhanced_memory_system():
    ...
    test_memories = [
        ...
        },
        {
            ...
            "context": {"type": "experience", "emotion": "happy", "importance": "medium"},
            "importance": 0.7
        },
        {
            "content": "코딩할 때 항상 같은 패턴을 사용하는 것을 발견했다.",
            "context": {"type": "pattern", "emotion": "neutral", "importance": "medium"},
            "importance": 0.6
        },
        {
            "content": "시험 결과가 나왔는데 실망스럽다.",
            "context": {"type": "emotion", "emotion": "disappointed", "importance": "high"},
            "importance": 0.8
        }
    ]
