...
async def test_reasoning_engine_modules():
    ...
    try:
        ...
        from reasoning_system.reasoning_engine import InferenceEngine, InferenceType, InferenceContext
        ...
        # Decision Maker 테스트
        from reasoning_system.reasoning_engine import DecisionMaker, DecisionType, DecisionContext, DecisionCriteria, DecisionOption
        
        decision_maker = DecisionMaker()
        criteria = [
            DecisionCriteria(criteria_id="c1", name="Criteria 1", weight=0.5, description="Test criteria", evaluation_method="scale"),
            DecisionCriteria(criteria_id="c2", name="Criteria 2", weight=0.5, description="Test criteria", evaluation_method="scale")
        ]
        options = [
...
async def test_reasoning_strategies_modules():
    ...
    try:
        ...
        from reasoning_system.reasoning_strategies import DeductiveReasoning, DeductiveRuleType, DeductivePremise
        ...
        from reasoning_system.reasoning_strategies import InductiveReasoning, InductiveType, InductiveObservation
        ...
        from reasoning_system.reasoning_strategies import AbductiveReasoning, AbductiveType, AbductiveObservation
...
async def test_reasoning_optimization_modules():
    ...
    try:
        from reasoning_system.reasoning_optimization import ReasoningOptimizer, OptimizationType, OptimizationTarget
        
        optimizer = ReasoningOptimizer()
        targets = [
            OptimizationTarget(target_id="t1", target_type="processing_time", current_value=100.0, target_value=50.0),
            OptimizationTarget(target_id="t2", target_type="memory_usage", current_value=512.0, target_value=256.0)
        ]
...
async def test_module_imports():
    ...
    try:
        ...
        from reasoning_system.reasoning_engine import (
            InferenceEngine, InferenceType, InferenceContext, InferenceResult,
            LogicProcessor, LogicType, LogicalRule, LogicalChain, LogicAnalysis,
            DecisionMaker, DecisionType, DecisionCriteria, DecisionOption, DecisionResult, DecisionContext
        )
        logger.info("Reasoning Engine imports 성공")
        
        # Reasoning Strategies imports
        from reasoning_system.reasoning_strategies import (
            DeductiveReasoning, DeductiveRuleType, DeductivePremise, DeductiveConclusion, DeductiveRule, DeductiveAnalysis,
            InductiveReasoning, InductiveType, InductiveObservation, InductivePattern, InductiveGeneralization, InductiveAnalysis,
            AbductiveReasoning, AbductiveType, AbductiveObservation, AbductiveHypothesis, AbductiveExplanation, AbductiveAnalysis
        )
        ...
        from reasoning_system.reasoning_optimization import (
            ReasoningOptimizer, OptimizationType, OptimizationTarget, OptimizationStrategy, OptimizationResult, OptimizationAnalysis
        )
