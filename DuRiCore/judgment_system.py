...
class JudgmentSystem:
    ...
    async def judge(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                from memory.judgment_strategy_applier import \
            ...
            try:
                ...
                sys.path.append(os.path.dirname(os.path.abspath(__file__)))
                from memory.judgment_trace_store import (get_reasoning_trace,
                                                         record_judgment_trace)
            ...
            try:
                ...
                sys.path.append(os.path.dirname(os.path.abspath(__file__)))
                from learning.strategic_learning_engine import \
    ...
    async def _generate_improvement_suggestions(
        ...
        try:
            ...
            if recent_errors > 3:
                return [
                    f"연속적인 오류로 인한 개선 제안 생성 실패: {e}. 시스템 상태를 점검해주세요."
                ]
            elif system_performance < 0.3:
                return [
    ...
    def _real_analyze_situation(
        self, input_data: Dict[str, Any], memory_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """실제 상황 분석 로직"""
        try:
            # 입력 데이터에서 키워드 추출
    ...
    def _real_make_decision(
        self, situation_analysis: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """실제 의사결정 로직"""
        try:
    ...
    def _calculate_ethical_score_real(
        self, decision: str, situation_analysis: Dict[str, Any]
    ) -> float:
        """실제 윤리적 점수 계산"""
        # 기본 윤리 점수
        base_score = 0.7
