...
class SelfDirectedLearningStrategy:
    ...
    async def _compile_learning_result(self, session_id: str, 
                                     ...
                                     learning_activities: List[LearningActivity],
                                     learning_outcomes: List[LearningOutcome],
                                     total_learning_time: timedelta) -> SelfDirectedLearningResult:
        """학습 결과 컴파일"""
        # 평균 참여도 계산
        if learning_activities:
            average_engagement = sum(activity.engagement_level for activity in learning_activities) / len(learning_activities)
        else:
            average_engagement = 0.0
    ...
    async def get_learning_summary(self) -> Dict[str, Any]:
        ...
        average_engagement = sum(r.average_engagement for r in self.learning_history) / total_sessions
