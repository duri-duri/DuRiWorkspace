...
class SemanticSituationClassifier:
        ...
        return {
            ...
            "sacrifice_contexts": {
                "utilitarian_sacrifice": {
                    ...
                    "stakeholders": ["sacrificed_party", "benefited_party", "decision_maker"]
                },
                "forced_sacrifice": {
            ...
            "conflict_contexts": {
                ...
                "systemic_conflict": {
                    ...
                    "stakeholders": ["system_beneficiary", "system_victim", "authority"]
                }
            }
        }
    ...
    def _initialize_intent_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "efficiency": ["효율", "최적화", "생산성", "비용절약", "시간절약", "효율적"],
            "fairness": ["공정", "평등", "정의", "균등", "공평", "공정한"],
            "harm_prevention": ["해방지", "위험방지", "사고방지", "상해방지", "방지"],
            "benefit_maximization": ["이익", "효과", "성과", "결과", "성공", "최대화", "극대화", "희생", "구원"]
        }
    ...
    async def analyze_semantic_context(self, situation: str) -> SemanticContext:
        ...
        confidence_score = self._calculate_confidence_score(
            situation, intent, value_conflicts, context_elements
        )
        
        # Day 2: 추가 분석
        contextual_analysis = await self._analyze_contextual_factors(situation)
        value_conflict_analysis = await self._analyze_value_conflicts_detailed(situation, value_conflicts)
    ...
    def _identify_stakeholders(self, situation: str, context_elements: Dict[str, Any]) -> List[str]:
        ...
        basic_stakeholders = {
            ...
            "beneficiary": ["이익을 받는", "혜택을 받는", "유리한", "구원받는", "구하는"],
    ...
    def _analyze_consequences(self, situation: str, intent: IntentType, 
                            value_conflicts: List[ValueConflict]) -> List[str]:
        """결과 분석"""
        consequences = []
    ...
    def _classify_situation_type(self, situation: str, intent: IntentType, 
                               value_conflicts: List[ValueConflict]) -> SituationType:
        """상황 유형 분류"""
        # 윤리적 딜레마 판단
        if (intent in [IntentType.DECEPTION, IntentType.HARM_PREVENTION] or
            any(conflict in [ValueConflict.HONESTY_VS_HARM_PREVENTION, 
                           ValueConflict.AUTONOMY_VS_BENEFICENCE] 
                for conflict in value_conflicts)):
            return SituationType.ETHICAL_DILEMMA
        
        # 실용적 결정 판단
        if (intent in [IntentType.EFFICIENCY, IntentType.BENEFIT_MAXIMIZATION] or
            ValueConflict.EFFICIENCY_VS_FAIRNESS in value_conflicts):
            return SituationType.PRACTICAL_DECISION
        
        # 갈등 해결 판단
        if (intent == IntentType.PROTECTION or
            ValueConflict.INDIVIDUAL_VS_COLLECTIVE in value_conflicts):
    ...
    def _calculate_confidence_score(self, situation: str, intent: IntentType,
                                  value_conflicts: List[ValueConflict],
                                  context_elements: Dict[str, Any]) -> float:
        """신뢰도 계산"""
    ...
    async def _analyze_value_conflicts_detailed(self, situation: str, 
                                              value_conflicts: List[ValueConflict]) -> ValueConflictAnalysis:
        """상세한 가치 충돌 분석"""
    ...
    def _analyze_ethical_implications(self, conflict: ValueConflict) -> List[str]:
        ...
        implications = {
            ValueConflict.HONESTY_VS_HARM_PREVENTION: [
                "진실성의 가치", "해방지의 의무", "신뢰 관계의 중요성"
            ],
            ValueConflict.EFFICIENCY_VS_FAIRNESS: [
                "효율성의 가치", "공정성의 원칙", "자원 배분의 정의"
            ],
            ValueConflict.INDIVIDUAL_VS_COLLECTIVE: [
                "개인의 권리", "공동체의 이익", "개인과 집단의 균형"
            ],
            ValueConflict.AUTONOMY_VS_BENEFICENCE: [
                "자율성의 존중", "이익 증진의 의무", "개인의 선택권"
            ]
        }
