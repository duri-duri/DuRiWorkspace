...
from ..data_structures import ReasoningType, ReasoningContext
    ...
    def _count_nested_levels(self, data: Any, current_level: int = 0) -> int:
        ...
        try:
            if isinstance(data, dict):
                return max(self._count_nested_levels(v, current_level + 1) for v in data.values()) if data else current_level
            elif isinstance(data, list):
                return max(self._count_nested_levels(item, current_level + 1) for item in data) if data else current_level
            else:
