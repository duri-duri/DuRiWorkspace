...
class DeductiveReasoning:
    ...
    async def perform_deductive_reasoning(self, premises: List[DeductivePremise], 
                                        rule_type: DeductiveRuleType = DeductiveRuleType.MODUS_PONENS) -> DeductiveAnalysis:
        """연역적 추론 수행"""
        try:
            ...
            self.logger.info(f"연역적 추론 완료: {rule_type.value}, 유효성: {analysis.validity_score:.2f}")
            return analysis
    ...
    def _create_deductive_rule(self, premises: List[DeductivePremise], 
                             rule_type: DeductiveRuleType) -> DeductiveRule:
        """연역적 규칙 생성"""
        try:
            # 규칙 유형에 따른 결론 생성
    ...
    def _generate_conclusion(self, premises: List[DeductivePremise], 
                           rule_type: DeductiveRuleType) -> DeductiveConclusion:
        """결론 생성"""
        try:
    ...
    def _modus_ponens_conclusion(self, premises: List[DeductivePremise]) -> DeductiveConclusion:
        ...
        try:
            if len(premises) >= 2:
                ...
                confidence = min(premises[0].confidence, premises[1].confidence if len(premises) > 1 else 1.0)
    ...
    def _hypothetical_syllogism_conclusion(self, premises: List[DeductivePremise]) -> DeductiveConclusion:
        """가설적 삼단논법 결론"""
        try:
            if len(premises) >= 3:
                # P → Q, Q → R ⊢ P → R
    ...
    def _disjunctive_syllogism_conclusion(self, premises: List[DeductivePremise]) -> DeductiveConclusion:
        """선언적 삼단논법 결론"""
        try:
            if len(premises) >= 2:
                # P ∨ Q, ¬P ⊢ Q
    ...
    def _constructive_dilemma_conclusion(self, premises: List[DeductivePremise]) -> DeductiveConclusion:
        """구성적 딜레마 결론"""
        try:
            if len(premises) >= 3:
                # (P → Q) ∧ (R → S), P ∨ R ⊢ Q ∨ S
    ...
    def _destructive_dilemma_conclusion(self, premises: List[DeductivePremise]) -> DeductiveConclusion:
        """파괴적 딜레마 결론"""
        try:
            if len(premises) >= 3:
                # (P → Q) ∧ (R → S), ¬Q ∨ ¬S ⊢ ¬P ∨ ¬R
    ...
    def _build_reasoning_chains(self, rule: DeductiveRule) -> List[List[str]]:
        ...
        try:
            ...
            for i, premise in enumerate(rule.premises):
                detailed_chain.append(f"전제 {i+1}: {premise.content} (신뢰도: {premise.confidence:.2f})")
            detailed_chain.append(f"적용된 규칙: {rule.rule_type.value}")
            detailed_chain.append(f"도출된 결론: {rule.conclusion.content} (신뢰도: {rule.conclusion.confidence:.2f})")
    ...
    def _analyze_deductive_reasoning(self, rule: DeductiveRule, 
                                   reasoning_chains: List[List[str]]) -> DeductiveAnalysis:
        """연역적 추론 분석"""
        try:
            # 유효성 분석
    ...
    def _identify_issues(self, rule: DeductiveRule) -> List[str]:
        ...
        try:
            ...
            # 신뢰도가 낮은 전제들
            low_confidence_premises = [premise for premise in rule.premises if premise.confidence < 0.5]
    ...
    def _update_performance_metrics(self, analysis: DeductiveAnalysis, processing_time: float):
        ...
        # 평균 유효성 업데이트
        total_validity = self.performance_metrics['average_validity'] * (self.performance_metrics['total_reasonings'] - 1)
        self.performance_metrics['average_validity'] = (total_validity + analysis.validity_score) / self.performance_metrics['total_reasonings']
        
        # 평균 건전성 업데이트
        total_soundness = self.performance_metrics['average_soundness'] * (self.performance_metrics['total_reasonings'] - 1)
        self.performance_metrics['average_soundness'] = (total_soundness + analysis.soundness_score) / self.performance_metrics['total_reasonings']
