...
class LogicProcessor:
    ...
    async def process_logic(self, input_data: Dict[str, Any], 
                          logic_type: LogicType = LogicType.PROPOSITIONAL) -> LogicAnalysis:
        """논리 처리 수행"""
        try:
    ...
    def _extract_logical_rules(self, input_data: Dict[str, Any], 
                             logic_type: LogicType) -> List[LogicalRule]:
        """논리적 규칙 추출"""
        rules = []
    ...
    def _analyze_logic(self, chains: List[LogicalChain], 
                      logic_type: LogicType) -> LogicAnalysis:
        """논리 분석 수행"""
        try:
            # 일관성 분석
    ...
    def _generate_suggestions(self, chains: List[LogicalChain], 
                            logic_type: LogicType) -> List[str]:
        """개선 제안 생성"""
        suggestions = []
    ...
    def _update_performance_metrics(self, analysis: LogicAnalysis, processing_time: float):
        ...
        # 평균 일관성 업데이트
        total_consistency = self.performance_metrics['average_consistency'] * (self.performance_metrics['total_processings'] - 1)
        self.performance_metrics['average_consistency'] = (total_consistency + analysis.consistency_score) / self.performance_metrics['total_processings']
        
        # 평균 완전성 업데이트
        total_completeness = self.performance_metrics['average_completeness'] * (self.performance_metrics['total_processings'] - 1)
        self.performance_metrics['average_completeness'] = (total_completeness + analysis.completeness_score) / self.performance_metrics['total_processings']
