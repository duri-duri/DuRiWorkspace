...
# Day 6: 학습 피드백 시스템
from learning_feedback_system import AdaptiveLearningEngine, JudgmentMemorySystem, SelfImprovementSystem
...
class IntegratedDuRiSystem:
    ...
    async def process_complex_situation(self, situation: str, action: str) -> Dict[str, Any]:
        ...
        # Day 3-4: 철학적 논증 분석
        print("\n🤔 Day 3-4: 철학적 논증 분석")
        philosophical_arguments = await self.philosophical_analysis.analyze_multiple_perspectives(action, situation)
        ...
        print(f"  • 공리주의 분석: {utilitarian_arg.final_conclusion[:50] if utilitarian_arg else 'N/A'}...")
        print(f"  • 통합 권고: {len(philosophical_arguments.get('integrated_recommendations', []))}개")
        ...
        insight_evaluation = await self.insight_evaluator.evaluate_insight(insight_content, reasoning_graph)
        ...
        print(
            f"  • 종합 품질: {quality_metrics.overall_quality if hasattr(quality_metrics, 'overall_quality') else 0:.2f}"  # noqa: E501
        )
        print(
            f"  • 진위성 수준: {authenticity_check.authenticity_level.value if hasattr(authenticity_check, 'authenticity_level') else 'unknown'}"  # noqa: E501
        )
        print(f"  • 종합 등급: {overall_assessment.get('grade', 'unknown') if overall_assessment else 'unknown'}")
...
async def test_integrated_system():
    ...
    print("\n🎯 핵심 통찰:")
    print("  • 상황 복잡도: 높음 (다중 이해관계자, 가치 충돌)")
    print("  • 철학적 분석: 칸트적 의무론 vs 공리주의 효용성")
    ...
    print(f"  • 통찰 등급: {result['insight_evaluation']['overall_assessment'].get('grade', 'unknown')}")
    ...
    print("\n🔄 다음 단계:")
    print("  • 지속적 학습을 통한 판단 품질 향상")
    print("  • 통찰 진위성 검증 강화")
    print("  • 다중 관점 통합 능력 개선")
