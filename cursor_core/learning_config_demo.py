...
class DuRiLearningConfigDemo:
    ...
    async def _introduce_learning_config(self):
        ...
        logger.info(
            f"  - 트리거 조건: {self.config.learning_loop['external_llm_call']['trigger_conditions']}"
        )
        logger.info(
            f"  - 사용 모델: {list(self.config.learning_loop['external_llm_call']['models'].keys())}"
        )

        logger.info("\n💰 자원 제한:")
        logger.info(
            f"  - 월 예산: ${self.config.resource_limits['monthly_token_budget_dollars']}"
        )
        logger.info(
            f"  - 최대 토큰/호출: {self.config.resource_limits['max_tokens_per_call']}"
        )
        logger.info(
            f"  - 호출 우선순위: {self.config.resource_limits['call_priority']}"
        )
        ...
        self.demo_results.append(
            {"section": "learning_config_introduction", "data": config_data}
        )
    ...
    async def _test_trigger_conditions(self):
        ...
        self.demo_results.append(
            {"section": "trigger_conditions_test", "data": trigger_results}
        )
    ...
    async def _simulate_budget_scenarios(self):
        ...
        estimated_cost = self.config.estimate_call_cost(
            LLMModel.CLAUDE3_HAIKU, estimated_tokens
        )
        ...
        if self.config.can_call_llm(LLMModel.CLAUDE3_HAIKU):
            self.config.update_budget_status(
                LLMModel.CLAUDE3_HAIKU, estimated_tokens, estimated_cost
            )
            logger.info("  ✅ 호출 성공")
        else:
            logger.info("  ❌ 호출 실패")
        ...
        estimated_cost = self.config.estimate_call_cost(
            LLMModel.GPT4O, estimated_tokens
        )
        ...
        if self.config.can_call_llm(LLMModel.GPT4O):
            self.config.update_budget_status(
                LLMModel.GPT4O, estimated_tokens, estimated_cost
            )
            logger.info("  ✅ 호출 성공")
        else:
            logger.info("  ❌ 호출 실패")
    ...
    async def _test_funding_request_system(self):
        ...
        logger.info(
            f"🎯 학습 우선순위 설정: {self.config.core_belief_score['learning_priority']}"
        )
        ...
        logger.info(
            f"⚠️ 예산 위기 상황: {self.config.budget_status.usage_percentage:.1f}% 사용"
        )

        # 자금 요청 생성
        funding_request = self.config.DuRi_generate_funding_request(
            LLMModel.CLAUDE3_HAIKU, 280
        )
        ...
        logger.info(
            f"  - 사용 모델: {funding_request['learning_opportunity']['model']}"
        )
        logger.info(
            f"  - 예상 토큰: {funding_request['learning_opportunity']['expected_tokens']}"
        )
        logger.info(
            f"  - 중요도 점수: {funding_request['learning_opportunity']['importance_score']}"
        )
    ...
    async def _test_cursor_monitoring(self):
        ...
        if funding_request:
            ...
            logger.info(
                f"  - 요청 금액: ${funding_request['requested_amount_usd']:.4f}"
            )
            logger.info(
                f"  - 사용률: {funding_request['budget_status']['used_percentage']:.1f}%"
            )
        else:
        ...
        CursorMonitor.notify_parent(
            channel="voice + visual", urgency="medium", message=test_message
        )
