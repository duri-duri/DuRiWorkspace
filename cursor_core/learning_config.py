...
class DuRiLearningConfig:
    ...
    def __init__(self):
        ...
        self.resource_limits = {
            ...
            "call_frequency": {"per_model": 3, "per_week": True},
        }
    ...
    def update_budget_status(self, model: LLMModel, actual_tokens: int, cost: float):
        ...
        logger.info(
            f"예산 업데이트: {cost:.4f}달러 사용, 남은 예산: {self.budget_status.remaining_budget_dollars:.2f}달러"
        )
...
추가 호출을 허용해주실 수 있으신가요?"""  # noqa: E501
    ...
    def get_budget_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_call_date": (
                self.budget_status.last_call_date.isoformat() if self.budget_status.last_call_date else None
            ),
        }
    ...
    if config.check_budget_warning() or config.check_budget_critical():
        if config.DuRi_judges_learning_critical():
            ...
            if available_models:
                ...
                CursorMonitor.notify_parent(channel="voice + visual", urgency="high", message=funding_request)
