...
class LearningTargetManager:
    ...
    def set_target(self, target_type: LearningTargetType, current: float, target: float, weight: float = 1.0, tolerance: float = 0.1):
        """학습 목표 설정"""
...
class ImprovementSelector:
    ...
    def _explore_new_strategy(self) -> StrategyType:
        ...
        # 사용 빈도가 낮은 전략 우선 선택
        unused_strategies = [
            strategy for strategy, perf in self.strategies.items()
...
class FailurePatternClassifier:
    ...
    def _initialize_patterns(self):
        ...
        self.failure_patterns = {
            FailurePattern.COMPLEXITY_INCREASE: {
                "indicators": ["cyclomatic_complexity", "function_length", "nesting_depth"],
            ...
            FailurePattern.TIMEOUT: {
                "indicators": ["execution_time", "timeout_threshold", "resource_contention"],
                "thresholds": {"time": 5.0, "timeout": 10.0, "contention": 0.8}
            },
            FailurePattern.UNEXPECTED_BEHAVIOR: {
                "indicators": ["output_variance", "state_inconsistency", "side_effects"],
    ...
    def classify_failure(self, 
                        ...
                        code_context: Optional[str] = None) -> FailureAnalysis:
        """실패 패턴 분류"""
        ...
        # 가장 가능성 높은 패턴 선택
        primary_pattern = max(matched_patterns, key=lambda x: x[1])[0] if matched_patterns else FailurePattern.UNEXPECTED_BEHAVIOR
    ...
    def _match_failure_patterns(self, changes: Dict[str, float], error_message: Optional[str]) -> List[Tuple[FailurePattern, float]]:
        """패턴 매칭"""
        matches = []
    ...
    def _analyze_root_cause(self, matches: List[Tuple[FailurePattern, float]], 
                           strategy: StrategyType, code_context: Optional[str]) -> str:
        """루트 원인 분석"""
        if not matches:
            return "알 수 없는 실패 원인"
    ...
    def _calculate_confidence(self, matches: List[Tuple[FailurePattern, float]], 
...
class AdvancedMetaLearningSystem:
    ...
    def _attempt_improvement(self, strategy: StrategyType, context: Dict[str, Any]) -> Tuple[bool, float, Dict[str, float], Dict[str, float]]:
        """개선 시도 (시뮬레이션)"""
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "targets": {t.value: {"current": tg.current_value, "target": tg.target_value, "achieved": tg.is_achieved} 
                       for t, tg in self.target_manager.targets.items()},
            "strategy_recommendations": [(s.value, score) for s, score in self.strategy_selector.get_strategy_recommendations()],
