...
import json, math, dataclasses, statistics
...
def load_sampleset(path: Path) -> SampleSet:
    ...
    groups = {g["id"]: GroupCfg(id=g["id"], weight=float(g.get("weight", 1.0)))
    ...
    for s in d.get("samples", []):
        samples.append(Sample(
            ...
            candidates=[{"tag": c.get("tag","default"), "text": c.get("text","")}
...
def save_md_table(report: Dict[str, Any]) -> str:
    ...
    if tail:
        lines += ["", "## Bottom 5", "", "| ID | Group | Score | Text |", "|---|---|---:|---|"]
        for r in reversed(tail):  # ascending
            lines.append(row(r))

    return "\n".join(lines)

def regress(baseline: Dict[str, Any], current: Dict[str, Any],
            threshold_overall: float = 0.005,
            threshold_group: float = 0.010) -> Dict[str, Any]:
    ...
    return {
        "passed": passed,
        "overall": {"baseline": base_o, "current": cur_o, "delta": delta_o,
...
def regression_md(summary: Dict[str, Any]) -> str:
    ...
    lines += ["", f"**Worst group drop:** `{s['worst_group_drop']:.3f}`  •  **Group threshold:** `{s['threshold_group']:.3f}`"]
    return "\n".join(lines)
