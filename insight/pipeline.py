...
class PromptPipeline:
    ...
    def clean_candidates(self, candidates: List[str]) -> List[str]:
        ...
        for candidate in candidates:
            ...
            if any(len(token_set & seen) / len(token_set) > self.cleaning_rules["duplicate_threshold"] 
    ...
    def process_pipeline(self, prompt: str, candidates: Optional[List[str]] = None, 
                        top_k: int = 3, use_enhanced_evaluation: bool = True) -> Dict[str, Any]:
        ...
        report = {
            ...
            "evaluation_method": "enhanced_integrated" if use_enhanced_evaluation and INTEGRATION_AVAILABLE else "insight_only",
            "rankings": rankings,
            "cleaning_stats": {
            ...
            "integration_status": {
                "available": INTEGRATION_AVAILABLE,
                "systems_used": ["ChatGPTEvaluator", "GeneticEvolutionEngine", "JudgmentSystem"] if INTEGRATION_AVAILABLE else []
            }
        }
    ...
    def save_report(self, report: Dict[str, Any], output_path: str, format: str = "json"):
        ...
        if format == "json":
            ...
            with open(path, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["rank", "score", "text", "novelty", "coherence", "brevity", "combined_score"])
