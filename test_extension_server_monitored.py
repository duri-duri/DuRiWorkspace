...
async def generate_human_ai_response(human_request: Dict[str, Any]):
    ...
    try:
        ...
        integrated_response = _generate_integrated_response(
            user_input, context_result, emotion_result, intuitive_result, adaptive_response
        )
...
def _generate_integrated_response(user_input: str, context: Dict, emotion: Dict, 
                                intuitive: Optional[Dict], adaptive: Dict) -> Dict[str, Any]:
    """통합 응답 생성"""
...
async def capture_conversation_endpoint(conversation_data: Dict[str, Any]):
    ...
    try:
        ...
        if auto_learn:
            ...
            print(f"   🤔 DuRi 자기성찰 완료: {len(reflection_result.get('improvement_proposal', {}).get('specific_improvements', []))}개 개선안")
            ...
            if evaluation_result.get("total_score", 0) < 0.5:  # 낮은 점수일 때만 논의
                discussion_result = duri_chatgpt_discussion.initiate_discussion(
                    reflection_result.get("improvement_proposal", {}),
                    evaluation_result
                )
            ...
            return {
                ...
                "learning_summary": {
                    "evaluation_score": evaluation_result.get("total_score", 0),
                    "improvement_suggestions": len(reflection_result.get("improvement_proposal", {}).get("specific_improvements", [])),
...
async def meta_evaluate_improvement_endpoint(meta_request: Dict[str, Any]):
    ...
    try:
        ...
        if not original_response or not improved_response or not user_question:
            raise HTTPException(status_code=400, detail="original_response, improved_response, user_question이 필요합니다")
...
async def add_realtime_conversation(conversation_data: Dict[str, Any]):
    ...
    try:
        ...
        if not user_input or not assistant_response:
            raise HTTPException(status_code=400, detail="user_input과 assistant_response가 필요합니다")
