...
class LearningVisualizer:
    ...
    def generate_learning_trend_chart(self) -> str:
        ...
        try:
            ...
            ax1.plot(range(len(learning_values)), learning_values, 'b-o', linewidth=2, markersize=6)
...
class IntegrationMonitor:
    ...
    def check_integration_needed(self) -> Dict[str, Any]:
        ...
        if self.learning_patterns_count >= INTEGRATION_THRESHOLDS["learning_patterns"]:
            alerts.append(f"⚠️ 학습 패턴이 {self.learning_patterns_count}개 축적됨 (임계값: {INTEGRATION_THRESHOLDS['learning_patterns']})")
        
        # 응답 시간 체크
        if self.response_times:
            avg_response_time = sum(self.response_times) / len(self.response_times)
            if avg_response_time >= INTEGRATION_THRESHOLDS["response_time"]:
                alerts.append(f"⚠️ 평균 응답 시간이 {avg_response_time:.2f}초 (임계값: {INTEGRATION_THRESHOLDS['response_time']}초)")
        
        # 코드 복잡도 체크
        if self.code_complexity_level >= INTEGRATION_THRESHOLDS["code_complexity"]:
            alerts.append(f"⚠️ 코드 복잡도가 {self.code_complexity_level}단계 (임계값: {INTEGRATION_THRESHOLDS['code_complexity']}단계)")
...
class LearningAnalyzer:
    ...
    def _extract_key_concepts(self, conversation: str) -> List[str]:
        ...
        technical_terms = re.findall(r'\b(?:API|HTTP|JSON|Python|JavaScript|React|Vue|Node|Server|Client|Database|Cache|Async|Sync)\b', conversation, re.IGNORECASE)
    ...
    def _calculate_complexity(self, conversation: str) -> float:
        ...
        # 기술적 용어 비율
        tech_terms = len(re.findall(r'\b(?:API|HTTP|JSON|Python|JavaScript|React|Vue|Node|Server|Client|Database|Cache|Async|Sync)\b', conversation, re.IGNORECASE))
    ...
    def _analyze_engagement(self, conversation: str) -> float:
        ...
        explanations = len(re.findall(r'\b(?:because|since|therefore|thus|hence|so|as|for)\b', conversation, re.IGNORECASE))
...
class ChatGPTEvaluator:
    ...
    def _calculate_6d_scores(self, response: str, question: str) -> Dict[str, float]:
        ...
        # 정확성: 기술적 용어와 개념의 정확성
        tech_terms = len(re.findall(r'\b(?:API|HTTP|JSON|Python|JavaScript|React|Vue|Node|Server|Client|Database|Cache|Async|Sync)\b', response, re.IGNORECASE))
        ...
        # 깊이: 분석적 내용의 깊이
        analytical_words = len(re.findall(r'\b(?:because|since|therefore|thus|hence|analysis|compare|difference|advantage|disadvantage)\b', response, re.IGNORECASE))
        scores["depth"] = min(analytical_words / max(len(response.split()), 1) * 5, 1.0)
        
        # 구조화: 논리적 구성
        structure_indicators = len(re.findall(r'\b(?:first|second|third|finally|however|moreover|in addition|conclusion)\b', response, re.IGNORECASE))
        ...
        # 실용성: 적용 가능한 내용
        practical_indicators = len(re.findall(r'\b(?:example|code|implementation|step|guide|tutorial|practice)\b', response, re.IGNORECASE))
    ...
    def _find_critical_issues(self, response: str) -> List[str]:
        ...
        if not re.search(r'\b(?:API|HTTP|JSON|Python|JavaScript|React|Vue|Node|Server|Client|Database|Cache|Async|Sync)\b', response, re.IGNORECASE):
...
class DuRiSelfReflector:
    ...
    def reflect_on_chatgpt_feedback(self, chatgpt_evaluation: Dict[str, Any], original_response: str, user_question: str) -> Dict[str, Any]:
    ...
    def _self_assess_response(self, response: str, question: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "technical_depth": len(re.findall(r'\b(?:API|HTTP|JSON|Python|JavaScript|React|Vue|Node|Server|Client|Database|Cache|Async|Sync)\b', response, re.IGNORECASE)),
            "has_examples": "example" in response.lower() or "code" in response.lower(),
            "has_structure": len(re.findall(r'\b(?:first|second|finally|however|moreover)\b', response, re.IGNORECASE)) > 0,
...
class DuRiChatGPTDiscussion:
    ...
    def initiate_discussion(self, duri_improvement_proposal: Dict[str, Any], chatgpt_evaluation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_discussion_points(self, duri_proposal: Dict[str, Any], chatgpt_eval: Dict[str, Any]) -> List[Dict[str, Any]]:
        """논의 포인트 생성"""
        points = []
...
'''
    ...
    def apply_improvement(self, improvement: Dict[str, Any], user_approval: bool = False) -> Dict[str, Any]:
        ...
        if improvement["discussion_score"] < self.approval_threshold:
            result["status"] = "rejected"
            result["message"] = f"합의 점수가 낮습니다 ({improvement['discussion_score']:.2f} < {self.approval_threshold})"
            return result
...
async def duri_self_reflect_endpoint(reflection_request: Dict[str, Any]):
    ...
    try:
        ...
        if not chatgpt_evaluation or not original_response:
            raise HTTPException(status_code=400, detail="chatgpt_evaluation과 original_response가 필요합니다")
...
async def apply_improvement_endpoint(improvement_request: Dict[str, Any]):
    ...
    try:
        ...
        result = safe_code_improvement.apply_improvement(
            improvement,
            improvement_request["user_approval"]
        )
...
async def create_improvement_proposal_endpoint(proposal_request: Dict[str, Any]):
    ...
    try:
        ...
        # 코드 개선안 생성
        improvement = safe_code_improvement.create_code_improvement(
            proposal_request["discussion_result"],
            proposal_request.get("target_file")
        )
