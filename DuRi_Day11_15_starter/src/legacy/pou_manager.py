...
class PoUManager:
    ...
    def update_pou_progress(self, domain: str, progress: float, 
                           quality: float = 0.0, safety: float = 0.0, 
                           performance: float = 0.0, error_count: int = 0) -> PoUStatus:
        """PoU 진행 상황 업데이트"""
        if domain not in self.pou_statuses:
    ...
    def generate_pou_dashboard(self) -> Dict[str, Any]:
        ...
        dashboard = {
            ...
            "completed_pilots": len([s for s in self.pou_statuses.values() if s.status == "completed"]),
        ...
        if self.pou_statuses:
            ...
            dashboard["summary"]["avg_quality_score"] = sum(s.quality_score for s in statuses) / len(statuses)
            dashboard["summary"]["avg_safety_score"] = sum(s.safety_score for s in statuses) / len(statuses)
            dashboard["summary"]["avg_performance_score"] = sum(s.performance_score for s in statuses) / len(statuses)
            dashboard["summary"]["avg_progress"] = sum(s.progress_percentage for s in statuses) / len(statuses)
    ...
    def generate_pou_report(self) -> Dict[str, Any]:
        ...
        for domain, data in dashboard["domains"].items():
            if data["status"] == "running":
                if data["progress"] < 50:
                    report["recommendations"].append(f"{domain} 도메인 진행률이 낮습니다. 추가 지원이 필요합니다.")
                if data["error_count"] > 5:
                    report["recommendations"].append(f"{domain} 도메인 오류가 많습니다. 디버깅이 필요합니다.")
                if data["quality_score"] < 80:
                    report["recommendations"].append(f"{domain} 도메인 품질 점수가 낮습니다. 개선이 필요합니다.")
