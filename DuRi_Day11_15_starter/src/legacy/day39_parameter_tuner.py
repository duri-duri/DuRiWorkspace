...
class Day39ParameterTuner:
    ...
    def run_plan_a_experiment(self, config_file: pathlib.Path) -> pathlib.Path:
        ...
        cmd = [
            ...
            "--glob", "'samples/logs/*_A_*.jsonl'",
            "--mapping", "configs/pou_ingest_mapping.yaml",
            "--outdir", str(out_dir),
            "--eval_config", str(config_file),
            "--weight_preset", "safety_first",
            "--evaluate_script", str(self.evaluator_script)
        ]
    ...
    def run_plan_b_experiment(self) -> pathlib.Path:
        ...
        cmd = [
            "python", str(self.ingest_script),
            "--glob", "'samples/logs/*_B_*.jsonl'",
            "--mapping", "configs/pou_ingest_mapping.yaml",
            "--outdir", str(out_dir),
            "--eval_config", str(self.objective_config),
            "--weight_preset", "quality",
            "--evaluate_script", str(self.evaluator_script)
        ]
    ...
    def run_ab_comparison(self, dir_a: pathlib.Path, dir_b: pathlib.Path) -> Dict[str, Any]:
        ...
        # A/B 테스트 실행
        cmd = [
            "python", str(self.ab_test_script),
            "--glob_a", f"'{dir_a}/ab_A_*.json'",
            "--glob_b", f"'{dir_b}/ab_A_*.json'"
        ]
    ...
    def generate_report(self, sensitivity: Dict[str, float], ab_results: Dict[str, Any], 
                       decision: str, canary_rules: Dict[str, Any]) -> str:
        """최종 리포트 생성"""
