...
def enforce_constraints(m, acc):
    ...
    if "max_p95_latency_ms" in acc and m["latency_ms"] > acc["max_p95_latency_ms"]:
        ok = False
        reasons.append(f"latency_ms>{acc['max_p95_latency_ms']}")
    if "min_accuracy" in acc and m["accuracy"] < acc["min_accuracy"]:
        ok = False
        reasons.append(f"accuracy<{acc['min_accuracy']}")
    if "min_explainability" in acc and m["explainability"] < acc["min_explainability"]:
        ok = False
        reasons.append(f"explainability<{acc['min_explainability']}")
    if "max_failure_rate" in acc and m["failure_rate"] > acc["max_failure_rate"]:
        ok = False
        reasons.append(f"failure_rate>{acc['max_failure_rate']}")
    return ok, reasons
...
def main():
    ...
    ap.add_argument(
        "--metrics",
        required=True,
        help="JSON file with latency_ms, accuracy, explainability, failure_rate",
    )
    ap.add_argument("--config", required=True, help="YAML file objective_params.yaml")
    ap.add_argument(
        "--weight_preset",
        default="balanced",
        help="weights key in YAML (balanced/speed/quality/safety_first)",
    )
    args = ap.parse_args()
