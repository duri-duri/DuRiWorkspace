...
def bench_cmd():
    ...
    if path_cmd:
        return path_cmd
    print(
        "[ERR] TRACE_BENCH_CMD not set and no trace_bench on PATH. No dummy allowed.",
        file=sys.stderr,
    )
    sys.exit(2)
...
def assert_real_bench(cmd: str):
    ...
    try:
        ...
        if (
            (not ver)
            or ("dummy" in ver.lower())
            or ("simulator" in ver.lower())
            or ("[bench]" in ver.lower())
        ):
...
def main():
    ...
    for i, (samp, ser, comp) in enumerate(levels, 1):
        ...
        p95 = float(d["p95_ms"])
        err = float(d.get("error_rate", 0.0))
        size = float(d.get("size_kb", 0.0))
        overhead = max(0.0, (p95 - base_p95) / base_p95)
        size_inc = max(0.0, (size / base_size) - 1.0)
        feasible = (overhead <= T_over) and (err <= T_err)
        J = (
            W["overhead"] * overhead
            + W["error"] * err
            + W["size"] * size_inc
            + (0.5 if not feasible else 0.0)
        )
        rec = {
            "sampling_rate": samp,
            "serialization": ser,
        ...
        results.append(rec)
        best = rec if (best is None or rec["J"] < best["J"]) else best
    summary = {
        "schema_version": "trace_tune.v2",
        "generated_at": ts,
        "source": {
            "sweep_cfg": str(CFG),
            "eval_metrics": str(EVAL),
            "slo_metrics": str(slo_path),
        },
        "baseline": {"p95_ms": base_p95, "error_rate": base_err, "size_kb": base_size},
        "target": {
            "max_overhead_pct": T_over * 100,
            "max_error_rate": T_err,
            "weight": W,
            "policy": policy,
        },
    ...
    md = [
        ...
        f"- J(score): **{best['J']:.6f}**, feasible: **{best['feasible']}**",
        "## Top-5 by J",
    ]
    for r in sorted(results, key=lambda x: x["J"])[:5]:
        md.append(
            f"- {r['sampling_rate']}, {r['serialization']}, {r['compression']} → "
