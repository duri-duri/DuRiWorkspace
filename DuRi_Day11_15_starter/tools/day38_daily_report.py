...
def finite_diff_sensitivity(evaluator, cfg, preset, m, delta=0.05):
    ...
    m2 = dict(m)
    m2["failure_rate"] = max(0.0, min(1.0, m["failure_rate"] + eps))
    J1 = eval_J(evaluator, cfg, preset, m2)
    dJ_dfail = (J1 - J0) / eps
    # latency 민감도도 같이
    m3 = dict(m)
    m3["latency_ms"] = m["latency_ms"] + 50.0
    ...
    for dom in cfg["domains"]:
        ...
        *_, last = open(f, encoding="utf-8").read().strip().splitlines()
        ...
        J0, dJ_dfail, dJ_dlat = finite_diff_sensitivity(
            cfg["objective"]["evaluator"],
            cfg["objective"]["config"],
            cfg["objective"]["weight_preset"],
            m,
        )
        lines.append(
            f'| {dom} | {r["ts_utc"]} | {r["n"]} | {r["latency_ms_p95"]} | {float(r["accuracy_mean"]):.3f} | {float(r["explain_mean"]):.3f} | {float(r["failure_rate"]):.4f} | {float(r["J"]):.4f} | {dJ_dfail:.3f} | {dJ_dlat:.6f} |'
        )
