...
for p in DER.glob("*.txt"):
    ...
    for line in txt.splitlines():
        if re.search(r"(금기|주의|악화|흉통|실신|어지러움)", line):
            out_triggers.append({"regex": re.escape(line[:24]), "action":"stop_and_consult"})
...
rules.write_text(yaml.safe_dump(y, allow_unicode=True, sort_keys=False), encoding="utf-8")
print("[OK] mapped contraindications -> risk_checks_rehab_protocol.yaml (+",len(out_triggers),")")
