...
class V1ProtocolRehabSystem:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        handler = logging.FileHandler(f"v1_protocol_rehab_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    def generate_v1_routine(self, user_id: str) -> PersonalizedRoutineV1:
        ...
        routine = PersonalizedRoutineV1(
            ...
            pain_management_score=self._calculate_v1_pain_management_score(selected_exercises, profile)
        )
    ...
    def _calculate_v1_difficulty_score(self, exercises: List[ExerciseV1], profile: UserProfileV1) -> float:
        """V1 프로토콜 난이도 점수"""
        if not exercises:
            return 0.0
    ...
    def _calculate_v1_safety_score(self, exercises: List[ExerciseV1], profile: UserProfileV1) -> float:
    ...
    def _calculate_v1_effectiveness_score(self, exercises: List[ExerciseV1], profile: UserProfileV1) -> float:
        """V1 프로토콜 효과성 점수"""
        if not exercises:
            return 0.0
    ...
    def _calculate_v1_compliance_score(self, exercises: List[ExerciseV1], profile: UserProfileV1) -> float:
        """V1 프로토콜 준수도 점수"""
        score = 100.0
    ...
    def _calculate_v1_pain_management_score(self, exercises: List[ExerciseV1], profile: UserProfileV1) -> float:
        """V1 프로토콜 통증 관리 점수"""
        if profile.current_pain_level < profile.v1_protocol.pain_threshold:
            return 100.0
    ...
    def generate_v1_report(self) -> Dict[str, Any]:
        ...
        if self.routines:
            ...
            report["summary"]["avg_difficulty_score"] = sum(r.difficulty_score for r in routines) / len(routines)
            report["summary"]["avg_safety_score"] = sum(r.safety_score for r in routines) / len(routines)
            report["summary"]["avg_effectiveness_score"] = sum(r.effectiveness_score for r in routines) / len(routines)
            report["summary"]["avg_v1_compliance_score"] = sum(r.v1_compliance_score for r in routines) / len(routines)
            report["summary"]["avg_rpe_score"] = sum(r.rpe_score for r in routines) / len(routines)
            report["summary"]["avg_pain_management_score"] = sum(r.pain_management_score for r in routines) / len(routines)
            report["summary"]["avg_routine_duration"] = sum(r.total_duration for r in routines) / len(routines)
