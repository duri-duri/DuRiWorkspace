...
class EnhancedObjectiveEvaluator:
    ...
    def _calculate_linear_minmax_utility(self, value: float, min_val: float, max_val: float) -> float:
        """선형 minmax 유틸리티 계산"""
        if max_val <= min_val:
            return 0.0
    ...
    def evaluate(self, metrics: Metrics, preset_name: str = 'balanced') -> ObjectiveResult:
        ...
        # 목적함수 J 계산
        J = (weights['latency'] * utilities.latency +
...
class PoUPerformanceMonitor:
    ...
    def generate_comprehensive_report(self, all_results: Dict[str, Dict[str, ObjectiveResult]]) -> str:
        """종합 보고서 생성"""
        report = f"""
...
"""
        ...
        for domain, results in all_results.items():
            optimal_preset, optimal_result = self.evaluator.find_optimal_preset(results['balanced'].metrics)
...
async def main():
    ...
    for domain, results in all_results.items():
        optimal_preset, optimal_result = monitor.evaluator.find_optimal_preset(results['balanced'].metrics)
