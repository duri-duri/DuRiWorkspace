...
class PoUPilotManager:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "active_pilots": len(
                [m for m in self.metrics.values() if m.get("status") == "running"]
            ),
        ...
        if self.metrics:
            ...
            if metrics_list:
                ...
                report["summary"]["avg_safety_score"] = sum(
                    m["safety_score"] for m in metrics_list
                ) / len(metrics_list)
                report["summary"]["avg_performance_ms"] = sum(
                    m["performance_ms"] for m in metrics_list
                ) / len(metrics_list)
                report["summary"]["avg_error_rate"] = sum(
                    m["error_rate"] for m in metrics_list
                ) / len(metrics_list)
                report["summary"]["avg_trace_coverage"] = sum(
                    m["trace_coverage"] for m in metrics_list
                ) / len(metrics_list)
