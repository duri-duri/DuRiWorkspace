...
def test_results_schema():
    ...
    # 필수 필드 검증
    required_fields = {"objective_delta", "p_value", "t_stat", "gate_pass", "gate_reasons"}
    assert required_fields.issubset(results.keys()), f"필수 필드 누락: {required_fields - set(results.keys())}"
