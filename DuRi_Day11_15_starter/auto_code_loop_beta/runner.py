...
def run_tests():
    ...
    if not gate_script.exists():
        # beta fallback
        test_json.write_text(json.dumps({"pass_rate": 0.85, "failures": []}, ensure_ascii=False, indent=2), encoding="utf-8")
        return True
...
def main():
    if not run_tests():
        print(json.dumps({"promote": False, "reason": "no_tests"})); sys.exit(1)

    tests_path = LOGS / "test_result.json"
    try:
        tests = json.loads(tests_path.read_text(encoding="utf-8"))
    except Exception as e:
        print(json.dumps({"promote": False, "reason": "bad_test_json", "error": str(e)})); sys.exit(1)

    if not risk_gate():
        print(json.dumps({"promote": False, "reason": "no_risk"})); sys.exit(1)

    risks_path = LOGS / "risk.json"
    try:
        risks = json.loads(risks_path.read_text(encoding="utf-8"))
    except Exception as e:
        print(json.dumps({"promote": False, "reason": "bad_risk_json", "error": str(e)})); sys.exit(1)
    ...
    if cond:
        ...
        print(json.dumps({"promote": False, "reason": "gate_failed", "tests": tests, "risks": risks}, ensure_ascii=False))
        sys.exit(1)
