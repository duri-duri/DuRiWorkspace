...
class RehabPersonalizationEngine:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        handler = logging.FileHandler(f"rehab_personalization_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    def generate_personalized_routine(self, user_id: str) -> PersonalizedRoutine:
        ...
        self.logger.info(f"Generated routine {routine.routine_id} with {len(selected_exercises)} exercises")
        return routine
    ...
    def _calculate_safety_score(self, exercises: List[Exercise], profile: UserProfile) -> float:
        ...
        final_safety = max(0.0, base_safety - injury_penalty - limitation_penalty + exercise_safety - 50.0)
        return min(100.0, final_safety)
    
    def _calculate_effectiveness_score(self, exercises: List[Exercise], profile: UserProfile) -> float:
        """효과성 점수 계산"""
        if not exercises:
            return 0.0
