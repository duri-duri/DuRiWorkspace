...
class BehaviorGenerator:
    ...
    def _initialize_templates(self) -> Dict[str, BehaviorTemplate]:
        ...
        templates["data_analysis"] = BehaviorTemplate(
            ...
            base_steps=[
                ...
                {"step": "데이터 전처리", "duration": 10.0, "resources": ["cpu", "memory"]},
        ...
        templates["learning_action"] = BehaviorTemplate(
            ...
            base_steps=[
                {"step": "학습 데이터 준비", "duration": 10.0, "resources": ["storage"]},
        ...
        templates["optimization_task"] = BehaviorTemplate(
            ...
            base_steps=[
                {"step": "현재 상태 분석", "duration": 20.0, "resources": ["cpu", "memory"]},
                {"step": "최적화 방안 탐색", "duration": 30.0, "resources": ["cpu", "memory"]},
                {"step": "최적화 적용", "duration": 40.0, "resources": ["cpu", "memory", "storage"]},
    ...
    async def generate_behavior_plan(self, decision_result: Dict[str, Any],
                                   available_resources: Dict[str, float],
                                   constraints: Dict[str, Any]) -> BehaviorPlan:
        """행동 계획 생성"""
        try:
            # 1. 행동 타입 결정
    ...
    async def _select_strategy(self, decision_result: Dict[str, Any],
                             constraints: Dict[str, Any]) -> BehaviorStrategy:
        """전략 선택"""
        urgency = decision_result.get("urgency_level", 0.0)
        complexity = decision_result.get("complexity_score", 0.0)
    ...
    async def _select_template(self, behavior_type: BehaviorType,
                             strategy: BehaviorStrategy) -> BehaviorTemplate:
        """템플릿 선택"""
        # 행동 타입에 따른 템플릿 매핑
        template_mapping = {
    ...
    async def _customize_steps(self, template: BehaviorTemplate,
    ...
    async def _define_success_criteria(self, template: BehaviorTemplate,
    ...
    async def _assess_risks(self, decision_result: Dict[str, Any],
                           steps: List[Dict[str, Any]]) -> Dict[str, float]:
        """위험 평가"""
        risks = {}
    ...
    async def _calculate_resource_requirements(self, template: BehaviorTemplate,
                                            steps: List[Dict[str, Any]],
    ...
    async def _estimate_time(self, template: BehaviorTemplate,
    ...
    async def _generate_description(self, decision_result: Dict[str, Any],
