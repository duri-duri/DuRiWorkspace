...
class LearningPatternAnalyzer:
    ...
    async def analyze_success_patterns(self, behavior_traces: List[Dict[str, Any]]) -> List[LearningPattern]:
        ...
        try:
            ...
            successful_behaviors = [
                trace for trace in behavior_traces 
    ...
    async def identify_learning_effectiveness(self, performance_history: List[Dict[str, Any]]) -> LearningEffectivenessReport:
        """학습 효과성 식별"""
        try:
    ...
    async def _extract_pattern_from_group(self, group_id: str, behaviors: List[Dict[str, Any]], 
                                        pattern_type: PatternType) -> Optional[LearningPattern]:
        """그룹에서 패턴 추출"""
        try:
            if len(behaviors) < 2:
                return None
    ...
    async def _calculate_pattern_confidence(self, frequency: float, success_rate: float, 
                                         average_performance: float, sample_size: int) -> float:
        """패턴 신뢰도 계산"""
        try:
            # 샘플 크기 기반 신뢰도
    ...
    async def _calculate_improvement_rate(self, performance_history: List[Dict[str, Any]]) -> float:
        ...
        try:
            ...
            previous = performance_history[-20:-10] if len(performance_history) >= 20 else performance_history[:-len(recent)]
    ...
    async def _generate_recommendations(self, effectiveness_score: float, 
                                      improvement_rate: float, stability_score: float) -> List[str]:
        """권장사항 생성"""
        try:
            recommendations = []
