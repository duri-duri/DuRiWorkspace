...
class AsyncLLMInterface:
    ...
    async def ask_llm(self, prompt: str, query_type: QueryType, 
                      context: Dict[str, Any] = None, 
                      provider: LLMProvider = LLMProvider.CHATGPT,
                      priority: int = 1) -> LLMResponse:
        """LLM에 질문 (비동기)"""
        try:
            # 캐시 확인
    ...
    async def _call_chatgpt(self, request: LLMRequest, config: Dict[str, Any]) -> LLMResponse:
        ...
        payload = {
            'model': config['model'],
            'messages': [
                {'role': 'system', 'content': self._create_system_prompt(request.query_type)},
    ...
    def _generate_cache_key(self, prompt: str, query_type: QueryType, 
                           context: Dict[str, Any], provider: LLMProvider) -> str:
        """캐시 키 생성"""
