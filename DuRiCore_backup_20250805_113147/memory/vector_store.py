...
class VectorMemoryStore:
    ...
    def store_memory(self, content: str, emotion_data: Dict[str, Any], context_data: Dict[str, Any], importance: float = 0.5) -> str:
        """메모리 저장"""
        try:
            # 고유 ID 생성
    ...
    def search_similar_memories(self, query: str, emotion_data: Optional[Dict[str, Any]] = None, 
                               context_data: Optional[Dict[str, Any]] = None, limit: int = 5) -> List[Tuple[str, float]]:
        """유사한 메모리 검색"""
        try:
            # 쿼리 벡터 생성
            query_semantic = self._create_semantic_vector(query)
            query_emotion = self._create_emotion_vector(emotion_data) if emotion_data else [0.0] * self.emotion_dim
            query_context = self._create_context_vector(context_data) if context_data else [0.0] * self.context_dim
    ...
    def _generate_memory_id(self, content: str, emotion_data: Dict[str, Any], context_data: Dict[str, Any]) -> str:
        """메모리 ID 생성"""
    ...
    def _create_emotion_vector(self, emotion_data: Dict[str, Any]) -> List[float]:
        ...
        emotion_mapping = {
            ...
            "disgust": 5, "love": 6, "contempt": 7, "anticipation": 8, "neutral": 9
        }
    ...
    def _create_context_vector(self, context_data: Dict[str, Any]) -> List[float]:
        ...
        # 맥락 정보를 벡터로 변환
        context_keywords = [
            "일상", "업무", "관계", "건강", "취미", "학습", "여행", "음식",
            "음악", "영화", "책", "운동", "쇼핑", "게임", "예술", "기술"
        ]
    ...
    def _extract_tags(self, content: str, emotion_data: Dict[str, Any], context_data: Dict[str, Any]) -> List[str]:
        """태그 추출"""
        tags = []
    ...
    def _find_and_link_associations(self, memory_id: str):
        ...
        for other_id, other_memory in self.memories.items():
            ...
            # 유사도 계산
            semantic_sim = self._cosine_similarity(current_memory.semantic_vector, other_memory.semantic_vector)
            emotion_sim = self._cosine_similarity(current_memory.emotion_vector, other_memory.emotion_vector)
    ...
    def _get_most_accessed_memories(self, limit: int) -> List[Dict[str, Any]]:
        """가장 많이 접근된 메모리들"""
        sorted_memories = sorted(self.memories.values(), key=lambda x: x.accessed_count, reverse=True)
        return [{"id": m.id, "content": m.content[:50], "access_count": m.accessed_count} 
    ...
    def _get_most_important_memories(self, limit: int) -> List[Dict[str, Any]]:
        ...
        return [{"id": m.id, "content": m.content[:50], "importance": m.importance} 
                for m in sorted_memories[:limit]]
