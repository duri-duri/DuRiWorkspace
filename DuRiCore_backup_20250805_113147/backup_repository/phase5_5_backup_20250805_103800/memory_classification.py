...
class MemoryClassifier:
    ...
    def __init__(self):
        ...
        self.keyword_dictionary = {
            MemoryType.EXPERIENCE: {
                "primary": ["경험", "발생", "일어났다", "했다", "했다", "만났다", "갔다", "왔다"],
                "secondary": ["오늘", "어제", "지난주", "이번주", "그때", "그날"],
                "context": ["친구", "가족", "회사", "학교", "집", "카페", "영화관"]
            },
            MemoryType.KNOWLEDGE: {
                "primary": ["알다", "이해", "학습", "정보", "지식", "배웠다", "알게되었다", "깨달았다"],
                "secondary": ["책", "강의", "수업", "강의", "튜토리얼", "문서", "매뉴얼"],
                "context": ["공부", "연구", "조사", "분석", "실험", "테스트"]
            },
            MemoryType.PATTERN: {
                "primary": ["패턴", "반복", "규칙", "습관", "경향", "항상", "늘", "보통"],
                "secondary": ["매번", "언제나", "대부분", "주로", "자주", "가끔"],
                "context": ["행동", "생각", "감정", "반응", "결과", "결과"]
            },
            MemoryType.EMOTION: {
                "primary": ["감정", "기분", "느낌", "행복", "슬픔", "화남", "기쁨", "우울"],
                "secondary": ["좋다", "나쁘다", "재미있다", "지루하다", "짜증나다", "신기하다"],
        ...
        self.emotion_keywords = {
            "positive": ["행복", "기쁨", "즐거움", "신기", "재미", "좋다", "만족", "감동"],
            "negative": ["슬픔", "화남", "짜증", "우울", "실망", "걱정", "불안", "스트레스"],
    ...
    def _hybrid_classification(self, results: List[ClassificationResult], 
                             content: str, context: Dict[str, Any]) -> ClassificationResult:
        """하이브리드 분류 (최종 결정)"""
        try:
            # 신뢰도 가중 평균 계산
            weighted_scores = {}
    ...
    def _analyze_sentence_structure(self, content: str) -> Dict[str, bool]:
        ...
        try:
            ...
            # 경험 문장 패턴
            experience_patterns = [
                r"했다$", r"했다$", r"갔다$", r"왔다$", r"만났다$",
                r"일어났다$", r"발생했다$", r"경험했다$"
            ]
            
            # 지식 문장 패턴
            knowledge_patterns = [
                r"알게되었다$", r"배웠다$", r"이해했다$", r"깨달았다$",
                r"학습했다$", r"알다$", r"이해하다$"
            ]
            
            # 패턴 문장 패턴
            pattern_patterns = [
                r"항상$", r"늘$", r"보통$", r"자주$", r"가끔$",
                r"패턴$", r"반복$", r"규칙$", r"습관$"
            ]
    ...
    def _analyze_semantic_features(self, content: str) -> Dict[str, float]:
        ...
        try:
            ...
            # 경험 관련 특징
            experience_words = ["경험", "발생", "일어나", "하다", "만나", "가다", "오다"]
            ...
            # 감정 관련 특징
            emotion_words = self.emotion_keywords["positive"] + self.emotion_keywords["negative"] + self.emotion_keywords["neutral"]
            ...
            return {"learning_related": 0.0, "experience_related": 0.0, "pattern_related": 0.0, "emotion_related": 0.0}
...
class MemoryTagger:
    ...
    async def extract_tags(self, content: str, context: Dict[str, Any], 
                          classification_result: ClassificationResult) -> List[TagInfo]:
        """태그 추출"""
        try:
            tags = []
    ...
    def _generate_auto_tags(self, content: str, context: Dict[str, Any], 
                           classification_result: ClassificationResult) -> List[TagInfo]:
        """자동 태그 생성"""
        tags = []
        ...
        if classification_result.confidence > 0.8:
            tags.append(TagInfo(tag="높은신뢰도", confidence=0.9, source="auto", category="confidence"))
        elif classification_result.confidence < 0.5:
            tags.append(TagInfo(tag="낮은신뢰도", confidence=0.9, source="auto", category="confidence"))
