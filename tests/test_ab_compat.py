...
B,140"""
            ...
            try:
                with tempfile.TemporaryDirectory() as output_dir:
                    ...
                    # 공통 필드 확인
                    common_fields = ["objective_delta", "t_stat", "gate_pass", "gate_reasons"]
...
B,132"""
            ...
            try:
                with tempfile.TemporaryDirectory() as output_dir:
                    ...
                    assert "gate_error" in str(result_with_gate["gate_reasons"]) or "gate_disabled" in str(result_with_gate["gate_reasons"])
            finally:
                os.unlink(csv_path)
        finally:
            os.unlink(config_path)
