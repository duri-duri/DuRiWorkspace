...
from insight.metrics import (bleu_like, brevity_prior, composite, distinct_n,
                             evaluate_candidates, evaluate_text,
                             repeat_penalty, tokenize)
...
class TestDistinctN:
    ...
    def test_distinct_repetition(self):
        ...
        assert (
            distinct_n(text, n=2) == 3 / 4
        )  # (the,the), (the,cat), (cat,cat) -> unique: (the,the), (the,cat), (cat,cat)
