...
def test_rules_matching_accuracy():
    ...
    from DuRiCore.duri_logging.autodetect import infer_component, get_all_components
...
def test_context_fields():
    ...
    from DuRiCore.duri_logging.context import (
        set_request_id, set_session_id, set_learning_session_id, set_phase,
...
def test_sampling_functionality():
    ...
    print(f"📊 예상 범위: {expected_min}~{expected_max} ({expected_min/total_calls:.1%}~{expected_max/total_calls:.1%})")
...
def test_logging_format():
    ...
    # 필수 필드 확인
    required_fields = ["component", "request_id", "session_id", "learning_session_id", "phase"]
    missing_fields = []
