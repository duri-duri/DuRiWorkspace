...
class EnhancedEmotionFilter:
    ...
    def __init__(self):
        ...
        self.bias_patterns = {
            JudgmentBias.EMOTIONAL_BIAS: [
                "감정적으로", "화가 나서", "기분이 나빠서", "짜증나서"
            ],
            JudgmentBias.CONFIRMATION_BIAS: [
                "역시", "예상대로", "당연히", "분명히"
            ],
            JudgmentBias.ANCHORING_BIAS: [
                "처음부터", "원래", "본래", "기본적으로"
            ],
    ...
    def analyze_emotion(self, text: str, context: Optional[Dict[str, Any]] = None) -> EmotionAnalysis:
        ...
        logger.info(f"[EnhancedEmotionFilter] 감정 분석: {primary_emotion[0].value} (강도: {intensity.value}, 편향: {bias_detected.value})")
    ...
    def _analyze_emotion_intensity(self, text: str, emotion: EmotionCategory) -> EmotionIntensity:
        ...
        if emotion in self.emotion_keywords:
            ...
            # 강도 지시어 개수
            indicator_count = sum(1 for indicator in config["intensity_indicators"] if indicator in text_lower)
