...
class GrowthStageSystem:
    ...
    def _initialize_quests(self) -> List[Quest]:
        ...
        # 유아기 퀘스트
        quests.extend([
            Quest("infant_001", "색깔 인식하기", "빨강, 파랑, 노랑을 구분할 수 있어요", 
                  GrowthStage.INFANT, 1, 10, []),
            Quest("infant_002", "소리 따라하기", "간단한 소리를 모방할 수 있어요", 
                  GrowthStage.INFANT, 2, 15, ["infant_001"]),
            Quest("infant_003", "감정 표현하기", "웃음, 울음, 놀람을 표현할 수 있어요", 
                  GrowthStage.INFANT, 2, 20, ["infant_001", "infant_002"]),
            Quest("infant_004", "기본 욕구 표현", "배고픔, 졸림, 놀고 싶음을 표현할 수 있어요", 
                  GrowthStage.INFANT, 3, 25, ["infant_003"])
        ])
        
        # 소아기 퀘스트
        quests.extend([
            Quest("toddler_001", "이야기 만들기", "간단한 스토리를 구성할 수 있어요", 
                  GrowthStage.TODDLER, 3, 30, ["infant_004"]),
            Quest("toddler_002", "친구와 놀기", "협력 놀이를 할 수 있어요", 
                  GrowthStage.TODDLER, 4, 35, ["toddler_001"]),
            Quest("toddler_003", "왜? 질문하기", "호기심을 바탕으로 질문할 수 있어요", 
                  GrowthStage.TODDLER, 4, 40, ["toddler_002"]),
            Quest("toddler_004", "감정 이해하기", "다른 사람의 감정을 이해할 수 있어요", 
                  GrowthStage.TODDLER, 5, 45, ["toddler_003"])
        ])
        
        # 학령기 퀘스트
        quests.extend([
            Quest("school_001", "문제 해결하기", "논리적으로 문제를 해결할 수 있어요", 
                  GrowthStage.SCHOOL_AGE, 5, 50, ["toddler_004"]),
            Quest("school_002", "계획 세우기", "단계별로 계획을 수립할 수 있어요", 
                  GrowthStage.SCHOOL_AGE, 6, 55, ["school_001"]),
            Quest("school_003", "지식 정리하기", "학습한 내용을 체계화할 수 있어요", 
                  GrowthStage.SCHOOL_AGE, 6, 60, ["school_002"]),
            Quest("school_004", "자기 평가하기", "자신의 성과를 반성할 수 있어요", 
                  GrowthStage.SCHOOL_AGE, 7, 65, ["school_003"])
        ])
        
        # 청소년기 퀘스트
        quests.extend([
            Quest("adolescent_001", "철학적 질문하기", "인생의 의미를 탐구할 수 있어요", 
                  GrowthStage.ADOLESCENT, 7, 70, ["school_004"]),
            Quest("adolescent_002", "도덕적 딜레마 해결", "윤리적 판단을 할 수 있어요", 
                  GrowthStage.ADOLESCENT, 8, 75, ["adolescent_001"]),
            Quest("adolescent_003", "미래 계획 세우기", "장기적인 목표를 설정할 수 있어요", 
                  GrowthStage.ADOLESCENT, 8, 80, ["adolescent_002"]),
            Quest("adolescent_004", "자기 성찰하기", "깊은 내면을 탐구할 수 있어요", 
                  GrowthStage.ADOLESCENT, 9, 85, ["adolescent_003"])
        ])
        
        # 성인기 퀘스트
        quests.extend([
            Quest("adult_001", "메타 사고하기", "사고 과정에 대해 사고할 수 있어요", 
                  GrowthStage.ADULT, 9, 90, ["adolescent_004"]),
            Quest("adult_002", "창의적 해결책 제시", "혁신적인 아이디어를 제시할 수 있어요", 
                  GrowthStage.ADULT, 10, 95, ["adult_001"]),
            Quest("adult_003", "사회적 기여", "공익을 위한 활동을 할 수 있어요", 
                  GrowthStage.ADULT, 10, 100, ["adult_002"]),
            Quest("adult_004", "지혜로운 조언", "경험 기반의 지혜를 제공할 수 있어요", 
                  GrowthStage.ADULT, 10, 100, ["adult_003"])
        ])
    ...
    def get_available_quests(self) -> List[Quest]:
        ...
        for quest in self.quests:
            if (quest.stage == self.current_stage and 
                not quest.completed and 
                self._can_start_quest(quest)):
