...
from application_system import ApplicationSystem, ApplicationDomain, ProblemType
...
async def test_domain_detection(app_system: ApplicationSystem):
    ...
    test_inputs = [
        ...
        ("복잡한 문제를 해결하는 방법을 알려주세요.", ApplicationDomain.PROBLEM_SOLVING),
        ("창의적인 이야기를 만들어주세요.", ApplicationDomain.CREATIVE_WRITING),
        ("코드 성능을 분석하는 방법을 알려주세요.", ApplicationDomain.TECHNICAL_ANALYSIS),
        ("새로운 아이디어를 떠올리는 방법을 알려주세요.", ApplicationDomain.PROBLEM_SOLVING)
    ]
...
async def test_performance_monitoring(app_system: ApplicationSystem):
    ...
    for i, user_input in enumerate(test_inputs, 1):
        try:
            ...
            print(f"      {i}. {result.domain.value}: 신뢰도 {result.confidence_score:.2f}, 시간 {result.execution_time:.2f}초")
    ...
    scenarios = [
        {
            ...
            "expected_domains": [ApplicationDomain.GENERAL_CONVERSATION, ApplicationDomain.PROBLEM_SOLVING]
        },
        {
            "name": "창의적 요청 + 기술적 분석",
            "input": "창의적인 웹사이트를 만들고 싶은데, 어떤 기술을 사용하는 것이 좋을까요?",
            "expected_domains": [ApplicationDomain.CREATIVE_WRITING, ApplicationDomain.TECHNICAL_ANALYSIS]
        },
        {
            "name": "전략적 계획 + 창의적 사고",
            "input": "새로운 비즈니스를 시작하려고 하는데, 창의적인 전략을 세우고 싶어요.",
            "expected_domains": [ApplicationDomain.PROBLEM_SOLVING, ApplicationDomain.CREATIVE_WRITING]
        }
    ]
