...
class PerformanceMonitoringSystem:
    ...
    async def analyze_performance_trends(self, trend_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 각 지표별 트렌드 분석
            for metric_type in PerformanceMetric:
                metric_data = [data for data in self.performance_data if data.metric_type == metric_type]
    ...
    async def _check_performance_alerts(self, performance_data: PerformanceData) -> None:
        ...
        if metric_type in self.alert_thresholds:
            ...
            alert = PerformanceAlert(
                ...
                threshold_value=thresholds["warning" if alert_level == AlertLevel.WARNING else "critical"],
            ...
            if len(metric_data) >= 5:
                ...
                if abs(latest_value - mean_value) > 2 * std_value:
                    anomalies.append({
                        ...
                        "severity": "high" if abs(latest_value - mean_value) > 3 * std_value else "medium"
                    })
    ...
    async def _generate_trend_recommendations(self, analysis_result: Dict[str, Any]) -> List[str]:
        ...
        for metric_name, trend_data in metric_trends.items():
            ...
            if trend == "increasing" and metric_name in ["cpu_usage", "memory_usage", "response_time", "error_rate"]:
                recommendations.append(f"{metric_name} 지표가 증가하고 있습니다. 모니터링을 강화하세요.")
            elif trend == "decreasing" and metric_name in ["throughput", "availability"]:
                recommendations.append(f"{metric_name} 지표가 감소하고 있습니다. 성능 개선이 필요합니다.")
        ...
        for anomaly in anomalies:
            ...
            recommendations.append(f"{metric_type} 지표에서 {severity} 수준의 이상 징후가 감지되었습니다.")
        
        return recommendations
    
    async def _create_performance_alert(self, performance_data: PerformanceData) -> Optional[PerformanceAlert]:
        """성능 알림 생성"""
        metric_type = performance_data.metric_type
        current_value = performance_data.value
    ...
    async def _create_condition_based_alerts(self, alert_conditions: Dict[str, Any]) -> List[PerformanceAlert]:
        """조건 기반 알림 생성"""
        alerts = []
    ...
    async def _analyze_improvement_details(self, improvement_data: Dict[str, Any]) -> Dict[str, Any]:
        """개선 세부사항 분석"""
        details = {
    ...
    async def _calculate_weighted_performance(self, performance_data: List[PerformanceData]) -> float:
        """가중 평균 성능 계산"""
