...
class InferenceEngine:
    ...
    async def perform_inference(self, context: InferenceContext, 
                              inference_type: InferenceType = InferenceType.INTEGRATED) -> InferenceResult:
        """추론 수행"""
        try:
    ...
    def _update_performance_metrics(self, result: InferenceResult, processing_time: float):
        ...
        # 평균 신뢰도 업데이트
        total_confidence = self.performance_metrics['average_confidence'] * (self.performance_metrics['total_inferences'] - 1)
        self.performance_metrics['average_confidence'] = (total_confidence + result.confidence) / self.performance_metrics['total_inferences']
        
        # 평균 처리 시간 업데이트
        total_time = self.performance_metrics['average_processing_time'] * (self.performance_metrics['total_inferences'] - 1)
        self.performance_metrics['average_processing_time'] = (total_time + processing_time) / self.performance_metrics['total_inferences']
