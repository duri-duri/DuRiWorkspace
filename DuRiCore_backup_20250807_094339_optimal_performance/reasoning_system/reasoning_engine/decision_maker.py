...
class DecisionMaker:
    ...
    async def make_decision(self, context: DecisionContext, 
                          decision_type: DecisionType = DecisionType.RATIONAL) -> DecisionResult:
        """의사결정 수행"""
        try:
            ...
            self.logger.info(f"의사결정 완료: {decision_type.value}, 신뢰도: {result.confidence:.2f}")
            return result
    ...
    def _calculate_confidence(self, options: List[DecisionOption], 
                            selected_option: DecisionOption) -> float:
        """신뢰도 계산"""
        try:
            if not options:
                return 0.0
    ...
    def _calculate_intuitive_confidence(self, options: List[DecisionOption], 
                                      selected_option: DecisionOption) -> float:
        """직관적 신뢰도 계산"""
        try:
            if not options:
    ...
    def _calculate_emotional_confidence(self, options: List[DecisionOption], 
                                     selected_option: DecisionOption) -> float:
        """감정적 신뢰도 계산"""
        try:
            if not options:
    ...
    def _calculate_collaborative_score(self, option: DecisionOption, context: DecisionContext) -> float:
        """협력적 점수 계산"""
        try:
            # 다중 관점을 고려한 점수 계산
            score = 0.0
    ...
    def _calculate_collaborative_confidence(self, options: List[DecisionOption], 
                                         selected_option: DecisionOption) -> float:
        """협력적 신뢰도 계산"""
        try:
            if not options:
    ...
    def _calculate_adaptive_confidence(self, options: List[DecisionOption], 
                                    selected_option: DecisionOption) -> float:
        """적응적 신뢰도 계산"""
        try:
            if not options:
    ...
    def _calculate_strategic_confidence(self, options: List[DecisionOption], 
                                     selected_option: DecisionOption) -> float:
        """전략적 신뢰도 계산"""
        try:
            if not options:
    ...
    def _update_performance_metrics(self, result: DecisionResult, processing_time: float):
        ...
        self.performance_metrics['average_confidence'] = (total_confidence + result.confidence) / self.performance_metrics['total_decisions']
        
        # 평균 처리 시간 업데이트
        total_time = self.performance_metrics['average_processing_time'] * (self.performance_metrics['total_decisions'] - 1)
        self.performance_metrics['average_processing_time'] = (total_time + processing_time) / self.performance_metrics['total_decisions']
