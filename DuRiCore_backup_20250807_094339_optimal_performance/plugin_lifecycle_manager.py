...
class PluginLifecycleManager:
    ...
    async def install_plugin(self, plugin_name: str, plugin_path: str, 
                           version: str = "1.0.0", author: str = "Unknown",
                           description: str = "", dependencies: List[str] = None,
                           priority: PluginPriority = PluginPriority.NORMAL) -> bool:
        """플러그인 설치"""
        try:
            # 플러그인 파일 검증
    ...
    async def update_plugin(self, plugin_name: str, new_version: str, 
                          update_path: str) -> bool:
        """플러그인 업데이트"""
        if plugin_name not in self.plugins:
            return False
    ...
    def _update_performance_metrics(self):
        ...
        # 업데이트 성공률 계산
        update_attempts = sum(1 for p in self.plugins.values() 
                            if p.last_updated != p.created_at)
        update_successes = sum(1 for p in self.plugins.values() 
        ...
        resolved_deps = sum(1 for p in self.plugins.values() 
                          for dep in p.dependencies if dep in self.plugins)
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_plugins': len(self.plugins),
            'plugin_states': {
                'active': sum(1 for p in self.plugins.values() 
                            if p.state == PluginState.ACTIVE),
                'loaded': sum(1 for p in self.plugins.values() 
                            if p.state == PluginState.LOADED),
                'inactive': sum(1 for p in self.plugins.values() 
                              if p.state == PluginState.INACTIVE),
                'error': sum(1 for p in self.plugins.values() 
...
""")
        ...
        success = await lifecycle_manager.install_plugin(
            plugin_name, plugin_file, "1.0.0", "Test Author", 
            f"샘플 플러그인 {plugin_name}", [], PluginPriority.NORMAL
        )
