...
async def test_cache_optimization():
    ...
    try:
        ...
        for i, test_input in enumerate(test_inputs):
            ...
            logger.info(f"   테스트 {i+1}: {execution_time:.3f}초, 성공: {result.success}, 캐시히트: {cache_hit}")
        ...
        test_cases = [
            ...
            {
                "name": "복잡한 데이터",
                "input": {"task": "complex", "data": "test_data", "extra": "additional_info", "nested": {"key": "value"}},
                "context": {"mode": "test", "performance": True}
            },
            {
                "name": "중요도 기반",
                "input": {"task": "important", "data": "test_data", "id": "123", "timestamp": "2025-08-06"},
        ...
        for test_case in test_cases:
            ...
            logger.info(f"   {test_case['name']}: {optimization_time:.3f}초, 키 길이: {len(cache_key)}")
