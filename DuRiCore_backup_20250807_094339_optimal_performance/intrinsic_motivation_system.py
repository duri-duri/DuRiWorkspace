...
class CuriosityMetrics:
    ...
    def overall_curiosity(self) -> float:
        """전체 호기심 수준"""
        return (self.novelty_seeking + self.complexity_preference + 
                self.exploration_drive + self.question_generation + 
                self.learning_interest) / 5.0
...
class AchievementMetrics:
    ...
    def overall_achievement(self) -> float:
        """전체 성취욕 수준"""
        return (self.mastery_orientation + self.performance_improvement + 
                self.skill_development + self.goal_setting + 
...
class IntrinsicMotivationSystem:
    ...
    async def update_achievement_metrics(self, performance: Dict[str, float]) -> None:
        ...
        try:
            ...
            self.motivation_state.last_update = datetime.now()
            logger.info(f"🏆 성취욕 메트릭 업데이트: {self.motivation_state.achievement_metrics.overall_achievement:.3f}")
