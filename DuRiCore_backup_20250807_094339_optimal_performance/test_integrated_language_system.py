...
class IntegratedLanguageSystemTester:
    ...
    async def _test_basic_functionality(self) -> Dict[str, Any]:
        ...
        for test_case in test_cases:
            try:
                ...
                success = (
                    result.understanding_result is not None and
                    result.generation_result is not None and
    ...
    async def _test_language_generation(self) -> Dict[str, Any]:
        ...
        for test_case in test_cases:
            try:
                ...
                success = (
                    generation_result.generated_text is not None and
                    len(generation_result.generated_text.strip()) > 0 and
                    generation_result.confidence_score > 0.3
                )
    ...
    async def _test_multilingual_processing(self) -> Dict[str, Any]:
        ...
        for test_case in test_cases:
            try:
                ...
                success = (
                    multilingual_analysis.get('multilingual_support', False) and
                    multilingual_analysis.get('detected_language') == test_case['expected_language']
                )
    ...
    async def _test_integration(self) -> Dict[str, Any]:
        ...
        for test_case in test_cases:
            try:
                ...
                logger.info(f"✅ {test_case['name']}: {'성공' if success else '실패'} (통합점수: {result.integration_score:.2f})")
    ...
    def _print_test_summary(self, results: Dict[str, Any]):
        ...
        for category, category_results in results['test_categories'].items():
            ...
            logger.info(f"  {category}: {category_results['successful_tests']}/{category_results['total_tests']} ({success_rate:.1f}%)")
