...
class EmotionalSelfAwarenessSystem:
    ...
    async def _update_response_monitoring_metrics(
        ...
        self.awareness_state.awareness_metrics.response_monitoring = min(
            1.0, self.awareness_state.awareness_metrics.response_monitoring + 0.01
        )

    async def _update_pattern_awareness_metrics(self, patterns: Dict[str, Any]) -> None:
        """패턴 인식 메트릭 업데이트"""
        # 실제 구현에서는 더 정교한 업데이트 로직 사용
        self.awareness_state.awareness_metrics.pattern_awareness = min(
            1.0, self.awareness_state.awareness_metrics.pattern_awareness + 0.01
        )
