...
class MemoryAllocator:
    ...
    async def allocate_memory(self, content: str, memory_type: MemoryType, 
                            size: Optional[int] = None, expires_at: Optional[datetime] = None) -> str:
        """메모리 할당"""
        try:
            ...
            self.performance_metrics["average_allocation_time"] = (
                (self.performance_metrics["average_allocation_time"] * (self.performance_metrics["total_allocations"] - 1) + 
                 (time.time() - start_time)) / self.performance_metrics["total_allocations"]
            )
    ...
    async def optimize_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            unused_blocks = []
            for block_id, memory_block in self.memory_blocks.items():
                if (memory_block.status == MemoryStatus.FREE and 
                    memory_block.last_accessed < datetime.now() - timedelta(hours=24)):
