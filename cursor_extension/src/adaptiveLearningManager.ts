...
export class AdaptiveLearningManager {
    ...
    private isAdaptiveLearning: boolean = false;
    private currentOptimalFormat: string = 'summary';
     ...
     * 적응적 학습으로 대화 전송
     */
    async sendAdaptiveConversation(speaker: string, content: string, context?: any): Promise<AdaptiveLearningResult | null> {
        try {
            ...
            if (response && response.success) {
                ...
                vscode.window.showInformationMessage(
                    `DuRi 적응적 학습 완료: ${response.selected_format} 형식 (효율성: ${(response.efficiency_metrics.overall_score * 100).toFixed(1)}%)`
                );
     ...
     * 특정 형식으로 테스트
     */
    async testSpecificFormat(formatType: string, content: string): Promise<AdaptiveLearningResult | null> {
        try {
            ...
            if (response && response.success) {
                ...
                vscode.window.showInformationMessage(
                    `${formatType} 형식 테스트 완료 (효율성: ${(response.efficiency_metrics.overall_score * 100).toFixed(1)}%)`
                );
     ...
     * 현재 최적 형식 조회
     */
    getCurrentOptimalFormat(): string {
