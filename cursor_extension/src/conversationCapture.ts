...
import { DuRiAPI, UnifiedConversationRequest } from './duriAPI';

export class ConversationCapture {
    private learningManager: DuRiLearningManager;
    private duriAPI: DuRiAPI;
    private isCapturing: boolean = false;
    private lastUserInput: string = '';
    private lastAssistantResponse: string = '';
    private sessionId: string;

    constructor(learningManager: DuRiLearningManager) {
        this.learningManager = learningManager;
        this.duriAPI = new DuRiAPI();
    ...
    private async checkDuRiConnection(): Promise<void> {
    ...
    private monitorCursorChat(): void {
    ...
    private extractConversationFromContent(content: string): void {
        // 사용자 입력과 AI 응답 추출
                ...
                if (result) {
                    ...
                    if (result.improvement_suggestions && result.improvement_suggestions.length > 0) {
                        vscode.window.showInformationMessage(
                            `DuRi 학습 완료! 점수: ${result.integrated_score}. 개선 제안: ${result.improvement_suggestions.length}개`
                        );
                    }
                } else {
    ...
    private async sendCodeGenerationToDuRi(codeContent: string, fileName: string): Promise<void> {
