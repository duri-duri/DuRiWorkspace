...
class SelfReflectionEngine:
    ...
    def create_reflection(
        ...
        # 반영 수준 결정
        reflection_level = self._determine_reflection_level(
            data, emotional_state, growth_impact
        )
        ...
        logger.info(
            f"반영 일지 생성 완료: {reflection_type.value} - {reflection_level.value}"
        )
        return entry

    def _determine_reflection_level(
        self, data: Dict[str, Any], emotional_state: str, growth_impact: float
    ) -> ReflectionLevel:
        """반영 수준 결정"""
    ...
    def _generate_insights(
        ...
        if reflection_type == ReflectionType.JUDGMENT:
            ...
            if experience_points > 100:
                insights.append(
                    "충분한 경험을 쌓았습니다. 다음 단계로 나아갈 준비가 되었습니다."
                )
            else:
    ...
    def _generate_action_items(
        self, insights: List[str], reflection_type: ReflectionType
    ) -> List[str]:
        """행동 항목 생성"""
        action_items = []
    ...
    def get_reflections_by_type(
        ...
        return [
            ...
            if entry.reflection_type == reflection_type
        ]
    ...
    def get_reflection_summary(self) -> Dict[str, Any]:
        ...
        for entry in self.reflection_log:
            type_counts[entry.reflection_type.value] = (
                type_counts.get(entry.reflection_type.value, 0) + 1
            )
            level_counts[entry.reflection_level.value] = (
                level_counts.get(entry.reflection_level.value, 0) + 1
            )
