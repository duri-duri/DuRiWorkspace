...
class PerformanceScorer:
    ...
    def score_loop_performance(
        ...
        for metric in PerformanceMetric:
            ...
            score = self._calculate_metric_score(
                metric, loop_data, emotional_state, growth_metrics, judgment_result
            )
        ...
        # 성장 기여도 계산
        growth_contribution = self._calculate_growth_contribution(
            scores, growth_metrics
        )
    ...
    def _score_emotional_growth(
        self, emotional_state: str, loop_data: Dict[str, Any]
    ) -> float:
        """감정 성장 점수"""
        ...
        score = (
            ...
            + emotional_insight * 0.2
        ) * 100
        return min(100.0, max(0.0, score))

    def _score_cognitive_development(
        self, growth_metrics: Dict[str, Any], loop_data: Dict[str, Any]
    ) -> float:
        """인지 발달 점수"""
    ...
    def _score_social_progress(
        self, loop_data: Dict[str, Any], emotional_state: str
    ) -> float:
        """사회성 진전 점수"""
        ...
        score = (
            social_interaction * 0.4 + empathy_level * 0.3 + communication_quality * 0.3
        ) * 100
        return min(100.0, max(0.0, score))

    def _score_creativity_level(
        self, loop_data: Dict[str, Any], growth_metrics: Dict[str, Any]
    ) -> float:
        """창의성 수준 점수"""
        ...
        score = (
            ...
            + artistic_sensitivity * 0.2
        ) * 100
        return min(100.0, max(0.0, score))

    def _score_problem_solving(
        self, judgment_result: Dict[str, Any], loop_data: Dict[str, Any]
    ) -> float:
        """문제 해결 점수"""
    ...
    def _score_adaptability(
        ...
        score = (
            change_response * 0.4 + learning_speed * 0.3 + stress_management * 0.3
        ) * 100
        return min(100.0, max(0.0, score))
    ...
    def _score_autonomy(
        ...
        score = (
            self_direction * 0.4 + decision_independence * 0.3 + initiative_level * 0.3
        ) * 100
        return min(100.0, max(0.0, score))
    ...
    def _generate_feedback(
        self, metric: PerformanceMetric, score: float, level: PerformanceLevel
    ) -> str:
        """피드백 생성"""
    ...
    def _calculate_overall_score(
        self, scores: Dict[PerformanceMetric, PerformanceScore]
    ) -> float:
        """전체 점수 계산"""
    ...
    def _calculate_growth_contribution(
        self,
        scores: Dict[PerformanceMetric, PerformanceScore],
        growth_metrics: Dict[str, Any],
    ) -> float:
    ...
    def _calculate_efficiency_rating(
        self, loop_data: Dict[str, Any], overall_score: float
    ) -> float:
        """효율성 평가"""
        ...
        efficiency = (
            (time_efficiency * 0.6 + complexity_efficiency * 0.4)
            * overall_score
            / 100.0
        )
    ...
    def _generate_recommendations(
        self, scores: Dict[PerformanceMetric, PerformanceScore], overall_score: float
    ) -> List[str]:
        """권장사항 생성"""
        recommendations = []
        ...
        for metric, score in scores.items():
            if metric not in self.baseline_scores:
                ...
                self.baseline_scores[metric] = (
                    self.baseline_scores[metric] * 0.9 + score.score * 0.1
                )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        avg_overall_score = sum(p.overall_score for p in recent_performances) / len(
            recent_performances
        )
        avg_efficiency = sum(p.efficiency_rating for p in recent_performances) / len(
            recent_performances
        )
        ...
        for metric in PerformanceMetric:
            if metric != PerformanceMetric.OVERALL_GROWTH:
                scores = [
                    p.scores[metric].score
                    for p in recent_performances
                    if metric in p.scores
                ]
    ...
    def get_performance_trends(
        self, metric: PerformanceMetric, limit: int = 20
    ) -> List[Dict[str, Any]]:
        """성과 트렌드 조회"""
        trends = []
