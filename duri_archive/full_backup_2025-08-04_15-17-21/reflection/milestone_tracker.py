...
class MilestoneTracker:
    ...
    def _initialize_milestones(self):
        """이정표 초기화"""
        # 레벨 1: NEWBORN
        self._create_level_milestones(
            1,
            [
                (
                    "basic_emotion_recognition",
                    MilestoneCategory.EMOTIONAL_MATURITY,
                    "기본 감정 인식",
                    "기본적인 감정을 인식하고 표현할 수 있다",
                ),
                (
                    "simple_interaction",
                    MilestoneCategory.SOCIAL_SKILLS,
                    "간단한 상호작용",
                    "기본적인 상호작용을 할 수 있다",
                ),
                (
        ...
        self._create_level_milestones(
            ...
            [
                ...
                (
                    ...
                    MilestoneCategory.PROBLEM_SOLVING,
                    "복합 문제 해결",
                    "복잡한 문제를 해결한다",
                ),
                (
                    "self_directed_learning",
                    MilestoneCategory.SELF_MOTIVATION,
                    "자기주도 학습",
                    "스스로 학습을 주도한다",
                ),
            ],
    ...
    def update_milestone_progress(
        self, milestone_id: str, progress: float, experience_gained: int = 0
    ):
        """이정표 진행도 업데이트"""
        ...
        logger.info(
            f"이정표 진행도 업데이트: {milestone_id} - {old_progress:.2f} → {milestone.progress:.2f}"
        )
    ...
    def get_milestones_by_category(
        self, category: MilestoneCategory
    ) -> List[Milestone]:
        """카테고리별 이정표 조회"""
    ...
    def get_completed_milestones(self) -> List[Milestone]:
        """완료된 이정표 조회"""
        return [
            m for m in self.milestones.values() if m.status == MilestoneStatus.COMPLETED
        ]
    ...
    def get_milestone_summary(self) -> Dict[str, Any]:
        ...
        for category in MilestoneCategory:
            category_progress[category.value] = self.calculate_category_progress(
                category
            )
        ...
        return {
            ...
            "completion_rate": (
                completed_milestones / total_milestones if total_milestones > 0 else 0.0
            ),
            "category_progress": category_progress,
            "level_progress": level_progress,
            "recent_progress": (
                self.progress_history[-10:] if self.progress_history else []
            ),
        }

    def get_recommended_milestones(
        self, current_level: int, emotional_state: str
    ) -> List[Milestone]:
        """추천 이정표 조회"""
        # 현재 레벨의 미완료 이정표
        current_milestones = [
            m
            for m in self.get_milestones_for_level(current_level)
        ...
        for category in priority_categories:
            category_milestones = [
                m for m in current_milestones if m.category == category
            ]
