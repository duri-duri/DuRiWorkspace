...
def analyze_self_performance(db: Session = Depends(get_db_session)):
    """자기 성능 분석"""
    try:
        ...
        return {"success": True, "message": "자기 성능 분석 완료", "data": result}
...
def auto_improve_system(db: Session = Depends(get_db_session)):
    """시스템 자동 개선"""
    try:
        se_service = SelfEvolutionService(db)
        ...
        return {"success": True, "message": "시스템 자동 개선 완료", "data": result}
...
def get_self_evolution_stats(db: Session = Depends(get_db_session)):
    """자기 진화 통계 조회"""
    try:
        se_service = SelfEvolutionService(db)
        stats = se_service.get_self_evolution_stats()

        return {"success": True, "message": "자기 진화 통계 조회 완료", "data": stats}
...
def improve_specific_system(
    improvement_request: Dict[str, Any], db: Session = Depends(get_db_session)
):
    """특정 시스템 개선"""
    try:
        se_service = SelfEvolutionService(db)
...
def get_detailed_performance_report(db: Session = Depends(get_db_session)):
    """상세 성능 리포트"""
    try:
        se_service = SelfEvolutionService(db)
        ...
        if overall_score < 70:
            report["recommendations"].append(
                "전체 시스템 성능이 낮습니다. 우선순위 개선이 필요합니다."
            )
        elif overall_score < 80:
            report["recommendations"].append(
                "시스템 성능이 양호하지만 개선 여지가 있습니다."
            )
        else:
            report["recommendations"].append(
                "시스템 성능이 우수합니다. 유지 관리에 집중하세요."
            )
