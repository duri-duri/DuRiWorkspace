...
class SelfModelEnhancer:
    ...
    def evaluate_current_self_state(
    ...
    def conduct_self_evaluation(
        self, evaluation_type: SelfEvaluationType, family_context: Dict[str, Any] = None
    ) -> SelfEvaluation:
        """자아 평가 수행"""
        try:
    ...
    def _get_evaluation_levels(
        self, evaluation_type: SelfEvaluationType
    ) -> tuple[float, float]:
        """평가 수준 결정"""
        level_mappings = {
    ...
    def _identify_improvement_areas(
        ...
        """개선 영역 식별"""
        improvement_areas = {
            SelfEvaluationType.KNOWLEDGE: [
                "고급 윤리학",
                "심화 창의성 이론",
                "복잡한 가족 역학",
            ],
            SelfEvaluationType.EMOTIONAL: [
                "감정 표현 다양화",
                "복잡한 감정 이해",
                "감정 조절 기술",
            ],
            SelfEvaluationType.SOCIAL: [
                "갈등 해결 기술",
                "다양한 관점 이해",
                "사회적 상황 적응",
            ],
            SelfEvaluationType.CREATIVE: [
                "창의적 표현 방법",
                "혁신적 사고",
                "예술적 감각",
            ],
            SelfEvaluationType.ETHICAL: [
                "복잡한 윤리적 상황",
                "가치 충돌 해결",
                "윤리적 판단 고도화",
            ],
            SelfEvaluationType.FAMILY: [
                "가족 갈등 해결",
                "세대 간 소통",
                "가족 역학 이해",
            ],
    ...
    def predict_self_growth(
        self, prediction_type: SelfPredictionType, timeframe: str = "3개월"
    ) -> SelfPrediction:
        """자아 성장 예측"""
        try:
            ...
            # 예측 유형별 결과 및 확률 결정
            predicted_outcome, probability = self._get_prediction_outcome(
    ...
    def _get_prediction_outcome(
        self, prediction_type: SelfPredictionType, timeframe: str
    ) -> tuple[str, float]:
        """예측 결과 및 확률 결정"""
        prediction_outcomes = {
        ...
        prediction = prediction_outcomes.get(
            prediction_type,
            {"outcome": "예측 결과를 분석 중입니다.", "probability": 0.5},
        )

        return prediction["outcome"], prediction["probability"]

    def _analyze_prediction_factors(
        self, prediction_type: SelfPredictionType
    ) -> List[str]:
        """예측 영향 요인 분석"""
        factors = {
            SelfPredictionType.GROWTH: [
    ...
    def _calculate_prediction_confidence(
        self, probability: float, factor_count: int
    ) -> float:
        """예측 신뢰도 계산"""
        base_score = 0.6
    ...
    def create_self_growth_plan(
        self, goal: str, family_context: Dict[str, Any] = None
    ) -> SelfGrowthPlan:
        """자아 성장 계획 생성"""
        try:
            ...
            action_steps = self._generate_action_steps(
                goal, current_state, target_state
            )
    ...
    def _generate_action_steps(
        self, goal: str, current_state: str, target_state: str
    ) -> List[str]:
        """행동 단계 생성"""
        goal_lower = goal.lower()
    ...
    def get_self_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for level in SelfAwarenessLevel:
                level_states = [
                    s for s in self.self_states if s.awareness_level == level
                ]
            ...
            for eval_type in SelfEvaluationType:
                type_evaluations = [
                    e for e in self.self_evaluations if e.evaluation_type == eval_type
                ]
            ...
            for pred_type in SelfPredictionType:
                type_predictions = [
                    p for p in self.self_predictions if p.prediction_type == pred_type
                ]
                prediction_stats[pred_type.value] = len(type_predictions)

            # 평균 신뢰도 계산
            avg_state_confidence = (
                sum(s.overall_confidence for s in self.self_states)
                / len(self.self_states)
                if self.self_states
                else 0
            )
            ...
            avg_prediction_confidence = (
                sum(p.confidence_score for p in self.self_predictions)
    ...
    def export_self_data(self) -> Dict[str, Any]:
        ...
        try:
            export_data = {
                ...
                "self_predictions": [
                    asdict(prediction) for prediction in self.self_predictions
                ],
        ...
        try:
            ...
            for state_data in data.get("self_states", []):
                ...
                if "timestamp" in state_data:
                    state_data["timestamp"] = datetime.fromisoformat(
            ...
            for evaluation_data in data.get("self_evaluations", []):
                ...
                if "timestamp" in evaluation_data:
                    evaluation_data["timestamp"] = datetime.fromisoformat(
            ...
            for prediction_data in data.get("self_predictions", []):
                ...
                if "timestamp" in prediction_data:
                    prediction_data["timestamp"] = datetime.fromisoformat(
...
def test_self_model_enhancer():
    ...
    # 2. 자아 평가 수행
    knowledge_evaluation = self_enhancer.conduct_self_evaluation(
        SelfEvaluationType.KNOWLEDGE, family_context
    )
    ...
    # 3. 자아 성장 예측
    growth_prediction = self_enhancer.predict_self_growth(
        SelfPredictionType.GROWTH, "6개월"
    )
    ...
    print(
        f"✅ 자아 통계: {statistics['total_states']}개 상태, {statistics['total_evaluations']}개 평가"
    )
    ...
    print(
        f"✅ 자아 데이터 내보내기: {len(export_data['self_states'])}개 상태, {len(export_data['self_evaluations'])}개 평가"
    )
