...
from .experience_recorder_service import (
    Experience,
    ExtractedLesson,
    GenerationalExperienceRecorder,
)

# Phase 10 시스템들 import
from .family_identity_service import (
    FamilyCulture,
    FamilyIdentity,
    FamilyIdentityCore,
    FamilyMember,
)
from .lesson_extractor_service import (
    BasicLessonExtractor,
)
from .lesson_extractor_service import ExtractedLesson as LessonExtractorLesson
from .relationship_formation_service import (
    FamilyRelationshipFormationSystem,
    Interaction,
    RoleUnderstanding,
)
...
class Phase10Integration:
    ...
    def initialize_phase10(self, family_name: str, initial_members: List[Dict]) -> Dict:
        ...
        try:
            # 1. 가족 정체성 초기화
            family_identity = self.family_identity_core.initialize_family_identity(
                family_name, initial_members
            )
    ...
    def record_comprehensive_experience(
        ...
        try:
            ...
            if (
                "participants" in experience_data
                and len(experience_data["participants"]) >= 2
            ):
                interaction_data = {
                    ...
                    "communication_quality": experience_data.get(
                        "communication_quality", 0.5
                    ),
                    "mutual_understanding": experience_data.get(
                ...
                interaction = self.relationship_system.record_interaction(
                    interaction_data
                )
                results["interaction_recorded"] = asdict(interaction)

            # 3. 교훈 추출
            lesson = self.lesson_extractor.extract_lesson_from_experience(
    ...
    def generate_family_wisdom_report(self) -> Dict:
        ...
        try:
            ...
            # 6. 다음 세대 전달 준비된 교훈들
            next_generation_lessons = (
                self.lesson_extractor.prepare_next_generation_lessons()
            )

            report = {
                "report_timestamp": datetime.now().isoformat(),
                "family_strength": family_insights.get("family_strength", 0.0),
                "relationship_health": relationship_insights.get(
                    "overall_relationship_health", 0.0
                ),
    ...
    def suggest_family_improvements(self) -> List[Dict]:
        ...
        try:
            ...
            relationship_suggestions = (
                self.relationship_system.suggest_relationship_improvements("DuRi")
            )
    ...
    def import_phase10_data(self, data: Dict):
        ...
        try:
            ...
            if "family_identity_data" in data:
                self.family_identity_core.import_family_data(
                    data["family_identity_data"]
                )
            ...
            if "relationship_data" in data:
                self.relationship_system.import_relationship_data(
    ...
    def _update_integration_status(self):
        ...
        # 지혜 성숙도 계산
        lesson_insights = self.lesson_extractor.get_lesson_insights()
        self.status.wisdom_maturity = lesson_insights.get(
            "maturity_progression", {}
        ).get("current_maturity", 0.0)
    ...
    def _calculate_phase10_progress(self) -> Dict:
        ...
        # 경험 기반 진행도
        experience_progress = min(
            self.status.total_experiences / 50 * 100, 100
        )  # 50개 경험 = 100%

        # 교훈 기반 진행도
        lesson_progress = min(
            self.status.total_lessons / 20 * 100, 100
        )  # 20개 교훈 = 100%
    ...
    def get_phase10_summary(self) -> Dict:
        ...
        try:
            ...
            summary = {
                ...
                "last_updated": (
                    self.status.last_updated.isoformat()
