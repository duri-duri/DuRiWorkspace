...
class TextBasedLearningSystem:
    ...
    def extract_knowledge_from_text(self, text_content_id: str) -> ExtractedKnowledge:
        ...
        try:
            # 텍스트 콘텐츠 찾기
            text_content = next((tc for tc in self.text_contents if tc.id == text_content_id), None)
            if not text_content:
            ...
            # 신뢰도 점수 계산
            confidence_score = self._calculate_confidence_score(text_content, key_concepts, main_ideas)
    ...
    def _determine_learning_category(self, content: str) -> LearningCategory:
        ...
        elif any(word in content_lower for word in ['창의', '상상', '혁신', '아이디어']):
    ...
    def _calculate_confidence_score(self, text_content: TextContent, key_concepts: List[str], main_ideas: List[str]) -> float:
        """신뢰도 점수 계산"""
        # 기본 점수
        base_score = 0.5
    ...
    def update_learning_progress(self, text_content_id: str, progress_data: Dict[str, Any]) -> LearningProgress:
        """학습 진도 업데이트"""
        try:
            # 기존 진도 찾기
            existing_progress = next((p for p in self.learning_progress if p.text_content_id == text_content_id), None)
            ...
            if existing_progress:
                ...
                existing_progress.total_time_spent_minutes += progress_data.get('time_spent_minutes', 0)
    ...
    def get_learning_recommendations(self, family_context: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """학습 추천 제공"""
        try:
            recommendations = []
            ...
            for content in relevant_contents:
                # 해당 콘텐츠의 지식 추출
                knowledge = next((k for k in self.extracted_knowledge if k.text_content_id == content.id), None)
                
                if knowledge:
                    recommendation = {
                        'text_content': asdict(content),
                        'extracted_knowledge': asdict(knowledge),
                        'recommendation_score': self._calculate_recommendation_score(content, knowledge, family_context),
    ...
    def _filter_family_relevant_contents(self, family_context: Dict[str, Any] = None) -> List[TextContent]:
        """가족 관련 콘텐츠 필터링"""
    ...
    def _calculate_recommendation_score(self, content: TextContent, knowledge: ExtractedKnowledge, family_context: Dict[str, Any] = None) -> float:
        """추천 점수 계산"""
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for category in LearningCategory:
                category_knowledge = [k for k in self.extracted_knowledge if k.learning_category == category]
                category_stats[category.value] = len(category_knowledge)
            
            # 수준별 통계
            level_stats = {}
            for level in LearningLevel:
                level_knowledge = [k for k in self.extracted_knowledge if k.learning_level == level]
                level_stats[level.value] = len(level_knowledge)
            
            # 평균 신뢰도
            avg_confidence = sum(k.confidence_score for k in self.extracted_knowledge) / len(self.extracted_knowledge) if self.extracted_knowledge else 0
            
            # 평균 학습 진도
            avg_completion = sum(p.completion_percentage for p in self.learning_progress) / len(self.learning_progress) if self.learning_progress else 0
    ...
    def export_learning_data(self) -> Dict[str, Any]:
        ...
        try:
            export_data = {
                ...
                'extracted_knowledge': [asdict(knowledge) for knowledge in self.extracted_knowledge],
                'learning_progress': [asdict(progress) for progress in self.learning_progress],
        ...
        try:
            ...
            for content_data in data.get('text_contents', []):
                ...
                if 'publish_date' in content_data and content_data['publish_date']:
                    content_data['publish_date'] = datetime.fromisoformat(content_data['publish_date'])
...
def test_text_learning_system():
    ...
    print(f"   가족 관련 인사이트: {len(extracted_knowledge.family_relevant_insights)}개")
    ...
    learning_progress = text_learning.update_learning_progress(text_content.id, progress_data)
    ...
    print(f"✅ 학습 통계 생성: {statistics['total_contents']}개 콘텐츠, {statistics['total_knowledge']}개 지식")
