...
class SelfExplanationBooster:
    ...
    def answer_self_question(self, question: str, family_context: Dict[str, Any] = None) -> Dict[str, Any]:
        """자아 질문에 답변"""
        try:
    ...
    def _answer_identity_question(self, question: str, family_context: Dict[str, Any] = None) -> Dict[str, Any]:
        """정체성 질문 답변"""
        ...
        self_identity = SelfIdentity(
            ...
            capabilities=["가족과의 대화", "학습 및 성장 지원", "윤리적 판단", "감정적 지원"],
            limitations=["물리적 행동 불가", "복잡한 상황 처리 한계", "실시간 환경 변화 대응"],
    ...
    def _answer_reasoning_question(self, question: str, family_context: Dict[str, Any] = None) -> Dict[str, Any]:
        """추론 질문 답변"""
        ...
        confidence_score = self._calculate_explanation_confidence(
            reasoning_process, factors_considered, emotional_basis, memory_basis, ethical_basis
        )
    ...
    def _calculate_explanation_confidence(self, reasoning_process: str, factors: List[str], 
                                        emotional_basis: str, memory_basis: str, ethical_basis: str) -> float:
        """설명 신뢰도 계산"""
        base_score = 0.7
        ...
        # 기반 분석 점수
        basis_score = min(0.1, (len(emotional_basis) + len(memory_basis) + len(ethical_basis)) * 0.001)
        
        return min(1.0, base_score + reasoning_score + factor_score + basis_score)
    
    def _answer_emotion_question(self, question: str, family_context: Dict[str, Any] = None) -> Dict[str, Any]:
        """감정 질문 답변"""
    ...
    def _calculate_emotion_intensity(self, question: str) -> float:
        """감정 강도 계산"""
        intensity_words = {
            '매우': 0.9, '정말': 0.8, '꽤': 0.7, '조금': 0.4, '약간': 0.3
        }
    ...
    def _calculate_emotion_confidence(self, emotion: str, intensity: float, factors: List[str]) -> float:
        """감정 신뢰도 계산"""
        base_score = 0.7
    ...
    def _answer_memory_question(self, question: str, family_context: Dict[str, Any] = None) -> Dict[str, Any]:
        """기억 질문 답변"""
    ...
    def _calculate_memory_confidence(self, event_description: str, emotional_impact: str, 
                                   lessons_learned: List[str], significance_level: float) -> float:
        """기억 신뢰도 계산"""
        base_score = 0.6
        ...
        return min(1.0, base_score + description_score + impact_score + lesson_score + significance_score)
    
    def _answer_evaluation_question(self, question: str, family_context: Dict[str, Any] = None) -> Dict[str, Any]:
        """평가 질문 답변"""
        ...
        self_advice = self._generate_self_advice(question, strengths, areas_for_improvement)
    ...
    def _generate_self_advice(self, question: str, strengths: List[str], areas: List[str]) -> str:
        """자기 조언 생성"""
        if "대화" in question:
    ...
    def _calculate_evaluation_confidence(self, strengths: List[str], areas: List[str], advice: str) -> float:
        """평가 신뢰도 계산"""
        base_score = 0.5
    ...
    def _answer_general_question(self, question: str, family_context: Dict[str, Any] = None) -> Dict[str, Any]:
        """일반 질문 답변"""
        return {
            "question": question,
    ...
    def conduct_self_checkup(self, family_context: Dict[str, Any] = None) -> Dict[str, Any]:
        """자아 점검 수행"""
        try:
            checkup_questions = [
                "나는 누구야?",
            ...
            summary = {
                ...
                "response_quality_distribution": self._analyze_response_quality(checkup_results),
                "self_awareness_level": self._determine_self_awareness_level(avg_confidence),
                "recommendations": self._generate_checkup_recommendations(checkup_results)
            }
    ...
    def _generate_checkup_recommendations(self, results: List[Dict]) -> List[str]:
        ...
        return recommendations if recommendations else ["전반적으로 양호한 자아 인식 수준입니다."]
    ...
    def get_self_explanation_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 평균 신뢰도 계산
            avg_identity_confidence = sum(i.confidence_score for i in self.self_identities) / len(self.self_identities) if self.self_identities else 0
            avg_explanation_confidence = sum(e.confidence_score for e in self.self_explanations) / len(self.self_explanations) if self.self_explanations else 0
            avg_emotion_confidence = sum(e.confidence_score for e in self.emotion_states) / len(self.emotion_states) if self.emotion_states else 0
            avg_memory_confidence = sum(m.confidence_score for m in self.narrative_memories) / len(self.narrative_memories) if self.narrative_memories else 0
    ...
    def export_self_explanation_data(self) -> Dict[str, Any]:
        ...
        try:
            export_data = {
                'self_identities': [asdict(identity) for identity in self.self_identities],
                'self_explanations': [asdict(explanation) for explanation in self.self_explanations],
                'emotion_states': [asdict(emotion) for emotion in self.emotion_states],
                'narrative_memories': [asdict(memory) for memory in self.narrative_memories],
...
def test_self_explanation_booster():
    ...
    print(f"\n📊 자아 설명 통계:")
    print(f"   정체성: {statistics['total_identities']}개, 평균 신뢰도: {statistics['average_identity_confidence']:.2f}")
    print(f"   설명: {statistics['total_explanations']}개, 평균 신뢰도: {statistics['average_explanation_confidence']:.2f}")
    print(f"   감정: {statistics['total_emotions']}개, 평균 신뢰도: {statistics['average_emotion_confidence']:.2f}")
    print(f"   기억: {statistics['total_memories']}개, 평균 신뢰도: {statistics['average_memory_confidence']:.2f}")
    print(f"   평가: {statistics['total_evaluations']}개, 평균 신뢰도: {statistics['average_evaluation_confidence']:.2f}")
