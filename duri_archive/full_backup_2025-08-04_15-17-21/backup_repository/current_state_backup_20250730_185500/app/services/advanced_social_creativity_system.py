...
class AdvancedSocialCreativitySystem:
    ...
    def _calculate_creativity_score(
        ...
        # 가족 영향의 긍정성에 따른 점수
        positive_impact = any(
            word in family_impact.lower()
            for word in ["향상", "개선", "증진", "발전", "성장"]
        )
        ...
        creativity_score = (
            base_score
            * (0.6 + 0.2 * innovation_factor + 0.2 * implementation_factor)
            * impact_factor
        )
    ...
    def _calculate_innovation_metrics(
        ...
        category_adjustments = {
            InnovationCategory.PROCESS_IMPROVEMENT: {
                "feasibility": 0.1,
                "sustainability": 0.1,
            },
            InnovationCategory.COMMUNICATION_ENHANCEMENT: {
                "usefulness": 0.1,
                "impact": 0.1,
            },
            InnovationCategory.PROBLEM_RESOLUTION: {"novelty": 0.1, "usefulness": 0.1},
            InnovationCategory.RELATIONSHIP_BUILDING: {
                "impact": 0.15,
                "sustainability": 0.1,
            },
            InnovationCategory.LEARNING_OPTIMIZATION: {
                "usefulness": 0.1,
                "feasibility": 0.05,
            },
        ...
        pattern_adjustments = {
            ...
            CreativityPattern.ANALOGICAL_THINKING: {
                "usefulness": 0.05,
                "feasibility": 0.05,
            },
            CreativityPattern.SYNTHETIC_THINKING: {"impact": 0.1},
            CreativityPattern.INTEGRATIVE_THINKING: {
                "sustainability": 0.1,
                "impact": 0.05,
            },
        }
    ...
    def _determine_implementation_status(
        self, solution: CreativeSolution, innovation_metrics: Dict[str, float]
    ) -> str:
        """구현 상태 결정"""
    ...
    def _generate_success_indicators(
        self, solution: CreativeSolution, innovation_category: InnovationCategory
    ) -> List[str]:
        """성공 지표 생성"""
        base_indicators = ["가족 만족도 향상", "문제 해결 효과성", "지속 가능성 확보"]
    ...
    def _identify_family_benefits(
        self, solution: CreativeSolution, innovation_category: InnovationCategory
    ) -> List[str]:
        """가족 혜택 식별"""
        base_benefits = ["가족 관계 개선", "문제 해결 능력 향상"]
    ...
    def _calculate_innovation_effectiveness(
        self, solution: CreativeSolution, innovation_metrics: Dict[str, float]
    ) -> float:
        """혁신 효과성 계산"""
        # 가중 평균 계산
    ...
    def analyze_creativity_patterns(self) -> CreativityAnalysis:
        ...
        for solution in self.creative_solutions:
            impact_type = (
                "positive"
                if "향상" in solution.family_impact or "개선" in solution.family_impact
                else "neutral"
            )
    ...
    def _calculate_analysis_reliability(
        self,
        creativity_patterns: Dict[str, List[str]],
        innovation_trends: Dict[str, List[str]],
        family_impact_patterns: Dict[str, List[str]],
        ...
        """분석 신뢰도 계산"""
        total_patterns = (
            len(creativity_patterns)
            + len(innovation_trends)
            + len(family_impact_patterns)
        )
        ...
        for patterns in [
            creativity_patterns.values(),
            innovation_trends.values(),
            family_impact_patterns.values(),
        ]:
    ...
    def optimize_creativity_process(
        self,
        solution: CreativeSolution,
        optimization_type: str,
        optimization_description: str,
        ...
        # 개선 지표 계산
        improvement_metrics = self._calculate_creativity_improvement_metrics(
            solution, optimization_type
        )
        ...
        family_impact_score = self._calculate_family_impact_score(
            solution, optimization_type
        )
    ...
    def _calculate_creativity_improvement_metrics(
        self, solution: CreativeSolution, optimization_type: str
    ) -> Dict[str, float]:
        """창의성 개선 지표 계산"""
        base_metrics = {
        ...
        optimization_improvements = {
            "pattern_enhancement": {
                "creativity_efficiency": 0.15,
                "innovation_quality": 0.1,
            },
            ...
            "innovation_acceleration": {
                "innovation_quality": 0.2,
                "problem_solving_ability": 0.1,
            },
        }
    ...
    def _evaluate_creativity_level(
        self, solution: CreativeSolution, optimization_type: str
    ) -> CreativityLevel:
        """창의성 수준 평가"""
    ...
    def _calculate_innovation_rate(
        ...
        adjusted_rate = (
            innovation_rate
            * (0.7 + 0.3 * innovation_factor)
            * (0.8 + 0.2 * creativity_factor)
        )

        return min(adjusted_rate, 1.0)

    def _calculate_family_impact_score(
        self, solution: CreativeSolution, optimization_type: str
    ) -> float:
        """가족 영향 점수 계산"""
        base_score = 0.8
        ...
        # 가족 영향의 긍정성에 따른 조정
        positive_impact = any(
            word in solution.family_impact
            for word in ["향상", "개선", "증진", "발전", "성장"]
        )
        impact_factor = 1.1 if positive_impact else 0.9
    ...
    def get_social_creativity_statistics(self) -> Dict[str, Any]:
        ...
        # 평균 창의성 점수
        avg_creativity = sum(s.creativity_score for s in self.creative_solutions) / max(
            total_solutions, 1
        )

        # 평균 혁신 효과성
        avg_innovation_effectiveness = sum(
            p.innovation_effectiveness for p in self.innovation_projects
        ) / max(total_projects, 1)

        # 평균 가족 영향 점수
        avg_family_impact = sum(
            o.family_impact_score for o in self.creativity_optimizations
        ) / max(total_optimizations, 1)

        # 평균 최적화 신뢰도
        avg_optimization_confidence = sum(
            o.optimization_confidence for o in self.creativity_optimizations
        ) / max(total_optimizations, 1)
    ...
    def export_social_creativity_data(self) -> Dict[str, Any]:
        ...
        return {
            "creative_solutions": [
                asdict(solution) for solution in self.creative_solutions
            ],
            "innovation_projects": [
                asdict(project) for project in self.innovation_projects
            ],
            "creativity_analyses": [
                asdict(analysis) for analysis in self.creativity_analyses
            ],
            "creativity_optimizations": [
                asdict(optimization) for optimization in self.creativity_optimizations
            ],
...
def test_advanced_social_creativity_system():
    ...
    project = system.create_innovation_project(
        ...
        creative_patterns=[
            CreativityPattern.DIVERGENT_THINKING,
            CreativityPattern.COLLABORATIVE,
        ],
    )
    ...
    print(
        f"📊 통계: 해결책 {stats['total_solutions']}개, 프로젝트 {stats['total_projects']}개"
    )
