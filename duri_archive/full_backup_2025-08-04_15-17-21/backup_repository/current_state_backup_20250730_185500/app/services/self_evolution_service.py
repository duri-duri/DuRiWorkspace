...
class SelfEvolutionService:
    ...
    def _analyze_memory_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_stats = (
                ...
                .filter(MemoryEntry.created_at >= cutoff_time)
            ...
            avg_importance = (
                sum(stat.avg_importance * stat.count for stat in memory_stats)
                / total_memories
                if total_memories > 0
                else 0
            )
            avg_promotions = (
                sum(stat.avg_promotions * stat.count for stat in memory_stats)
                / total_memories
                if total_memories > 0
                else 0
            )
    ...
    def _analyze_emotional_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 감정 처리 효율성
            emotional_efficiency = len(emotional_memories) / max(
                1, len(emotional_memories) + 10
            )  # 처리된 감정 비율

            # 복합 감정 처리 능력
            complex_emotions = sum(
                1
                for memory in emotional_memories
                if memory.tags
                and len(
            ...
            return {
                ...
                "performance_score": (
                    ...
                    + emotional_efficiency * 0.3
                    + complex_emotion_ratio * 0.3
                )
                * 100,
            }
    ...
    def _analyze_evolution_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            evolution_memories = (
                ...
                .filter(
                    and_(
                        MemoryEntry.created_at >= cutoff_time,
                        MemoryEntry.tags.contains(
                            ["evolution", "learning", "improvement", "pattern"]
                        ),
                    )
                )
                .all()
            ...
            # 진화 효과성 (중요도 점수 기반)
            evolution_effectiveness = sum(
                memory.importance_score for memory in evolution_memories
            ) / max(1, len(evolution_memories))

            # 패턴 발견 능력
            pattern_memories = [
                m for m in evolution_memories if "pattern" in (m.tags or [])
            ]
            pattern_discovery_rate = len(pattern_memories) / max(
                1, len(evolution_memories)
            )
            ...
            return {
                ...
                "performance_score": (
                    evolution_frequency * 0.3
                    + evolution_effectiveness * 0.4
        ...
        try:
            ...
            return {
                "trend_direction": (
                    ...
                    else "declining" if trend_direction < 0 else "stable"
                ),
    ...
    def _measure_improvement_effect(
        ...
        try:
            ...
            total_expected_improvement = sum(
                imp.get("expected_improvement", 0) for imp in improvements_made
            )

            return {
                "total_improvements_applied": total_improvements,
                "total_expected_improvement": total_expected_improvement,
                "average_improvement_per_system": total_expected_improvement
                / max(1, total_improvements),
                "improvement_efficiency": min(
                    1.0, total_improvements / 5.0
                ),  # 최대 5개 시스템 개선 가능
            }
    ...
    def get_self_evolution_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "self_evolution_score": sum(evolution_progress.values())
                / len(evolution_progress),
            }
