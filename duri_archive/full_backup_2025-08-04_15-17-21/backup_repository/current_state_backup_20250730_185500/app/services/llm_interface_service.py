...
class LLMInterface:
    ...
    def ask_llm(
        ...
        try:
            ...
            response_text = self._simulate_llm_response(
                question, query_type, family_context, provider
            )
            ...
            # 가족 관련성 점수 계산
            family_relevance_score = self._calculate_family_relevance_score(
                response_text, family_context
            )
    ...
    def _evaluate_response_quality(
        self, response_text: str, query_type: QueryType
    ) -> ResponseQuality:
        """응답 품질 평가"""
        ...
        if word_count > 100 and any(
            word in response_text.lower() for word in ["가족", "사랑", "이해", "지지"]
        ):
    ...
    def _calculate_family_relevance_score(
        self, response_text: str, family_context: Dict[str, Any]
    ) -> float:
        """가족 관련성 점수 계산"""
        family_keywords = [
            "가족",
        ...
        relevant_words = sum(
            1 for keyword in family_keywords if keyword in response_text.lower()
        )

        return min(1.0, relevant_words * 0.1)

    def get_learning_help(
        self, question: str, family_context: Dict[str, Any] = None
    ) -> LLMResponse:
        """학습 도움 요청"""
        return self.ask_llm(
            question, QueryType.LEARNING_HELP, family_context, LLMProvider.CHATGPT
        )

    def get_family_advice(
        self, question: str, family_context: Dict[str, Any] = None
    ) -> LLMResponse:
        """가족 조언 요청"""
        return self.ask_llm(
            question, QueryType.FAMILY_ADVICE, family_context, LLMProvider.CLAUDE
        )
    ...
    def get_emotional_support(
        ...
        """정서적 지원 요청"""
        return self.ask_llm(
            question, QueryType.EMOTIONAL_SUPPORT, family_context, LLMProvider.CHATGPT
        )

    def get_creative_inspiration(
        self, question: str, family_context: Dict[str, Any] = None
    ) -> LLMResponse:
        """창의적 영감 요청"""
        return self.ask_llm(
            question, QueryType.CREATIVE_INSPIRATION, family_context, LLMProvider.GEMINI
        )
    ...
    def get_knowledge_answer(
        ...
        """지식 질문 답변"""
        return self.ask_llm(
            question, QueryType.KNOWLEDGE_QUESTION, family_context, LLMProvider.CHATGPT
        )
    ...
    def get_llm_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for provider in LLMProvider:
                provider_responses = [
                    r for r in self.responses if r.provider == provider
                ]
            ...
            for quality in ResponseQuality:
                quality_responses = [
                    r for r in self.responses if r.response_quality == quality
                ]
                quality_stats[quality.value] = len(quality_responses)

            # 평균 처리 시간
            avg_processing_time = (
                sum(r.processing_time_seconds for r in self.responses)
                / len(self.responses)
                if self.responses
                else 0
            )
            ...
            # 평균 가족 관련성
            avg_family_relevance = (
                sum(r.family_relevance_score for r in self.responses)
                / len(self.responses)
                if self.responses
                else 0
            )
        ...
        try:
            ...
            for query_data in data.get("queries", []):
                ...
                if "timestamp" in query_data:
                    query_data["timestamp"] = datetime.fromisoformat(
...
def test_llm_interface():
    ...
    learning_response = llm_interface.get_learning_help(
        learning_question, family_context
    )
    print(
        f"✅ 학습 도움 응답: {learning_response.response_quality.value} 품질, {learning_response.confidence_score:.2f} 신뢰도"
    )
    ...
    print(
        f"✅ 가족 조언 응답: {advice_response.response_quality.value} 품질, {advice_response.family_relevance_score:.2f} 가족 관련성"
    )

    # 3. 정서적 지원 요청
    emotional_question = "아이가 학교에서 친구와 다퉈서 속상해해요."
    emotional_response = llm_interface.get_emotional_support(
        emotional_question, family_context
    )
    print(
        f"✅ 정서적 지원 응답: {emotional_response.response_quality.value} 품질, {emotional_response.processing_time_seconds:.2f}초 처리시간"
    )

    # 4. 창의적 영감 요청
    creative_question = "주말에 가족과 함께할 수 있는 재미있는 활동을 제안해주세요."
    creative_response = llm_interface.get_creative_inspiration(
        creative_question, family_context
    )
    print(
        f"✅ 창의적 영감 응답: {creative_response.response_quality.value} 품질, {creative_response.confidence_score:.2f} 신뢰도"
    )

    # 5. 지식 질문
    knowledge_question = "아이의 창의력을 키우는 방법은 무엇인가요?"
    knowledge_response = llm_interface.get_knowledge_answer(
        knowledge_question, family_context
    )
    print(
        f"✅ 지식 질문 응답: {knowledge_response.response_quality.value} 품질, {knowledge_response.family_relevance_score:.2f} 가족 관련성"
    )

    # 6. LLM 사용 통계
    statistics = llm_interface.get_llm_statistics()
    print(
        f"✅ LLM 사용 통계: {statistics['total_queries']}개 질문, {statistics['total_responses']}개 응답"
    )
    ...
    print(
        f"✅ LLM 데이터 내보내기: {len(export_data['queries'])}개 질문, {len(export_data['responses'])}개 응답"
    )
