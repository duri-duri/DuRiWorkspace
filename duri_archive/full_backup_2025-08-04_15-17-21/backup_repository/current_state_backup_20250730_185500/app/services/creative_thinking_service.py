...
class CreativeThinkingService:
    ...
    def _analyze_problem_complexity(self, problem: str, domain: str) -> Dict[str, Any]:
        ...
        try:
            ...
            adjusted_complexity = complexity_score * domain_complexity_factors.get(
                domain, 1.0
            )
    ...
    def _analyze_constraints(self, constraints: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for constraint in constraints:
                ...
                if any(
                    word in constraint_lower
                    for word in ["기술", "technical", "code", "system"]
                ):
                    constraint_types["technical"] += 1
                elif any(
                    word in constraint_lower
                    for word in ["자원", "resource", "인력", "manpower"]
                ):
                    constraint_types["resource"] += 1
                elif any(
                    word in constraint_lower
                    for word in ["시간", "time", "deadline", "기한"]
                ):
                    constraint_types["time"] += 1
                elif any(
                    word in constraint_lower
                    for word in ["예산", "budget", "비용", "cost"]
                ):
                    constraint_types["budget"] += 1
                elif any(
                    word in constraint_lower
                    for word in ["법적", "legal", "규정", "regulation"]
                ):
            ...
            constraint_intensity = min(
                1.0, total_constraints / 5
            )  # 5개를 최대 강도로 가정
            ...
            return {
                ...
                "primary_constraint": (
                    max(constraint_types.items(), key=lambda x: x[1])[0]
    ...
    def _analyze_goals(self, goals: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for goal in goals:
                ...
                if any(
                    word in goal_lower
                    for word in ["효율", "efficiency", "성능", "performance"]
                ):
                    goal_types["efficiency"] += 1
                elif any(
                    word in goal_lower
                    for word in ["혁신", "innovation", "창의", "creative"]
                ):
                    goal_types["innovation"] += 1
                elif any(
                    word in goal_lower
                    for word in ["품질", "quality", "정확", "accuracy"]
                ):
                    goal_types["quality"] += 1
                elif any(
                    word in goal_lower
                    for word in ["비용절감", "cost", "경제", "economic"]
                ):
                    goal_types["cost_reduction"] += 1
                elif any(
                    word in goal_lower
                    for word in ["사용자경험", "user", "사용성", "usability"]
                ):
            ...
            primary_goal = (
                max(goal_types.items(), key=lambda x: x[1])[0]
                if total_goals > 0
                else "none"
            )
            ...
            return {
                ...
                "requires_creative_approach": goal_complexity > 0.5
                or goal_types["innovation"] > 0,
            }
    ...
    def _analyze_existing_patterns(
    ...
    def _analyze_problem_patterns(self, problem: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for component in key_components:
                ...
                if any(
                    word in component_lower
                    for word in ["최적화", "optimization", "개선", "improve"]
                ):
                    problem_types["optimization"] += 1
                elif any(
                    word in component_lower
                    for word in ["자동화", "automation", "자동", "auto"]
                ):
                    problem_types["automation"] += 1
                elif any(
                    word in component_lower
                    for word in ["통합", "integration", "연결", "connect"]
                ):
                    problem_types["integration"] += 1
                elif any(
                    word in component_lower
                    for word in ["사용자", "user", "경험", "experience"]
                ):
    ...
    def _analyze_pattern_connections(
        self, domain_patterns: Dict[str, Any], problem_patterns: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """패턴 연결 분석"""
        try:
            connections = []
                    ...
                    connections.append(
                        {
                            ...
                            "connection_type": (
                                "strong" if connection_strength > 0.6 else "moderate"
                            ),
                        }
                    )
    ...
    def _detect_innovation_opportunities(
        ...
        try:
            ...
            strong_connections = [
                conn
                for conn in pattern_connections
                if conn.get("connection_strength", 0) > 0.6
            ]

            for connection in strong_connections:
                for innovation_area in innovation_areas:
                    # 혁신 영역과 문제 유형의 적합성 평가
                    innovation_fit = self._calculate_connection_strength(
                        innovation_area, problem_type
                    )
                        ...
                        opportunities.append(
                            {
                                ...
                                "potential_impact": (
                                    "high" if innovation_fit > 0.7 else "medium"
                                ),
                            }
                        )
    ...
    def _generate_ideas(
        self, context_analysis: Dict[str, Any], pattern_analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """아이디어 생성"""
        try:
            ideas = []
            ...
            # 1. 패턴 기반 아이디어 생성
            pattern_based_ideas = self._generate_pattern_based_ideas(
                context_analysis, pattern_analysis
            )
            ideas.extend(pattern_based_ideas)

            # 2. 혁신 기반 아이디어 생성
            innovation_based_ideas = self._generate_innovation_based_ideas(
                context_analysis, pattern_analysis
            )
            ideas.extend(innovation_based_ideas)

            # 3. 제약 조건 기반 아이디어 생성
            constraint_based_ideas = self._generate_constraint_based_ideas(
                context_analysis
            )
    ...
    def _generate_constraint_based_ideas(
        self, context_analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """제약 조건 기반 아이디어 생성"""
        try:
    ...
    def _generate_goal_based_ideas(
        self, context_analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """목표 기반 아이디어 생성"""
        try:
    ...
    def _assess_innovation(
        self, ideas: List[Dict[str, Any]], pattern_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """혁신성 평가"""
        try:
            if not ideas:
            ...
            return {
                ...
                "high_innovation_ideas": len(
                    [score for score in innovation_scores if score >= 0.7]
                ),
            }
    ...
    def _analyze_feasibility(
        self, ideas: List[Dict[str, Any]], context_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """실현 가능성 분석"""
        try:
            if not ideas:
            ...
            return {
                ...
                "high_feasibility_ideas": len(
                    [score for score in feasibility_scores if score >= 0.7]
                ),
            }
    ...
    def _calculate_creativity_score(
        ...
        try:
            ...
            diversity_score = min(
                1.0, idea_diversity / 4
            )  # 4가지 타입을 최대 다양성으로 가정

            # 종합 창의성 점수 계산
            creativity_score = (
                innovation_score * 0.4 + feasibility_score * 0.3 + diversity_score * 0.3
            )
    ...
    def get_creative_thinking_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 창의성 점수 계산
            creativity_score = self._calculate_overall_creativity_score(
                creative_patterns
            )
            ...
            for memory in recent_memories:
                if memory.context and "creative" in memory.context.lower():
                    ...
                    creative_pattern = {
                        ...
                        "creativity_level": (
                            "high"
                            if "innovation" in memory.context.lower()
                            else "moderate"
                        ),
                    }
    ...
    def _calculate_overall_creativity_score(
        ...
        try:
            ...
            level_scores = [
                creativity_levels.get(p.get("creativity_level", "moderate"), 0.5)
                for p in patterns
            ]
