...
class EmotionalIntelligenceService:
    ...
    def _detect_emotion_conflicts(
        ...
        for emotion1, emotion2 in opposing_pairs:
            if (
                emotion_vector.get(emotion1, 0) > 0.3
                and emotion_vector.get(emotion2, 0) > 0.3
            ):
                conflicts.append(
                    {
                        ...
                        "severity": (
                            "moderate"
                            if min(emotion_vector[emotion1], emotion_vector[emotion2])
                            < 0.6
                            else "high"
                        ),
                    }
    ...
    def _calculate_emotion_stability(self, emotion_vector: Dict[str, float]) -> float:
        ...
        try:
            ...
            stability = max(
                0, 1 - (std_intensity / mean_intensity if mean_intensity > 0 else 0)
            )
    ...
    def _determine_complex_emotion_type(self, emotion_vector: Dict[str, float]) -> str:
        ...
        try:
            ...
            top_emotions = [
                emotion for emotion, intensity in sorted_emotions if intensity > 0.3
            ]
    ...
    def _are_opposing_emotions(self, emotion1: str, emotion2: str) -> bool:
        ...
        return (emotion1, emotion2) in opposing_pairs or (
            emotion2,
            emotion1,
        ) in opposing_pairs
    ...
    def _analyze_contextual_emotion(
        ...
        try:
            ...
            contextual_interpretation = {
                ...
                "social_context": self._interpret_social_emotion(
                    emotion_vector, social_context
                ),
                "personal_context": self._interpret_personal_emotion(
                    emotion_vector, context
                ),
            }
    ...
    def _interpret_work_emotion(
        ...
        work_satisfaction = emotion_vector.get("joy", 0) + emotion_vector.get(
            "trust", 0
        )
        ...
        return {
            ...
            "productivity_impact": (
                "positive" if work_satisfaction > work_stress else "negative"
            ),
    ...
    def _interpret_social_emotion(
        ...
        social_anxiety = emotion_vector.get("fear", 0) + emotion_vector.get(
            "sadness", 0
        )
        ...
        return {
            ...
            "social_readiness": (
                "ready" if social_comfort > social_anxiety else "cautious"
            ),
    ...
    def _interpret_personal_emotion(
        ...
        """개인적 맥락 감정 해석"""
        personal_wellbeing = emotion_vector.get("joy", 0) + emotion_vector.get(
            "trust", 0
        )
        personal_distress = emotion_vector.get("sadness", 0) + emotion_vector.get(
            "fear", 0
        )
    ...
    def _evaluate_context_fit(
        ...
        try:
            ...
            if context_type == "work":
                ...
                satisfaction = emotion_vector.get("joy", 0) + emotion_vector.get(
                    "trust", 0
                )
                ...
                anxiety = emotion_vector.get("fear", 0) + emotion_vector.get(
                    "sadness", 0
                )
                ...
                negative = emotion_vector.get("sadness", 0) + emotion_vector.get(
                    "fear", 0
                )
    ...
    def _calculate_emotion_reason_balance(
        ...
        try:
            ...
            # 균형 타입 결정
            balance_type = self._determine_balance_type(
                emotion_dominance, reason_dominance
            )
    ...
    def _determine_balance_type(
        self, emotion_dominance: float, reason_dominance: float
    ) -> str:
        """균형 타입 결정"""
    ...
    def _generate_empathetic_response(
        ...
        try:
            ...
            # 공감 효과 예측
            predicted_effect = self._predict_empathy_effect(
                empathetic_response, interpretation
            )
    ...
    def _create_empathetic_response(
        ...
        try:
            ...
            if social.get("anxiety_level", 0) > 0.5:
                responses.append(
                    "사회적 상황에서 불안하신 것 같습니다. 천천히 진행하세요"
                )

            # 업무 스트레스에 대한 공감
            if work.get("stress_level", 0) > 0.6:
                responses.append(
                    "업무 스트레스가 많으신 것 같습니다. 휴식이 필요합니다"
                )
    ...
    def _predict_empathy_effect(
        self, response: str, interpretation: Dict[str, Any]
    ) -> Dict[str, Any]:
        """공감 효과 예측"""
        try:
            personal = interpretation.get("personal_context", {})
            ...
            return {
                "predicted_wellbeing_improvement": predicted_wellbeing
                - current_wellbeing,
            ...
            for memory in recent_memories:
                if memory.emotion_data:
                    ...
                    emotion_pattern = {
                        ...
                        "complexity": len(
                            memory.emotion_data.get("secondary_emotions", [])
                        ),
    ...
    def _calculate_emotional_intelligence_score(
        ...
        try:
            ...
            # 종합 점수 계산
            total_score = (
                diversity_score * 0.4 + intensity_score * 0.3 + complexity_score * 0.3
            )
