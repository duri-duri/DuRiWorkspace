...
class AdvancedSocialLearningSystem:
    ...
    def record_social_learning_experience(
        self,
        learning_pattern: LearningPatternType,
        context: str,
        participants: List[str],
    ...
    def optimize_learning_process(
        ...
        improvement_metrics = self._calculate_improvement_metrics(
            experience, optimization_type
        )
        learning_quality = self._evaluate_learning_quality(
            experience, optimization_type
        )
    ...
    def _calculate_retention_rate(
        ...
        adjusted_retention = (
            retention_rate
            * (0.8 + 0.2 * insight_factor)
            * (0.9 + 0.1 * behavior_factor)
        )
    ...
    def _calculate_application_rate(
        ...
        # 행동 변화와 가족 영향에 따른 조정
        behavior_factor = min(len(experience.behavioral_changes) / 2, 1.0)
        family_impact_positive = (
            "긍정" in experience.family_impact or "향상" in experience.family_impact
        )
        impact_factor = 1.1 if family_impact_positive else 0.9

        adjusted_application = (
            application_rate * (0.8 + 0.2 * behavior_factor) * impact_factor
        )
    ...
    def _calculate_integration_effectiveness(
        ...
        adjusted_effectiveness = (
            base_effectiveness
            * (0.7 + 0.3 * source_factor)
            * (0.8 + 0.2 * knowledge_factor)
            * (0.9 + 0.1 * application_factor)
        )
    ...
    def analyze_learning_patterns(self) -> LearningPattern:
        ...
        for experience in self.social_learning_experiences:
            ...
            effectiveness_score = min(
                len(experience.insights_gained) / 3
                + len(experience.behavioral_changes) / 2,
                1.0,
            )
    ...
    def _calculate_pattern_reliability(
        ...
        """패턴 신뢰도 계산"""
        total_patterns = (
            len(effectiveness_patterns)
            + len(application_patterns)
    ...
    def get_social_learning_statistics(self) -> Dict[str, Any]:
        ...
        # 평균 보유율
        avg_retention = sum(
            o.retention_rate for o in self.learning_optimizations
        ) / max(total_optimizations, 1)

        # 평균 적용율
        avg_application = sum(
            o.application_rate for o in self.learning_optimizations
        ) / max(total_optimizations, 1)

        # 평균 신뢰도
        avg_confidence = sum(
            o.confidence_level for o in self.learning_optimizations
        ) / max(total_optimizations, 1)

        # 평균 통합 효과성
        avg_integration_effectiveness = sum(
            i.integration_effectiveness for i in self.knowledge_integrations
    ...
    def export_social_learning_data(self) -> Dict[str, Any]:
        """사회적 학습 데이터 내보내기"""
        return {
            "social_learning_experiences": [
                asdict(experience) for experience in self.social_learning_experiences
            ],
            "learning_optimizations": [
                asdict(optimization) for optimization in self.learning_optimizations
            ],
            "knowledge_integrations": [
                asdict(integration) for integration in self.knowledge_integrations
            ],
            "learning_patterns": [
...
def test_advanced_social_learning_system():
    ...
    print(
        f"📊 통계: 경험 {stats['total_experiences']}개, 최적화 {stats['total_optimizations']}개"
    )
