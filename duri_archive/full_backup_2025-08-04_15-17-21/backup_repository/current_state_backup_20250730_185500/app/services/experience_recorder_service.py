...
class GenerationalExperienceRecorder:
    ...
    def extract_lesson_from_experience(self, experience_id: str) -> ExtractedLesson:
        ...
        try:
            experience = next(
                (exp for exp in self.experiences if exp.id == experience_id), None
            )
            if not experience:
    ...
    def get_experience_insights(self, time_period_days: int = 30) -> Dict:
        ...
        try:
            ...
            recent_experiences = [
                exp for exp in self.experiences if exp.timestamp >= cutoff_date
            ]
            ...
            insights = {
                ...
                "family_interaction_patterns": self._analyze_family_patterns(
                    recent_experiences
                ),
    ...
    def _update_learning_progress(self, experience: Experience):
        ...
        progress_increment = experience.learning_value * (
            1 + abs(experience.emotional_impact)
        )
        self.learning_progress[category] = min(
            1.0, self.learning_progress[category] + progress_increment
        )
    ...
    def _identify_applicable_situations(self, experience: Experience) -> List[str]:
        ...
        if experience.type == ExperienceType.FAMILY_INTERACTION:
            situations.extend(["가족 갈등 해결", "가족 소통 개선", "가족 활동 계획"])

        if experience.type == ExperienceType.EMOTIONAL:
            situations.extend(["감정 관리", "스트레스 대처", "타인 감정 이해"])

        if experience.type == ExperienceType.PROBLEM_SOLVING:
            situations.extend(["문제 해결", "의사결정", "위기 대응"])
    ...
    def _is_family_specific(self, experience: Experience) -> bool:
        """가족 특화 여부 판단"""
        return (
            experience.type == ExperienceType.FAMILY_INTERACTION
            or "family" in experience.description.lower()
            or len(experience.participants) > 1
        )
    ...
    def _analyze_time_patterns(self) -> List[Dict]:
        ...
        # 시간대별 분석
        morning_experiences = [
            exp for exp in self.experiences if exp.timestamp.hour < 12
        ]
        afternoon_experiences = [
            exp for exp in self.experiences if 12 <= exp.timestamp.hour < 18
        ]
        evening_experiences = [
            exp for exp in self.experiences if exp.timestamp.hour >= 18
        ]

        for time_period, experiences in [
            ("morning", morning_experiences),
            ("afternoon", afternoon_experiences),
            ("evening", evening_experiences),
        ]:
            if len(experiences) >= 3:
                avg_emotional = sum(exp.emotional_impact for exp in experiences) / len(
                    experiences
                )
                avg_learning = sum(exp.learning_value for exp in experiences) / len(
                    experiences
                )
    ...
    def _analyze_family_patterns(
        ...
        family_experiences = [
            exp for exp in experiences if exp.type == ExperienceType.FAMILY_INTERACTION
        ]

        if len(family_experiences) >= 3:
            avg_emotional = sum(
                exp.emotional_impact for exp in family_experiences
            ) / len(family_experiences)
            avg_learning = sum(exp.learning_value for exp in family_experiences) / len(
                family_experiences
            )
            ...
            return [
                {
                    ...
                    "family_members": list(
                        set(
                            [
                                member
                                for exp in family_experiences
                                for member in exp.participants
                            ]
                        )
                    ),
                    "significance": (
                        len(family_experiences) / len(experiences) if experiences else 0
                    ),
                }
            ]
    ...
    def _analyze_emotional_patterns(self) -> List[Dict]:
        ...
        positive_experiences = [
            exp for exp in self.experiences if exp.emotional_impact > 0.3
        ]
        ...
        if len(positive_experiences) >= 3:
            avg_learning = sum(
                exp.learning_value for exp in positive_experiences
            ) / len(positive_experiences)
            patterns.append(
                {
                    ...
                    "avg_learning_value": avg_learning,
                    "common_elements": [
                        exp.type.value for exp in positive_experiences[:3]
                    ],
                    "family_members": [],
        ...
        if len(negative_experiences) >= 3:
            avg_learning = sum(
                exp.learning_value for exp in negative_experiences
            ) / len(negative_experiences)
            patterns.append(
                {
                    ...
                    "avg_learning_value": avg_learning,
                    "common_elements": [
                        exp.type.value for exp in negative_experiences[:3]
                    ],
                    "family_members": [],
    ...
    def _analyze_emotional_trends(self, experiences: List[Experience]) -> Dict:
        ...
        early_avg = sum(
            ...
            for exp in sorted_experiences[: len(sorted_experiences) // 2]
        ) / (len(sorted_experiences) // 2)
        late_avg = sum(
            exp.emotional_impact
            for exp in sorted_experiences[len(sorted_experiences) // 2 :]
        ) / (len(sorted_experiences) // 2)
        ...
        return {
            "trend": trend,
            "average_impact": sum(exp.emotional_impact for exp in experiences)
            / len(experiences),
    ...
    def _get_recent_learning_focus(self, experiences: List[Experience]) -> List[str]:
        ...
        recent_experiences = [
            exp for exp in experiences if exp.timestamp >= recent_cutoff
        ]
        ...
        sorted_categories = sorted(
            category_learning.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _get_top_lessons(self, experiences: List[Experience]) -> List[Dict]:
        ...
        high_value_experiences = [
            exp for exp in experiences if exp.learning_value > 0.7
        ]

        # 학습 가치 순으로 정렬
        sorted_experiences = sorted(
            high_value_experiences, key=lambda x: x.learning_value, reverse=True
        )
    ...
    def _collect_life_lessons(self) -> List[Dict]:
        ...
        life_lessons = [
            ...
            if lesson.lesson_type == LessonType.LIFE_LESSON
        ]
    ...
    def _collect_skill_lessons(self) -> List[Dict]:
        ...
        skill_lessons = [
            lesson
            for lesson in self.extracted_lessons
            if lesson.lesson_type == LessonType.SKILL_LESSON
        ]
    ...
    def _collect_emotional_lessons(self) -> List[Dict]:
        ...
        emotional_lessons = [
            lesson
            for lesson in self.extracted_lessons
            if lesson.lesson_type == LessonType.EMOTIONAL_LESSON
        ]
    ...
    def _collect_relationship_lessons(self) -> List[Dict]:
        ...
        relationship_lessons = [
            lesson
            for lesson in self.extracted_lessons
            if lesson.lesson_type == LessonType.RELATIONSHIP_LESSON
        ]
    ...
    def _collect_family_specific_lessons(self) -> List[Dict]:
        """가족 특화 교훈 수집"""
        family_lessons = [
            lesson for lesson in self.extracted_lessons if lesson.family_specific
        ]
    ...
    def _calculate_wisdom_strength(self) -> float:
        ...
        avg_generational_value = sum(
            l.generational_value for l in self.extracted_lessons
        ...
        family_factor = len(
            [l for l in self.extracted_lessons if l.family_specific]
        ) / len(self.extracted_lessons)
    ...
    def export_experience_data(self) -> Dict:
        ...
        return {
            ...
            "experience_patterns": [
                asdict(pattern) for pattern in self.experience_patterns
            ],
