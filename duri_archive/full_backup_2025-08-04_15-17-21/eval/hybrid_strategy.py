...
class HybridStrategyExecutor:
    ...
    def start_hybrid_execution(self, reality_strategy: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Dream Engine 시작
            self.dream_engine.start_dream_engine(
                evaluation_callback=self._dream_evaluation_callback
            )

            # 하이브리드 실행 시작
            self.is_running = True
            self.execution_thread = threading.Thread(
                target=self._run_hybrid_execution, args=(execution_id,)
            )
    ...
    def _execute_reality_strategies(self):
        ...
        for strategy_id, execution in self.reality_strategies.items():
            if execution.execution_status == ExecutionStatus.RUNNING:
                try:
                    # 현실 전략 실행 (시뮬레이션)
                    performance_score = self._simulate_strategy_execution(
                    ...
                    logger.debug(
                        f"현실 전략 {strategy_id} 실행 완료: {performance_score:.2f}"
                    )
        ...
        for strategy_id, execution in self.dream_strategies.items():
            if execution.execution_status == ExecutionStatus.RUNNING:
                try:
                    ...
                    logger.debug(
                        f"Dream 전략 {strategy_id} 실행 완료: {performance_score:.2f}"
                    )
    ...
    def _simulate_strategy_execution(self, strategy_data: Dict[str, Any]) -> float:
        ...
        # 랜덤 변동 추가
        random_factor = 0.1
        performance_score = base_score * weight + random.uniform(
    ...
    def _compare_strategies(self):
        ...
        completed_reality = [
            execution
            for execution in self.reality_strategies.values()
        ...
        completed_dreams = [
            execution
            for execution in self.dream_strategies.values()
            if execution.execution_status == ExecutionStatus.COMPLETED
        ]
        ...
        for reality_execution in completed_reality:
            for dream_execution in completed_dreams:
                # 이미 비교된 조합인지 확인
                comparison_key = (
                    f"{reality_execution.strategy_id}_{dream_execution.strategy_id}"
                )
                if any(
                    c.reality_strategy_id == reality_execution.strategy_id
                    and c.dream_strategy_id == dream_execution.strategy_id
                ...
                comparison_result = self._execute_comparison(
                    reality_execution, dream_execution
                )
                if comparison_result:
                    self.comparison_history.append(comparison_result)

    def _execute_comparison(
        self, reality_execution: StrategyExecution, dream_execution: StrategyExecution
        ...
        try:
            ...
            if dream_score > reality_score:
                ...
                promotion_reason = (
                    f"Dream 전략이 현실 전략보다 {score_difference:.2f}점 높음"
                )
            elif reality_score > dream_score:
                winner = StrategyType.REALITY
                promotion_reason = (
                    f"현실 전략이 Dream 전략보다 {score_difference:.2f}점 높음"
                )
            else:
            ...
            logger.info(
                f"전략 비교 완료: {winner.value} 승리 (차이: {score_difference:.2f})"
            )
    ...
    def _make_promotion_decisions(self):
        ...
        for comparison in self.comparison_history:
            ...
            if (
                ...
                < self.execution_config["min_score_difference"]
            ):
                continue
    ...
    def get_hybrid_statistics(self) -> Dict[str, Any]:
        ...
        for status in ExecutionStatus:
            reality_status[status.value] = len(
                [
                    s
                    for s in self.reality_strategies.values()
                    if s.execution_status == status
                ]
            )
            dream_status[status.value] = len(
                [
                    s
                    for s in self.dream_strategies.values()
                    if s.execution_status == status
                ]
            )
        ...
        # 승격 통계
        promotions = len(
            [c for c in self.comparison_history if c.winner == StrategyType.DREAM]
        )
        ...
        return {
            ...
            "average_score_difference": (
                sum(c.score_difference for c in self.comparison_history)
                / total_comparisons
