...
from duri_core.utils.evaluation_lock import (
    get_evaluation_lock,
    get_evaluation_state_manager,
)
...
class CoreEval:
        ...
        try:
            ...
            with self.evaluation_lock.evaluation_context(evaluation_id):
                ...
                performance_score = self._calculate_performance_score(
                    dream_data, current_performance
                )
                ...
                # 3. 유레카 감지
                eureka_detected = self._detect_eureka(
                    combined_score, novelty_score, performance_score
                )

                # 4. 평가 유형 결정
                evaluation_type = self._determine_evaluation_type(
                    dream_data, eureka_detected
                )
                ...
                logger.info(
                    f"Dream 전략 평가 완료: {dream_id}, 결과: {result.value}, 점수: {combined_score:.3f}"
                )
                return decision
            ...
            # Fallback 평가 시도
            try:
                return self.evaluate_dream_strategy_fallback(
                    dream_data, current_performance, context
                )
    ...
    def _calculate_performance_score(
        self, dream_data: Dict[str, Any], current_performance: float
    ) -> float:
        """성과 점수를 계산합니다."""
        # 기본 성과 점수
    ...
    def _detect_eureka(
        self, combined_score: float, novelty_score: float, performance_score: float
    ) -> bool:
        """유레카를 감지합니다."""
        ...
        if (
            combined_score >= eureka_threshold
        ...
        # 예상 외의 고성능 조합
        if combined_score >= 0.9 and novelty_score >= 0.9:
            return True

        return False

    def _determine_evaluation_type(
        self, dream_data: Dict[str, Any], eureka_detected: bool
    ) -> EvaluationType:
        """평가 유형을 결정합니다."""
        if eureka_detected:
    ...
    def _determine_evaluation_result(
        ...
        if combined_score >= criteria.adoption_threshold:
            reasoning.append(
                f"종합 점수 {combined_score:.2f}로 채택 임계값 {criteria.adoption_threshold} 초과"
            )
        ...
        if combined_score <= criteria.rejection_threshold:
            reasoning.append(
                f"종합 점수 {combined_score:.2f}로 거부 임계값 {criteria.rejection_threshold} 미만"
            )
    ...
    def evaluate_dream_strategy_fallback(
        self,
        dream_data: Dict[str, Any],
        current_performance: float = 0.0,
        context: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            # 제한된 데이터로 기본 점수 계산
            performance_score = self._calculate_fallback_performance_score(
                dream_data, current_performance
            )
            ...
            logger.info(
                f"Fallback 평가 완료: {dream_id}, 결과: {result.value}, 점수: {combined_score:.3f}"
            )
            return decision
    ...
    def _calculate_fallback_performance_score(
        self, dream_data: Dict[str, Any], current_performance: float
    ) -> float:
    ...
    def get_evaluation_statistics(self) -> Dict[str, Any]:
        ...
        # 평균 신뢰도
        avg_confidence = (
            sum(d.confidence for d in self.evaluation_history)
        ...
        # 평균 종합 점수
        avg_combined_score = (
            sum(d.combined_score for d in self.evaluation_history)
            / len(self.evaluation_history)
            if self.evaluation_history
            else 0
        )
    ...
    def cleanup_expired_evaluations(self):
        ...
        ttl_threshold = current_time - timedelta(
            hours=self.evaluation_criteria.ttl_hours
        )
        expired_count = 0
        ...
        self.eureka_detections = [
            dream_id
            for dream_id in self.eureka_detections
            if any(
                eval_result.dream_id == dream_id
                and eval_result.evaluation_time > ttl_threshold
                for eval_result in self.evaluation_history
            )
        ]
        ...
        self.adoption_history = [
            dream_id
            for dream_id in self.adoption_history
            if any(
                eval_result.dream_id == dream_id
                and eval_result.evaluation_time > ttl_threshold
                for eval_result in self.evaluation_history
    ...
    def enforce_max_candidates(self):
        ...
        if len(self.evaluation_history) > self.evaluation_criteria.max_dream_candidates:
            ...
            sorted_evaluations = sorted(
                self.evaluation_history, key=lambda x: x.evaluation_time
            )
            excess_count = (
                len(self.evaluation_history)
                - self.evaluation_criteria.max_dream_candidates
            )
