...
class GPTCursorWorkflow:
    ...
    def generate_gpt_recommendation(
    ...
    def create_refactoring_task(
        self, analysis: CodeAnalysis, refactoring_type: RefactoringType
    ) -> RefactoringTask:
        """리팩토링 작업 생성"""
    ...
    def execute_workflow_stage(
        ...
        """워크플로우 단계 실행"""
        stage_id = (
            f"workflow_stage_{stage.value}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _execute_design_stage(
        self, context: Dict[str, Any], recommendation: GPTRecommendation
    ) -> Dict[str, Any]:
        """설계 단계 실행"""
        return {
            "status": "completed",
    ...
    def _execute_implementation_stage(
        self, context: Dict[str, Any], recommendation: GPTRecommendation
    ) -> Dict[str, Any]:
        """구현 단계 실행"""
        return {
    ...
    def _execute_refactoring_stage(
        self, context: Dict[str, Any], recommendation: GPTRecommendation
    ) -> Dict[str, Any]:
        """리팩토링 단계 실행"""
        return {
    ...
    def get_workflow_statistics(self) -> Dict[str, Any]:
        ...
        for stage in WorkflowStage:
            stage_recommendations = [
                r for r in self.recommendations if r.stage == stage
            ]
...
"""
    ...
    # 3. 리팩토링 작업 생성
    refactoring_task = workflow.create_refactoring_task(
        analysis, RefactoringType.CODE_STRUCTURE
    )
    ...
    # 4. 워크플로우 단계 실행
    workflow_result = workflow.execute_workflow_stage(
        WorkflowStage.IMPLEMENTATION, context
    )
    ...
    print(
        f"✅ 워크플로우 데이터 내보내기: {len(export_data['recommendations'])}개 추천사항"
    )
