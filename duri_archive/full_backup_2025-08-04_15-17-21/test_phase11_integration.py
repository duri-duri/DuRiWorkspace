...
from app.services.developmental_thinking_conversation_service import (
    DevelopmentalThinkingConversationSystem,
)
from app.services.family_conversation_precision_service import (
...
def test_phase11_integration():
    ...
    sample_subtitles = [
        {
            "start_time": 0.0,
            "end_time": 30.0,
            "text": "안녕하세요! 오늘은 가족과 함께할 수 있는 창의적인 놀이를 소개해드릴게요.",
        },
        {
            "start_time": 30.0,
            "end_time": 60.0,
    ...
    visual_knowledge = subtitle_learning.extract_visual_knowledge_from_video(
        video_content.id
    )
    ...
    # 자막 학습 → 대화 시스템 연동
    visual_recommendations = subtitle_learning.get_visual_learning_recommendations(
        family_context
    )
    ...
    print(
        f"✅ 시스템 활성화율: {system_activation_rate:.2f} ({active_systems}/{total_systems})"
    )
