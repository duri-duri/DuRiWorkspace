...
class DreamEngine:
        ...
        for dream_type in DreamType:
            if random.random() < 0.25:  # 25% 확률로 생성
                ...
                if dream_candidate:
                    ...
                    logger.debug(f"Dream 후보 생성: {dream_candidate.dream_id} ({dream_type.value})")
    
    def _create_dream_candidate(self, dream_type: DreamType) -> Optional[DreamCandidate]:
        """Dream 후보를 생성합니다."""
        try:
            # Dream 생성기 호출
    ...
    def _generate_concept_fusion(self) -> Dict[str, Any]:
        ...
        # 융합할 개념들
        concepts = [
            'learning', 'creativity', 'efficiency', 'innovation',
            'stability', 'adaptability', 'precision', 'flexibility'
        ]
    ...
    def _generate_intuition_exploration(self) -> Dict[str, Any]:
        ...
        # 직관적 요소들
        intuition_elements = [
            'unexpected_combination', 'counter_intuitive_approach',
            'simplified_complexity', 'complex_simplicity',
            'reverse_logic', 'lateral_thinking'
        ]
    ...
    def _evaluate_dream_candidates(self):
        ...
        for dream_id, candidate in list(self.dream_candidates.items()):
            # 평가 간격 확인
            if (candidate.last_evaluation and 
                (current_time - candidate.last_evaluation).seconds < self.dream_config['evaluation_interval']):
                continue
    ...
    def _evaluate_candidate(self, candidate: DreamCandidate) -> Optional[Dict[str, Any]]:
        """후보를 평가합니다."""
        try:
    ...
    def _default_evaluation(self, candidate: DreamCandidate) -> Dict[str, Any]:
        """기본 평가 로직"""
        # 종합 점수 계산
        total_score = (candidate.confidence_score * 0.4 + 
    ...
    def _process_evaluation_result(self, candidate: DreamCandidate, evaluation_result: Dict[str, Any]):
        """평가 결과를 처리합니다."""
    ...
    def _cleanup_candidates(self):
        """후보를 정리합니다."""
        # 폐기된 후보 제거
        discarded_candidates = [dream_id for dream_id, candidate in self.dream_candidates.items() 
        ...
        # 승격된 후보 기록
        promoted_candidates = [candidate for candidate in self.dream_candidates.values() 
    ...
    def get_dream_statistics(self) -> Dict[str, Any]:
        ...
        active_candidates = len([c for c in self.dream_candidates.values() if c.status == DreamStatus.ACTIVE])
        promoted_candidates = len([c for c in self.dream_candidates.values() if c.status == DreamStatus.PROMOTED])
        discarded_candidates = len([c for c in self.dream_candidates.values() if c.status == DreamStatus.DISCARDED])
        ...
        type_stats = {}
        for dream_type in DreamType:
            type_count = len([c for c in self.dream_candidates.values() if c.dream_type == dream_type])
