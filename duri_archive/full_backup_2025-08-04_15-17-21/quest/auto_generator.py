...
class QuestAutoGenerator:
        ...
        self.quest_templates[QuestCategory.EMOTIONAL_GROWTH] = [
            {
                ...
                "duration": 30,
                "success_criteria": ["감정을 정확히 식별", "감정 변화 패턴 발견", "자신의 감정에 대한 통찰 얻기"],
                "reflection_prompts": ["어떤 감정이 가장 자주 나타났나요?", "감정 변화의 원인은 무엇인가요?", "감정을 조절하는 방법을 발견했나요?"]
            },
            {
                "title": "공감 능력 향상하기",
                ...
                "type": QuestType.PRACTICE,
                "duration": 60,
                "success_criteria": ["다른 사람의 감정을 정확히 파악", "적절한 공감 표현", "관계 개선 경험"],
                "reflection_prompts": ["공감할 때 어떤 어려움이 있었나요?", "공감 능력이 향상된 것을 느꼈나요?", "앞으로 어떻게 개선할 수 있을까요?"]
            }
        ]
        ...
        self.quest_templates[QuestCategory.COGNITIVE_DEVELOPMENT] = [
            {
                ...
                "duration": 90,
                "success_criteria": ["새로운 개념을 정확히 이해", "실제 적용 가능성 발견", "학습 과정에서의 통찰"],
                "reflection_prompts": ["어떤 부분이 가장 어려웠나요?", "새로운 관점을 얻었나요?", "이 지식을 어떻게 활용할 수 있을까요?"]
            },
            {
                "title": "복잡한 문제 분석하기",
                ...
                "duration": 120,
                "success_criteria": ["문제의 핵심 파악", "체계적 분석", "실행 가능한 해결책 제시"],
                "reflection_prompts": ["문제 분석 과정에서 무엇을 배웠나요?", "어떤 분석 도구가 유용했나요?", "다음에는 어떻게 개선할 수 있을까요?"]
            }
        ]
        ...
        self.quest_templates[QuestCategory.SOCIAL_SKILLS] = [
            {
                ...
                "type": QuestType.PRACTICE,
                "duration": 60,
                "success_criteria": ["적극적 경청 실천", "명확한 의사전달", "대화 품질 향상"],
                "reflection_prompts": ["대화에서 어떤 어려움이 있었나요?", "대화 기술이 향상된 것을 느꼈나요?", "앞으로 어떤 부분을 개선하고 싶나요?"]
            }
        ]
        ...
        self.quest_templates[QuestCategory.CREATIVITY] = [
            {
                ...
                "duration": 180,
                "success_criteria": ["독창적인 아이디어 생성", "구체적 실행 계획", "창의적 결과물 완성"],
                "reflection_prompts": ["창의적 아이디어는 어떻게 떠올렸나요?", "실행 과정에서 어떤 어려움이 있었나요?", "결과물에 만족하나요?"]
            }
        ]
        ...
        self.quest_templates[QuestCategory.PROBLEM_SOLVING] = [
            {
                ...
                "duration": 120,
                "success_criteria": ["문제의 정확한 파악", "다양한 해결 방안 탐색", "효과적인 해결책 실행"],
                "reflection_prompts": ["문제 해결 과정에서 무엇을 배웠나요?", "어떤 해결 방법이 가장 효과적이었나요?", "다음에는 어떻게 개선할 수 있을까요?"]
            }
        ]
        ...
        self.quest_templates[QuestCategory.SELF_REFLECTION] = [
            {
                ...
                "reflection_prompts": ["자기 성찰을 통해 무엇을 발견했나요?", "어떤 패턴이 보이나요?", "앞으로 어떻게 변화하고 싶나요?"]
            }
        ]
    
    def generate_quest_from_reflection(self, 
                                      reflection_data: Dict[str, Any],
                                      current_level: int,
                                      emotional_state: str) -> AutoQuest:
        """성찰 결과 기반 퀘스트 생성"""
        # 성찰 데이터 분석
        ...
        # 적절한 카테고리 선택
        category = self._select_quest_category(reflection_type, insights, emotional_state)
        
        # 난이도 결정
        difficulty = self._determine_quest_difficulty(current_level, emotional_state, insights)
        
        # 템플릿 선택 및 커스터마이징
        quest_template = self._select_and_customize_template(category, difficulty, insights, action_items)
        ...
        quest = AutoQuest(
            ...
            requirements=self._generate_requirements(category, difficulty, current_level),
        ...
        logger.info(f"자동 퀘스트 생성 완료: {quest_id} - {category.value} - {difficulty.value}")
        return quest
    
    def _select_quest_category(self, reflection_type: str, insights: List[str], emotional_state: str) -> QuestCategory:
        """퀘스트 카테고리 선택"""
        # 성찰 유형에 따른 기본 카테고리
        category_mapping = {
        ...
        base_category = category_mapping.get(reflection_type, QuestCategory.SELF_REFLECTION)
    ...
    def _determine_quest_difficulty(self, current_level: int, emotional_state: str, insights: List[str]) -> QuestDifficulty:
        """퀘스트 난이도 결정"""
        # 레벨 기반 기본 난이도
        if current_level <= 2:
    ...
    def _select_and_customize_template(self, category: QuestCategory, difficulty: QuestDifficulty, 
                                      insights: List[str], action_items: List[str]) -> Dict[str, Any]:
        """템플릿 선택 및 커스터마이징"""
        if category not in self.quest_templates:
            ...
            return {
                ...
                "reflection_prompts": ["무엇을 배웠나요?", "어떤 어려움이 있었나요?", "앞으로 어떻게 개선할 수 있을까요?"]
            }
        ...
        # 난이도에 맞는 템플릿 필터링
        suitable_templates = [t for t in available_templates if t["difficulty"] == difficulty]
        
        if not suitable_templates:
            # 난이도에 맞는 템플릿이 없으면 가장 가까운 것 선택
            difficulty_order = [QuestDifficulty.EASY, QuestDifficulty.MEDIUM, QuestDifficulty.HARD, QuestDifficulty.EXPERT]
            current_index = difficulty_order.index(difficulty)
            
            for i in range(len(difficulty_order)):
                test_difficulty = difficulty_order[(current_index + i) % len(difficulty_order)]
                suitable_templates = [t for t in available_templates if t["difficulty"] == test_difficulty]
        ...
        if insights:
            ...
            customized_template["description"] += f" 이전 성찰에서 '{insight_summary}'를 발견했습니다."
    ...
    def _generate_requirements(self, category: QuestCategory, difficulty: QuestDifficulty, current_level: int) -> List[str]:
        """요구사항 생성"""
    ...
    def _generate_rewards(self, category: QuestCategory, difficulty: QuestDifficulty) -> Dict[str, Any]:
        """보상 생성"""
        base_experience = {
    ...
    def get_recommended_categories(self, reflection_history: List[Dict[str, Any]]) -> List[QuestCategory]:
        """추천 카테고리 조회"""
