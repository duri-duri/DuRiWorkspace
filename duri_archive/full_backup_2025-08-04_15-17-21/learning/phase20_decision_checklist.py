...
class Phase20DecisionChecklist:
    ...
    def initialize_phase_20_integration(self):
        ...
        try:
            ...
            sys.path.append(".")
            from duri_brain.learning.experience_based_decision_learning import (
                get_experience_learning_system,
            )
    ...
    def run_quantitative_evaluation(self) -> QuantitativeMetrics:
        ...
        # 종합 점수 계산
        overall_score = (
            decision_confidence + decision_success_rate + decision_consistency
        ) / 3
    ...
    def run_qualitative_evaluation(self) -> List[DecisionTest]:
        ...
        for test_data in qualitative_tests:
            ...
            result, score, explanation = self._evaluate_answer(
                test_data["expected_answer"], actual_answer
            )
    ...
    def _evaluate_answer(
        ...
        matched_keywords = sum(
            1 for keyword in expected_keywords if keyword in actual_lower
        )
        match_rate = (
            matched_keywords / len(expected_keywords) if expected_keywords else 0
        )
    ...
    def _evaluate_explanation(
        ...
        included_elements = sum(
            1 for element in required_elements if element in actual_lower
        )
    ...
    def run_complete_evaluation(self) -> Dict[str, Any]:
        ...
        overall_result = self._evaluate_overall_performance(
            quantitative_result, qualitative_result, self_explanation_result
        )
    ...
    def _evaluate_overall_performance(
        ...
        qualitative: List[DecisionTest],
        self_explanation: List[DecisionTest],
    ) -> Dict[str, Any]:
        """전체 성능 평가"""
        # 정량 평가 통과 여부
        quantitative_pass = (
            quantitative.decision_confidence >= 0.700
            and quantitative.decision_success_rate >= 0.70
            and quantitative.decision_consistency >= 0.80
        )

        # 정성 평가 통과 여부
        qualitative_scores = [test.score for test in qualitative]
        qualitative_avg = (
            sum(qualitative_scores) / len(qualitative_scores)
        ...
        explanation_avg = (
            sum(explanation_scores) / len(explanation_scores)
    ...
    def _save_failure_result(self, result: Dict[str, Any]):
        ...
        try:
            ...
            with open(
                "failure_log/phase20_decision_fail.log", "w", encoding="utf-8"
            ) as f:
...
if __name__ == "__main__":
    ...
    if "error" not in result:
        if result["overall_pass"]:
            ...
            print(
                f"   정량 평가: {'통과' if result['quantitative']['pass'] else '실패'}"
            )
            print(
                f"   정성 평가: {'통과' if result['qualitative']['pass'] else '실패'}"
            )
            print(
                f"   자기설명 평가: {'통과' if result['self_explanation']['pass'] else '실패'}"
            )
        else:
            print("❌ Phase 20 체크리스트 실패")
            print(
                "   개선 제안이 failure_log/phase20_decision_fail.log에 저장되었습니다"
            )
    else:
