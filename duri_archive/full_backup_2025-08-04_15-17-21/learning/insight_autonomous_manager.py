...
class InsightAutonomousManager:
    ...
    def evaluate_insight(self, insight: Dict[str, Any]) -> InsightEvaluation:
        ...
        logger.info(
            f"🔍 평가 완료 - 종합 점수: {total_score:.3f}, 신뢰도: {confidence:.3f}"
        )
        return evaluation
    ...
    def _evaluate_relevance(self, insight: Dict[str, Any]) -> float:
        ...
        return min(
            (relevance_count / len(learning_keywords)) * 0.6 + problem_relevance * 0.4,
            1.0,
        )
    ...
    def _evaluate_feasibility(self, insight: Dict[str, Any]) -> float:
        ...
        complexity_score = sum(
            1 for indicator in complexity_indicators if indicator in strategy
        ) / len(complexity_indicators)

        # 리소스 요구사항 평가
        resource_indicators = ["메모리", "비용", "시간", "계산"]
        resource_score = sum(
            1 for indicator in resource_indicators if indicator in strategy
    ...
    def _evaluate_novelty(self, insight: Dict[str, Any]) -> float:
        ...
        # 기존 통찰과의 차별성
        existing_insights = list(self.stored_insights.values()) + list(
            self.held_insights.values()
        )
        ...
        for existing in existing_insights:
            if "strategy" in existing and "strategy" in insight:
                # 간단한 유사도 계산
                common_words = set(existing["strategy"].split()) & set(
                    insight["strategy"].split()
                )
                total_words = set(existing["strategy"].split()) | set(
                    insight["strategy"].split()
                )
                if total_words:
        ...
        novelty_score = (novelty_count / len(novelty_keywords)) * 0.7 + (
            1 - similarity_score
        ) * 0.3
    ...
    def make_decision(
        self, insight: Dict[str, Any], evaluation: InsightEvaluation
    ) -> InsightDecision:
        """통찰에 대한 결정"""
        ...
        decision = InsightDecision(
            insight_id=insight.get(
                "session_id", f"insight_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ),
            action=action,
            category=category,
            priority=priority,
        ...
        logger.info(
            f"🎯 결정 완료: {action.value} - {category.value} - {priority.value}"
        )
        return decision
    ...
    def _determine_priority(
        self, evaluation: InsightEvaluation, category: InsightCategory
    ) -> InsightPriority:
        """우선순위 결정"""
        total_score = evaluation.total_score
    ...
    def _generate_reasoning(
        ...
        if action == InsightAction.STORE:
            reasoning += (
                f"종합 점수 {evaluation.total_score:.2f}로 높은 가치를 인정하여 저장"
            )
    ...
    def _calculate_expected_benefit(
        self, evaluation: InsightEvaluation, action: InsightAction
    ) -> float:
        """예상 이익 계산"""
    ...
    def _assess_risk(
        self, evaluation: InsightEvaluation, insight: Dict[str, Any]
    ) -> str:
        """위험 평가"""
        risk_score = evaluation.risk_score
    ...
    def get_management_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_processed": len(self.stored_insights)
            + len(self.held_insights)
            + len(self.discarded_insights),
            "recent_decisions": [
                {
