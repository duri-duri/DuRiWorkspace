...
class Phase2AdvancedLearning:
    ...
    def create_advanced_learning_task(self, problem: str, complexity: ProblemComplexity) -> AdvancedLearningTask:
        """고급 학습 작업 생성"""
    ...
    def _determine_required_capabilities(self, complexity: ProblemComplexity) -> List[AdvancedLearningCapability]:
        """복잡도에 따른 필요한 능력 결정"""
    ...
    def execute_multi_step_reasoning(self, problem: str) -> MultiStepReasoning:
        ...
        # 추론 체인 생성
        reasoning_chain = " → ".join([f"단계{i+1}: {step['action']}" for i, step in enumerate(steps)])
    ...
    def _validate_hypotheses(self, hypotheses: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """가설 검증"""
        validated = []
    ...
    def _derive_conclusion(self, validated_hypotheses: List[Dict[str, Any]]) -> str:
        ...
        conclusion = f"{best_hypothesis['element']}의 최적화를 통해 문제를 해결할 수 있습니다. "
        conclusion += f"예상 효과: {best_hypothesis['validation_score']:.1%}의 성능 향상"
        
        return conclusion
        
    def execute_strategic_thinking(self, problem: str, reasoning: MultiStepReasoning) -> StrategicThinking:
        """전략적 사고 실행"""
        ...
        # 최적 전략 선택
        selected_strategy = max(strategic_options, key=lambda x: x['success_probability'])
    ...
    def execute_complex_problem_solving(self, task: AdvancedLearningTask) -> Dict[str, Any]:
        """복잡한 문제 해결 실행"""
        ...
        if self.insight_engine:
            insight_result = self.insight_engine.execute_dual_response(task.problem_description)
        ...
        solution = {
            ...
            "confidence": (reasoning.confidence + strategy.selected_strategy['success_probability']) / 2,
    ...
    def _enhance_capabilities(self, task: AdvancedLearningTask, solution: Dict[str, Any]):
        """능력 향상"""
    ...
    def get_phase_2_status(self) -> Dict[str, Any]:
        ...
        if self.reasoning_history:
            avg_confidence = sum(reasoning.confidence for reasoning in self.reasoning_history) / len(self.reasoning_history)
