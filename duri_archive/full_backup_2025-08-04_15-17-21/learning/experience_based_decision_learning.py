...
class ExperienceBasedDecisionLearning:
    ...
    def evaluate_decision(self, decision_question: str) -> DecisionEvaluation:
        ...
        # 종합 점수
        overall_score = (
            quality_score + confidence_score + outcome_score + learning_score
        ) / 4

        # 개선 제안
        improvement_suggestions = self._generate_improvement_suggestions(
            decision_analysis, overall_score
        )
    ...
    def _generate_improvement_suggestions(
        self, analysis: Dict[str, Any], overall_score: float
    ) -> List[str]:
        """개선 제안 생성"""
        suggestions = []
    ...
    def _create_experience_record(
        self, evaluation: DecisionEvaluation, analysis: Dict[str, Any]
    ):
        """경험 기록 생성"""
    ...
    def analyze_learning_patterns(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "learning_insights": self._generate_learning_insights(
                decision_types, quality_distribution, avg_score
            ),
        }
    ...
    def _generate_learning_insights(
        ...
        # 의사결정 유형별 통찰
        if (
            "긍정적 의사결정" in decision_types
            and decision_types["긍정적 의사결정"] > 0
        ):
            insights.append(
                "긍정적 의사결정 경험이 풍부하여 성공 패턴을 활용할 수 있습니다"
            )

        if (
            "부정적 의사결정" in decision_types
            and decision_types["부정적 의사결정"] > 0
        ):
            insights.append(
                "부정적 의사결정을 통해 실패 원인을 학습하고 개선할 수 있습니다"
            )

        # 품질 분포별 통찰
        if (
            "excellent" in quality_distribution
            and quality_distribution["excellent"] > 0
        ):
            insights.append(
                "우수한 의사결정 능력을 보유하고 있어 이를 확장할 수 있습니다"
            )
        ...
        if avg_score > 0.7:
            insights.append(
                "전반적으로 높은 신뢰도를 보이며 안정적인 의사결정 능력을 갖추고 있습니다"
            )
        elif avg_score < 0.5:
    ...
    def get_learning_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": sum(
                eval.confidence_score for eval in self.decision_evaluations
            )
            / max(len(self.decision_evaluations), 1),
