...
class StrategySurvivalJudge:
    ...
    def update_strategy_performance(
        self, strategy_id: str, performance: float, emotion: str = None
    ) -> bool:
        """
        전략 성과를 업데이트합니다.
    ...
    def evaluate_strategy_survival(
        ...
        try:
            ...
            logger.info(
                f"전략 {strategy_id} 생존 판단 완료: {judgment_result.action.value}"
            )
            return decision
    ...
    def _determine_recommended_action(
        self, judgment_result: JudgmentResult, record: StrategyRecord
    ) -> str:
        """권장 행동을 결정합니다."""
    ...
    def get_strategy_info(self, strategy_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "average_performance": (
                sum(record.performance_history) / len(record.performance_history)
    ...
    def get_survival_statistics(self) -> Dict[str, Any]:
        ...
        modified_strategies = len(
            [
                r
                for r in self.strategy_records.values()
                if r.status == StrategyStatus.MODIFIED
            ]
        )
        discarded_strategies = len(
            [
                r
                for r in self.strategy_records.values()
                if r.status == StrategyStatus.DISCARDED
            ]
        )

        total_decisions = len(self.decision_history)
        continue_decisions = len(
            [d for d in self.decision_history if d.action == JudgmentAction.CONTINUE]
        )
        modify_decisions = len(
            [d for d in self.decision_history if d.action == JudgmentAction.MODIFY]
        )
        discard_decisions = len(
            [d for d in self.decision_history if d.action == JudgmentAction.DISCARD]
        )
        ...
        avg_performance_score = (
            sum(d.performance_score for d in self.decision_history) / total_decisions
            if total_decisions > 0
            else 0
        )
        avg_emotion_score = (
            sum(d.emotion_score for d in self.decision_history) / total_decisions
            if total_decisions > 0
            else 0
        )
