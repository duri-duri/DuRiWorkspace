...
class SelfEvolutionAnalyzer:
    ...
    def _identify_applied_strategy(
    ...
    def generate_learning_module(self) -> Dict[str, Any]:
        ...
        learning_module = {
            ...
            "success_rate": (
                len([c for c in self.evolution_cases if c.success])
                / len(self.evolution_cases)
    ...
    def _select_best_strategy(
        self, problem_type: str, description: str
    ) -> EvolutionStrategy:
        """가장 적합한 전략을 선택합니다."""
        # 문제 유형에 따른 전략 매핑
        strategy_mapping = {
    ...
    def _apply_strategy(
        self, strategy: EvolutionStrategy, problem: Dict[str, Any]
    ) -> Dict[str, Any]:
        """전략을 적용합니다."""
        try:
            result = {
