...
class DegradationPredictor:
    ...
    def _prediction_loop(self):
        ...
        while self.is_predicting:
            try:
                ...
                logger.info(
                    f"예측 완료: {len(predictions)}개 예측, {len(recommendations)}개 권장사항"
                )
    ...
    def _check_chatgpt_trigger_conditions(self, history) -> bool:
        ...
        try:
            ...
            if decline_count > 0:
                ...
                if (
                    avg_decline
                    > self.trigger_conditions["performance_decline_threshold"] * 100
                ):  # 8%
                    logger.warning(
                        f"🚨 평균 성능 하락폭 {avg_decline:.1f}% > 8% 임계값"
                    )
                    return True
    ...
    def _check_stagnation_and_exhaustion(self, history) -> bool:
        ...
        try:
            ...
            complexity_trends = [
                t for t in recent_trends if t.metric_name == "system_complexity"
            ]
            performance_trends = [
                t
                for t in recent_trends
                if t.metric_name in ["cpu_usage", "memory_usage", "learning_cycle_time"]
            ]
            ...
            if complexity_trends:
                ...
                if (
                    latest_complexity
                    > self.trigger_conditions["structure_exhaustion_threshold"]
                ):
                    # 성능 개선이 없는지 확인
                    improving_count = sum(
                        1
                    ...
                    if improving_count == 0:
                        logger.warning(
                            f"🚨 시스템 복잡도 {latest_complexity:.1f}% > 85% 임계값, 성능 개선 없음"
                        )
                        return True
    ...
    def _predict_metric_degradation(
        self, metric_name: str, history
    ...
    def _linear_regression_prediction(
        self, trends: List, metric_name: str
    ) -> Optional[DegradationPrediction]:
        """선형 회귀를 사용한 예측"""
        try:
            # 시간과 값 데이터 준비
            ...
            # 리팩터링 우선순위 결정
            refactor_priority = self.priority_mapping.get(
                degradation_level, RefactorPriority.NONE
            )
    ...
    def _create_refactor_recommendation(
        self, prediction: DegradationPrediction
    ) -> Optional[RefactorRecommendation]:
    ...
    def get_active_recommendations(self) -> List[RefactorRecommendation]:
        ...
        return [
            ...
            and r.priority in [RefactorPriority.HIGH, RefactorPriority.URGENT]
        ]
    ...
    def get_prediction_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_predictions = [
                ...
                if p.degradation_level == DegradationLevel.CRITICAL
            ]
            high_predictions = [
                p
                for p in recent_predictions
                if p.degradation_level == DegradationLevel.HIGH
            ]
            medium_predictions = [
                p
                for p in recent_predictions
                if p.degradation_level == DegradationLevel.MEDIUM
            ]
            ...
            return {
                ...
                "prediction_confidence_avg": (
                    np.mean([p.confidence for p in recent_predictions])
