...
def test_integrated_autonomous_system():
    ...
    try:
        ...
        if phase_request:
            print(
                f"   🎯 Phase Up 요청: {phase_request.current_phase.value} → {phase_request.target_phase.value}"
            )
        ...
        print(
            f"   결정: {decision.action.value} - {decision.category.value} - {decision.priority.value}"
        )
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "phase":
            ...
            print(
                "사용법: python autonomous_insight_demo.py [phase|reflection|management|integrated|architecture|examples]"
            )
    else:
