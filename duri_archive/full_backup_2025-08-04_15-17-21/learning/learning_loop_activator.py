...
from duri_brain.learning.learning_loop_manager import get_learning_loop_manager, LearningLoopManager
...
class LearningLoopSchedule:
    ...
    active_hours: List[int] = field(default_factory=lambda: list(range(24)))  # 24시간 활성
...
class DuRiLearningLoopActivator:
    ...
    def _start_learning_loop_with_timeout(self, initial_strategy: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            def start_loop():
                try:
                    cycle_id = self.learning_loop_manager.start_learning_loop(initial_strategy)
    ...
    def _start_scheduler_non_blocking(self):
        ...
        self.scheduler_thread = threading.Thread(
            target=self._run_scheduler_with_timeout,
            daemon=True
        )
        ...
        while self.is_activated and iteration_count < max_iterations:
            try:
                ...
                if iteration_count % 60 == 0:  # 1분마다
                    logger.debug(f"스케줄러 실행 중... ({iteration_count}/{max_iterations})")
    ...
    def _store_deactivation_memory(self):
        ...
        try:
            content = {
                ...
                "total_runtime": (datetime.now() - self.activation_time).total_seconds() if self.activation_time else 0,
                "status": "deactivated"
            }
