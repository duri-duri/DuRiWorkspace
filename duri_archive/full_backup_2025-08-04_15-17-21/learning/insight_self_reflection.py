...
class InsightSelfReflector:
    ...
    def record_session(self, session_record: InsightSessionRecord):
        ...
        logger.info(
            f"📝 세션 기록: {session_record.session_id} - {session_record.outcome.value}"
        )
    ...
    def analyze_recent_sessions(self, hours: int = 24) -> Dict[str, Any]:
        ...
        # 성공률 계산
        success_count = len(
            [s for s in recent_sessions if s.outcome == InsightOutcome.SUCCESS]
        )
        ...
        avg_confidence = sum(
            s.confidence for s in recent_sessions if s.confidence
        ) / len(recent_sessions)
        avg_candidates = sum(s.candidates_generated for s in recent_sessions) / len(
            recent_sessions
        )
        ...
        failure_sessions = [
            ...
            if s.outcome in [InsightOutcome.FAILURE, InsightOutcome.NO_INSIGHT]
        ]
    ...
    def _analyze_failure_patterns(
        self, failure_sessions: List[InsightSessionRecord]
    ) -> Dict[str, Any]:
        """실패 패턴 분석"""
        ...
        for session in failure_sessions:
            ...
            patterns["common_problems"][problem_type] = (
                patterns["common_problems"].get(problem_type, 0) + 1
            )

            # 단계별 실패 분석
            for phase in session.phases_completed:
                patterns["phase_failures"][phase] = (
                    patterns["phase_failures"].get(phase, 0) + 1
                )
    ...
    def _analyze_success_patterns(
        self, success_sessions: List[InsightSessionRecord]
    ) -> Dict[str, Any]:
        """성공 패턴 분석"""
        ...
        patterns = {
            "optimal_duration": sum(s.duration for s in success_sessions)
            / len(success_sessions),
            "optimal_confidence": sum(s.confidence for s in success_sessions)
            / len(success_sessions),
            "optimal_candidates": sum(s.candidates_generated for s in success_sessions)
            / len(success_sessions),
            "successful_phases": {},
        }
        ...
        for session in success_sessions:
            for phase in session.phases_completed:
                patterns["successful_phases"][phase] = (
                    patterns["successful_phases"].get(phase, 0) + 1
                )
    ...
    def _recognize_patterns(self) -> Optional[ReflectionInsight]:
        ...
        if len(recent_failures) >= 3:
            common_phase = max(
                set([phase for s in recent_failures for phase in s.phases_completed]),
                key=lambda x: sum(
                    1 for s in recent_failures if x in s.phases_completed
                ),
            )
    ...
    def _optimize_parameters(self) -> Optional[ReflectionInsight]:
        ...
        successful_sessions = [
            s for s in self.session_history if s.outcome == InsightOutcome.SUCCESS
        ]
        ...
        if len(successful_sessions) >= 3:
            ...
            avg_confidence = sum(s.confidence for s in successful_sessions) / len(
                successful_sessions
            )
            ...
            if abs(avg_duration - self.parameter_history["pause_duration"]) > 1.0:
                adjustments.append(
                    f"일시정지 시간: {self.parameter_history['pause_duration']} → {avg_duration:.1f}"
                )

            if (
                abs(avg_confidence - self.parameter_history["confidence_threshold"])
                > 0.1
            ):
                adjustments.append(
                    f"신뢰도 임계값: {self.parameter_history['confidence_threshold']} → {avg_confidence:.2f}"
                )
    ...
    def apply_reflection_insights(
        ...
        for insight in insights:
            if insight.reflection_type == ReflectionType.PARAMETER_OPTIMIZATION:
                ...
                if "일시정지 시간" in insight.action_plan:
                    new_duration = float(
                        insight.action_plan.split("→")[1].strip().split()[0]
                    )
        ...
        return {
            ...
            "expected_improvement": (
                sum(insight.expected_improvement for insight in insights)
                / len(insights)
                if insights
                else 0
            ),
