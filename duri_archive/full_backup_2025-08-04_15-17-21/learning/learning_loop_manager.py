...
from duri_brain.creativity.advanced_creativity_system import (
    get_advanced_creativity_system,
)
...
from .challenge_trigger import ChallengeType, get_challenge_trigger
from .practice_engine import PracticeType, get_practice_engine
from .self_improvement_engine import ImprovementType, get_self_improvement_engine
from .strategy_imitator import ImitationType, get_strategy_imitator
...
class LearningLoopManager:
    ...
    def start_learning_loop(
        self, initial_strategy: Dict[str, Any], context: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        학습 루프를 시작합니다.
        ...
        self.loop_thread = threading.Thread(
            target=self._run_learning_loop, args=(context,), daemon=True
        )
        ...
        try:
            ...
            while self.is_running and cycle_count < self.learning_config["max_cycles"]:
                ...
                if (time.time() - start_time) > self.learning_config["cycle_timeout"]:
                    logger.warning(
                        f"⚠️ 학습 루프 타임아웃 ({self.learning_config['cycle_timeout']}초)"
                    )
                    break
                ...
                if (
                    last_meta_learning_time is None
                    or (current_time - last_meta_learning_time).total_seconds()
                    >= self.learning_config["meta_learning_interval"]
                ):
                ...
                if result and result.cycle.success:
                    failure_streak = 0
                    logger.info(
                        f"사이클 {cycle_count} 성공: 개선점수 {result.improvement_score:.2f}"
                    )
                else:
                    failure_streak += 1
                    logger.warning(
                        f"사이클 {cycle_count} 실패 (연속 실패: {failure_streak})"
                    )
                ...
                if (
                    result
                    and result.improvement_score
                    < self.learning_config["min_improvement_threshold"]
                ):
                    logger.info(
                        f"개선이 미미하여 학습 루프 중단: {result.improvement_score:.2f}"
                    )
                    break
    ...
    def _execute_learning_cycle_with_timeout(
    ...
    def _execute_feedback_stage(
        self, practice_result: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """3단계: 피드백 단계를 실행합니다."""
        try:
            # 연습 결과를 바탕으로 피드백 생성
    ...
    def _execute_challenge_stage(
        self, feedback_result: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """4단계: 도전 단계를 실행합니다."""
        try:
            # 전략 히스토리 구성
            strategy_history = {
                "success_streak": (
                    len(self.learning_cycles) if self.learning_cycles else 0
                ),
                "performance_stability": 0.8,  # 시뮬레이션
                "new_feedback_detected": feedback_result.get("negative_feedback", 0)
                > 0,
            }

            # 도전 판단
            challenge_decision = self.challenge_trigger.should_explore(
                strategy_history=strategy_history, current_context=context
            )
            ...
            return {
                ...
                "challenge_type": (
                    challenge_decision.challenge_type.value
                    if challenge_decision.challenge_type
                    else None
                ),
    ...
    def _execute_improvement_stage(
        self, challenge_result: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """5단계: 개선 단계를 실행합니다."""
        try:
            # 피드백 데이터 구성
            ...
            improvement_result = self.self_improvement_engine.improve(
                old_strategy=self.current_cycle.strategy, feedback_data=feedback_data
            )
    ...
    def _calculate_overall_performance(
        self, stage_results: Dict[LearningStage, Dict[str, Any]]
    ) -> float:
        """전체 성과를 계산합니다."""
        performances = []
        ...
        for stage, result in stage_results.items():
            if result.get("success", False):
                if stage == LearningStage.IMITATION:
                    ...
                    feedback_ratio = result.get("positive_feedback", 0) / max(
                        result.get("positive_feedback", 0)
                        + result.get("negative_feedback", 1),
                        1,
                    )
    ...
    def _evaluate_strategy_survival(
        self, overall_performance: float, context: Optional[Dict[str, Any]]
    ) -> Optional[Any]:
        """전략 생존을 평가합니다."""
        try:
            ...
            if not self.strategy_survival_judge.get_strategy_info(strategy_id):
                self.strategy_survival_judge.register_strategy(
                    ...
                    initial_emotion=(
                        context.get("emotion", "neutral") if context else "neutral"
                    ),
                )
            ...
            # 생존 판단
            survival_decision = self.strategy_survival_judge.evaluate_strategy_survival(
                strategy_id
            )
    ...
    def _bury_failed_strategy(
        self, survival_decision: Any, context: Optional[Dict[str, Any]]
    ):
        """실패한 전략을 묘지에 안장합니다."""
        try:
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        for stage in LearningStage:
            completed_count = sum(
                1 for c in self.learning_cycles if stage in c.stages_completed
            )
            stage_completion_rates[stage.value] = (
                completed_count / total_cycles if total_cycles > 0 else 0
            )

        avg_performance = (
            sum(
                c.performance_metrics.get("overall_performance", 0)
        ...
        avg_improvement = (
            sum(
                c.performance_metrics.get("improvement_score", 0)
                for c in self.learning_cycles
            )
            / total_cycles
            if total_cycles > 0
    ...
    def _apply_critical_improvement(self, suggestion):
        ...
        try:
            ...
            category = (
                ...
                else getattr(suggestion, "category", None)
            )

            if category == "error_handling":
                # 오류 처리 개선
                self.learning_config["max_failure_streak"] = max(
                    3, self.learning_config["max_failure_streak"] - 1
                )
                logger.info("오류 처리 개선: 실패 허용 횟수 감소")

            elif category == "performance":
                # 성능 최적화
                self.learning_config["cycle_timeout"] = min(
                    600, self.learning_config["cycle_timeout"] + 60
                )
                logger.info("성능 최적화: 사이클 타임아웃 증가")

            elif category == "learning":
                # 학습 전략 개선
                self.learning_config["min_improvement_threshold"] = max(
                    0.05, self.learning_config["min_improvement_threshold"] - 0.01
                )
    ...
    def _apply_high_priority_improvement(self, suggestion):
        ...
        try:
            ...
            if category == "learning":
                # 학습 전략 개선
                self.learning_config["min_improvement_threshold"] = max(
                    0.05, self.learning_config["min_improvement_threshold"] - 0.02
                )
                logger.info("학습 전략 개선: 개선 임계값 조정")

            elif category == "system_health":
                # 시스템 건강도 개선
                self.learning_config["meta_learning_interval"] = min(
                    7200, self.learning_config["meta_learning_interval"] + 600
                )
        ...
        try:
            if strategy_update.strategy_name == "learning_efficiency":
                ...
                if strategy_update.current_performance < 1.0:
                    self.learning_config["cycle_timeout"] = max(
                        120, self.learning_config["cycle_timeout"] - 30
                    )
        ...
        try:
            ...
            for improvement in specific_improvements:
                ...
                if priority == "critical":
                    self._apply_critical_improvement(
                        {"category": "learning", "description": description}
                    )
                    result["applied_improvements"].append(
                        {"priority": "critical", "description": description}
                    )
                    logger.info(f"🔴 중요 개선안 적용: {description}")

                elif priority == "high":
                    self._apply_high_priority_improvement(
                        {"category": "learning", "description": description}
                    )
                    result["applied_improvements"].append(
                        {"priority": "high", "description": description}
                    )
                    ...
                    # 일반 개선안은 학습 강도 조정
                    self._adjust_learning_intensity("increase")
                    result["strategy_updates"].append(
                        {"type": "intensity_increase", "description": description}
                    )
            ...
            if self_assessment.get("overall_rating") == "needs_improvement":
                self._adjust_learning_intensity("reduce")
                result["strategy_updates"].append(
                    {"type": "intensity_reduce", "reason": "성과 개선 필요"}
                )
            ...
            logger.info(
                f"✅ 학습 전략 업데이트 완료 - 적용된 개선안: {len(result['applied_improvements'])}개"
            )
            return result
    ...
    def _log_meta_reflection(self, retrospection: Dict[str, Any]):
        ...
        try:
            ...
            reflection_log = {
                ...
                "improvement_proposals": retrospection.get(
    ...
    def _adjust_learning_intensity(self, direction: str):
        ...
        try:
            if direction == "reduce":
                ...
                # 학습 강도 증가
                self.learning_config["cycle_timeout"] = min(
                    300, self.learning_config["cycle_timeout"] - 30
                )
                self.learning_config["meta_learning_interval"] = max(
                    1800, self.learning_config["meta_learning_interval"] - 300
                )
    ...
    def get_current_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_stage": (
                self.current_cycle.current_stage.value
                if self.current_cycle.current_stage
                else None
            ),
            "stages_completed": [
                stage.value for stage in self.current_cycle.stages_completed
            ],
            "start_time": self.current_cycle.start_time.isoformat(),
            "elapsed_time": (
                (datetime.now() - self.current_cycle.start_time).total_seconds()
                if self.current_cycle.start_time
                else 0
            ),
            ...
            "last_self_assessment": (
                self.self_assessment_manager.last_assessment_time.isoformat()
                if self.self_assessment_manager.last_assessment_time
                else None
            ),
    ...
    def _run_self_assessment_cycle(self):
        ...
        try:
            ...
            # 종합 자기 평가 실행
            assessment_result = (
                self.self_assessment_manager.run_comprehensive_assessment()
            )
            ...
            logger.info(
                f"자기 평가 사이클 완료 - 전체 점수: {assessment_result.overall_score:.2f}"
            )
    ...
    def _run_emotional_ethical_judgment_cycle(self):
        ...
        try:
            ...
            if result:
                ...
                logger.info(
                    f"점수 - 윤리적: {result.ethical_score:.2f}, 감정적: {result.emotional_score:.2f}, 전체: {result.overall_score:.2f}"
                )
    ...
    def _run_autonomous_goal_setting_cycle(self):
        ...
        try:
            ...
            # 자율 목표 생성
            new_autonomous_goals = (
                self.autonomous_goal_setting.generate_autonomous_goals()
            )
            ...
            if new_autonomous_goals:
                ...
                # 목표 우선순위 결정
                prioritization_result = self.autonomous_goal_setting.prioritize_goals(
                    new_autonomous_goals
                )

                if prioritization_result:
                    logger.info(
                        f"목표 우선순위 결정 완료: {prioritization_result.reasoning}"
                    )
    ...
    def _run_creativity_enhancement_cycle(self):
        ...
        try:
            ...
            session = self.advanced_creativity_system.run_creativity_session(
                context="시스템 성능 및 기능 개선", duration_minutes=15
            )

            if session:
                logger.info(
                    f"창의성 세션 완료: {len(session.ideas_generated)}개 아이디어 생성"
                )
    ...
    def _execute_learning_cycle(
        ...
        try:
            ...
            improvement_result = self._execute_improvement_stage(
                challenge_result, context
            )
            ...
            # 전략 생존 판단
            survival_decision = self._evaluate_strategy_survival(
                overall_performance, context
            )
            ...
            self.current_cycle.performance_metrics = {
                ...
                "survival_action": (
                    survival_decision.action.value if survival_decision else "unknown"
                ),
            }
            self.current_cycle.success = improvement_score > 0.0

            if improvement_result.get("modified_strategy"):
                self.current_cycle.modified_strategy = improvement_result[
                    "modified_strategy"
                ]
            ...
            logger.info(
                f"학습 사이클 완료: 성과 {overall_performance:.2f}, 개선점수 {improvement_score:.2f}"
            )
            return result
