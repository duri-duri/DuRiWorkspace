...
def test_smart_learning_checker():
    ...
    try:
        ...
        print(
            f"   - 최소/최대 대기: {status.get('min_wait_time', 0)}~{status.get('max_wait_time', 0)}초"
        )
        ...
        if diagnostic_history:
            ...
            for i, diagnostic in enumerate(diagnostic_history[-3:], 1):
                print(
                    f"      {i}. {diagnostic.timestamp.strftime('%H:%M:%S')}: {diagnostic.stuck_reason}"
                )
...
if __name__ == "__main__":
    ...
    print(
        "💡 실패한 경우 trace_learning_stuck_reason() 함수가 자동으로 실행되어 원인을 진단합니다."
    )
