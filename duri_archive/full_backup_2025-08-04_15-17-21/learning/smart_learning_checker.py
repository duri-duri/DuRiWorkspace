...
class SmartLearningChecker:
    ...
    def _smart_check_activation_with_timeout(
        self, max_wait: int, activation_start: datetime
    ) -> bool:
        ...
        try:
            ...
            while wait < max_wait:
                ...
                if status.get("is_activated", False):
                    ...
                    print(
                        f"✅ {wait}초 후 학습 루프 활성화 확인됨 (지연시간: {latency:.2f}초)"
                    )
                    return True
    ...
    def _trace_learning_stuck_reason(self, stuck_reason: str):
        ...
        try:
            ...
            print(f"📋 트리거 정보:")
            print(
                f"   - 마지막 트리거 시간: {trigger_info.get('last_trigger_time', 'Unknown')}"
            )
            print(
                f"   - 트리거 단계: {', '.join(trigger_info.get('trigger_steps', []))}"
            )
            ...
            print(
                f"   - 스케줄러 스레드 활성: {scheduler_status.get('thread_alive', False)}"
            )
    ...
    def _check_trigger_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "last_trigger_time": (
                    getattr(activator, "last_trigger_time", None) if activator else None
                ),
                "trigger_steps": (
                    getattr(activator, "trigger_steps", []) if activator else []
                ),
            }
        except Exception as e:
    ...
    def _get_adaptive_wait_time(self) -> int:
        ...
        successful_latencies = [
            data.latency_seconds for data in self.latency_history if data.success
        ]
        ...
        logger.info(
            f"적응형 대기 시간 계산: 평균 {avg_latency:.2f}초 → {adaptive_wait}초"
        )

        return adaptive_wait

    def set_adaptive_waiting_time(
        self, based_on: str = "last_successful_cycle_latency"
    ):
    ...
    def get_latency_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "success_rate": (
                len(successful_latencies) / len(self.latency_history)
