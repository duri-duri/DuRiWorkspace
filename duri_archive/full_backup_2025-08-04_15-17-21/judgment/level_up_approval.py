...
class LevelUpApproval:
    ...
    def approve_level_up(
        ...
        try:
            ...
            # 4. 종합 판단
            approval_result = self._make_final_decision(
                bias_analysis, growth_score, quest_score
            )
    ...
    def _analyze_emotion_bias(self, emotion_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        bias_analysis = self.bias_detector.detect_biases(
            judgment_context, decision_data
        )
    ...
    def _make_final_decision(
        self, bias_analysis: Dict[str, Any], growth_score: float, quest_score: float
    ) -> Dict[str, Any]:
        """최종 판단"""
        # 편향 점수에 따른 신뢰도 계산
        ...
        # 승인 여부 결정
        approved = (
            combined_score >= approval_threshold
        ...
        # 거부 이유 생성
        reason = self._generate_approval_reason(
            approved, combined_score, reliability, bias_score
        )
...
class GrowthAnalyzer:
    ...
    def evaluate_growth_trend(self, growth_history: list) -> Dict[str, Any]:
        ...
        return {"trend": trend, "growth_rate": growth_rate, "consistency": consistency}
