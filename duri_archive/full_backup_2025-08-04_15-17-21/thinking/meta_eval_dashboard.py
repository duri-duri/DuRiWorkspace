...
class MetaEvalDashboard:
    ...
    def compare_phases(
        self, phase1: str, phase2: str, metric_type: MetricType
    ) -> PhaseComparison:
        """단계 비교"""
        ...
        if not phase1_metrics or not phase2_metrics:
            ...
            if metric_type == MetricType.EVOLUTION_SCORE:
                phase1_value = sum(m.evolution_score for m in phase1_metrics) / len(
                    phase1_metrics
                )
                phase2_value = sum(m.evolution_score for m in phase2_metrics) / len(
                    phase2_metrics
                )
            elif metric_type == MetricType.IMPROVEMENT_RATE:
                phase1_value = sum(m.improvement_rate for m in phase1_metrics) / len(
                    phase1_metrics
                )
                phase2_value = sum(m.improvement_rate for m in phase2_metrics) / len(
                    phase2_metrics
                )
            elif metric_type == MetricType.SUCCESS_RATE:
                phase1_value = sum(m.success_rate for m in phase1_metrics) / len(
                    phase1_metrics
                )
                phase2_value = sum(m.success_rate for m in phase2_metrics) / len(
                    phase2_metrics
                )
            else:
                phase1_value = random.uniform(0.6, 0.8)
    ...
    def analyze_trends(
        self, metric_type: MetricType, time_period: str = "recent"
    ) -> TrendAnalysis:
        """트렌드 분석"""
        ...
        trend_direction = (
            ...
            else "하락" if end_value < start_value else "유지"
        )
        growth_rate = (
    ...
    def generate_overview_dashboard(self) -> DashboardReport:
        ...
        avg_evolution_score = (
            sum(m.evolution_score for m in self.loop_metrics) / total_loops
            if total_loops > 0
            else 0.0
        )
        avg_success_rate = (
            sum(m.success_rate for m in self.loop_metrics) / total_loops
            if total_loops > 0
            else 0.0
        )
        avg_improvement_rate = (
            sum(m.improvement_rate for m in self.loop_metrics) / total_loops
            if total_loops > 0
            else 0.0
        )
        ...
        summary = {
            ...
            "overall_performance": (
                "우수"
                if avg_evolution_score > 0.8
                else "양호" if avg_evolution_score > 0.6 else "개선 필요"
            ),
        }
    ...
    def generate_detailed_dashboard(self) -> DashboardReport:
        ...
        detailed_summary = {
            "phase_breakdown": {
                phase: len(metrics) for phase, metrics in phase_breakdown.items()
            },
            "best_performing_phase": (
                max(
                    phase_breakdown.keys(),
                    key=lambda p: sum(m.evolution_score for m in phase_breakdown[p])
                    / len(phase_breakdown[p]),
                )
                if phase_breakdown
                else "N/A"
            ),
    ...
    def generate_comparative_dashboard(self) -> DashboardReport:
        ...
        comparative_summary = {
            ...
            "average_improvement": (
                sum(c.improvement for c in self.phase_comparisons)
                / len(self.phase_comparisons)
                if self.phase_comparisons
                else 0.0
            ),
            "best_improvement": max(
                (c.improvement for c in self.phase_comparisons), default=0.0
            ),
        }
    ...
    def generate_trend_dashboard(self) -> DashboardReport:
        ...
        downward_trends = [
            t for t in self.trend_analyses if t.trend_direction == "하락"
        ]
        ...
        trend_summary = {
            ...
            "average_growth_rate": (
                sum(t.growth_rate for t in self.trend_analyses)
                / len(self.trend_analyses)
                if self.trend_analyses
                else 0.0
            ),
            "strongest_growth": max(
                (t.growth_rate for t in self.trend_analyses), default=0.0
            ),
        }
    ...
    def get_dashboard_status(self) -> Dict[str, Any]:
        ...
        if total_metrics > 0:
            avg_evolution_score = (
                sum(m.evolution_score for m in self.loop_metrics) / total_metrics
            )
            avg_success_rate = (
                sum(m.success_rate for m in self.loop_metrics) / total_metrics
            )
        else:
