...
class JudgmentConsciousness:
    ...
    def begin_conscious_judgment(
        ...
        try:
            ...
            judgment_id = (
                f"conscious_judgment_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            # 2. 철학적 확인
            philosophical_check = self._perform_philosophical_check(
                judgment_type, context
            )

            # 3. 윤리적 평가
            ethical_evaluation = self._perform_ethical_evaluation(
                judgment_type, context
            )
            ...
            logger.info(
                f"✅ 의식적 판단 완료: {judgment_type} - 상태: {consciousness_state.value}, 신뢰도: {confidence:.3f}"
            )
            return judgment
    ...
    def _perform_philosophical_check(
        self, judgment_type: str, context: Dict[str, Any]
    ) -> str:
        """철학적 확인 수행"""
        checks = [
    ...
    def _perform_ethical_evaluation(
        self, judgment_type: str, context: Dict[str, Any]
    ) -> str:
        """윤리적 평가 수행"""
        evaluations = [
    ...
    def _determine_consciousness_state(
        self, philosophical_check: str, ethical_evaluation: str, goal_alignment: str
    ) -> ConsciousnessState:
        """자각 상태 결정"""
    ...
    def _calculate_consciousness_confidence(
        self, philosophical_check: str, ethical_evaluation: str, goal_alignment: str
    ) -> float:
        """자각 신뢰도 계산"""
    ...
    def _create_philosophy_alignment(
        self, judgment: ConsciousJudgment
    ) -> PhilosophyAlignment:
        """철학적 일치 기록 생성"""
        alignment_id = (
            f"philosophy_alignment_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def get_conscious_judgment_history(
        self, limit: int = 10
    ) -> List[ConsciousJudgment]:
        """의식적 판단 기록 조회"""
        return self.conscious_judgments[-limit:]

    def get_philosophy_alignment_history(
        self, limit: int = 10
    ) -> List[PhilosophyAlignment]:
        """철학적 일치 기록 조회"""
    ...
    def get_consciousness_metrics(self) -> Dict[str, Any]:
        ...
        confirmed_count = len(
            [
                j
                for j in self.conscious_judgments
                if j.consciousness_state == ConsciousnessState.CONFIRMED
            ]
        )
        rejected_count = len(
            [
                j
                for j in self.conscious_judgments
                if j.consciousness_state == ConsciousnessState.REJECTED
            ]
        )
        total_count = len(self.conscious_judgments)

        avg_confidence = (
            sum(j.confidence for j in self.conscious_judgments) / total_count
            if total_count > 0
            else 0
        )
        ...
        return {
            ...
            "confirmation_rate": (
                confirmed_count / total_count if total_count > 0 else 0
            ),
