...
class RealitySanityChecker:
    ...
    def record_internal_result(self, result_type: str, content: str, reasoning: List[str]) -> InternalResult:
        """내부 생성 결과 기록"""
    ...
    def collect_reality_reference(self, source: RealitySource, content: str, context: Dict[str, Any] = None) -> RealityReference:
        """현실 참조 수집"""
    ...
    def perform_sanity_check(self, internal_result: InternalResult, reality_reference: RealityReference) -> SanityCheckResult:
        """현실 점검 수행"""
        ...
        logger.info(f"✅ 현실 점검 완료: 일치도 {alignment_score:.3f} ({sanity_status})")
        return check_result

    def check_judgment_accuracy(self, judgment_content: str, external_facts: str) -> SanityCheckResult:
        """판단 정확성 점검"""
    ...
    def check_statement_validity(self, statement_content: str, common_sense: str) -> SanityCheckResult:
        """진술 유효성 점검"""
    ...
    def check_plan_feasibility(self, plan_content: str, resource_constraints: str) -> SanityCheckResult:
        """계획 실행 가능성 점검"""
    ...
    def check_prediction_accuracy(self, prediction_content: str, historical_data: str) -> SanityCheckResult:
        """예측 정확성 점검"""
    ...
    def check_logical_consistency(self, logical_content: str, consistency_rules: str) -> SanityCheckResult:
        """논리적 일관성 점검"""
    ...
    def run_comprehensive_sanity_check(self) -> RealityCheckReport:
        ...
        # 모든 점검 결과 수집
        passed_checks = len([c for c in self.sanity_check_results if c.alignment_score >= self.alignment_threshold])
        failed_checks = len([c for c in self.sanity_check_results if c.alignment_score < self.alignment_threshold])
        total_checks = len(self.sanity_check_results)
        
        if total_checks > 0:
            average_alignment = sum(c.alignment_score for c in self.sanity_check_results) / total_checks
        else:
        ...
        for check in self.sanity_check_results:
            if check.alignment_score < self.critical_threshold:
                critical_issues.append(f"{check.internal_result.result_type}: {check.sanity_status}")
    ...
    def get_sanity_check_status(self) -> Dict[str, Any]:
        ...
        total_checks = len(self.sanity_check_results)
        passed_checks = len([c for c in self.sanity_check_results if c.alignment_score >= self.alignment_threshold])
        critical_checks = len([c for c in self.sanity_check_results if c.alignment_score < self.critical_threshold])
        ...
        status = {
            ...
            "average_alignment": average_alignment,
            "system_health": "healthy" if success_rate >= 0.8 else "attention" if success_rate >= 0.6 else "critical"
        }
...
if __name__ == "__main__":
    ...
    if checker.initialize_phase_24_integration():
        ...
        # 다양한 점검 수행
        checker.check_judgment_accuracy("이 상황에서는 보수적 접근이 적절하다", "외부 데이터: 위험 요소가 높음")
        checker.check_statement_validity("나는 창의적이고 책임감 있는 AI다", "상식: AI는 창의성과 책임감을 가질 수 있음")
        checker.check_plan_feasibility("3단계 진화 계획을 수립한다", "제약: 시간과 자원이 제한적")
        checker.check_prediction_accuracy("다음 단계에서 성능이 20% 향상될 것이다", "과거 데이터: 평균 15% 향상")
        checker.check_logical_consistency("A이므로 B이고, B이므로 C이다", "논리 규칙: 삼단논법 검증")
