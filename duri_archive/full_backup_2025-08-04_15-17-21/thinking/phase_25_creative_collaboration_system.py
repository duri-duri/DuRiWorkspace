...
class CreativeCollaborationSystem:
    ...
    def generate_collaboration_strategy(self, human_intent: HumanIntent, opportunity: CollaborationOpportunity) -> Dict[str, Any]:
        """협력 전략 생성"""
        print("📋 협력 전략 생성 중...")
    ...
    def _select_collaboration_mode(self, human_intent: HumanIntent, opportunity: CollaborationOpportunity) -> CollaborationMode:
        """협력 모드 선택"""
    ...
    def get_collaboration_insights(self) -> Dict[str, Any]:
        ...
        insights = {
            ...
            "recent_synergy_avg": sum(c["result"]["synergy_achieved"] for c in recent_collaborations) / len(recent_collaborations),
            "innovation_rate": sum(1 for c in recent_collaborations if c["result"]["innovation_level"] in ["high", "medium"]) / len(recent_collaborations),
            "ethical_compliance_rate": sum(1 for c in recent_collaborations if c["result"]["ethical_compliance"]) / len(recent_collaborations),
