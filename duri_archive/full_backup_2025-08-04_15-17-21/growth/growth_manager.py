...
class GrowthManager:
    ...
    def _evaluate_growth_condition(
        self, quest_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """성장 조건 평가"""
        ...
        if approval_result.get("approved", False):
            ...
            if next_level <= 8:  # 최대 레벨
                self.level_system.current_level = (
                    self.level_system.current_level.__class__(next_level)
                )
    ...
    def receive_stimulus(
        ...
        """자극 수신 (대역폭 관리)"""
        return self.bandwidth_manager.receive_stimulus(
            stimulus, stimulus_type, intensity, source
        )
