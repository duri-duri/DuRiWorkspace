...
class QuestEngine:
    ...
    def update_progress(
        self, user_input: str, emotion_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """퀘스트 진행도 업데이트"""
    ...
    def _select_optimal_quest(
        self, available_quests: List[Quest], weak_points: List[str]
    ) -> Quest:
        """최적 퀘스트 선택"""
    ...
    def _create_custom_quest(self, weak_points: List[str]) -> Quest:
        ...
        template = quest_templates.get(
            weak_point, quest_templates["emotional_maturity"]
        )
    ...
    def evaluate_input_for_quest(
        self, user_input: str, emotion_result: Dict[str, Any], quest: Quest
    ) -> Dict[str, Any]:
        """퀘스트를 위한 입력 평가"""
        # 간소화된 평가 로직
        performance_data = {
