...
class QuestCalculator:
    ...
    def calculate_quest_score(
        self, quest: Quest, performance_data: Dict[str, Any]
    ) -> QuestEvaluation:
        """퀘스트 점수 계산"""
        try:
            ...
            # 2. 난이도 보정
            difficulty_adjusted_score = self._apply_difficulty_multiplier(
                base_score, quest.difficulty
            )

            # 3. 시간 보너스/페널티 적용
            time_adjusted_score = self._apply_time_adjustments(
                difficulty_adjusted_score, quest, performance_data
            )

            # 4. 시도 횟수 페널티 적용
            final_score = self._apply_attempt_penalty(
                time_adjusted_score, quest.attempts
            )
            ...
            logger.info(
                f"퀘스트 점수 계산 완료: {quest.id}, 점수: {final_score:.3f}, 통과: {passed}"
            )
    ...
    def _apply_difficulty_multiplier(
        self, base_score: float, difficulty: QuestDifficulty
    ) -> float:
        """난이도 보정 적용"""
    ...
    def _apply_time_adjustments(
        self, score: float, quest: Quest, performance_data: Dict[str, Any]
    ) -> float:
        """시간 보정 적용"""
        ...
        if quest.time_limit and "completion_time" in performance_data:
            ...
            if time_ratio < 0.5:  # 50% 이내 완료
                ...
                adjusted_score += self.scoring_weights["time_bonus"][
                    "efficient_execution"
                ]
    ...
    def _generate_feedback(
        self, quest: Quest, score: float, performance_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """피드백 생성"""
        # 점수별 피드백 템플릿 선택
        if score >= 0.9:
    ...
    def _generate_improvement_suggestions(
        ...
        if category == "emotional":
            if performance_data.get("emotion_regulation", 0.0) < 0.7:
                suggestions.append(
                    "감정 조절 능력을 향상시키기 위해 명상이나 호흡 운동을 시도해보세요."
                )
            if performance_data.get("empathy_level", 0.0) < 0.7:
                suggestions.append(
                    "다른 사람의 관점을 이해하는 연습을 통해 공감 능력을 키워보세요."
                )
            ...
            if performance_data.get("creativity", 0.0) < 0.7:
                suggestions.append(
                    "다양한 관점에서 생각해보는 연습을 통해 창의성을 키워보세요."
                )
    ...
    def _create_error_evaluation(
        self, quest_id: str, error_message: str
    ) -> QuestEvaluation:
        """오류 평가 생성"""
