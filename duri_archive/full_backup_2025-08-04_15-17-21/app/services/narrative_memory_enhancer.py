...
class NarrativeMemoryEnhancer:
    ...
    def create_narrative_memory(
        ...
        # 서사 구조 결정
        narrative_structure = self._determine_narrative_structure(
            memory_type, emotional_tone, description
        )

        # 가족 영향 평가
        family_impact = self._assess_family_impact(
            description, key_emotions, lessons_learned
        )
    ...
    def _determine_narrative_structure(
        self, memory_type: MemoryType, emotional_tone: EmotionalTone, description: str
    ) -> NarrativeStructure:
        """서사 구조 결정"""
    ...
    def _assess_family_impact(
        self, description: str, key_emotions: List[str], lessons_learned: List[str]
    ) -> str:
        """가족 영향 평가"""
        ...
        positive_count = sum(
            1 for emotion in key_emotions if emotion in positive_emotions
        )
        negative_count = sum(
            1 for emotion in key_emotions if emotion in negative_emotions
        )

        if positive_count > negative_count:
            return (
                "이 경험은 가족 간의 유대감을 강화하고 긍정적인 분위기를 조성했습니다."
            )
    ...
    def create_memory_connection(
        ...
        connection_strength = self._calculate_connection_strength(
            source_memory, target_memory, connection_type
        )
        ...
        logger.info(
            f"기억 연결 생성 완료: {source_memory.title} → {target_memory.title}"
        )

        return connection

    def _calculate_connection_strength(
        self, source: NarrativeMemory, target: NarrativeMemory, connection_type: str
    ) -> float:
        """연결 강도 계산"""
        base_strength = 0.5
        ...
        if (
            ...
            and source.emotional_tone == target.emotional_tone
        ):
            base_strength += 0.3
    ...
    def generate_narrative_story(
        self, theme: str, related_memories: List[NarrativeMemory]
    ) -> NarrativeStory:
        """서사적 이야기 생성"""
    ...
    def _find_related_memories(self, theme: str) -> List[NarrativeMemory]:
        ...
        for memory in self.narrative_memories:
            # 제목이나 설명에서 테마 검색
            if (
                theme_lower in memory.title.lower()
                or theme_lower in memory.description.lower()
                or any(
                    theme_lower in lesson.lower() for lesson in memory.lessons_learned
                )
            ):
    ...
    def _find_memory_connections(
        self, memories: List[NarrativeMemory]
        ...
        for connection in self.memory_connections:
            if (
                connection.source_memory_id in memory_ids
                and connection.target_memory_id in memory_ids
            ):
    ...
    def _extract_growth_insights(self, memories: List[NarrativeMemory]) -> List[str]:
        ...
        for memory in memories:
            if (
                memory.memory_type == MemoryType.LEARNING_EXPERIENCE
                or memory.memory_type == MemoryType.GROWTH_MILESTONE
            ):
                insights.extend(memory.lessons_learned)

        return list(set(insights))  # 중복 제거

    def recall_narrative_memory(
        self, query: str, family_context: Dict[str, Any]
        ...
        for memory in self.narrative_memories:
            ...
            if (
                ...
                or any(
                    query_lower in lesson.lower() for lesson in memory.lessons_learned
                )
                or any(
                    query_lower in emotion.lower() for emotion in memory.key_emotions
                )
            ):
    ...
    def create_family_narrative(self, family_context: Dict[str, Any]) -> NarrativeStory:
        ...
        recent_memories = [
            m
            for m in self.narrative_memories
    ...
    def get_narrative_statistics(self) -> Dict[str, Any]:
        ...
        for memory_type in MemoryType:
            type_memories = [
                m for m in self.narrative_memories if m.memory_type == memory_type
            ]
        ...
        for tone in EmotionalTone:
            tone_memories = [
                m for m in self.narrative_memories if m.emotional_tone == tone
            ]
        ...
        statistics = {
            ...
            "average_confidence": sum(
                m.confidence_score for m in self.narrative_memories
            )
            / max(1, total_memories),
...
def test_narrative_memory_enhancer():
    ...
    # 3. 서사적 이야기 생성
    story = narrative_system.generate_narrative_story(
        "가족과 함께하는 활동", [memory1, memory2]
    )
    ...
    print(
        f"✅ 서사적 기억 통계: {statistics['total_memories']}개 기억, {statistics['total_connections']}개 연결"
    )
    ...
    print(
        f"✅ 서사적 기억 데이터 내보내기: {len(export_data['narrative_memories'])}개 기억"
    )
