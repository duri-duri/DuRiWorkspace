...
class QuasiFamilyDemo:
    ...
    async def _introduce_quasi_family_members(self):
        ...
        introduction_data = {"title": "DuRi의 준 가족 구성원들", "members": []}
        ...
        self.demo_results.append(
            {"section": "member_introduction", "data": introduction_data}
        )
    ...
    async def _demonstrate_feedback_processing(self):
        ...
        for example in feedback_examples:
            ...
            if member:
                # 피드백 처리
                processing_result = (
                    self.relationship_system.process_feedback_from_member(
                ...
                logger.info(
                    f"   - 피드백 민감도: {processing_result['feedback_sensitivity']}"
                )
                logger.info(
                    f"   - 전략 반영 우선순위: {processing_result['strategy_priority']}"
                )
                logger.info(
                    f"   - 감정 학습 가중치: {processing_result['emotion_weight']}"
                )
                logger.info(
                    f"   - 신념 강화 기여도: {processing_result['belief_contribution']}"
                )
                logger.info("")

        self.demo_results.append(
            {"section": "feedback_processing", "data": feedback_demo_data}
        )
    ...
    async def _show_learning_recommendations(self):
        ...
        for member_id, member in self.relationship_system.relationship_registry.items():
            recommendations = self.relationship_system.get_learning_recommendations(
                member_id
            )
    ...
    async def _show_relationship_statistics(self):
        ...
        self.demo_results.append(
            {"section": "relationship_statistics", "data": statistics}
        )
    ...
    async def _export_relationship_registry(self):
        ...
        for member_id, member_data in registry_data["DuRiRelationshipRegistry"].items():
            ...
            logger.info(
                f"    - learning_influence: {member_data['learning_influence']}"
            )

        self.demo_results.append(
            {"section": "relationship_registry_export", "data": registry_data}
        )
        ...
        return {
            ...
            "summary": {
                ...
                "relationship_types": list(
                    set(
                        member.relation_type.value
                        for member in self.relationship_system.relationship_registry.values()
                    )
                ),
                "average_trust": (
                    sum(
                    ...
                    / len(self.relationship_system.relationship_registry)
                    if self.relationship_system.relationship_registry
                    else 0.0
                ),
            },
