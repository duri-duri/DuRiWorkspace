...
class EmotionalConversationSystem:
    ...
    def analyze_emotional_state(
        self, message: str, family_context: Dict[str, Any], speaker_info: Dict[str, Any]
    ) -> EmotionalAnalysis:
        """감정 상태 분석"""
        # 감정 키워드 분석
        ...
        secondary_emotions = self._identify_secondary_emotions(
            emotion_keywords, primary_emotion
        )
        ...
        underlying_needs = self._identify_underlying_needs(
            primary_emotion, message, family_context
        )

        # 지원 요구사항 결정
        support_requirements = self._determine_support_requirements(
            primary_emotion, intensity, underlying_needs
        )

        # 신뢰도 계산
        confidence_score = self._calculate_emotional_confidence(
            message, emotion_keywords, primary_emotion
        )
    ...
    def _determine_primary_emotion(
        self, emotion_keywords: List[str], message: str
    ) -> EmotionalState:
        """주요 감정 결정"""
        message_lower = message.lower()
    ...
    def _identify_secondary_emotions(
        self, emotion_keywords: List[str], primary_emotion: EmotionalState
    ) -> List[EmotionalState]:
        """보조 감정 식별"""
        secondary_emotions = []
    ...
    def _assess_emotional_intensity(
        self, message: str, emotion_keywords: List[str]
    ) -> EmotionalIntensity:
        """감정 강도 평가"""
        # 강조 표현 확인
    ...
    def _identify_emotional_triggers(
        ...
        if (
            "가족" in message_lower
            or "엄마" in message_lower
            or "아빠" in message_lower
        ):
    ...
    def _identify_underlying_needs(
        self,
        primary_emotion: EmotionalState,
        message: str,
    ...
    def _determine_support_requirements(
        self,
        primary_emotion: EmotionalState,
        intensity: EmotionalIntensity,
        underlying_needs: List[str],
    ...
    def _calculate_emotional_confidence(
        self, message: str, emotion_keywords: List[str], primary_emotion: EmotionalState
    ) -> float:
        """감정 분석 신뢰도 계산"""
        base_score = 0.7
    ...
    def generate_emotional_response(
        self, analysis: EmotionalAnalysis, family_context: Dict[str, Any]
    ) -> EmotionalResponse:
        """감정적 응답 생성"""
        # 응답 유형 결정
        primary_support = (
            analysis.support_requirements[0]
            if analysis.support_requirements
            else SupportType.LISTENING
        )

        # 대화 톤 결정
        conversation_tone = self._determine_conversation_tone(
            analysis.primary_emotion, analysis.intensity
        )

        # 응답 내용 생성
        response_content = self._generate_response_content(
            analysis, primary_support, conversation_tone
        )
    ...
    def _determine_conversation_tone(
        self, primary_emotion: EmotionalState, intensity: EmotionalIntensity
    ) -> ConversationTone:
        """대화 톤 결정"""
    ...
    def _generate_emotional_validation(self, analysis: EmotionalAnalysis) -> str:
        ...
        elif analysis.primary_emotion in [
    ...
    def _generate_practical_support(
        self, analysis: EmotionalAnalysis, family_context: Dict[str, Any]
    ) -> Optional[str]:
        """실용적 지원 생성"""
    ...
    def _calculate_response_confidence(
        self, analysis: EmotionalAnalysis, response_content: str
    ) -> float:
        """응답 신뢰도 계산"""
    ...
    def conduct_emotional_conversation(
        self, family_member: str, initial_message: str, family_context: Dict[str, Any]
    ) -> EmotionalConversation:
        """감정적 대화 수행"""
        conversation_id = (
            f"emotional_conversation_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )

        # 초기 감정 분석
        initial_analysis = self.analyze_emotional_state(
            initial_message, family_context, {"name": family_member}
        )
        ...
        # 첫 번째 응답
        first_response = self.generate_emotional_response(
            initial_analysis, family_context
        )
        ...
        # 해결 달성 여부
        resolution_achieved = len(emotional_progress) > 1 and emotional_progress[
            -1
        ] in [EmotionalState.CALM, EmotionalState.HAPPY]
    ...
    def get_emotional_statistics(self) -> Dict[str, Any]:
        ...
        for emotion in EmotionalState:
            emotion_conversations = [
                c for c in self.emotional_conversations if c.initial_emotion == emotion
            ]
        ...
        for support_type in SupportType:
            support_count = sum(
                1
                for conv in self.emotional_conversations
                for support in conv.support_provided
        ...
        # 해결률
        resolution_rate = sum(
            1 for conv in self.emotional_conversations if conv.resolution_achieved
        ) / max(1, total_conversations)

        # 유대감 강화율
        bond_strengthening_rate = sum(
            1
            for conv in self.emotional_conversations
            if conv.emotional_bond_strengthened
        ) / max(1, total_conversations)
    ...
    def export_emotional_data(self) -> Dict[str, Any]:
        ...
        return {
            "emotional_conversations": [
                asdict(c) for c in self.emotional_conversations
            ],
            "emotional_patterns": self.emotional_patterns,
            "support_effectiveness": {
                k.value: v for k, v in self.support_effectiveness.items()
            },
...
def test_emotional_conversation_system():
    ...
    analysis = emotional_system.analyze_emotional_state(
        test_message, family_context, speaker_info
    )
    ...
    # 3. 감정적 대화 수행
    conversation = emotional_system.conduct_emotional_conversation(
        "아이1", test_message, family_context
    )
    ...
    print(
        f"✅ 감정적 대화 데이터 내보내기: {len(export_data['emotional_conversations'])}개 대화"
    )
