...
class MetacognitiveLearningSystem:
    ...
    def conduct_learning_session(
        ...
        # 메타인지 과정 결정
        metacognitive_process = self._determine_metacognitive_process(
            strategy, learning_content
        )

        # 학습 효과성 시뮬레이션
        effectiveness = self._simulate_learning_effectiveness(
            strategy, initial_confidence, duration_minutes
        )

        # 최종 신뢰도 계산
        final_confidence = self._calculate_final_confidence(
            initial_confidence, effectiveness, duration_minutes
        )

        # 적응 여부 결정
        adaptation_made = self._determine_adaptation(
            strategy, effectiveness, final_confidence
        )

        # 통찰 획득
        insights_gained = self._generate_learning_insights(
            strategy, effectiveness, adaptation_made
        )
    ...
    def _determine_metacognitive_process(
        self, strategy: LearningStrategy, learning_content: str
    ) -> MetacognitiveProcess:
        """메타인지 과정 결정"""
        if strategy in [
            LearningStrategy.ACTIVE_RECALL,
            LearningStrategy.PRACTICE_TESTING,
        ]:
    ...
    def _simulate_learning_effectiveness(
        self,
        strategy: LearningStrategy,
        initial_confidence: float,
        duration_minutes: int,
    ...
    def _generate_learning_insights(
        ...
        strategy: LearningStrategy,
        effectiveness: LearningEffectiveness,
        adaptation: Optional[AdaptationType],
    ) -> List[str]:
        """학습 통찰 생성"""
        insights = []
        ...
        elif effectiveness in [
            LearningEffectiveness.LOW,
            LearningEffectiveness.VERY_LOW,
        ]:
    ...
    def analyze_metacognitive_performance(
        self, session: LearningSession
    ) -> MetacognitiveAnalysis:
        """메타인지 성과 분석"""
        analysis_id = (
            f"metacognitive_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        improvement_suggestions = self._generate_improvement_suggestions(
            session, strategy_effectiveness
        )
    ...
    def _analyze_strategy_effectiveness(
        ...
        effectiveness_scores[session.strategy_used] = current_effectiveness.get(
            session.effectiveness, 0.5
        )
    ...
    def _analyze_process_insights(
        self, session: LearningSession
    ) -> Dict[MetacognitiveProcess, List[str]]:
        """과정 통찰 분석"""
        process_insights = {}
        ...
        if current_process == MetacognitiveProcess.PLANNING:
            process_insights[current_process] = [
                "학습 계획이 체계적으로 수립되었습니다.",
                "목표 설정이 명확했습니다.",
            ]
        elif current_process == MetacognitiveProcess.MONITORING:
            process_insights[current_process] = [
                "학습 진행 상황을 지속적으로 모니터링했습니다.",
                "문제점을 적시에 발견했습니다.",
            ]
    ...
    def _calculate_analysis_confidence(
        ...
        if session.effectiveness in [
            ...
            LearningEffectiveness.VERY_HIGH,
        ]:
            base_score += 0.1
        elif session.effectiveness in [
            LearningEffectiveness.LOW,
            LearningEffectiveness.VERY_LOW,
        ]:
            base_score -= 0.1
    ...
    def optimize_learning_strategy(
        ...
        """학습 전략 최적화"""
        optimization_id = (
            f"learning_optimization_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        logger.info(
            f"학습 전략 최적화 완료: {current_strategy.value} → {recommended_strategy.value}"
        )

        return optimization

    def _evaluate_current_strategy(
    ...
    def _recommend_optimal_strategy(
        self, performance: Dict[str, Any]
    ) -> LearningStrategy:
        """최적 전략 추천"""
        # 성과 기반 전략 추천
    ...
    def _generate_optimization_reason(
        self, current: LearningStrategy, recommended: LearningStrategy
    ) -> str:
        """최적화 이유 생성"""
    ...
    def _generate_implementation_steps(self, strategy: LearningStrategy) -> List[str]:
        ...
        if strategy == LearningStrategy.SPACED_REPETITION:
            ...
            steps.extend(
                ["자체 평가 문제 생성", "정기적인 테스트 실시", "오답 분석 및 개선"]
            )
    ...
    def get_metacognitive_statistics(self) -> Dict[str, Any]:
        ...
        for strategy in LearningStrategy:
            strategy_sessions = [
                s for s in self.learning_sessions if s.strategy_used == strategy
            ]
            if strategy_sessions:
                avg_effectiveness = sum(
                    1
                    for s in strategy_sessions
                    if s.effectiveness
                    in [LearningEffectiveness.HIGH, LearningEffectiveness.VERY_HIGH]
                ) / len(strategy_sessions)
                strategy_stats[strategy.value] = {
                    "session_count": len(strategy_sessions),
                    "avg_effectiveness": avg_effectiveness,
                    "avg_confidence_gain": sum(
                        s.final_confidence - s.initial_confidence
                        for s in strategy_sessions
                    )
                    / len(strategy_sessions),
                }
        ...
        for process in MetacognitiveProcess:
            process_sessions = [
                s for s in self.learning_sessions if s.metacognitive_process == process
            ]
...
def test_metacognitive_learning_system():
    ...
    print(
        f"   신뢰도 변화: {session.initial_confidence:.2f} → {session.final_confidence:.2f}"
    )
    print(
        f"   적응 여부: {session.adaptation_made.value if session.adaptation_made else '없음'}"
    )
    ...
    print(
        f"✅ 학습 전략 최적화: {optimization.current_strategy.value} → {optimization.recommended_strategy.value}"
    )
    ...
    print(
        f"✅ 메타인지 데이터 내보내기: {len(export_data['learning_sessions'])}개 세션"
    )
