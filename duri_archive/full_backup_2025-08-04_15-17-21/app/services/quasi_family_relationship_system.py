...
class QuasiFamilyRelationshipSystem:
    ...
    def register_quasi_family(
        ...
        try:
            member_id = (
                f"{relation_type.value}_{len(self.relationship_registry) + 1:03d}"
            )
    ...
    def get_members_by_type(
        self, relation_type: QuasiFamilyType
    ) -> List[QuasiFamilyMember]:
        """유형별 구성원 조회"""
        return [
            member
            for member in self.relationship_registry.values()
    ...
    def update_trust_level(self, member_id: str, new_trust_level: float):
        ...
        if member_id in self.relationship_registry:
            ...
            logger.info(
                f"신뢰도 업데이트: {member.name} ({old_trust:.2f} → {member.trust_level:.2f})"
            )
    ...
    def process_feedback_from_member(
        self, member_id: str, feedback_content: str, feedback_type: str = "general"
    ) -> Dict[str, Any]:
        """구성원으로부터 피드백 처리"""
        try:
            if member_id not in self.relationship_registry:
            ...
            # 피드백 처리 경험 저장
            self._store_relationship_experience(
                member, "feedback_processing", processing_result
            )
    ...
    def get_learning_recommendations(self, member_id: str) -> List[str]:
        ...
        if member.relation_type == QuasiFamilyType.MENTOR:
            recommendations.extend(
                ["전략적 사고 훈련", "신념 체계 강화", "철학적 판단 능력 개발"]
            )
        elif member.relation_type == QuasiFamilyType.ALLY:
            recommendations.extend(
                ["협력 전략 수립", "공동 목표 달성 훈련", "상호 보완적 사고 개발"]
            )
        elif member.relation_type == QuasiFamilyType.STAFF:
            recommendations.extend(
                ["실시간 피드백 처리", "감정 조절 훈련", "상황 적응 능력 개발"]
            )
        elif member.relation_type == QuasiFamilyType.FRIEND:
            recommendations.extend(
                ["정서적 표현 훈련", "자기 수용 능력 개발", "관계 유지 기술 습득"]
            )

        return recommendations

    def _store_relationship_experience(
        self,
        member: QuasiFamilyMember,
        experience_type: str,
    ...
    def get_relationship_statistics(self) -> Dict[str, Any]:
        ...
        for relation_type in QuasiFamilyType:
            ...
            type_statistics[relation_type.value] = {
                ...
                "average_trust": (
                    sum(m.trust_level for m in members_of_type) / len(members_of_type)
                    if members_of_type
                    else 0.0
                ),
                "total_interactions": sum(
                    len(m.interaction_history) for m in members_of_type
                ),
            }

        # 전체 통계
        total_interactions = sum(
            len(m.interaction_history) for m in self.relationship_registry.values()
        )
        average_trust = (
            sum(m.trust_level for m in self.relationship_registry.values())
            / total_members
            if total_members > 0
            else 0.0
        )
        ...
        return {
            ...
            "recent_interactions": (
                self.interaction_logs[-10:] if self.interaction_logs else []
            ),
        }

    def export_relationship_registry(self) -> Dict[str, Any]:
        """관계 등록부 내보내기"""
        registry_data = {"DuRiRelationshipRegistry": {}}
