...
class SubtitleBasedLearningSystem:
    ...
    def add_subtitle_segments(
        self, video_content_id: str, subtitle_data: List[Dict[str, Any]]
    ...
    def extract_visual_knowledge_from_video(
        self, video_content_id: str
    ) -> ExtractedVisualKnowledge:
        """영상에서 시각적 지식 추출"""
        try:
            # 영상 콘텐츠 찾기
            video_content = next(
                (vc for vc in self.video_contents if vc.id == video_content_id), None
            )
            ...
            subtitle_segments = [
                ...
                if s.video_content_id == video_content_id
            ]

            # 전체 자막 텍스트 결합
            full_subtitle_text = " ".join(
                [segment.text for segment in subtitle_segments]
            )
            ...
            for segment in subtitle_segments:
                if any(
                    word in segment.text.lower()
                    for word in ["보세요", "보시면", "이렇게", "이런", "그림", "화면"]
                ):
            ...
            for segment in subtitle_segments:
                if any(keyword in segment.text for keyword in family_keywords):
                    family_scenes.append(
                        f"{segment.start_time:.1f}s-{segment.end_time:.1f}s: {segment.text.strip()}"
                    )

            # 학습 카테고리 결정
            learning_category = self._determine_visual_learning_category(
                video_content, full_subtitle_text
            )

            # 난이도 결정
            difficulty_level = self._determine_difficulty_level(
                video_content, subtitle_segments
            )

            # 신뢰도 점수 계산
            confidence_score = self._calculate_visual_confidence_score(
                video_content, subtitle_segments, key_concepts, visual_insights
            )
    ...
    def _determine_visual_learning_category(
        self, video_content: VideoContent, subtitle_text: str
    ) -> VisualLearningCategory:
        """시각적 학습 카테고리 결정"""
        ...
        if any(
            ...
            for word in ["가족", "부모", "자식", "아이", "육아"]
        ):
            return VisualLearningCategory.FAMILY_ACTIVITIES
        elif any(
            word in text_lower
            for word in ["감정", "표정", "기쁨", "슬픔", "화남", "놀람"]
        ):
            return VisualLearningCategory.EMOTIONAL_EXPRESSION
        elif any(
            word in title_lower for word in ["방법", "하는법", "튜토리얼", "가이드"]
        ):
            return VisualLearningCategory.LIFE_SKILLS_DEMO
        elif any(
            word in text_lower for word in ["만들기", "그리기", "공작", "창작", "예술"]
        ):
    ...
    def _determine_difficulty_level(
        self, video_content: VideoContent, subtitle_segments: List[SubtitleSegment]
    ) -> str:
        """난이도 결정"""
        # 영상 길이와 자막 복잡도 기반
        avg_words_per_segment = (
            sum(len(segment.text.split()) for segment in subtitle_segments)
        ...
        if (
            video_content.duration_seconds < 300 and avg_words_per_segment < 10
        ):  # 5분 미만, 간단한 자막
            return "beginner"
        elif (
            video_content.duration_seconds < 900 and avg_words_per_segment < 20
        ):  # 15분 미만, 보통 자막
            return "intermediate"
        elif (
            video_content.duration_seconds < 1800 and avg_words_per_segment < 30
        ):  # 30분 미만, 복잡한 자막
            return "advanced"
        else:
            return "expert"

    def _calculate_visual_confidence_score(
        self,
        video_content: VideoContent,
        ...
        # 자막 품질 점수
        subtitle_quality = (
            sum(segment.confidence for segment in subtitle_segments)
            / len(subtitle_segments)
            if subtitle_segments
            else 0
        )
    ...
    def update_visual_learning_progress(
        self, video_content_id: str, progress_data: Dict[str, Any]
    ) -> VisualLearningProgress:
        """시각적 학습 진도 업데이트"""
        try:
            # 기존 진도 찾기
            existing_progress = next(
                (
                    p
                    for p in self.visual_learning_progress
                    if p.video_content_id == video_content_id
                ),
                None,
            )
            ...
            if existing_progress:
                ...
                existing_progress.visual_comprehension_score = progress_data.get(
                    "visual_comprehension_score",
                    existing_progress.visual_comprehension_score,
                )
                ...
                existing_progress.total_watch_time_seconds += progress_data.get(
                ...
                new_progress = VisualLearningProgress(
                    ...
                    visual_comprehension_score=progress_data.get(
    ...
    def get_visual_learning_recommendations(
        ...
        try:
            ...
            for video in relevant_videos:
                ...
                knowledge = next(
                    (
                        k
                        for k in self.extracted_visual_knowledge
                        if k.video_content_id == video.id
                    ),
                    None,
                )
                ...
                if knowledge:
                    recommendation = {
                        ...
                        "recommendation_score": self._calculate_visual_recommendation_score(
                            video, knowledge, family_context
                        ),
    ...
    def _filter_family_relevant_videos(
        self, family_context: Dict[str, Any] = None
    ) -> List[VideoContent]:
        """가족 관련 영상 필터링"""
    ...
    def _calculate_visual_recommendation_score(
        self,
        video: VideoContent,
        knowledge: ExtractedVisualKnowledge,
        ...
        # 영상 품질 점수 (조회수, 좋아요 기반)
        quality_score = min(
            0.2, (video.view_count / 1000) * 0.01 + (video.like_count / 100) * 0.01
        )
        ...
        return min(
            1.0, base_score + family_relevance + quality_score + difficulty_score
        )
    ...
    def get_visual_learning_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 평균 신뢰도
            avg_confidence = (
                sum(k.confidence_score for k in self.extracted_visual_knowledge)
                / len(self.extracted_visual_knowledge)
                if self.extracted_visual_knowledge
                else 0
            )

            # 평균 학습 진도
            avg_completion = (
                sum(p.completion_percentage for p in self.visual_learning_progress)
                / len(self.visual_learning_progress)
                if self.visual_learning_progress
                else 0
            )
    ...
    def export_visual_learning_data(self) -> Dict[str, Any]:
        ...
        try:
            export_data = {
                ...
                "subtitle_segments": [
                    asdict(segment) for segment in self.subtitle_segments
                ],
                "extracted_visual_knowledge": [
                    asdict(knowledge) for knowledge in self.extracted_visual_knowledge
                ],
                "visual_learning_progress": [
                    asdict(progress) for progress in self.visual_learning_progress
                ],
        ...
        try:
            ...
            for content_data in data.get("video_contents", []):
                ...
                if "upload_date" in content_data and content_data["upload_date"]:
                    content_data["upload_date"] = datetime.fromisoformat(
                        content_data["upload_date"]
                    )
...
def test_subtitle_learning_system():
    ...
    sample_subtitles = [
        {
            "start_time": 0.0,
            "end_time": 5.0,
            "text": "안녕하세요! 오늘은 가족과 함께할 수 있는 재미있는 놀이를 소개해드릴게요.",
        },
        {
            "start_time": 5.0,
            "end_time": 15.0,
            "text": "먼저 준비물을 보시면 종이와 색연필이 필요합니다.",
        },
        {
            "start_time": 15.0,
            "end_time": 30.0,
            "text": "이렇게 접어보세요. 아이들이 따라하기 쉬운 단계별로 설명드릴게요.",
        },
        {
            "start_time": 30.0,
            "end_time": 45.0,
            "text": "아이들의 창의력을 키워주는 좋은 방법이에요. 부모님도 함께 참여하시면 더욱 즐거워집니다.",
        },
        {
            "start_time": 45.0,
            "end_time": 60.0,
            "text": "완성된 작품을 보시면 아이들의 얼굴에 웃음이 가득할 거예요.",
        },
    ]

    subtitle_segments = subtitle_learning.add_subtitle_segments(
        video_content.id, sample_subtitles
    )
    print(f"✅ 자막 세그먼트 추가: {len(subtitle_segments)}개")

    # 3. 시각적 지식 추출
    extracted_knowledge = subtitle_learning.extract_visual_knowledge_from_video(
        video_content.id
    )
    print(
        f"✅ 시각적 지식 추출 완료: {len(extracted_knowledge.key_concepts)}개 키 컨셉"
    )
    ...
    visual_progress = subtitle_learning.update_visual_learning_progress(
        video_content.id, progress_data
    )
    print(
        f"✅ 시각적 학습 진도 업데이트: {visual_progress.completion_percentage}% 완료"
    )
    ...
    recommendations = subtitle_learning.get_visual_learning_recommendations(
        family_context
    )
    ...
    print(
        f"✅ 시각적 학습 통계 생성: {statistics['total_videos']}개 영상, {statistics['total_visual_knowledge']}개 지식"
    )

    # 7. 데이터 내보내기/가져오기
    export_data = subtitle_learning.export_visual_learning_data()
    print(
        f"✅ 시각적 학습 데이터 내보내기: {len(export_data['video_contents'])}개 영상"
    )
