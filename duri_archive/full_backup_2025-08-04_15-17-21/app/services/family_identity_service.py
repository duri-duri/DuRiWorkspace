...
class FamilyIdentityCore:
    ...
    def initialize_family_identity(
        self, family_name: str, initial_members: List[Dict]
    ) -> FamilyIdentity:
        """
        가족 정체성 초기화
        """
    ...
    def record_interaction(
        ...
        try:
            ...
            if member_id in self.relationship_memory:
                ...
                # 신뢰도 계산
                total_interactions = (
                    memory["positive_interactions"] + memory["negative_interactions"]
                )
                if total_interactions > 0:
                    memory["trust_level"] = (
                        memory["positive_interactions"] / total_interactions
                    )
    ...
    def update_family_culture(self, culture_updates: Dict) -> FamilyCulture:
        ...
        try:
            ...
            if "traditions" in culture_updates:
                self.family_identity.culture.traditions.extend(
                    culture_updates["traditions"]
                )
            ...
            if "decision_making_style" in culture_updates:
                self.family_identity.culture.decision_making_style = culture_updates[
                    "decision_making_style"
                ]

            if "conflict_resolution_style" in culture_updates:
                self.family_identity.culture.conflict_resolution_style = (
                    culture_updates["conflict_resolution_style"]
                )

            if "emotional_expression_style" in culture_updates:
                self.family_identity.culture.emotional_expression_style = (
                    culture_updates["emotional_expression_style"]
                )
    ...
    def _calculate_family_strength(self) -> float:
        ...
        total_trust = sum(
            memory["trust_level"] for memory in self.relationship_memory.values()
        )
        total_bonds = sum(
            memory["emotional_bond"] for memory in self.relationship_memory.values()
        )
    ...
    def _analyze_relationship_health(self) -> Dict:
        ...
        for member_id, memory in self.relationship_memory.items():
            member = next(
                (m for m in self.family_identity.members if m.id == member_id), None
            )
            if member:
        ...
        return {
            "overall_health": sum(r["health_score"] for r in relationships)
            / len(relationships),
    ...
    def _analyze_cultural_evolution(self) -> Dict:
        ...
        return {
            ...
            "communication_patterns_count": len(
                self.family_identity.culture.communication_patterns
            ),
            "formation_duration_days": (
                datetime.now() - self.family_identity.formation_date
            ).days,
        }
    ...
    def _get_trust_distribution(self) -> Dict:
        ...
        high_trust = sum(
            1
            for memory in self.relationship_memory.values()
            if memory["trust_level"] >= 0.7
        )
        medium_trust = sum(
            1
            for memory in self.relationship_memory.values()
            if 0.4 <= memory["trust_level"] < 0.7
        )
        low_trust = sum(
            1
            for memory in self.relationship_memory.values()
            if memory["trust_level"] < 0.4
        )
    ...
    def _get_emotional_bond_distribution(self) -> Dict:
        ...
        strong_bonds = sum(
            1
            for memory in self.relationship_memory.values()
            if memory["emotional_bond"] >= 0.7
        )
        moderate_bonds = sum(
            1
            for memory in self.relationship_memory.values()
            if 0.4 <= memory["emotional_bond"] < 0.7
        )
        weak_bonds = sum(
            1
            for memory in self.relationship_memory.values()
            if memory["emotional_bond"] < 0.4
        )
        ...
        try:
            if data.get("family_identity"):
                ...
                self.family_identity = FamilyIdentity(
                    ...
                    members=members,
                    culture=culture,
                    formation_date=datetime.fromisoformat(
                        family_data["formation_date"]
                    ),
