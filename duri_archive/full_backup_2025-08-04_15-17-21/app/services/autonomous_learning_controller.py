...
from .virtual_family_learning_system import (
    VirtualFamilyRole,
    get_virtual_family_learning_system,
)
...
class AutonomousLearningController:
        ...
        try:
            ...
            if topic_scores:
                ...
                best_topic = next(
                    topic
                    for topic in self.learning_topics
                    if topic.topic_id == best_topic_id
                )

                logger.info(
                    f"다음 학습 주제 선택: {best_topic.title} (점수: {topic_scores[best_topic_id]:.2f})"
                )
                return best_topic
    ...
    def _calculate_topic_score(
        self, topic: LearningTopic, preferences: Dict[str, Any]
    ) -> float:
        """주제별 점수 계산"""
        score = 0.0
        ...
        # 카테고리 선호도 점수
        category_pref = preferences["category_preferences"].get(
            topic.category.value, 0.5
        )
        score += category_pref * 0.2

        # 난이도 적합성 점수
        difficulty_diff = abs(
            topic.difficulty_level - preferences["difficulty_preference"]
        )
        score += (1.0 - difficulty_diff) * 0.15

        # 지속시간 적합성 점수
        duration_diff = abs(
            topic.estimated_duration - preferences["duration_preference"]
        )
    ...
    async def start_autonomous_learning(self) -> str:
        ...
        try:
            ...
            logger.info(
                f"자율 학습 시작: {selected_topic.title} (참여자: {len(participants)}명)"
            )
    ...
    async def _monitor_learning_session(self):
        ...
        try:
            ...
            while datetime.now() < start_time + timedelta(
                minutes=session.topic.estimated_duration
            ):
    ...
    async def _complete_learning_session(self):
        ...
        try:
            ...
            logger.info(
                f"학습 세션 완료: {session.topic.title} (만족도: {session.satisfaction_score:.2f})"
            )
    ...
    def _calculate_satisfaction_score(
        self, session: AutonomousLearningSession
    ) -> float:
        """만족도 점수 계산"""
        score = 0.5  # 기본 점수
