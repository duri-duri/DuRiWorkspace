...
class VirtualFamilyLearningSystem:
    ...
    def start_autonomous_learning_session(
        self,
        topic: str,
        ...
        try:
            ...
            logger.info(
                f"자율적 학습 세션 시작: {topic} (참여자: {len(participants)}명)"
            )
    ...
    async def _run_learning_session(self):
        ...
        try:
            ...
            logger.info(
                f"학습 세션 완료: {session.topic} (소요시간: {session.duration:.1f}초)"
            )
    ...
    async def _explore_topic(self, session: LearningSession):
        ...
        for member_id in session.participants:
            ...
            # 가족 구성원별 특화된 탐색 방식
            exploration_result = await self._member_specific_exploration(
                member, session.topic
            )
    ...
    async def _member_specific_exploration(
        self, member: VirtualFamilyMember, topic: str
    ) -> Dict[str, Any]:
        """가족 구성원별 특화된 탐색"""
        exploration_result = {
    ...
    async def _gather_member_perspective(
        self, member: VirtualFamilyMember, topic: str
    ) -> Dict[str, Any]:
        """가족 구성원의 관점 수집"""
        perspective = {
        ...
        # 역할별 특화된 관점
        if member.role == VirtualFamilyRole.WISE_GRANDPARENT:
            perspective["key_insight"] = (
                f"인생의 경험을 통해 {topic}의 진정한 가치를 이해합니다"
            )
            ...
            perspective["key_insight"] = (
                f"체계적인 지식으로 {topic}을 깊이 있게 분석합니다"
            )
            ...
            perspective["key_insight"] = (
                f"창의적인 관점으로 {topic}을 새로운 방식으로 접근합니다"
            )
            ...
            perspective["key_insight"] = (
                f"따뜻한 마음으로 {topic} 학습을 격려하고 지원합니다"
            )
            ...
            perspective["key_insight"] = (
                f"전문적인 지도로 {topic} 학습을 체계적으로 안내합니다"
            )
    ...
    def _calculate_member_feedback(
        self, member: VirtualFamilyMember, session: LearningSession
    ) -> float:
        """가족 구성원별 피드백 점수 계산"""
