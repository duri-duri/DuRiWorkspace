...
class EnhancedPreFamilyLearningSystem:
    ...
    def _generate_question_text(self, topic: LearningTopic, context: Dict[str, Any]) -> str:
        """질문 텍스트 생성"""
        question_templates = {
    ...
    async def _get_response_from_pre_family(self, question: LearningQuestion) -> Optional[LearningResponse]:
        """준 가족 구성원으로부터 응답 받기"""
        try:
            # 적절한 준 가족 구성원 선택
            ...
            logger.info(f"준 가족 응답 생성: {selected_member.name} -> {response_text[:50]}...")
    ...
    def _select_appropriate_member(self, question: LearningQuestion) -> Optional[PreFamilyMember]:
        """적절한 준 가족 구성원 선택"""
        # 주제별 적합한 구성원 매핑
    ...
    async def _generate_response(self, member: PreFamilyMember, question: LearningQuestion) -> str:
        """응답 생성 (시뮬레이션)"""
        # 실제로는 API 호출이 필요
        response_templates = {
            "gpt4_mentor": [
    ...
    async def _evaluate_response(self, response: LearningResponse) -> ResponseEvaluation:
        """응답 평가"""
        try:
            # 의미적 정확성 평가
            ...
            # 종합 점수 계산
            overall_score = (meaning_score + emotional_score + ethical_score + belief_alignment) / 4
            ...
            logger.info(f"응답 평가 완료: {overall_score:.2f} (수용: {evaluation.accepted})")
    ...
    def _generate_evaluation_feedback(self, meaning_score: float, emotional_score: float, 
                                    ethical_score: float, belief_alignment: float) -> str:
        """평가 피드백 생성"""
        feedback_parts = []
    ...
    async def _check_identity_protection(self, evaluation: ResponseEvaluation) -> bool:
        ...
        if evaluation.belief_alignment < self.belief_deviation_threshold:
            logger.warning(f"정체성 보호: 신념 일치도 {evaluation.belief_alignment:.2f}가 임계값 {self.belief_deviation_threshold} 미만")
            return False
        
        return True
    
    async def _accept_and_learn(self, response: LearningResponse, evaluation: ResponseEvaluation):
        """응답 수용 및 학습"""
        try:
            # 메모리에 학습 경험 저장
            learning_experience = {
    ...
    async def _reject_and_improve(self, response: LearningResponse, evaluation: ResponseEvaluation):
        """응답 거부 및 전략 교정"""
        try:
            # 거부 경험 저장
    ...
    def _update_learning_statistics(self, evaluation: ResponseEvaluation):
        ...
        self.average_quality_score = total_score / len(self.response_evaluations) if self.response_evaluations else 0.0
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "acceptance_rate": self.accepted_responses / self.total_interactions if self.total_interactions > 0 else 0.0,
