...
def main():
    ...
    if pass_rate is None and reg:
        ...
        passed = reg.get("passed"); total = reg.get("total")
    ...
    fail_rate_reg_pct = pct(fail_rate_reg) if isinstance(fail_rate_reg, (int,float)) and fail_rate_reg<=1 else (f"{fail_rate_reg}%" if fail_rate_reg is not None else "TBD")
    safety_hit_pct = pct(safety_hit) if isinstance(safety_hit, (int,float)) and safety_hit<=1 else (f"{safety_hit}%" if safety_hit is not None else "TBD")
    explain_score_fmt = f"{explain_score:.2f} / 5.0" if isinstance(explain_score,(int,float)) else "TBD"
    hitl_accept_pct = pct(hitl_accept) if isinstance(hitl_accept,(int,float)) and hitl_accept<=1 else (f"{hitl_accept}%" if hitl_accept is not None else "TBD")
    kappa_fmt = f"{kappa:.2f}" if isinstance(kappa,(int,float)) else "TBD"
    p95_h_fmt = f"{p95_h} h" if isinstance(p95_h,(int,float)) else "TBD"
    quality_fmt = f"{quality:.0f}%" if isinstance(quality,(int,float)) and quality<=1 else (f"{quality:.0f}%" if isinstance(quality,(int,float)) and quality>1 else "TBD")
