...
try:
    ...
    from dynamic_reasoning_graph import DynamicReasoningGraphBuilder, DynamicReasoningGraphAnalyzer
...
class DuRiThoughtFlow:
    ...
    async def counter_argue(self) -> None:
        ...
        for arg in counter_arguments:
            if arg.get('strength', 0) > 0.7:  # 강한 반론
    ...
    async def decide(self, self_reflect: bool = True) -> Dict[str, Any]:
        ...
        logger.info(f"✅ 최종 결정 완료 - 반성 점수: {self.reflection_scores[-1] if self.reflection_scores else 0.0:.2f}")
        return decision
