...
class DuRiOrchestrator:
    ...
    def __init__(self):
        ...
        # ACT-R 병렬 처리 시스템 초기화
        if ACTR_AVAILABLE:
            self.parallel_processor = ACTRParallelProcessor(max_concurrent_tasks=10)
            logger.info("🚀 ACT-R 병렬 처리 시스템 활성화")
        else:
            self.parallel_processor = None
            logger.info("⚠️  ACT-R 병렬 처리 시스템 비활성화 - 기본 모드")
        
        # LIDA 주의 시스템 초기화
        if LIDA_AVAILABLE:
            self.attention_system = LIDAAttentionSystem()
            logger.info("🧠 LIDA 주의 시스템 활성화")
        else:
            self.attention_system = None
            logger.info("⚠️  LIDA 주의 시스템 비활성화 - 기본 모드")
    ...
    async def _execute_judgment_phase(self):
        ...
        try:
            if self.attention_system and self.parallel_processor:
                ...
                judgment_tasks = [
                    lambda: self._call_judgment_system() if 'judgment_system' in self.systems else self._default_judgment(),
    ...
    async def _execute_feedback_phase(self):
        ...
        try:
            if self.parallel_processor:
                # ACT-R 병렬 처리 사용
                feedback_tasks = [
                    lambda: self._call_feedback_system() if 'feedback_system' in self.systems else self._default_feedback(),
