...
class MemoryAssociationSystem:
            ...
            if memory_id in self.memory_graph:
                for related_id in self.memory_graph[memory_id]:
                    ...
                    if link_key in self.associations:
                        ...
                        if (association_type is None or link.association_type == association_type) and \
                           link.strength >= min_strength:
    ...
    async def strengthen_association(self, source_id: str, target_id: str, 
                                   association_type: AssociationType, 
                                   additional_evidence: List[str]) -> bool:
        """연관성 강화"""
        try:
    ...
    # 내부 메서드들
    async def _analyze_semantic_associations(self, memory_id: str, memory_content: str,
    ...
    async def _analyze_temporal_associations(self, memory_id: str, memory_context: Dict[str, Any],
                                           all_memories: Dict[str, Any]) -> List[AssociationLink]:
        """시간적 연관성 분석"""
        try:
            associations = []
    ...
    async def _analyze_emotional_associations(self, memory_id: str, memory_content: str,
        ...
        try:
            ...
            for other_id, other_memory in all_memories.items():
                ...
                if hasattr(other_memory, 'content'):
                    other_emotion = self._extract_emotion(other_memory.content, getattr(other_memory, 'context_info', {}))
                    ...
                    if emotion_similarity > self.emotional_threshold:
                        ...
                        association = AssociationLink(
                            ...
                            confidence=0.8,
                            created_at=datetime.now(),
                            metadata={"current_emotion": current_emotion, "other_emotion": other_emotion},
                            evidence=evidence
                        )
    ...
    async def _analyze_contextual_associations(self, memory_id: str, memory_context: Dict[str, Any],
                                             all_memories: Dict[str, Any]) -> List[AssociationLink]:
        """맥락적 연관성 분석"""
        try:
            associations = []
    ...
    async def _analyze_thematic_associations(self, memory_id: str, memory_content: str,
                                           all_memories: Dict[str, Any]) -> List[AssociationLink]:
        """주제적 연관성 분석"""
        try:
            associations = []
                ...
                if hasattr(other_memory, 'content'):
                    ...
                    if theme_similarity > 0.5:  # 주제 임계값
                        ...
                        association = AssociationLink(
                            ...
                            confidence=0.6,
                            created_at=datetime.now(),
                            metadata={"current_themes": current_themes, "other_themes": other_themes},
                            evidence=evidence
                        )
...
async def test_memory_association():
    ...
    test_memories = {
        "mem1": {
            ...
            "context_info": {"type": "learning", "emotion": "excited", "importance": "high"},
            "created_at": datetime.now()
        },
        "mem2": {
            "content": "친구와 함께 영화를 봤다. 정말 재미있었다.",
            "vector_data": [0.2 + i * 0.01 for i in range(128)],
            "context_info": {"type": "experience", "emotion": "happy", "importance": "medium"},
            "created_at": datetime.now() - timedelta(hours=2)
        },
        "mem3": {
            "content": "코딩할 때 항상 같은 패턴을 사용하는 것을 발견했다.",
            "vector_data": [0.3 + i * 0.01 for i in range(128)],
            "context_info": {"type": "pattern", "emotion": "neutral", "importance": "medium"},
            "created_at": datetime.now() - timedelta(hours=1)
        },
        "mem4": {
            "content": "시험 결과가 나왔는데 실망스럽다.",
            "vector_data": [0.4 + i * 0.01 for i in range(128)],
            "context_info": {"type": "emotion", "emotion": "disappointed", "importance": "high"},
