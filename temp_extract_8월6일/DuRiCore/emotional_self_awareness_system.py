...
class SelfAwarenessLevel(Enum):
    """자기 인식 수준"""
    UNCONSCIOUS = "unconscious"  # 무의식적 (0.0-0.2)
    AWARE = "aware"             # 인식 (0.2-0.4)
    UNDERSTANDING = "understanding" # 이해 (0.4-0.6)
    MASTERY = "mastery"         # 숙달 (0.6-0.8)
    TRANSCENDENT = "transcendent" # 초월 (0.8-1.0)

@dataclass
class EmotionalTrigger:
    """감정적 트리거"""
    trigger_id: str
    trigger_type: str  # external, internal, memory, expectation
    description: str
...
class SelfAwarenessMetrics:
    """자기 인식 측정 지표"""
    emotional_clarity: float = 0.5      # 감정 명확성 (0.0-1.0)
    trigger_recognition: float = 0.5    # 트리거 인식 (0.0-1.0)
    pattern_awareness: float = 0.5      # 패턴 인식 (0.0-1.0)
    response_monitoring: float = 0.5    # 반응 모니터링 (0.0-1.0)
    emotional_maturity: float = 0.5     # 감정적 성숙도 (0.0-1.0)
    
    @property
    def overall_self_awareness(self) -> float:
        """전체 자기 인식 수준"""
        return (self.emotional_clarity + self.trigger_recognition + 
                self.pattern_awareness + self.response_monitoring + 
                self.emotional_maturity) / 5.0
...
class EmotionalSelfAwarenessSystem:
    ...
    async def identify_emotion_patterns(self) -> Dict[str, Any]:
        ...
        # 패턴 분석
        patterns = {
            "dominant_emotion": self._find_dominant_emotion(emotion_sequence),
            "intensity_trend": self._analyze_intensity_trend(intensity_sequence),
            "emotion_cycles": self._identify_emotion_cycles(emotion_sequence),
            "trigger_response_patterns": self._analyze_trigger_response_patterns()
        }
    ...
    async def assess_emotional_maturity(self) -> Dict[str, Any]:
        ...
        # 전체 성숙도 점수
        maturity_score = (emotional_stability + response_appropriateness + 
                         self_regulation_ability + empathy_level + 
        ...
        improvement_areas = self._identify_improvement_areas({
            ...
            "self_regulation_ability": self_regulation_ability,
    ...
    def get_awareness_state(self) -> Dict[str, Any]:
        """자기 인식 상태 반환"""
        return {
            "awareness_metrics": asdict(self.awareness_state.awareness_metrics),
            "trigger_count": len(self.awareness_state.emotional_triggers),
            "response_count": len(self.awareness_state.emotional_responses),
            "last_update": self.awareness_state.last_update.isoformat()
        }
    ...
    def _identify_emotion_cause(self, trigger_data: Dict[str, Any]) -> EmotionCategory:
        ...
        emotion_mapping = {
            'success': EmotionCategory.JOY,
            'failure': EmotionCategory.SADNESS,
            'threat': EmotionCategory.FEAR,
            'injustice': EmotionCategory.ANGER,
            'surprise': EmotionCategory.SURPRISE,
            'disgust': EmotionCategory.DISGUST,
            'excitement': EmotionCategory.EXCITEMENT,
            'satisfaction': EmotionCategory.CONTENTMENT,
            'uncertainty': EmotionCategory.ANXIETY
        }
    ...
    def _identify_emotion_cycles(self, emotions: List[EmotionCategory]) -> List[Dict[str, Any]]:
        ...
        for i in range(len(emotions) - 3):
            ...
            if len(set(cycle)) >= 3:  # 최소 3개 다른 감정
                cycles.append({
                    "start_index": i,
                    "emotions": [e.value for e in cycle],
                    "duration": len(cycle)
                })
    ...
    async def _update_trigger_recognition_metrics(self, trigger: EmotionalTrigger) -> None:
        ...
        self.awareness_state.awareness_metrics.trigger_recognition = min(1.0, 
            self.awareness_state.awareness_metrics.trigger_recognition + 0.01)
    
    async def _update_response_monitoring_metrics(self, response: EmotionalResponse) -> None:
        """반응 모니터링 메트릭 업데이트"""
        # 실제 구현에서는 더 정교한 업데이트 로직 사용
        self.awareness_state.awareness_metrics.response_monitoring = min(1.0, 
            self.awareness_state.awareness_metrics.response_monitoring + 0.01)
    
    async def _update_pattern_awareness_metrics(self, patterns: Dict[str, Any]) -> None:
        """패턴 인식 메트릭 업데이트"""
        # 실제 구현에서는 더 정교한 업데이트 로직 사용
        self.awareness_state.awareness_metrics.pattern_awareness = min(1.0, 
            self.awareness_state.awareness_metrics.pattern_awareness + 0.01)
    ...
    test_triggers = [
        {
            ...
            "description": "프로젝트 완료",
            "intensity": 0.8,
            "context": {"external_event": True}
        },
        {
            "type": "failure",
            "description": "목표 달성 실패",
            "intensity": 0.6,
            "context": {"internal_thought": True}
        },
        {
            "type": "threat",
            "description": "새로운 도전",
            "intensity": 0.7,
            "context": {"expectation": True}
        }
    ]
    ...
    test_responses = [
        {
            ...
            "intensity": 0.8,
            "duration": 120.0,
            "physical": {"heart_rate_increased": True},
            "cognitive": {"attention_focused": True, "memory_enhanced": True},
            "behavioral": {"problem_solving": True}
        },
        {
            "emotion": "sadness",
            "intensity": 0.6,
            "duration": 300.0,
            "physical": {"muscle_tension": True},
            "cognitive": {"thought_racing": True},
            "behavioral": {"withdrawal": True}
        },
        {
            "emotion": "fear",
            "intensity": 0.7,
            "duration": 180.0,
            "physical": {"sweating": True, "trembling": True},
            "cognitive": {"judgment_impaired": True},
            "behavioral": {"seeking_support": True}
        }
    ]
