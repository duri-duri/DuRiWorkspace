...
class ReasoningGraphBuilder:
    ...
    def build_reasoning_chain(self, situation: str, 
                            semantic_context: Dict[str, Any],
                            philosophical_arguments: Dict[str, Any]) -> ReasoningGraph:
        """추론 체인 구축"""
    ...
    def _create_situation_nodes(self, situation: str, semantic_context) -> Dict[str, ReasoningNode]:
        ...
        situation_type_node = ReasoningNode(
            ...
            metadata={"situation_type": semantic_context.situation_type.value if hasattr(semantic_context, 'situation_type') else 'unknown'}
        )
    ...
    def _create_philosophical_nodes(self, philosophical_arguments: Dict[str, Any]) -> Dict[str, ReasoningNode]:
        ...
        if "kantian" in philosophical_arguments:
            ...
            kantian_node = ReasoningNode(
                ...
                metadata={"reasoning_type": "kantian", "strength": kantian.strength if hasattr(kantian, 'strength') else 0.5}
            )
        ...
        if "utilitarian" in philosophical_arguments:
            ...
            utilitarian_node = ReasoningNode(
                ...
                metadata={"reasoning_type": "utilitarian", "strength": utilitarian.strength if hasattr(utilitarian, 'strength') else 0.5}
            )
        ...
        if "integrated" in philosophical_arguments:
            ...
            integrated_node = ReasoningNode(
                ...
                metadata={"reasoning_type": "integrated", "strength": integrated.get('strength')}
            )
    ...
    def _create_reasoning_edges(self, nodes: Dict[str, ReasoningNode], 
                               situation: str, 
                               philosophical_arguments: Dict[str, Any]) -> Dict[str, ReasoningEdge]:
        """추론 엣지 생성"""
        edges = {}
    ...
    def _create_conclusion_nodes(self, nodes: Dict[str, ReasoningNode], 
                                edges: Dict[str, ReasoningEdge]) -> Dict[str, ReasoningNode]:
        """결론 노드 생성"""
        conclusion_nodes = {}
    ...
    def _calculate_graph_confidence(self, nodes: Dict[str, ReasoningNode], 
                                  edges: Dict[str, ReasoningEdge]) -> float:
        """그래프 신뢰도 계산"""
        if not nodes:
            return 0.0
    ...
    def _calculate_graph_complexity(self, nodes: Dict[str, ReasoningNode], 
                                  edges: Dict[str, ReasoningEdge]) -> float:
        """그래프 복잡도 계산"""
        complexity = 0.0
...
class LogicalInferenceEngine:
    ...
    async def apply_logical_inference(self, premises: List[str], 
    ...
    def _are_contradictory(self, premise1: str, premise2: str) -> bool:
        ...
        for pattern1, pattern2 in contradiction_patterns:
            if (any(word in premise1 for word in pattern1) and 
                any(word in premise2 for word in pattern2)):
                return True
...
class ReasoningGraphAnalyzer:
    ...
    async def analyze_reasoning_process(self, situation: str, 
                                      semantic_context: Dict[str, Any],
        ...
        # 2. 논리적 추론 적용
        premises = [node.content for node in reasoning_graph.nodes.values() 
                   if node.node_type == NodeType.PREMISE]
        conclusions = [node.content for node in reasoning_graph.nodes.values() 
    ...
    def _assess_reasoning_quality(self, graph: ReasoningGraph, 
