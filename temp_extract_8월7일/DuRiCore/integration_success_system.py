...
class ConflictDetectionSystem:
    ...
    async def _check_for_integration_conflict(self, element1: Dict[str, Any], 
                                            element2: Dict[str, Any]) -> Optional[IntegrationConflict]:
        """통합 충돌 확인"""
...
class IntegrationPrioritySystem:
    ...
    async def determine_integration_priorities(self, knowledge_elements: List[Dict[str, Any]]) -> List[IntegrationPriorityItem]:
        """지식 통합의 우선순위 결정"""
        priorities = []
...
class IntegrationSuccessSystem:
    ...
    async def _calculate_completion_rate(self, knowledge_elements: List[Dict[str, Any]], 
                                       conflicts: List[IntegrationConflict]) -> float:
        """완성도 계산"""
        ...
        completed_elements = len([elem for elem in knowledge_elements 
                                if str(elem.get('id', 'unknown')) not in conflict_element_ids])
        
        return completed_elements / len(knowledge_elements)
    
    async def _calculate_conflict_resolution_rate(self, conflicts: List[IntegrationConflict], 
                                                resolved_conflicts: List[Dict[str, Any]]) -> float:
        """충돌 해결률 계산"""
        if not conflicts:
            return 1.0
    ...
    async def _evaluate_element_consistency(self, element1: Dict[str, Any], 
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_success_score': np.mean([imp['improvement_score'] for imp in self.improvement_history]) if self.improvement_history else 0.0,
            'last_improvement_time': self.improvement_history[-1]['improvement_details']['improvement_time'] if self.improvement_history else None
        }
