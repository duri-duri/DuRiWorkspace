...
try:
    ...
    _HAS_DATA_ADAPTER = True
except Exception:
    _HAS_DATA_ADAPTER = False
...
class Phase1ProblemSolver:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.force_topk_features = int(self.config.get("force_topk_features", 2))   # ★ 3→2 (과적합 완화)
        ...
        # [PATCH1] 하드닝 옵션 추가
        self.load_fixed_split = bool(self.config.get("load_fixed_split", False))
        self.split_dir = self.config.get("split_dir", "./artifacts_phase1")
        self.reseed_on_imbalance = bool(self.config.get("reseed_on_imbalance", False))
        self.gate = {
            "r2_gap_max": float(self.config.get("gate_r2_gap_max", 0.02)),
            "nrmse_max": float(self.config.get("gate_nrmse_max", 1.30)),
            "mse_ratio_max": float(self.config.get("gate_mse_ratio_max", 1.50)),
        }
        
        # 실데이터 옵션
        self.data_path: Optional[str] = self.config.get("data_path") or self.config.get("data_source")  # 두 키 모두 허용
        self.schema_map: Optional[Dict[str, Any]] = self.config.get("schema_map")  # 스키마 dict
        self.sample_rows: Optional[int] = self.config.get("sample_rows")
    ...
    def load_data(self) -> pd.DataFrame:
        """
        실데이터 우선, 없으면 더미 데이터 생성.
        """
        # ★ 실데이터가 설정된 경우 무조건 RealDataAdapter 사용
        if self.data_path and self.schema_map:
            try:
                adapter = RealDataAdapter(self.data_path, self.schema_map, self.sample_rows)
                df = adapter.load()
                print(f"   📁 실데이터 로드 완료: {df.shape[0]}행 × {df.shape[1]}열")
                
                # ★ 실데이터의 경우 split_xy를 통해 ML 학습용 특성만 선택
                X, y = adapter.split_xy()
                df_ml = pd.concat([X, y], axis=1)
                print(f"   🔧 ML 학습용 특성 선택됨: {X.shape[1]}개 특성")
                print(f"   🔧 선택된 특성: {list(X.columns)}")
                return df_ml
            except Exception as e:
                print(f"   ⚠️ 실데이터 로드 실패: {e}")
                print("   🔄 더미 데이터로 폴백")
    ...
    def generate_base_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        # ★ 디버깅: 컬럼 상태 확인
        print(f"   🔍 generate_base_features: X 컬럼 {len(X.columns)}개")
        print(f"   🔍 컬럼 목록: {list(X.columns)}")
        print(f"   🔍 데이터 타입: {X.dtypes.to_dict()}")
        
        return X, y

    def generate_advanced_features(self, X: pd.DataFrame) -> pd.DataFrame:
        # ★ 디버깅: 입력 상태 확인
        print(f"   🔍 generate_advanced_features 입력: X 컬럼 {len(X.columns)}개")
        print(f"   🔍 입력 컬럼 목록: {list(X.columns)}")
        ...
        # ★ 디버깅: 출력 상태 확인
        print(f"   🔍 generate_advanced_features 출력: X 컬럼 {len(X.columns)}개")
        print(f"   🔍 출력 컬럼 목록: {list(X.columns)}")
    ...
    # [PATCH1] 분할 저장/로드 헬퍼
    def _save_split_indices(self, idx_train, idx_valid, idx_test):
        os.makedirs(self.split_dir, exist_ok=True)
        np.save(os.path.join(self.split_dir, "split_idx_train.npy"), idx_train)
        np.save(os.path.join(self.split_dir, "split_idx_valid.npy"), idx_valid)
        np.save(os.path.join(self.split_dir, "split_idx_test.npy"),  idx_test)

    def _try_load_split_indices(self, n_total: int) -> Tuple[np.ndarray,np.ndarray,np.ndarray]:
        p_tr = os.path.join(self.split_dir, "split_idx_train.npy")
        p_va = os.path.join(self.split_dir, "split_idx_valid.npy")
        p_te = os.path.join(self.split_dir, "split_idx_test.npy")
        if self.load_fixed_split and all(os.path.exists(p) for p in [p_tr,p_va,p_te]):
            tr, va, te = np.load(p_tr), np.load(p_va), np.load(p_te)
            if tr.ndim==va.ndim==te.ndim==1 and len(set(tr)|set(va)|set(te))<=n_total:
                print("📎 fixed split loaded from artifacts.")
                return tr.astype(int), va.astype(int), te.astype(int)
        return None, None, None
    
    # [PATCH2] 층화 품질 가드
    def _check_strata_balance(self, bins: np.ndarray, max_ratio: float = 2.0) -> bool:
        cnt = np.bincount(bins, minlength=bins.max()+1)
        pr  = cnt / max(1, cnt.sum())
        ok = (pr.min() > 0) and ((pr.max() / pr.min()) <= max_ratio)
        if not ok:
            print(f"⚠️ strata imbalance: max/min={pr.max():.3f}/{pr.min():.3f} > {max_ratio}")
        return ok
    ...
    def _train_random_forest(self, X_tr, y_tr, X_val, y_val) -> Dict[str, Any]:
        if self.strong_reg:
            mdl = RandomForestRegressor(
                n_estimators=400,         # ★ 300 → 400 (안정성 향상)
                max_depth=6,              # ★ None → 6 (과적합 방지)
                min_samples_leaf=8,       # 8 유지
                min_samples_split=16,     # ★ 2 → 16 (과적합 방지)
                max_features=0.6,         # ★ "sqrt" → 0.6 (과적합 방지)
                bootstrap=True,           # ★ 명시적 설정
        ...
        # [PATCH3] 누수 가드: 파이프라인으로 래핑
        mdl = self._make_rf_pipeline(mdl)
        mdl.fit(X_tr, y_tr)
    ...
    def _train_xgboost(self, X_tr, y_tr, X_val, y_val) -> Optional[Dict[str, Any]]:
        ...
        if self.strong_reg:
            params = dict(
                n_estimators=400,         # 300 → 400 (안정성 향상)
                max_depth=3,              # 3 유지
                min_child_weight=8,       # 8 유지
                gamma=0.2,                # ★ 0 → 0.2 (과적합 방지)
                subsample=0.7,            # 0.7 유지
                colsample_bytree=0.6,     # ★ 0.7 → 0.6 (과적합 방지)
                reg_lambda=2.5,           # ★ 8.0 → 2.5 (균형 조정)
                reg_alpha=0.3,            # ★ 0 → 0.3 (L1 정규화 추가)
                learning_rate=0.06,       # 0.06 유지
                random_state=self.random_state,
                n_jobs=-1,
                tree_method="hist",
            )
        ...
        # [PATCH3] 누수 가드: 파이프라인으로 래핑
        mdl = self._make_xgb_pipeline(mdl)
        # Pipeline에서는 eval_set을 직접 전달할 수 없으므로, XGBoost 모델에만 적용
        if hasattr(mdl, 'named_steps') and 'est' in mdl.named_steps:
            # Pipeline의 XGBoost 모델에만 eval_set 적용
            mdl.fit(X_tr, y_tr, est__eval_set=[(X_val, y_val)], est__verbose=False)
        else:
            mdl.fit(X_tr, y_tr)
        pred_val = mdl.predict(X_val)
    ...
    # [PATCH3] 누수 가드용 전처리 + 파이프라인 래퍼
    def _safe_preproc(self):
        def _f(X):
            # 옵션: 보수적 clip (config에서 on/off)
            if bool(self.config.get("pre_clip_iqr", False)):
                X = X.copy()
                ...
                return X
            return X
        return FunctionTransformer(_f, validate=False)
        
    def _make_rf_pipeline(self, rf_reg):
        return Pipeline([("pre", self._safe_preproc()), ("est", rf_reg)])

    def _make_xgb_pipeline(self, xgb_reg):
        return Pipeline([("pre", self._safe_preproc()), ("est", xgb_reg)])
    ...
    # [PATCH5] 평가 게이트 + 폴백
    def _passes_gate(self, gap: dict) -> bool:
        return (gap.get("r2_gap", 1.0) <= self.gate["r2_gap_max"] and
                gap.get("nrmse_ratio", 1e9) <= self.gate["nrmse_max"] and
                gap.get("mse_ratio", 1e9)  <= self.gate["mse_ratio_max"])
    ...
    # [PATCH4] 이소토닉 보정 샘플 캡
    def _fit_isotonic_capped(self, iso_model, y_val, pred_val, max_n: int = 2000):  # ★ 5000→2000 (과보정 방지)
        yv = np.asarray(y_val).ravel()
        pv = np.asarray(pred_val).ravel()
        n  = min(len(yv), max_n)
        rs = np.random.RandomState(self.random_state)
        idx = rs.choice(len(yv), n, replace=False) if len(yv) > n else np.arange(len(yv))
        iso_model.fit(pv[idx], yv[idx])
        return iso_model
    ...
    def backup_pipeline(self, when: str, tag: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # [PATCH8] 스냅샷 메타에 환경 버전 기록
            try:
                import platform
                env = {
                    "python": platform.python_version(),
                    "numpy": np.__version__,
                    "sklearn": getattr(sklearn, "__version__", "n/a") if 'sklearn' in globals() else "n/a",
                    "xgboost": getattr(xgboost, "__version__", "n/a") if 'xgboost' in globals() else "n/a",
                }
                try:
                    import psutil
                    env.update({
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if self.data_path and self.schema_map:
            print(f"   📊 실데이터 스키마:")
            print(f"      - Target: {self.schema_map.get('target', 'N/A')}")
            print(f"      - Numeric: {len(self.schema_map.get('numeric', []))}개")
            print(f"      - Categorical: {len(self.schema_map.get('categorical', []))}개")
            print(f"      - Drop: {len(self.schema_map.get('drop', []))}개")
            print(f"      - Datetime: {len(self.schema_map.get('datetime', []))}개")
        ...
        # ★ 디버깅: 특성 선택 직전 데이터 상태 확인
        print(f"   🔍 특성 선택 직전: X 컬럼 {len(X.columns)}개")
        print(f"   🔍 컬럼 목록: {list(X.columns)}")
        print(f"   🔍 데이터 타입: {X.dtypes.to_dict()}")
        
        # datetime/categorical 컬럼이 있다면 강제 제거
        datetime_cols = [col for col in X.columns if X[col].dtype == 'object' and X[col].str.contains('2023-|2024-', na=False).any()]
        if datetime_cols:
            print(f"   ⚠️ datetime 컬럼 강제 제거: {datetime_cols}")
        ...
        # [PATCH6] 분할 저장/로드 + strata 가드 통합
        # 0) 로드 시도
        tr, va, te = self._try_load_split_indices(len(y))
        if tr is not None:
            idx_train, idx_valid, idx_test = tr, va, te
            X_tr, X_val, X_te = X.iloc[idx_train], X.iloc[idx_valid], X.iloc[idx_test]
            y_tr, y_val, y_te = y.iloc[idx_train], y.iloc[idx_valid], y.iloc[idx_test]
            print(f"   📎 고정 분할 로드됨: 훈련 {len(X_tr)}개, 검증 {len(X_val)}개, 테스트 {len(X_te)}개")
        else:
            # 기존 분할 로직으로 idx_train/idx_valid/idx_test 계산
            if getattr(self, "use_stratified_split", True):
                ...
                # strata 품질 체크
                y_bins = self._make_strat_labels(y, q=10)
                if not self._check_strata_balance(y_bins):
                    if self.reseed_on_imbalance:
                        print("🔁 reseed on imbalance enabled → reseeding split")
                        self.random_state = int(self.random_state) + 1
                        X_tr, X_val, X_te, y_tr, y_val, y_te = self._split_train_valid_test_stratified(
                            X, y, valid_size=0.2, test_size=0.2, q=10
                        )
            ...
            # 최초 저장
            idx_train = X.index.get_indexer(X_tr.index)
            idx_valid = X.index.get_indexer(X_val.index)
            idx_test = X.index.get_indexer(X_te.index)
            self._save_split_indices(idx_train, idx_valid, idx_test)
        ...
        # [PATCH5] 평가 게이트 적용
        gap_ok = self._passes_gate(final_metrics['gap_analysis'])
        if gap_ok:
            print("✅ 평가 게이트 통과: 과적합 없음")
        else:
            print("⚠️ 평가 게이트 경고: 과적합 의심 (보정/스태킹 비활성화 권장)")
            # 게이트 실패 시 단순 모델로 폴백 고려
            print("   💡 폴백 옵션: RF 단일 모델 또는 단순 평균 앙상블")
            
            # [PATCH5] 게이트 자동 폴백 강화
            print("   🔄 자동 폴백: RF 단일 모델로 전환 (견고한 베이스라인)")
            # RF 단일 모델로 최종 선택 변경
            best = next(c for c in cands if c["name"] == "rf")
            print(f"   ✅ 폴백 완료: {best['name']} 모델 선택됨")
