...
class DeepLearningModel(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_sizes: List[int] = None, dropout_rate: float = 0.1):
        super(DeepLearningModel, self).__init__()
        
        if hidden_sizes is None:
            hidden_sizes = [64, 32]  # 더 간단한 구조
...
class Phase2DeepLearningIntegration:
    ...
    def __init__(self, phase1_models: Dict[str, Any] = None):
        ...
        self.integration_config = {
            'ensemble_weight_ml': 0.6,      # ML 모델 가중치 (증가)
            'ensemble_weight_dl': 0.4,      # 딥러닝 모델 가중치 (감소)
            'ensemble_method': 'weighted_average',  # 앙상블 방법 추가
            'training_epochs': 200,         # 학습 에포크 (증가)
            'learning_rate': 0.0005,        # 학습률 (감소 - 더 안정적)
            'batch_size': 64,               # 배치 크기 (증가)
            'early_stopping_patience': 15   # 조기 종료 인내심 (증가)
        }
    ...
    def integrate_with_phase1(self, phase1_solver) -> Dict[str, Any]:
        """Phase 1과의 통합 수행"""
        try:
            if not hasattr(phase1_solver, 'get_integration_interface'):
                return {'success': False, 'reason': 'Phase 1 통합 인터페이스 없음'}
            
            # Phase 1 데이터 가져오기
            phase1_data = phase1_solver.get_integration_interface()
            
            # 통합 상태 확인
            if not phase1_data.get('status') == 'ready_for_integration':
                return {'success': False, 'reason': 'Phase 1이 통합 준비되지 않음'}
            
            # Phase 1 모델들을 Phase 2에 통합
            self.phase1_models = phase1_data.get('models', {})
            
            # 통합 결과 반환
            integration_result = {
                'success': True,
                'phase1_integrated': True,
                'phase1_models_count': len(self.phase1_models),
                'deep_learning_ready': bool(self.deep_learning_model),
                'hybrid_system_ready': bool(self.deep_learning_model and self.phase1_models),
                'integration_timestamp': datetime.now().isoformat()
            }
            
            logger.info("Phase 1과의 통합 완료")
            return integration_result
            
        except Exception as e:
            logger.error(f"Phase 1 통합 실패: {e}")
            return {'success': False, 'reason': str(e)}
    
    def get_phase1_integration_status(self) -> Dict[str, Any]:
        """Phase 1 통합 상태 확인"""
        return {
            'phase1_models_available': bool(self.phase1_models),
            'phase1_models_count': len(self.phase1_models),
            'deep_learning_ready': bool(self.deep_learning_model),
            'integration_ready': bool(self.deep_learning_model and self.phase1_models)
        }
    
    def create_enhanced_hybrid_system(self) -> Dict[str, Any]:
        """Phase 1과 Phase 2를 결합한 향상된 하이브리드 시스템"""
        try:
            if not self.phase1_models:
                return {'success': False, 'reason': 'Phase 1 모델이 통합되지 않음'}
            
            if not self.deep_learning_model:
                return {'success': False, 'reason': '딥러닝 모델이 학습되지 않음'}
            
            # 향상된 하이브리드 시스템 생성
            hybrid_result = self.create_hybrid_system()
            
            if not hybrid_result.get('success'):
                return hybrid_result
            
            # Phase 1과 Phase 2 통합 정보 추가
            enhanced_result = {
                **hybrid_result,
                'phase1_integration': {
                    'models_available': list(self.phase1_models.keys()),
                    'integration_status': 'completed'
                },
                'phase2_integration': {
                    'deep_learning_ready': True,
                    'model_performance': self.performance_metrics.get('deep_learning', {})
                },
                'hybrid_system': {
                    'status': 'fully_integrated',
                    'total_models': len(self.phase1_models) + 1,  # +1 for deep learning
                    'ensemble_method': self.integration_config['ensemble_method']
                }
            }
