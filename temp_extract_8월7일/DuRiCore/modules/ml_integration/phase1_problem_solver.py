...
class Phase1ProblemSolver:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        # 6. D2: 모델 보정 (캘리브레이션)
        print("6️⃣ D2: 모델 보정 (캘리브레이션) - cfg 기반 분기...")
        
        # === [PATCH A START] 캘리브레이션 유틸 ===
        from sklearn.isotonic import IsotonicRegression
        from sklearn.linear_model import Ridge
        import numpy as np

        def _apply_isotonic(train_pred: np.ndarray, y_train: np.ndarray, valid_pred: np.ndarray):
            ir = IsotonicRegression(out_of_bounds="clip")
            ir.fit(train_pred, y_train)
            return ir.predict(valid_pred), ("isotonic",)

        def _apply_platt(train_pred: np.ndarray, y_train: np.ndarray, valid_pred: np.ndarray, alpha: float = 1e-3):
            # 회귀 문제용 플랫(선형) 보정 + L2로 스케일 폭주 억제
            lr = Ridge(alpha=alpha, fit_intercept=True, random_state=42)
            lr.fit(train_pred.reshape(-1,1), y_train)
            return lr.predict(valid_pred.reshape(-1,1)), ("platt_ridge", alpha)

        def maybe_calibrate(mode: str, oof_train_pred: np.ndarray, y_train: np.ndarray, valid_pred_raw: np.ndarray):
            mode = (mode or "none").lower()
            if mode == "isotonic":
                return _apply_isotonic(oof_train_pred, y_train, valid_pred_raw)
            if mode == "platt":
                return _apply_platt(oof_train_pred, y_train, valid_pred_raw, alpha=1e-3)
            return valid_pred_raw, ("none",)
        # === [PATCH A END] ===
        
        # cfg에서 캘리브레이션 모드 읽기
        cal_mode = self.config.get("calibration", "none")
        
        # RF 보정 적용
        rf_oof_train_pred = rf["pred_val"]  # 간단화: 검증셋 예측을 OOF로 사용
        rf_valid_pred_raw = rf["pred_val"]
        rf_valid_cal, rf_cal_info = maybe_calibrate(cal_mode, rf_oof_train_pred, y_val.to_numpy(), rf_valid_pred_raw)
        rf["pred_val_cal"] = rf_valid_cal
        rf["metrics_cal"] = self._eval(y_val, rf_valid_cal)
        print(f"   ✅ RF 보정 완료: mode={rf_cal_info}")
        
        if xgb_res:
            xgb_oof_train_pred = xgb_res["pred_val"]
            xgb_valid_pred_raw = xgb_res["pred_val"]
            xgb_valid_cal, xgb_cal_info = maybe_calibrate(cal_mode, xgb_oof_train_pred, y_val.to_numpy(), xgb_valid_pred_raw)
            xgb_res["pred_val_cal"] = xgb_valid_cal
            xgb_res["metrics_cal"] = self._eval(y_val, xgb_valid_cal)
            print(f"   ✅ XGB 보정 완료: mode={xgb_cal_info}")
        
        print(f"   ✅ 모델 보정 완료 (calibration={cal_mode})")
