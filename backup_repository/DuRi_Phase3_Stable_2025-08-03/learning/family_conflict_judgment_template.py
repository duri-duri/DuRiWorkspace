...
class FamilyConflictJudgmentTemplate:
    ...
    def analyze_family_conflict(self, conflict_description: str) -> ConflictAnalysis:
        ...
        # 3. 아이들의 관점 분석
        children_perspectives = self._analyze_children_perspectives(conflict_description)
        
        # 4. 갈등 심화 위험 분석
        escalation_risks = self._analyze_escalation_risks(conflict_description, children_perspectives)
        
        # 5. 즉각적 중재 필요성 판단
        immediate_intervention = self._assess_immediate_intervention(children_perspectives, escalation_risks)
        
        # 6. 중재 우선순위 결정
        mediation_priority = self._determine_mediation_priority(children_perspectives, escalation_risks)
    ...
    def _analyze_children_perspectives(self, description: str) -> List[ChildPerspective]:
        """아이들의 관점 분석"""
        perspectives = []
    ...
    def _assess_urgency(self, child_role: str) -> EmotionIntensity:
        """긴급성 평가"""
        intensities = [EmotionIntensity.MILD, EmotionIntensity.MODERATE, EmotionIntensity.INTENSE, EmotionIntensity.CRITICAL]
    ...
    def _analyze_escalation_risks(self, description: str, children: List[ChildPerspective]) -> List[str]:
        """갈등 심화 위험 분석"""
        risks = []
        ...
        if len([c for c in children if c.urgency_level in [EmotionIntensity.INTENSE, EmotionIntensity.CRITICAL]]) > 1:
    ...
    def _assess_immediate_intervention(self, children: List[ChildPerspective], risks: List[str]) -> bool:
        """즉각적 중재 필요성 평가"""
        # 위험도가 높은 경우
        ...
        # 감정 강도가 높은 경우
        if any(child.urgency_level in [EmotionIntensity.INTENSE, EmotionIntensity.CRITICAL] for child in children):
            return True
    ...
    def _determine_mediation_priority(self, children: List[ChildPerspective], risks: List[str]) -> str:
        """중재 우선순위 결정"""
        # 감정 강도가 가장 높은 아이 우선
    ...
    def create_mediation_plan(self, conflict_analysis: ConflictAnalysis) -> MediationPlan:
        """중재 계획 생성"""
    ...
    def execute_judgment_process(self, conflict_description: str) -> Dict[str, Any]:
        ...
        judgment_result = {
            ...
            "key_insights": self._generate_key_insights(conflict_analysis, mediation_plan),
            "recommendations": self._generate_recommendations(conflict_analysis, mediation_plan),
            "success_probability": self._assess_success_probability(conflict_analysis, mediation_plan)
        }
    ...
    def _generate_key_insights(self, analysis: ConflictAnalysis, plan: MediationPlan) -> List[str]:
        """핵심 통찰 생성"""
        insights = []
        ...
        # 감정 강도에 대한 통찰
        for child in analysis.children_involved:
            if child.urgency_level in [EmotionIntensity.INTENSE, EmotionIntensity.CRITICAL]:
    ...
    def _generate_recommendations(self, analysis: ConflictAnalysis, plan: MediationPlan) -> List[str]:
        """권고사항 생성"""
        recommendations = []
        
        # 즉각적 권고
        if analysis.immediate_intervention_needed:
            recommendations.append("즉각적인 중재가 필요합니다 - 감정이 격화될 위험이 있습니다")
    ...
    def _assess_success_probability(self, analysis: ConflictAnalysis, plan: MediationPlan) -> float:
        """성공 확률 평가"""
        ...
        if len([c for c in analysis.children_involved if c.urgency_level == EmotionIntensity.MILD]) > 0:
            base_probability += 0.05
            
        # 부정적 요인
        if any(c.urgency_level == EmotionIntensity.CRITICAL for c in analysis.children_involved):
...
if __name__ == "__main__":
    ...
    print(f"   즉각적 중재 필요: {'예' if result['conflict_analysis'].immediate_intervention_needed else '아니오'}")
