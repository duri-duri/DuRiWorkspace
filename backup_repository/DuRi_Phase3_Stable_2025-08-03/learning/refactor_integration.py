        ...
        try:
            if self.performance_history:
                ...
                if summary.get("warning_metrics", 0) > 0:
                    logger.warning(f"성능 경고 지표 발견: {summary['warning_metrics']}개")
        ...
        try:
            if self.degradation_predictor:
                ...
                if summary.get("critical_predictions", 0) > 0:
                    logger.critical(f"심각한 성능 저하 예측: {summary['critical_predictions']}개")
                    self._trigger_urgent_refactor()
                
                elif summary.get("high_predictions", 0) > 0:
                    logger.warning(f"높은 우선순위 예측: {summary['high_predictions']}개")
        ...
        try:
            if self.refactor_controller:
                ...
                if summary.get("running_tasks", 0) > 0:
                    logger.info(f"실행 중인 리팩터링 작업: {summary['running_tasks']}개")
    ...
    def get_integration_status(self) -> RefactorIntegrationStatus:
        ...
        try:
            return RefactorIntegrationStatus(
                ...
                last_prediction_time=datetime.now() if self.degradation_predictor else None,
                last_refactor_time=datetime.now() if self.refactor_controller else None,
                total_predictions=len(self.degradation_predictor.predictions) if self.degradation_predictor else 0,
                total_refactors=len(self.refactor_controller.tasks) if self.refactor_controller else 0,
