...
class PhaseSelfEvaluator:
    ...
    def evaluate_phase_readiness(self, target_phase: PhaseLevel) -> List[PhaseAchievement]:
        """특정 Phase 준비도 평가"""
    ...
    def should_request_phase_up(self) -> Optional[PhaseUpRequest]:
        ...
        if all_achieved:
            # Phase Up 요청 생성
            confidence = sum(achievement.current_score for achievement in achievements) / len(achievements)
            ...
            logger.info(f"🎯 Phase Up 요청 생성: {self.current_phase.value} → {next_phase.value}")
            return request
        else:
            logger.info(f"🎯 Phase Up 준비 부족: {len([a for a in achievements if not a.achieved])}개 기준 미달성")
            return None
...
if __name__ == "__main__":
    ...
    if request:
        print(f"🎯 Phase Up 요청: {request.current_phase.value} → {request.target_phase.value}")
