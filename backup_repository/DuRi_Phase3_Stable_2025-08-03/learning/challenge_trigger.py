...
class ChallengeTrigger:
    ...
    def should_explore(self, strategy_history: Dict[str, Any], 
                      current_context: Optional[Dict[str, Any]] = None) -> ChallengeDecision:
        """
        도전 여부를 판단합니다.
        ...
        try:
            # 도전 조건들 평가
            conditions = self._evaluate_challenge_conditions(strategy_history, current_context)
            ...
            # 도전 유형 결정
            challenge_type = self._determine_challenge_type(conditions, current_context) if should_challenge else None
            
            # 신뢰도 계산
            confidence = self._calculate_confidence(conditions, challenge_score)
            
            # 추론 과정
            reasoning = self._generate_reasoning(conditions, challenge_score, should_challenge)
            
            # 추천 전략
            recommended_strategy = self._generate_recommended_strategy(challenge_type, current_context) if should_challenge else None
            ...
            if should_challenge:
                ...
                logger.info(f"도전 결정: {challenge_type.value if challenge_type else 'None'}, 신뢰도: {confidence:.2f}, 위험도: {risk_level:.2f}")
            else:
                logger.info(f"도전 보류: 점수 {challenge_score:.2f}, 신뢰도: {confidence:.2f}")
    ...
    def _evaluate_challenge_conditions(self, strategy_history: Dict[str, Any], 
                                     current_context: Optional[Dict[str, Any]]) -> List[ChallengeCondition]:
        """도전 조건들을 평가합니다."""
        conditions = []
        ...
        conditions.append(ChallengeCondition(
    ...
    def _determine_challenge_type(self, conditions: List[ChallengeCondition], 
                                 current_context: Optional[Dict[str, Any]]) -> ChallengeType:
        """도전 유형을 결정합니다."""
        # 조건에 따른 도전 유형 결정
    ...
    def _calculate_confidence(self, conditions: List[ChallengeCondition], challenge_score: float) -> float:
        """신뢰도를 계산합니다."""
        # 조건 충족률 기반 신뢰도
        met_conditions = len([c for c in conditions if c.met])
        total_conditions = len(conditions)
        
        condition_confidence = met_conditions / total_conditions if total_conditions > 0 else 0
    ...
    def _generate_reasoning(self, conditions: List[ChallengeCondition], 
                           challenge_score: float, should_challenge: bool) -> List[str]:
        """추론 과정을 생성합니다."""
        reasoning = []
        ...
        if should_challenge:
            ...
            for condition in met_conditions:
                reasoning.append(f"- {condition.condition_type}: {condition.current_value:.1f} >= {condition.threshold:.1f}")
        else:
    ...
    def _generate_recommended_strategy(self, challenge_type: ChallengeType, 
                                     current_context: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """추천 전략을 생성합니다."""
    ...
    def _calculate_risk_level(self, challenge_type: ChallengeType, 
                             current_context: Optional[Dict[str, Any]]) -> float:
        """위험도를 계산합니다."""
        base_risk = {
    ...
    def get_challenge_statistics(self) -> Dict[str, Any]:
        ...
        challenge_decisions = len([d for d in self.challenge_history if d.should_challenge])
        ...
        avg_confidence = sum(d.confidence for d in self.challenge_history) / total_decisions if total_decisions > 0 else 0
        avg_risk = sum(d.risk_level for d in self.challenge_history) / total_decisions if total_decisions > 0 else 0
        
        return {
            "total_decisions": total_decisions,
            "challenge_decisions": challenge_decisions,
            "challenge_rate": challenge_decisions / total_decisions if total_decisions > 0 else 0,
