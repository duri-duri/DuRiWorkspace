...
# 통합 개선 시스템 import
from duri_modules.autonomous.unified_improvement_system import UnifiedImprovementSystem, ImprovementCategory
...
class UnifiedConversationProcessor:
    ...
    async def process_conversation(self, user_input: str, duri_response: str, metadata: Optional[Dict[str, Any]] = None) -> UnifiedConversationResult:
        """통합 대화 처리: 저장 + 분석 + 평가 + 학습 + 자동 개선 + 통합 개선"""
    ...
    def _evaluate_comprehensive(self, user_input: str, duri_response: str, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        """통합 평가"""
        try:
            # ChatGPT 평가
            ...
            # 자기성찰
            self_reflection = self.duri_self_reflector.reflect_on_conversation(user_input, duri_response, chatgpt_evaluation)
            ...
            logger.info(f"📊 통합 평가 완료: ChatGPT({chatgpt_evaluation.get('total_score', 0):.3f}), 결과({result_evaluation.get('overall_score', 0):.3f})")
    ...
    def _learn_comprehensive(self, conversation_id: str, analysis_results: Dict[str, Any], evaluation_results: Dict[str, Any]) -> Dict[str, Any]:
        """통합 학습"""
        try:
    ...
    async def _execute_automatic_improvements(self, evaluation_results: Dict[str, Any], user_input: str, duri_response: str) -> Optional[Dict[str, Any]]:
        """자동 개선 실행 (2단계)"""
        try:
            # 컨텍스트 준비
            context = {
            ...
            if improvement_result.get("status") == "completed":
                logger.info(f"✅ 자동 개선 완료: {improvement_result['summary']['success_count']}/{improvement_result['summary']['total_count']} 성공")
    ...
    async def _execute_unified_improvement(self, evaluation_results: Dict[str, Any], user_input: str, duri_response: str) -> Optional[Dict[str, Any]]:
        """통합 개선 실행 (3단계)"""
        try:
            # 컨텍스트 준비
    ...
    async def _execute_autonomous_learning_session(self, evaluation_results: Dict[str, Any], analysis_results: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """자율 학습 세션 실행"""
        try:
