...
class CursorIntegration:
    ...
    async def process_cursor_feedback(self, session_id: str, user_input: str, duri_response: str, 
                                    cursor_evaluation: Dict[str, Any]) -> CursorFeedback:
        """Cursor 피드백 처리"""
        try:
            # 피드백 분석
            ...
            if session_id in self.active_sessions:
                ...
                self.active_sessions[session_id]["improvement_suggestions"].extend(improvement_suggestions)
            ...
            logger.info(f"✅ Cursor 피드백 처리 완료: {len(learning_insights)}개 인사이트, {len(improvement_suggestions)}개 제안")
    ...
    async def end_learning_session(self, session_id: str) -> LearningSession:
        ...
        logger.info(f"🏁 학습 세션 종료: {session_id} - 효율성 {learning_session.learning_efficiency:.3f}")
    ...
    async def get_learning_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 평균 지표 계산
            avg_learning_efficiency = sum(session.learning_efficiency for session in self.learning_history) / total_sessions
            avg_improvement_rate = sum(session.improvement_rate for session in self.learning_history) / total_sessions
            avg_cursor_satisfaction = sum(session.cursor_satisfaction for session in self.learning_history) / total_sessions
    ...
    async def apply_improvements(self, improvement_suggestions: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "success_rate": len(applied_improvements) / len(improvement_suggestions) if improvement_suggestions else 0.0
            }
    ...
    def _create_learning_session_summary(self, session_data: Dict[str, Any]) -> LearningSession:
        ...
        # Cursor 만족도 계산
        if exchanges:
            avg_confidence = sum(exchange.confidence_score for exchange in exchanges) / len(exchanges)
        else:
            avg_confidence = 0.5
    ...
    def _identify_improvement_areas(self) -> List[str]:
        ...
        if self.learning_history:
            ...
            # 낮은 학습 효율성
            avg_efficiency = sum(session.learning_efficiency for session in recent_sessions) / len(recent_sessions)
            if avg_efficiency < 0.6:
                improvement_areas.append("학습 효율성 향상 필요")
            
            # 낮은 개선율
            avg_improvement = sum(session.improvement_rate for session in recent_sessions) / len(recent_sessions)
            if avg_improvement < 0.5:
                improvement_areas.append("개선 제안 적용률 향상 필요")
            
            # 낮은 Cursor 만족도
            avg_satisfaction = sum(session.cursor_satisfaction for session in recent_sessions) / len(recent_sessions)
