...
class ConversationStore:
    ...
    def store_conversation(self, user_input: str, duri_response: str, metadata: Dict[str, Any] = None) -> str:
        """대화 데이터 저장"""
    ...
    def _calculate_learning_value(self, user_input: str, duri_response: str) -> float:
        ...
        # 기술적 키워드 포함 여부
        tech_keywords = ["API", "HTTP", "JSON", "async", "await", "FastAPI", "Flask", "Python", "코드", "구현"]
        tech_score = sum(1 for keyword in tech_keywords if keyword.lower() in duri_response.lower()) / len(tech_keywords)
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        avg_learning_value = sum(conv["learning_value"] for conv in self.conversation_history) / total_conversations
        total_response_time = sum(conv.get("response_time", 0) for conv in self.conversation_history)
        ...
        return {
            ...
            "total_conversation_length": sum(conv["conversation_length"] for conv in self.conversation_history),
            "high_value_conversations": len([conv for conv in self.conversation_history if conv["learning_value"] > 0.7])
        }
    ...
    def extract_learning_patterns(self) -> List[Dict[str, Any]]:
        ...
        patterns.append({
            ...
            "data": {"average_length": avg_length, "total_conversations": len(response_lengths)},
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_conversations": len(self.conversation_history),
                "recent_conversations": len(self.conversation_history[-10:]) if self.conversation_history else 0
            }
