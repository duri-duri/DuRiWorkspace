...
# 기존 시스템들 import
try:
    from emotional_self_awareness_system import EmotionalSelfAwarenessSystem, EmotionCategory, EmotionIntensity
...
class EmotionalThinkingSystem:
    ...
    async def _identify_secondary_emotions(self, context: Dict[str, Any], primary_emotion: EmotionCategory) -> List[EmotionCategory]:
        """보조 감정 식별"""
        secondary_emotions = []
        
        # 주요 감정과 관련된 보조 감정
        related_emotions = {
            EmotionCategory.JOY: [EmotionCategory.EXCITEMENT, EmotionCategory.CONTENTMENT],
            ...
            EmotionCategory.SURPRISE: [EmotionCategory.EXCITEMENT, EmotionCategory.FEAR],
            EmotionCategory.DISGUST: [EmotionCategory.ANGER, EmotionCategory.FEAR],
            EmotionCategory.EXCITEMENT: [EmotionCategory.JOY, EmotionCategory.CONTENTMENT],
            EmotionCategory.CONTENTMENT: [EmotionCategory.JOY, EmotionCategory.EXCITEMENT],
    ...
    async def _generate_emotional_insights(self, emotional_state: EmotionalState, 
                                         context: Dict[str, Any]) -> List[EmotionalInsight]:
        """감정적 통찰 생성"""
        insights = []
    ...
    async def _generate_secondary_emotion_insight(self, secondary_emotion: EmotionCategory, 
                                                emotional_state: EmotionalState) -> Optional[EmotionalInsight]:
        """보조 감정 기반 통찰 생성"""
    ...
    async def _generate_context_insight(self, emotional_state: EmotionalState, 
                                      context: Dict[str, Any]) -> Optional[EmotionalInsight]:
        """컨텍스트 기반 통찰 생성"""
    ...
    async def _generate_empathetic_responses(self, emotional_state: EmotionalState, 
                                           context: Dict[str, Any]) -> List[EmpatheticResponse]:
        """공감적 반응 생성"""
        responses = []
    ...
    async def _generate_primary_empathetic_response(self, emotional_state: EmotionalState) -> Optional[EmpatheticResponse]:
        """주요 감정에 대한 공감적 반응 생성"""
        empathy_responses = {
    ...
    async def _generate_secondary_empathetic_response(self, secondary_emotion: EmotionCategory, 
                                                    emotional_state: EmotionalState) -> Optional[EmpatheticResponse]:
        """보조 감정에 대한 공감적 반응 생성"""
    ...
    async def _generate_intensity_empathetic_response(self, emotional_state: EmotionalState) -> Optional[EmpatheticResponse]:
        """감정 강도에 따른 공감적 반응 생성"""
        if emotional_state.intensity >= 0.8:
    ...
    async def _make_emotional_decisions(self, emotional_state: EmotionalState, 
                                      insights: List[EmotionalInsight], 
                                      context: Dict[str, Any]) -> List[EmotionalDecision]:
        """감정적 의사결정"""
        decisions = []
    ...
    async def _make_emotional_state_decision(self, emotional_state: EmotionalState) -> Optional[EmotionalDecision]:
        ...
        decision_text = decision_strategies.get(emotional_state.primary_emotion, "감정을 고려한 균형잡힌 접근을 선택한다.")
    ...
    async def _make_insight_based_decision(self, insights: List[EmotionalInsight], 
                                         emotional_state: EmotionalState) -> Optional[EmotionalDecision]:
        """통찰 기반 의사결정"""
        if not insights:
            return None
    ...
    async def _make_context_based_decision(self, context: Dict[str, Any], 
                                         emotional_state: EmotionalState) -> Optional[EmotionalDecision]:
        """컨텍스트 기반 의사결정"""
        context_text = str(context)
    ...
    async def _apply_emotional_intuition(self, emotional_state: EmotionalState, 
                                       context: Dict[str, Any]) -> List[EmotionalInsight]:
        """감정적 직관 활용"""
        intuitive_insights = []
    ...
    async def _calculate_emotional_accuracy(self, insights: List[EmotionalInsight], 
                                          responses: List[EmpatheticResponse]) -> float:
        """감정적 정확도 계산"""
...
async def test_emotional_thinking_system():
    ...
    context2 = {"situation": "인간관계", "emotion": "기쁨", "relationship": "친구", "context": "성공"}
