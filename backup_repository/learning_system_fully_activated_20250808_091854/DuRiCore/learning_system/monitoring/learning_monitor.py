...
class LearningMonitoringSystem:
    ...
    async def record_learning_event(self, session_id: str, event_type: str, 
                                  phase: LearningPhase, data: Dict[str, Any] = None) -> str:
        """학습 이벤트 기록"""
        if session_id not in self.monitoring_sessions:
            logger.error(f"세션을 찾을 수 없음: {session_id}")
            return ""
    ...
    async def record_learning_metrics(self, session_id: str, progress_score: float = 0.0,
                                    efficiency_score: float = 0.0, quality_score: float = 0.0,
                                    engagement_score: float = 0.0) -> str:
        """학습 메트릭 기록"""
        if session_id not in self.monitoring_sessions:
            logger.error(f"세션을 찾을 수 없음: {session_id}")
            return ""
    ...
    async def get_monitoring_status(self, session_id: str = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_sessions": len([s for s in self.monitoring_sessions.values() if s.status == MonitoringStatus.ACTIVE]),
