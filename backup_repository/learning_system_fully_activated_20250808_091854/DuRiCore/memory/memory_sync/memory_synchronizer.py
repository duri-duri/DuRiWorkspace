...
class MemorySynchronizer:
    ...
    async def start_sync_operation(self, sync_type: SyncType, source_id: str, 
                                 target_id: str, metadata: Dict[str, Any] = None) -> str:
        """동기화 작업 시작"""
        try:
        ...
        try:
            ...
            self.performance_metrics["average_sync_time"] = (
                (self.performance_metrics["average_sync_time"] * (self.performance_metrics["total_sync_operations"] - 1) + 
                 sync_time) / self.performance_metrics["total_sync_operations"]
            )
    ...
    async def detect_conflicts(self, memory_id: str, source_version: Dict[str, Any], 
                             target_version: Dict[str, Any]) -> Optional[MemoryConflict]:
        """메모리 충돌 감지"""
        try:
            # 버전 비교
    ...
    async def get_sync_statistics(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "pending_operations": len([op for op in self.sync_operations.values() if op.status == SyncStatus.PENDING]),
                "in_progress_operations": len([op for op in self.sync_operations.values() if op.status == SyncStatus.IN_PROGRESS]),
                "completed_operations": len([op for op in self.sync_operations.values() if op.status == SyncStatus.COMPLETED]),
                "failed_operations": len([op for op in self.sync_operations.values() if op.status == SyncStatus.FAILED]),
