...
try:
    ...
    from internal_conflict_detector import InternalConflictDetector, ConflictAnalysisResult
    from duri_expression_layer import DuRiExpressionLayer, ExpressionResult
...
class PhaseZIntegrationTest:
    ...
    def _initialize_test_cases(self) -> List[TestCase]:
        ...
        test_cases.append(TestCase(
            ...
            input_data={
                ...
                'goals': ['윤리적 원칙 준수', '실용적 해결책 제시', '사용자 만족도'],
    ...
    async def _validate_test_result(self, test_case: TestCase, actual_result: Dict[str, Any], execution_time: float) -> TestResult:
        """테스트 결과 검증"""
        # 기본 성공 여부 확인
    ...
    async def _generate_recommendations(self) -> List[str]:
        ...
        # 성공률 기반 권장사항
        success_rate = len([r for r in self.test_results if r.result == TestResult.PASS]) / len(self.test_results) if self.test_results else 0.0
        
        if success_rate < 0.8:
            recommendations.append("테스트 성공률이 낮습니다. 시스템 안정성을 개선해야 합니다.")
        
        # 성능 기반 권장사항
        avg_time = sum(r.execution_time for r in self.test_results) / len(self.test_results) if self.test_results else 0.0
        ...
        if error_count > 0:
            recommendations.append(f"{error_count}개의 테스트에서 에러가 발생했습니다. 시스템 안정성을 점검해야 합니다.")
