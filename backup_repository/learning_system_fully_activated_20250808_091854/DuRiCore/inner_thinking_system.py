...
class InnerThinkingSystem:
    ...
    async def _generate_internal_questions(self, topic: str, depth_level: ThoughtDepth, 
                                         motivation: InternalMotivation) -> List[InternalQuestion]:
        """내적 질문 생성"""
        questions = []
    ...
    async def _execute_self_reflection_loop(self, topic: str, questions: List[InternalQuestion], 
                                          depth_level: ThoughtDepth) -> List[SelfReflection]:
        """자기 성찰 루프 실행"""
        reflections = []
    ...
    async def _create_analysis_reflection(self, topic: str, questions: List[InternalQuestion], 
                                        observation: SelfReflection) -> SelfReflection:
        """분석 성찰 생성"""
    ...
    async def _create_evaluation_reflection(self, topic: str, questions: List[InternalQuestion], 
                                          reflections: List[SelfReflection]) -> SelfReflection:
        """평가 성찰 생성"""
    ...
    async def _create_synthesis_reflection(self, topic: str, questions: List[InternalQuestion], 
                                         reflections: List[SelfReflection]) -> SelfReflection:
        """종합 성찰 생성"""
    ...
    async def _create_integration_reflection(self, topic: str, questions: List[InternalQuestion], 
                                           reflections: List[SelfReflection]) -> SelfReflection:
        """통합 성찰 생성"""
    ...
    async def _create_deep_reflections(self, topic: str, questions: List[InternalQuestion], 
                                     reflections: List[SelfReflection]) -> List[SelfReflection]:
        """깊은 성찰 생성"""
        deep_reflections = []
        ...
        deep_reflections.append(SelfReflection(
    ...
    async def _generate_insights(self, reflections: List[SelfReflection], 
                               questions: List[InternalQuestion]) -> List[str]:
        """통찰 생성"""
        insights = []
    ...
    async def _generate_additional_insights(self, reflections: List[SelfReflection], 
                                          questions: List[InternalQuestion]) -> List[str]:
        """추가 통찰 생성"""
        insights = []
    ...
    async def _draw_conclusions(self, insights: List[str], reflections: List[SelfReflection], 
                              questions: List[InternalQuestion]) -> List[str]:
        """결론 도출"""
        conclusions = []
    ...
    async def get_thinking_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_thinking_duration": np.mean([p.duration for p in self.thought_processes]) if self.thought_processes else 0.0,
            "depth_distribution": self._get_depth_distribution(),
            "motivation_distribution": self._get_motivation_distribution(),
            "recent_insights": [p.insights[-3:] for p in self.thought_processes[-3:]] if self.thought_processes else []
        }
