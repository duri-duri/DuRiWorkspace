...
class DuRiExpressionEngine(BaseModule):
    ...
    async def execute(self, context: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"표현 생성 중 오류 발생: {str(e)}"
            }
    
    async def express_judgment(self, 
                             ...
                             style: Optional[Union[ExpressionStyle, str]] = None) -> ExpressionResult:
        """판단 결과를 자연어로 표현"""
    ...
    async def _extract_key_points(self, 
                                ...
                                decision_tree: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """핵심 판단 포인트 추출"""
        key_points = {
            'decision': None,
            'reasoning': None,
    ...
    async def _construct_expression(self, 
        ...
        try:
            ...
            if key_points['alternatives'] and len(key_points['alternatives']) > 0:
                alternatives_template = template.get('with_alternatives', "다른 방법도 고려했지만, {alternatives}보다는 이 방법이 더 적절하다고 판단했어요.")
    ...
    def _initialize_templates(self) -> Dict[ExpressionType, Dict[ExpressionStyle, Dict[str, str]]]:
        """표현 템플릿 초기화"""
        return {
...
# 편의 함수
async def express_duri_judgment(judgment_data: Optional[Dict[str, Any]] = None,
                              thought_flow: Optional[Dict[str, Any]] = None,
                              decision_tree: Optional[Dict[str, Any]] = None,
