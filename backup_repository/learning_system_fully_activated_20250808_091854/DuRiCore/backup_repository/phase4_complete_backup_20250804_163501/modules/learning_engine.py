...
class LearningEngine:
    ...
    def _integrate_learning_results(
        ...
        try:
            ...
            # 학습 점수 조정
            adjusted_score = min(1.0, learning_score * (1 + llm_analysis.get('learning_potential', 0) * 0.3))
    ...
    def _update_learning_stats(self, learning_result: LearningResult):
        ...
        try:
            ...
            new_avg = (current_avg * (total_sessions - 1) + learning_result.learning_score) / total_sessions
...
class TextLearningSystem:
    ...
    def process(self, content: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"learning_score": 0.0, "insights": ["오류 발생"], "knowledge_gained": {}, "skills_improved": [], "next_steps": []}
...
class MetacognitiveLearningSystem:
    ...
    def _assess_reflection_level(self, content: str) -> float:
        """반성 수준 평가"""
        try:
            reflection_keywords = ["생각", "느낌", "이해", "배운", "깨달", "반성", "분석"]
...
class FamilyLearningSystem:
    ...
    def _analyze_family_relationship(self, content: str) -> float:
        """가족 관계 분석"""
        try:
            family_keywords = ["가족", "부모", "아이", "아버지", "어머니", "형제", "자매"]
