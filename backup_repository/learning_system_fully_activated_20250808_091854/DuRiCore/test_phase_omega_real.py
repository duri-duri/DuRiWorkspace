...
from self_goal_generator import SelfGoalGenerator, ImprovementAreaEnum
...
class PhaseOmegaRealTest:
    ...
    async def _test_survival_instinct_working(self):
        ...
        try:
            ...
            # 검증
            if (survival_status and 
                hasattr(survival_status, 'survival_probability') and
                hasattr(survival_status, 'status') and
                0 <= survival_status.survival_probability <= 1 and
                survival_status.status in [SurvivalStatusEnum.CRITICAL, SurvivalStatusEnum.DANGEROUS, 
                                         SurvivalStatusEnum.STABLE, SurvivalStatusEnum.SECURE, SurvivalStatusEnum.THRIVING]):
                
                logger.info(f"✅ 생존 본능 작동 확인: 상태={survival_status.status.value}, 확률={survival_status.survival_probability:.2f}")
                self.test_results.append(("생존 본능", True, f"상태: {survival_status.status.value}, 확률: {survival_status.survival_probability:.2f}"))
            else:
    ...
    async def _test_self_goal_generation_working(self):
        ...
        try:
            ...
            # 검증
            if (self_goals and len(self_goals) > 0 and
                all(hasattr(goal, 'goal_id') and hasattr(goal, 'title') and hasattr(goal, 'description') 
    ...
    async def _test_evolution_system_working(self):
        ...
        try:
            ...
            if (evolution_progress and 
                hasattr(evolution_progress, 'evolution_score') and
                0 <= evolution_progress.evolution_score <= 1):
                
                logger.info(f"✅ 진화 시스템 작동 확인: 진화 점수={evolution_progress.evolution_score:.2f}")
                self.test_results.append(("진화 시스템", True, f"진화 점수: {evolution_progress.evolution_score:.2f}"))
            else:
    ...
    async def _test_survival_assessment_working(self):
        ...
        try:
            ...
            if (survival_score and 
                hasattr(survival_score, 'overall_score') and
                0 <= survival_score.overall_score <= 1):
                
                logger.info(f"✅ 생존 평가 작동 확인: 생존 점수={survival_score.overall_score:.2f}")
                self.test_results.append(("생존 평가", True, f"생존 점수: {survival_score.overall_score:.2f}"))
            else:
    ...
    async def _test_phase_omega_integration_working(self):
        ...
        try:
            ...
            if (result and 
                result.success and
                result.survival_status and
                len(result.self_goals) > 0 and
                result.integration_time > 0):
                
                logger.info(f"✅ Phase Ω 통합 작동 확인: {len(result.self_goals)}개 목표, {result.integration_time:.2f}초")
                ...
                self.test_results.append(("Phase Ω 통합", True, f"{len(result.self_goals)}개 목표, {result.integration_time:.2f}초"))
            else:
