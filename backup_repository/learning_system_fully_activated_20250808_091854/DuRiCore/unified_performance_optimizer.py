...
class UnifiedPerformanceOptimizer:
    ...
    async def _collect_current_metrics(self) -> PerformanceMetrics:
        ...
        try:
            ...
            throughput = len(self.performance_history) / max(1, time.time() - self.start_time if hasattr(self, 'start_time') else 1)
            latency = 0.0  # 실제 측정 필요
            error_rate = 0.0  # 실제 측정 필요
            quality_score = 1.0  # 기본값
            efficiency_score = await self._calculate_efficiency_score(cpu_usage, memory_usage, throughput, quality_score)
    ...
    async def _calculate_efficiency_score(self, cpu_usage: float, memory_usage: float, 
                                        throughput: float, quality_score: float) -> float:
        """효율성 점수 계산"""
        try:
            # 각 메트릭의 가중치
            weights = {
            ...
            try:
                ...
                if await self._needs_optimization(current_metrics):
                    ...
                    if optimization_result.status == OptimizationStatus.COMPLETED:
                        logger.info(f"자동 최적화 완료: 개선 점수 {optimization_result.improvement_score:.2f}")
                    else:
    ...
    async def _optimize_memory(self, metrics: PerformanceMetrics) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"improvement_score": improvement_score, "optimized_metrics": metrics}
        ...
        try:
            ...
            return {
                ...
                "optimization_history": {
                    ...
                    "successful_optimizations": len([o for o in self.optimization_history if o.status == OptimizationStatus.COMPLETED]),
                    "failed_optimizations": len([o for o in self.optimization_history if o.status == OptimizationStatus.FAILED]),
                    "average_improvement": np.mean([o.improvement_score for o in self.optimization_history if o.status == OptimizationStatus.COMPLETED]) if self.optimization_history else 0.0
                },
                "cache_status": {
                    "cache_size": len(self.cache),
                    "max_cache_size": self.max_cache_size,
                    "cache_utilization": len(self.cache) / self.max_cache_size if self.max_cache_size > 0 else 0
                }
            }
