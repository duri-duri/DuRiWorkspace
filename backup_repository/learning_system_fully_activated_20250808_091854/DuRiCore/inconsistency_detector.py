...
class InconsistencyDetector:
    ...
    async def _detect_structural_inconsistencies(self, graph: 'DynamicReasoningGraph') -> List[Inconsistency]:
        ...
        if isolated_nodes:
            inconsistency = Inconsistency(
                inconsistency_id=f"structural_inconsistency_isolated_nodes",
    ...
    async def _detect_circular_reasoning(self, graph: 'DynamicReasoningGraph') -> List[Inconsistency]:
        ...
        for node_id in graph.nodes:
            if node_id not in all_visited:
                ...
                if cycle:
                    circular_reasoning.append(Inconsistency(
    ...
    async def _detect_missing_premises(self, graph: 'DynamicReasoningGraph') -> List[Inconsistency]:
        ...
        for conclusion in conclusion_nodes:
            ...
            supporting_premises = []
            for edge in graph.edges.values():
                if edge.target_node == conclusion.node_id and edge.edge_type.value in ["supports", "infers"]:
                    source_node = graph.nodes.get(edge.source_node)
            ...
            if len(supporting_premises) < 2:
                missing_premises.append(Inconsistency(
    ...
    def _is_logical_contradiction(self, node1: 'DynamicReasoningNode', node2: 'DynamicReasoningNode') -> bool:
        ...
        contradiction_keywords = {
            "찬성": "반대", "옳다": "틀리다", "참": "거짓", "있음": "없음",
            "가능": "불가능", "필요": "불필요", "중요": "중요하지 않음"
        }
...
class InconsistencyResolver:
    ...
    def _initialize_resolution_strategies(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "node_modification": {
                "weight": 0.3,
                "description": "노드 수정 전략"
            },
            "edge_adjustment": {
                "weight": 0.25,
                "description": "엣지 조정 전략"
            },
            "path_reconstruction": {
                "weight": 0.25,
                "description": "경로 재구성 전략"
            },
            "evidence_weighting": {
                "weight": 0.2,
    ...
    async def resolve_inconsistencies(self, graph: 'DynamicReasoningGraph', 
                                    inconsistencies: List[Inconsistency]) -> List[ResolutionResult]:
        """불일치 해결"""
    ...
    async def _execute_resolution_strategy(self, graph: 'DynamicReasoningGraph', 
                                         inconsistency: Inconsistency, 
                                         strategy: ResolutionStrategy,
                                         modified_nodes: Set[str],
                                         modified_edges: Set[str]) -> Optional[ResolutionResult]:
        """해결 전략 실행"""
        try:
            if strategy == ResolutionStrategy.NODE_MODIFICATION:
                ...
                return await self._resolve_contradiction_resolution(graph, inconsistency, modified_nodes, modified_edges)
            else:
                return None
    ...
    async def _resolve_node_modification(self, graph: 'DynamicReasoningGraph', 
                                       inconsistency: Inconsistency,
                                       modified_nodes: Set[str]) -> ResolutionResult:
        """노드 수정 전략"""
        nodes_to_modify = []
    ...
    async def _resolve_edge_adjustment(self, graph: 'DynamicReasoningGraph', 
                                     inconsistency: Inconsistency,
                                     modified_edges: Set[str]) -> ResolutionResult:
        """엣지 조정 전략"""
        edges_to_modify = []
    ...
    async def _resolve_path_reconstruction(self, graph: 'DynamicReasoningGraph', 
                                         inconsistency: Inconsistency,
                                         modified_nodes: Set[str]) -> ResolutionResult:
        """경로 재구성 전략"""
        # 고립된 노드들을 다른 노드와 연결
    ...
    async def _resolve_evidence_weighting(self, graph: 'DynamicReasoningGraph', 
                                        inconsistency: Inconsistency,
                                        modified_nodes: Set[str]) -> ResolutionResult:
        """증거 가중치 조정 전략"""
        nodes_to_modify = []
    ...
    async def _resolve_contradiction_resolution(self, graph: 'DynamicReasoningGraph', 
                                              inconsistency: Inconsistency,
                                              modified_nodes: Set[str],
                                              modified_edges: Set[str]) -> ResolutionResult:
        """모순 해결 전략"""
...
class InconsistencyValidator:
    ...
    async def validate_resolution_results(self, graph: 'DynamicReasoningGraph', 
                                        inconsistencies: List[Inconsistency],
                                        resolution_results: List[ResolutionResult]) -> Dict[str, Any]:
        """해결 결과 검증"""
