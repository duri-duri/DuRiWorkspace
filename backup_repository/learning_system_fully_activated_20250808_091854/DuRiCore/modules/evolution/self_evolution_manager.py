...
class SelfEvolutionManager:
        ...
        try:
            ...
            if os.path.exists(self.evolution_file):
                with open(self.evolution_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self.evolution_steps = [EvolutionStep(**step) for step in data.get('evolution_steps', [])]
    ...
    def execute_self_improvement_sequence(self, reflection_insights: List[Any] = None) -> Dict[str, Any]:
        """
        자기개선 시퀀스를 실행합니다.
        ...
        print(f"✅ 자기개선 시퀀스 완료: {len(updated_beliefs)}개 신념, {len(updated_rules)}개 규칙, {len(updated_behaviors)}개 행동 패턴 업데이트")
        ...
        for insight in reflection_insights:
            try:
                ...
                if abs(confidence_impact) > 0.3:
                    belief_update = {
                        "insight_id": getattr(insight, 'judgment_trace_id', 'unknown') if hasattr(insight, 'judgment_trace_id') else insight.get('judgment_trace_id', 'unknown'),
                        "content": getattr(insight, 'improvement_suggestion', '') if hasattr(insight, 'improvement_suggestion') else insight.get('improvement_suggestion', ''),
        ...
        for insight in reflection_insights:
            try:
                ...
                if improvement and ";" in improvement:
                    ...
                    rule_update = {
                        "insight_id": getattr(insight, 'judgment_trace_id', 'unknown') if hasattr(insight, 'judgment_trace_id') else insight.get('judgment_trace_id', 'unknown'),
                        "condition": getattr(insight, 'analysis', '') if hasattr(insight, 'analysis') else insight.get('analysis', ''),
                        "action": improvement,
                        "confidence_impact": confidence_impact,
                        "priority": abs(confidence_impact)
                    }
            ...
            try:
                ...
                if improvement:
                    ...
                    behavior_value = {
                        "insight_id": getattr(insight, 'judgment_trace_id', 'unknown') if hasattr(insight, 'judgment_trace_id') else insight.get('judgment_trace_id', 'unknown'),
                        "pattern": improvement,
                        "trigger_condition": getattr(insight, 'analysis', '') if hasattr(insight, 'analysis') else insight.get('analysis', ''),
                        "confidence_impact": confidence_impact,
                        "created_at": datetime.now().isoformat(),
                        "version": "2.0"
                    }
    ...
    def _record_evolution_steps(self, beliefs_to_update: List[Dict], 
                              rules_to_update: List[Dict], 
                              updated_behaviors: List[str],
                              reflection_insights: List[Any]) -> List[EvolutionStep]:
        """진화 단계들을 기록합니다."""
        evolution_steps = []
        ...
        if updated_behaviors:
            step = EvolutionStep(
                ...
                previous_state={"behaviors_count": len(self.behavior_patterns) - len(updated_behaviors)},
    ...
    def get_evolution_summary(self) -> Dict:
        ...
        return {
            ...
            "recent_evolution_steps": len(self.evolution_steps[-10:]) if self.evolution_steps else 0,
            "average_impact_score": sum(step.impact_score for step in self.evolution_steps) / len(self.evolution_steps) if self.evolution_steps else 0
        }
    ...
    def _adjust_judgment_with_beliefs(self, judgment_data: Dict, relevant_beliefs: List[Dict]) -> Dict:
        """신념들을 바탕으로 판단을 조정합니다."""
