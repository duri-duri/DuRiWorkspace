...
async def test_integrated_performance_optimization():
    ...
    try:
        ...
        logger.info(f"   - 기본 성능: {integrated_test_results.get('basic_performance', {}).get('execution_time', 0):.3f}초")
        logger.info(f"   - 병렬 처리: {integrated_test_results.get('parallel_processing', {}).get('execution_time', 0):.3f}초")
        logger.info(f"   - 캐시 성능: {integrated_test_results.get('cache_performance', {}).get('cache_lookup_time', 0):.3f}초")
        ...
        if baseline_time > 0 and current_time > 0:
            ...
            target_achievement = (baseline_time - current_time) / (baseline_time - target_time) * 100 if baseline_time > target_time else 100
