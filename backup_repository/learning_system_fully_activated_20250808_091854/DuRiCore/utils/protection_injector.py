...
"""
    ...
    def _add_judgment_trace(self, content: str, module_name: str) -> str:
        ...
        for func_name in functions:
            if any(keyword in func_name.lower() for keyword in ['analyze', 'process', 'handle', 'manage', 'control', 'reasoning', 'judgment']):
                # 함수에 판단 흐름 Trace 구조 추가
                trace_structure = self.injection_templates["judgment_trace_structure"].format(
                    module_name=module_name,
                    intent=f"{func_name} 실행"
                )
                ...
                if in_function:
                    ...
                    if line.strip() and not line.strip().startswith('#'):
                        ...
                        if not line.strip().startswith('pass') and not line.strip().startswith('return'):
                            ...
                            trace_indented = '\n'.join(' ' * indent + trace_line if trace_line.strip() else trace_line 
