...
class MetaLoopManager:
    ...
    def execute_meta_judgment(self, judgment_type: JudgmentType, context: Dict[str, Any], 
                            existing_system_result: Optional[Dict[str, Any]] = None) -> MetaJudgment:
        """메타 판단 실행"""
        try:
            ...
            # 3. 재귀적 평가
            recursive_evaluation = self._perform_recursive_evaluation(meta_review, context)
            
            # 4. 최종 결정
            final_decision = self._make_final_decision(recursive_evaluation, context)
            
            # 5. 신뢰도 계산
            confidence = self._calculate_meta_confidence(initial_reason, meta_review, recursive_evaluation)
            ...
            if existing_system_result:
                comparison = self._compare_with_existing_system(judgment, existing_system_result)
                self.loop_comparisons.append(comparison)
            
            logger.info(f"✅ 메타 판단 완료: {judgment_type.value} - 신뢰도: {confidence:.3f}")
            return judgment
    ...
    def _make_initial_judgment(self, judgment_type: JudgmentType, context: Dict[str, Any]) -> str:
        """초기 판단 수행"""
    ...
    def _perform_recursive_evaluation(self, meta_review: str, context: Dict[str, Any]) -> str:
        """재귀적 평가 수행"""
    ...
    def _make_final_decision(self, recursive_evaluation: str, context: Dict[str, Any]) -> str:
        """최종 결정 수행"""
        decisions = [
    ...
    def _calculate_meta_confidence(self, initial_reason: str, meta_review: str, recursive_evaluation: str) -> float:
        """메타 신뢰도 계산"""
    ...
    def _compare_with_existing_system(self, meta_judgment: MetaJudgment, 
                                    existing_system_result: Dict[str, Any]) -> LoopComparison:
        """기존 시스템과 비교"""
        ...
        # 진화 잠재력 계산
        evolution_potential = 1.0 - agreement_level  # 일치도가 낮을수록 진화 잠재력 높음
        ...
        logger.info(f"🔄 시스템 비교 완료: 일치도 {agreement_level:.3f}, 진화 잠재력 {evolution_potential:.3f}")
        return comparison
    
    def _create_error_judgment(self, judgment_type: JudgmentType, error_message: str) -> MetaJudgment:
        """오류 판단 생성"""
        return MetaJudgment(
    ...
    def get_evolution_metrics(self) -> Dict[str, Any]:
        ...
        avg_agreement = sum(c.agreement_level for c in self.loop_comparisons) / len(self.loop_comparisons)
        avg_evolution_potential = sum(c.evolution_potential for c in self.loop_comparisons) / len(self.loop_comparisons)
