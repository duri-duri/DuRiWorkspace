...
class CognitiveBandwidthManager:
    ...
    def receive_stimulus(self, stimulus: str, stimulus_type: StimulusType, 
    ...
    def _calculate_overload_risk(self) -> float:
        ...
        recent_stimuli = len([s for s in self.stimulus_history 
                            if (datetime.now() - datetime.fromisoformat(s.timestamp)).seconds < 60])
        
        # 처리 중인 자극 비율
        processing_ratio = processing_count / self.current_config.max_concurrent_processing
