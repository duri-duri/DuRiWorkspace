...
class StrategyImitator:
    ...
    def imitate(self, reference_strategy: Dict[str, Any], 
                imitation_type: ImitationType = ImitationType.EXACT_COPY,
                context: Optional[Dict[str, Any]] = None) -> ImitationResult:
        """
        전략을 모방합니다.
        ...
        try:
            ...
            result = ImitationResult(
                ...
                adaptation_notes=self._generate_adaptation_notes(imitation_type, context),
            ...
            logger.info(f"전략 모방 완료: {imitation_type.value}, 신뢰도: {result.confidence:.2f}")
            return result
    ...
    def _adaptive_copy(self, strategy: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """상황에 맞게 조정된 복사"""
    ...
    def _creative_copy(self, strategy: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """창의적 변형"""
        copied_strategy = copy.deepcopy(strategy)
    ...
    def _calculate_confidence(self, imitation_type: ImitationType, context: Optional[Dict[str, Any]]) -> float:
        """모방 신뢰도 계산"""
        base_confidence = {
    ...
    def _generate_adaptation_notes(self, imitation_type: ImitationType, context: Optional[Dict[str, Any]]) -> List[str]:
        """적응 노트 생성"""
        notes = []
    ...
    def _update_successful_patterns(self, result: ImitationResult):
        """성공적인 패턴 업데이트"""
        if result.success:
            pattern_key = f"{result.imitation_type.value}_{len(result.original_strategy)}"
    ...
    def get_imitation_statistics(self) -> Dict[str, Any]:
        ...
        avg_confidence = sum(r.confidence for r in self.imitation_history) / total_imitations if total_imitations > 0 else 0
        
        return {
            "total_imitations": total_imitations,
            "successful_imitations": successful_imitations,
            "success_rate": successful_imitations / total_imitations if total_imitations > 0 else 0,
            "type_distribution": type_counts,
            "average_confidence": avg_confidence,
            "successful_patterns": self.successful_patterns
        }
    
    def recommend_imitation_type(self, strategy_complexity: str = "medium") -> ImitationType:
        """모방 유형 추천"""
