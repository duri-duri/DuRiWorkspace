...
class RefactorPredictiveController:
    ...
    def _check_new_recommendations(self):
        ...
        try:
            ...
            for recommendation in active_recommendations:
                # 이미 처리된 권장사항인지 확인
                if not self._is_recommendation_processed(recommendation.recommendation_id):
                    # 새로운 리팩터링 작업 생성
                    ...
                    if task:
                        ...
                        if self.auto_approval_enabled and task.priority in ["high", "urgent"]:
    ...
    def _execute_pending_tasks(self):
        ...
        try:
            ...
            approved_tasks = [
                ...
                if task.status == RefactorStatus.APPROVED
            ]
            
            # 실행 중인 작업 수 확인
            running_tasks = [
                task for task in self.tasks 
                if task.status == RefactorStatus.IN_PROGRESS
            ]
            ...
            if available_slots > 0:
                # 우선순위 순으로 정렬하여 실행
                approved_tasks.sort(key=lambda x: self._get_priority_score(x.priority), reverse=True)
    ...
    def _create_refactor_result(self, task: RefactorTask, success: bool, 
                              execution_time: float, before_performance: Dict[str, float],
                              after_performance: Dict[str, float]) -> RefactorResult:
        """리팩터링 결과를 생성합니다."""
        try:
            # 성능 개선률 계산
            ...
            if before_performance and after_performance:
                ...
                cpu_improvement = (before_performance.get("cpu_usage", 0) - after_performance.get("cpu_usage", 0)) / max(before_performance.get("cpu_usage", 1), 1)
                memory_improvement = (before_performance.get("memory_usage", 0) - after_performance.get("memory_usage", 0)) / max(before_performance.get("memory_usage", 1), 1)
                improvement_percentage = (cpu_improvement + memory_improvement) / 2 * 100
    ...
    def _analyze_completed_tasks(self):
        ...
        try:
            completed_tasks = [
                task for task in self.tasks 
                if task.status == RefactorStatus.COMPLETED
            ]
            
            for task in completed_tasks:
                # 성능 개선 효과 분석
                if task.actual_impact and task.actual_impact > 0:
                    logger.info(f"리팩터링 성공: {task.task_id} - 개선률: {task.actual_impact:.1f}%")
                else:
    ...
    def cancel_task(self, task_id: str) -> bool:
        ...
        try:
            task = next((t for t in self.tasks if t.task_id == task_id), None)
            if task and task.status in [RefactorStatus.PENDING, RefactorStatus.APPROVED]:
    ...
    def get_task_summary(self) -> Dict[str, Any]:
        """작업 요약을 반환합니다."""
        try:
            pending_tasks = [t for t in self.tasks if t.status == RefactorStatus.PENDING]
            approved_tasks = [t for t in self.tasks if t.status == RefactorStatus.APPROVED]
            running_tasks = [t for t in self.tasks if t.status == RefactorStatus.IN_PROGRESS]
            completed_tasks = [t for t in self.tasks if t.status == RefactorStatus.COMPLETED]
