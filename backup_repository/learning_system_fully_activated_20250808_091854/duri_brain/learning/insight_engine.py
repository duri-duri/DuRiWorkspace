...
class SemanticDriftGenerator:
    ...
    def __init__(self):
        self.memory_fragments = [
            "오래된 학습 데이터", "과거 실패 경험", "무관한 성공 사례",
            "잊혀진 전략", "우연한 발견", "예상치 못한 연결",
            "감정적 기억", "직관적 판단", "실수에서 배운 교훈"
        ]
...
class DisruptiveMappingEngine:
    ...
    def __init__(self):
        self.disruption_patterns = [
            "논리 역전", "가정 뒤집기", "관계 재정의",
            "우선순위 변경", "목표 재설정", "방법론 혼합"
        ]
        
    def create_disruptive_composition(self, reasoning: str, fragments: List[str]) -> List[InsightCandidate]:
        """기존 전략에서 의도적으로 논리 깨기"""
...
class MetaEvaluator:
    ...
    def evaluate_candidate(self, candidate: InsightCandidate) -> float:
        ...
        feasibility_score = 1.0 - candidate.confidence * 0.5  # 낮은 신뢰도 = 높은 실현 가능성
    ...
    def select_best_candidate(self, candidates: List[InsightCandidate]) -> Optional[InsightCandidate]:
        """최고 점수 후보 선택"""
        if not candidates:
            return None
            
        scores = [(candidate, self.evaluate_candidate(candidate)) for candidate in candidates]
        ...
        # 임계값 확인 (0.6 이상)
        if best_score >= 0.6:
            logger.info(f"✅ 최고 통찰 후보 선택: {best_candidate.strategy[:50]}... (점수: {best_score:.3f})")
...
class InsightTriggerEngine:
    ...
    def trigger_insight_session(self, problem: str, trigger_type: InsightTriggerType) -> Optional[InsightSession]:
        """통찰 세션 트리거"""
        ...
        try:
            ...
            reasoning = self.retrograde_reasoning.apply_retrograde_reasoning(problem, fragments)
            phases_completed.append(InsightPhase.RETROGRADE_REASONING)
            
            # 4. 비논리적 연결 탐색
            logger.info("📌 4단계: 파괴적 구성")
            candidates = self.disruptive_mapping.create_disruptive_composition(reasoning, fragments)
...
class DualResponseSystem:
    ...
    def execute_dual_response(self, problem: str) -> Dict[str, Any]:
        ...
        if self.should_trigger_insight(refactor_success):
            ...
            # 통찰 세션 실행
            trigger_type = InsightTriggerType.REPEATED_FAILURE if not refactor_success else InsightTriggerType.NO_GAIN
            insight_session = self.insight_engine.trigger_insight_session(problem, trigger_type)
