...
def test_insight_integration():
    ...
    try:
        ...
        print(f"   이성적 리팩터링 횟수: {status['dual_response_system']['rational_refactor_count']}")
        print(f"   통찰 트리거 횟수: {status['dual_response_system']['insight_trigger_count']}")
...
def test_insight_phases():
    ...
    try:
        ...
        from duri_brain.learning.insight_engine import (
            CognitivePauseManager, SemanticDriftGenerator,
            RetrogradeReasoningEngine, DisruptiveMappingEngine,
            MetaEvaluator, InsightTriggerEngine
        )
...
def test_full_insight_session():
    ...
    try:
        import sys
        sys.path.append('.')
        from duri_brain.learning.insight_engine import InsightTriggerEngine, InsightTriggerType
