...
class PracticeEngine:
    ...
    def practice_strategy(self, strategy: Dict[str, Any], 
                        practice_type: PracticeType = PracticeType.REPETITION,
                        iterations: int = 10,
                        context: Optional[Dict[str, Any]] = None) -> PracticeResult:
        """
        전략을 연습합니다.
        ...
        try:
            # 연습 실행
            modified_strategy = self._execute_practice(strategy, practice_type, iterations, context, session)
            
            # 성능 계산
            success_rate = session.success_count / iterations if iterations > 0 else 0
            avg_performance = sum(session.performance_metrics.values()) / len(session.performance_metrics) if session.performance_metrics else 0
            ...
            # 추천사항 생성
            recommendations = self._generate_recommendations(session, success_rate, avg_performance)
            ...
            logger.info(f"전략 연습 완료: {practice_type.value}, 성공률: {success_rate:.2f}, 개선점수: {improvement_score:.2f}")
            return result
    ...
    def _execute_practice(self, strategy: Dict[str, Any], practice_type: PracticeType, 
                         iterations: int, context: Optional[Dict[str, Any]], 
                         session: PracticeSession) -> Dict[str, Any]:
        """연습 실행"""
    ...
    def _execute_repetition(self, strategy: Dict[str, Any], iteration: int, context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """단순 반복 실행"""
        # 실제 전략 실행 시뮬레이션
    ...
    def _execute_variation(self, strategy: Dict[str, Any], iteration: int, context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """변형 연습 실행"""
        # 파라미터 변형
    ...
    def _execute_progressive(self, strategy: Dict[str, Any], iteration: int, context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """점진적 난이도 증가"""
    ...
    def _execute_adaptive(self, strategy: Dict[str, Any], iteration: int, context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """적응형 연습"""
        # 이전 성과에 따른 적응
    ...
    def _calculate_improvement_score(self, session: PracticeSession) -> float:
        ...
        improvement = (final_performance - initial_performance) / initial_performance if initial_performance > 0 else 0
        return max(improvement, 0.0)
    
    def _generate_recommendations(self, session: PracticeSession, success_rate: float, avg_performance: float) -> List[str]:
        """추천사항 생성"""
        recommendations = []
    ...
    def get_practice_statistics(self) -> Dict[str, Any]:
        ...
        successful_sessions = len([s for s in self.practice_sessions if s.success_count > s.failure_count])
        ...
        avg_success_rate = sum(s.success_count / s.iterations for s in self.practice_sessions if s.iterations > 0) / total_sessions if total_sessions > 0 else 0
