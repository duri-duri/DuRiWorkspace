...
class FamilyRelationshipFormationSystem:
    ...
    def analyze_relationship_health(self, member_id: str) -> Dict:
        ...
        try:
            # 해당 구성원과의 모든 상호작용
            member_interactions = [i for i in self.interactions if member_id in i.participants]
            ...
            for interaction in member_interactions:
                other_participants = [p for p in interaction.participants if p != member_id]
                for other_id in other_participants:
                    ...
                    relationships[other_id]['total_emotional_impact'] += interaction.emotional_impact
                    relationships[other_id]['total_satisfaction'] += interaction.satisfaction_level
                    relationships[other_id]['total_communication_quality'] += interaction.communication_quality
            ...
            for other_id, data in relationships.items():
                ...
                data['avg_communication_quality'] = data['total_communication_quality'] / count
                data['relationship_health'] = (data['avg_emotional_impact'] + data['avg_satisfaction'] + data['avg_communication_quality']) / 3
            
            # 전체 건강도 계산
            overall_health = sum(data['relationship_health'] for data in relationships.values()) / len(relationships)
    ...
    def understand_family_role(self, member_id: str, role_data: Dict) -> RoleUnderstanding:
        """
        가족 내 역할 이해 및 적응
        """
        try:
            ...
            logger.info(f"역할 이해 업데이트: {member_id} - {role_understanding.role.value}")
        ...
        try:
            ...
            for other_id, relationship_data in health_analysis['relationships'].items():
                if relationship_data['relationship_health'] < 0.6:  # 개선 필요
                    suggestion = self._generate_improvement_suggestion(member_id, other_id, relationship_data)
                    suggestions.append(suggestion)
            
            # 역할 적응도 개선 제안
            if member_id in self.role_understandings:
                role_suggestions = self._generate_role_improvement_suggestions(member_id)
    ...
    def _update_relationship_patterns(self, interaction: Interaction):
        ...
        if existing_pattern:
            ...
            total_quality = (interaction.emotional_impact + interaction.satisfaction_level + 
                           interaction.communication_quality) / 3
            existing_pattern.average_quality = (
                (existing_pattern.average_quality * (existing_pattern.frequency - 1) + total_quality) / 
                existing_pattern.frequency
            )
            
            if interaction.interaction_type.value not in existing_pattern.common_interactions:
                existing_pattern.common_interactions.append(interaction.interaction_type.value)
            ...
            new_pattern = RelationshipPattern(
                ...
                frequency=1,
                average_quality=(interaction.emotional_impact + interaction.satisfaction_level + 
                               interaction.communication_quality) / 3,
    ...
    def _update_trust_metrics(self, interaction: Interaction):
        ...
        for participant in interaction.participants:
            ...
            other_participant = [p for p in interaction.participants if p != participant][0]
            
            # 신뢰도 계산 (의사소통 품질, 상호 이해도, 만족도 기반)
            trust_score = (interaction.communication_quality + interaction.mutual_understanding + 
                          interaction.satisfaction_level) / 3
    ...
    def _update_communication_patterns(self, interaction: Interaction):
        ...
        for participant in interaction.participants:
            ...
            if interaction_type not in self.communication_patterns[participant]['preferred_interaction_types']:
                self.communication_patterns[participant]['preferred_interaction_types'][interaction_type] = 0
            self.communication_patterns[participant]['preferred_interaction_types'][interaction_type] += 1
            
            # 의사소통 품질 트렌드 업데이트
            self.communication_patterns[participant]['communication_quality_trend'].append({
    ...
    def _update_support_network(self, interaction: Interaction):
        """지원 네트워크 업데이트"""
        if interaction.interaction_type in [InteractionType.SUPPORT, InteractionType.EMOTIONAL_SUPPORT, InteractionType.PRACTICAL_HELP]:
            for participant in interaction.participants:
                if participant not in self.support_network:
                    self.support_network[participant] = []
                
                other_participants = [p for p in interaction.participants if p != participant]
    ...
    def _update_role_adaptation(self, member_id: str, role_understanding: RoleUnderstanding):
        """역할 적응도 업데이트"""
        # 최근 상호작용을 기반으로 적응도 계산
        recent_interactions = [i for i in self.interactions 
                             if member_id in i.participants and 
                             i.timestamp >= datetime.now() - timedelta(days=30)]
        
        if recent_interactions:
            # 역할 수행 점수 계산
            role_performance = sum(i.satisfaction_level for i in recent_interactions) / len(recent_interactions)
            role_understanding.performance_score = role_performance
            
            # 적응도 업데이트 (성능과 만족도의 조합)
            adaptation_score = (role_performance + role_understanding.satisfaction_score) / 2
            role_understanding.adaptation_level = min(1.0, adaptation_score)
    
    def _generate_relationship_insights(self, member_id: str, relationships: Dict) -> List[str]:
        """관계 통찰력 생성"""
        insights = []
        
        # 강한 관계 식별
        strong_relationships = [other_id for other_id, data in relationships.items() 
                              if data['relationship_health'] > 0.8]
        if strong_relationships:
            insights.append(f"강한 관계를 가진 구성원: {', '.join(strong_relationships)}")
        
        # 개선이 필요한 관계 식별
        weak_relationships = [other_id for other_id, data in relationships.items() 
        ...
        if member_id in self.communication_patterns:
            preferred_types = self.communication_patterns[member_id]['preferred_interaction_types']
            if preferred_types:
    ...
    def _calculate_overall_relationship_health(self) -> float:
        ...
        total_health = sum(pattern.average_quality for pattern in self.relationship_patterns)
    ...
    def _analyze_communication_patterns(self) -> Dict:
        ...
        for member_id, patterns in self.communication_patterns.items():
            ...
            # 선호하는 상호작용 유형
            for interaction_type, count in patterns['preferred_interaction_types'].items():
    ...
    def _analyze_conflict_patterns(self) -> Dict:
        """갈등 해결 패턴 분석"""
        conflict_interactions = [i for i in self.interactions if i.interaction_type == InteractionType.CONFLICT]
        ...
        if conflict_interactions:
            ...
            resolved_conflicts = [i for i in conflict_interactions if i.mood_after and i.mood_before]
            if resolved_conflicts:
                successful_resolutions = [i for i in resolved_conflicts 
                                       if self._is_mood_improved(i.mood_before, i.mood_after)]
                analysis['resolution_success_rate'] = len(successful_resolutions) / len(resolved_conflicts)
    ...
    def _analyze_support_network(self) -> Dict:
        ...
        if self.support_network:
            total_connections = sum(len(connections) for connections in self.support_network.values())
            analysis['average_support_connections'] = total_connections / len(self.support_network)
            ...
            # 네트워크 밀도 계산
            total_possible_connections = len(self.support_network) * (len(self.support_network) - 1)
            if total_possible_connections > 0:
                analysis['support_network_density'] = total_connections / total_possible_connections
    ...
    def _analyze_role_effectiveness(self) -> Dict:
        ...
        if self.role_understandings:
            total_performance = sum(role.performance_score for role in self.role_understandings.values())
            total_satisfaction = sum(role.satisfaction_score for role in self.role_understandings.values())
            total_adaptation = sum(role.adaptation_level for role in self.role_understandings.values())
            
            analysis['average_performance'] = total_performance / len(self.role_understandings)
            analysis['average_satisfaction'] = total_satisfaction / len(self.role_understandings)
            analysis['average_adaptation'] = total_adaptation / len(self.role_understandings)
    ...
    def _analyze_relationship_evolution(self) -> Dict:
        ...
        improving_patterns = [p for p in self.relationship_patterns if p.emotional_trend == "improving"]
        declining_patterns = [p for p in self.relationship_patterns if p.emotional_trend == "declining"]
        stable_patterns = [p for p in self.relationship_patterns if p.emotional_trend == "stable"]
        ...
        evolution_data = {
            ...
            'average_trust': sum(p.trust_level for p in self.relationship_patterns) / total_patterns
        }
    ...
    def _generate_improvement_suggestion(self, member_id: str, other_id: str, relationship_data: Dict) -> Dict:
        """개선 제안 생성"""
        suggestion = {
    ...
    def export_relationship_data(self) -> Dict:
        ...
        return {
            ...
            'relationship_patterns': [asdict(pattern) for pattern in self.relationship_patterns],
            'role_understandings': {member_id: asdict(understanding) 
    ...
    def import_relationship_data(self, data: Dict):
        ...
        try:
            ...
            for pattern_data in data.get('relationship_patterns', []):
                pattern_data['participant_pair'] = tuple(pattern_data['participant_pair'])
                self.relationship_patterns.append(RelationshipPattern(**pattern_data))
            
            # 역할 이해 데이터 복원
            self.role_understandings = {}
            for member_id, understanding_data in data.get('role_understandings', {}).items():
                understanding_data['last_updated'] = datetime.fromisoformat(understanding_data['last_updated'])
                understanding_data['role'] = RoleExpectation(understanding_data['role'])
                self.role_understandings[member_id] = RoleUnderstanding(**understanding_data)
