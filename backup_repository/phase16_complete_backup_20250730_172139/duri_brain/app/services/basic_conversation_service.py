...
class BasicConversationSystem:
    ...
    def start_conversation(self, family_member_id: str, family_member_name: str, relationship: str, family_context: Dict[str, Any] = None) -> ConversationSession:
        """대화 세션 시작"""
        try:
    ...
    def process_message(self, session_id: str, speaker_id: str, speaker_name: str, message: str) -> ConversationResponse:
        """메시지 처리 및 응답 생성"""
        try:
            # 메시지 생성
            ...
            # 응답 생성
            response_text = self._generate_response(message, conversation_type, emotion_detected, speaker_name)
    ...
    def _analyze_conversation_type(self, message: str) -> ConversationType:
        ...
        # 감정 공유 패턴
        if any(word in message_lower for word in ['슬퍼', '화나', '기뻐', '걱정', '스트레스', '힘들어']):
            return ConversationType.EMOTIONAL_SUPPORT
        
        # 질문 패턴
        if any(word in message_lower for word in ['뭐', '어떻게', '왜', '언제', '어디', '누가']) and '?' in message:
            return ConversationType.QUESTION
        
        # 조언 요청 패턴
        if any(word in message_lower for word in ['조언', '도움', '어떻게 해야', '방법']):
            return ConversationType.ADVICE_REQUEST
        
        # 학습 패턴
        if any(word in message_lower for word in ['배우고 싶어', '알려줘', '가르쳐', '설명']):
    ...
    def _detect_emotion(self, message: str) -> EmotionType:
        ...
        # 기쁨 관련
        if any(word in message_lower for word in ['기뻐', '행복', '좋아', '재미', '웃겨', '즐거워']):
            return EmotionType.HAPPY
        
        # 슬픔 관련
        if any(word in message_lower for word in ['슬퍼', '우울', '속상', '힘들어', '지쳐']):
        ...
        # 혼란 관련
        if any(word in message_lower for word in ['모르겠', '헷갈려', '어려워', '복잡']):
    ...
    def _determine_response_style(self, conversation_type: ConversationType, emotion: EmotionType) -> ResponseStyle:
        """응답 스타일 결정"""
    ...
    def _generate_response(self, message: str, conversation_type: ConversationType, emotion: EmotionType, speaker_name: str) -> str:
        """응답 생성"""
        message_lower = message.lower()
    ...
    def _evaluate_emotion_appropriateness(self, response: str, emotion: EmotionType) -> bool:
        """감정 적절성 평가"""
        response_lower = response.lower()
        
        if emotion == EmotionType.SAD and any(word in response_lower for word in ['아프', '슬프', '위로', '힘들']):
            return True
        elif emotion == EmotionType.HAPPY and any(word in response_lower for word in ['기뻐', '좋아', '행복', '즐거워']):
            return True
        elif emotion == EmotionType.ANGRY and any(word in response_lower for word in ['화', '짜증', '이해', '함께']):
            return True
        elif emotion == EmotionType.WORRIED and any(word in response_lower for word in ['걱정', '불안', '함께', '도움']):
            return True
    ...
    def _evaluate_family_relevance(self, response: str) -> bool:
        """가족 관련성 평가"""
        family_keywords = ['가족', '부모', '자식', '사랑', '관계', '소통', '이해', '지지']
        response_lower = response.lower()
    ...
    def end_conversation_session(self, session_id: str) -> ConversationSession:
        ...
        try:
            ...
            if session:
                ...
                # 세션 품질 계산
                session_messages = [m for m in self.messages if m.id.startswith(session_id)]
                if session_messages:
                    avg_emotion_score = sum(self._emotion_to_score(m.emotion_detected) for m in session_messages) / len(session_messages)
                    session.session_quality = min(1.0, avg_emotion_score * 0.5 + len(session_messages) * 0.1)
    ...
    def get_conversation_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for conv_type in ConversationType:
                type_messages = [m for m in self.messages if m.conversation_type == conv_type]
                conversation_type_stats[conv_type.value] = len(type_messages)
            
            # 감정별 통계
            emotion_stats = {}
            for emotion in EmotionType:
                emotion_messages = [m for m in self.messages if m.emotion_detected == emotion]
                emotion_stats[emotion.value] = len(emotion_messages)
            
            # 응답 스타일별 통계
            response_style_stats = {}
            for style in ResponseStyle:
                style_responses = [r for r in self.responses if r.response_style == style]
                response_style_stats[style.value] = len(style_responses)
            
            # 평균 세션 품질
            avg_session_quality = sum(s.session_quality for s in self.conversation_sessions) / len(self.conversation_sessions) if self.conversation_sessions else 0
    ...
    def export_conversation_data(self) -> Dict[str, Any]:
        ...
        try:
            export_data = {
                'conversation_sessions': [asdict(session) for session in self.conversation_sessions],
...
def test_basic_conversation_system():
    ...
    # 2. 인사말 처리
    greeting_response = conversation_system.process_message(session.id, "member_1", "엄마", "안녕하세요!")
    print(f"✅ 인사말 응답: {greeting_response.response_style.value} 스타일, {greeting_response.confidence_score:.2f} 신뢰도")
    
    # 3. 감정 공유 처리
    emotional_response = conversation_system.process_message(session.id, "member_1", "엄마", "아이가 학교에서 친구와 다퉈서 속상해요.")
    print(f"✅ 감정 공유 응답: {emotional_response.response_style.value} 스타일, {emotional_response.emotion_appropriate} 감정 적절성")
    
    # 4. 질문 처리
    question_response = conversation_system.process_message(session.id, "member_1", "엄마", "아이와 어떻게 대화해야 할까요?")
    print(f"✅ 질문 응답: {question_response.response_style.value} 스타일, {question_response.family_relevant} 가족 관련성")
    
    # 5. 학습 요청 처리
    learning_response = conversation_system.process_message(session.id, "member_1", "엄마", "아이의 창의력을 키우는 방법을 알려주세요.")
    print(f"✅ 학습 요청 응답: {learning_response.response_style.value} 스타일, {learning_response.confidence_score:.2f} 신뢰도")
    
    # 6. 장난스러운 대화 처리
    playful_response = conversation_system.process_message(session.id, "member_1", "엄마", "재미있는 이야기 해주세요!")
    print(f"✅ 장난스러운 대화 응답: {playful_response.response_style.value} 스타일, {playful_response.family_relevant} 가족 관련성")
    ...
    print(f"✅ 대화 통계: {statistics['total_sessions']}개 세션, {statistics['total_messages']}개 메시지")
    ...
    print(f"✅ 대화 데이터 내보내기: {len(export_data['conversation_sessions'])}개 세션, {len(export_data['messages'])}개 메시지")
