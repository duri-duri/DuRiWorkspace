...
class BasicLessonExtractor:
    ...
    def extract_lesson_from_experience(self, experience_data: Dict, 
                                     family_context: Dict = None) -> ExtractedLesson:
        """
        경험에서 교훈 추출
        """
        try:
            ...
            # 적용성 결정
            applicability = self._determine_lesson_applicability(experience_data, family_context)
            ...
            lesson = ExtractedLesson(
                ...
                generational_value=self._calculate_generational_value(experience_data, family_context),
                extraction_timestamp=datetime.now(),
                source_experiences=[experience_data.get('id', 'unknown')],
                tags=self._generate_lesson_tags(experience_data, category),
                next_generation_ready=self._is_next_generation_ready(experience_data, family_context)
            )
    ...
    def analyze_lesson_patterns(self) -> Dict:
        ...
        try:
            analysis = {
                ...
                'generational_ready_lessons': len([l for l in self.extracted_lessons if l.next_generation_ready]),
                'family_specific_lessons': len([l for l in self.extracted_lessons if l.applicability == LessonApplicability.FAMILY_SPECIFIC]),
                'universal_lessons': len([l for l in self.extracted_lessons if l.applicability == LessonApplicability.UNIVERSAL]),
                'average_confidence': sum(l.confidence_score for l in self.extracted_lessons) / len(self.extracted_lessons) if self.extracted_lessons else 0,
                'average_generational_value': sum(l.generational_value for l in self.extracted_lessons) / len(self.extracted_lessons) if self.extracted_lessons else 0
            }
    ...
    def generate_family_wisdom(self) -> WisdomCollection:
        ...
        try:
            # 가족 특화 교훈 수집
            family_lessons = [l for l in self.extracted_lessons if l.applicability == LessonApplicability.FAMILY_SPECIFIC]
            
            # 보편적 교훈 수집
            universal_lessons = [l for l in self.extracted_lessons if l.applicability == LessonApplicability.UNIVERSAL]
            
            # 세대 전달 준비된 교훈 수집
            generational_lessons = [l for l in self.extracted_lessons if l.next_generation_ready]
            ...
            wisdom = WisdomCollection(
                ...
                generational_advice=self._extract_generational_advice(generational_lessons),
    ...
    def prepare_next_generation_lessons(self) -> List[Dict]:
        ...
        """
        try:
            ready_lessons = [l for l in self.extracted_lessons if l.next_generation_ready]
    ...
    def get_lesson_insights(self) -> Dict:
        ...
        try:
            insights = {
                ...
                'next_generation_ready': len([l for l in self.extracted_lessons if l.next_generation_ready]),
    ...
    def _determine_lesson_complexity(self, experience_data: Dict) -> LessonComplexity:
        ...
        complexity_score = (learning_value * 0.4 + emotional_impact * 0.3 + 
    ...
    def _determine_lesson_applicability(self, experience_data: Dict, family_context: Dict) -> LessonApplicability:
        """교훈 적용성 결정"""
    ...
    def _extract_emotional_context(self, experience_data: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'emotional_direction': 'positive' if experience_data.get('emotional_impact', 0) > 0 else 'negative'
        }
    ...
    def _calculate_confidence_score(self, experience_data: Dict) -> float:
        ...
        return min(1.0, learning_value * 0.4 + emotional_clarity * 0.4 + description_quality * 0.2)
    
    def _calculate_generational_value(self, experience_data: Dict, family_context: Dict) -> float:
        """세대 가치 계산"""
    ...
    def _generate_lesson_tags(self, experience_data: Dict, category: LessonCategory) -> List[str]:
        """교훈 태그 생성"""
        tags = [
            category.value,
    ...
    def _is_next_generation_ready(self, experience_data: Dict, family_context: Dict) -> bool:
        """다음 세대 전달 준비 여부"""
        learning_value = experience_data.get('learning_value', 0)
        emotional_impact = abs(experience_data.get('emotional_impact', 0))
    ...
    def _update_lesson_patterns(self, lesson: ExtractedLesson):
        """교훈 패턴 업데이트"""
        # 카테고리별 패턴
        category_pattern = next((p for p in self.lesson_patterns if p.pattern_type == f"category_{lesson.category.value}"), None)
    ...
    def _analyze_maturity_progression(self) -> Dict:
        ...
        # 시간순 정렬
        sorted_lessons = sorted(self.extracted_lessons, key=lambda x: x.extraction_timestamp)
        
        # 성숙도 계산 (복잡도와 신뢰도 기반)
        early_lessons = sorted_lessons[:len(sorted_lessons)//2]
        late_lessons = sorted_lessons[len(sorted_lessons)//2:]
        
        early_maturity = sum(l.confidence_score for l in early_lessons) / len(early_lessons) if early_lessons else 0
        late_maturity = sum(l.confidence_score for l in late_lessons) / len(late_lessons) if late_lessons else 0
    ...
    def _extract_family_insights(self, family_lessons: List[ExtractedLesson]) -> List[str]:
        """가족 통찰력 추출"""
        insights = []
    ...
    def _extract_universal_truths(self, universal_lessons: List[ExtractedLesson]) -> List[str]:
        """보편적 진리 추출"""
        truths = []
    ...
    def _extract_generational_advice(self, generational_lessons: List[ExtractedLesson]) -> List[str]:
        """세대 조언 추출"""
        advice = []
    ...
    def _calculate_wisdom_maturity(self) -> float:
        ...
        avg_complexity = sum(complexity_scores[l.complexity] for l in self.extracted_lessons) / len(self.extracted_lessons)
    ...
    def _calculate_wisdom_completeness(self) -> float:
        ...
        complexity_levels_covered = len(set(l.complexity for l in self.extracted_lessons))
        
        category_completeness = min(categories_covered / 8, 1.0)  # 8개 카테고리
        complexity_completeness = min(complexity_levels_covered / 4, 1.0)  # 4개 복잡도 레벨
    ...
    def export_lesson_data(self) -> Dict:
        ...
        return {
            ...
            'wisdom_collections': [asdict(wisdom) for wisdom in self.wisdom_collections],
    ...
    def import_lesson_data(self, data: Dict):
        ...
        try:
            ...
            for lesson_data in data.get('extracted_lessons', []):
                ...
                lesson_data['applicability'] = LessonApplicability(lesson_data['applicability'])
            ...
            for wisdom_data in data.get('wisdom_collections', []):
                wisdom_data['collection_timestamp'] = datetime.fromisoformat(wisdom_data['collection_timestamp'])
