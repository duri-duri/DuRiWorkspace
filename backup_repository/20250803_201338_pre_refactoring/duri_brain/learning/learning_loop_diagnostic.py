...
class LearningLoopDiagnostic:
    ...
    def diagnose_learning_loop_activation(self) -> Dict[str, Any]:
        ...
        logger.info(f"🔍 === 학습 루프 활성화 진단 시작: {self.diagnostic_start_time} ===")
    ...
    def _check_initial_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            initial_state = {
                ...
                "learning_loop_running": learning_loop_manager.is_running if learning_loop_manager else False,
                "activator_activated": activator.is_activated if activator else False,
                "current_cycle_id": learning_loop_manager.current_cycle_id if learning_loop_manager else None,
                "timestamp": datetime.now().isoformat()
            }
            
            logger.info(f"✅ 초기 상태: {initial_state}")
            return {"step": "initial_state_check", "success": True, "data": initial_state}
    ...
    def _check_learning_loop_manager(self) -> Dict[str, Any]:
        ...
        try:
            ...
            manager_state = {
                ...
                "loop_thread_alive": learning_loop_manager.loop_thread.is_alive() if learning_loop_manager.loop_thread else False,
                "learning_config": learning_loop_manager.learning_config,
                "timestamp": datetime.now().isoformat()
            }
            
            logger.info(f"✅ 학습 루프 매니저 상태: {manager_state}")
            return {"step": "learning_loop_manager_check", "success": True, "data": manager_state}
            
        except Exception as e:
            logger.error(f"❌ 학습 루프 매니저 확인 실패: {e}")
            return {"step": "learning_loop_manager_check", "success": False, "error": str(e)}
    ...
    def _attempt_activation_with_timeout(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if thread.is_alive():
                ...
                result["timeout"] = True
                return {"step": "activation_attempt", "success": False, "timeout": True, "elapsed_time": timeout}
            
            if result["error"]:
                logger.error(f"❌ 활성화 실패: {result['error']}")
                return {"step": "activation_attempt", "success": False, "error": result["error"]}
            
            logger.info(f"✅ 활성화 성공: {result['activation_result']}")
            return {"step": "activation_attempt", "success": True, "data": result["activation_result"]}
    ...
    def _check_post_activation_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            post_activation_state = {
                ...
                "current_cycle_id": learning_loop_manager.current_cycle_id if learning_loop_manager.current_cycle else None,
                "loop_thread_alive": learning_loop_manager.loop_thread.is_alive() if learning_loop_manager.loop_thread else False,
                "scheduler_thread_alive": activator.scheduler_thread.is_alive() if activator.scheduler_thread else False,
                "timestamp": datetime.now().isoformat()
            }
            
            logger.info(f"✅ 활성화 후 상태: {post_activation_state}")
            return {"step": "post_activation_check", "success": True, "data": post_activation_state}
    ...
    def _analyze_blocking_points(self) -> list:
        ...
        try:
            ...
            return [{"type": "analysis_error", "issue": "분석 중 오류 발생", "details": str(e)}]
    ...
    def _get_final_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            final_status = {
                "diagnostic_duration": (datetime.now() - self.diagnostic_start_time).total_seconds(),
                "learning_loop_running": learning_loop_manager.is_running if learning_loop_manager else False,
                "activator_activated": activator.is_activated if activator else False,
                "current_cycle_id": learning_loop_manager.current_cycle_id if learning_loop_manager and learning_loop_manager.current_cycle else None,
                "timestamp": datetime.now().isoformat()
            }
    ...
    def _generate_recommendations(self, diagnostic_result: Dict[str, Any]) -> list:
        ...
        for step in diagnostic_result.get("steps", []):
            if step.get("timeout"):
                recommendations.append("🕐 활성화 타임아웃 발생: 타임아웃 시간을 늘리거나 블로킹 지점을 수정하세요")
            
            if not step.get("success"):
                recommendations.append(f"❌ {step.get('step')} 실패: {step.get('error', 'Unknown error')}")
        
        # 블로킹 포인트가 있는 경우
        blocking_points = diagnostic_result.get("blocking_points", [])
        if blocking_points:
            for point in blocking_points:
                recommendations.append(f"🔒 블로킹 포인트: {point['type']} - {point['issue']}")
        
        # 학습 루프가 실행되지 않는 경우
        final_status = diagnostic_result.get("final_status", {})
        if not final_status.get("learning_loop_running", False):
            recommendations.append("🔄 학습 루프가 실행되지 않음: 수동으로 활성화하거나 오류를 수정하세요")
