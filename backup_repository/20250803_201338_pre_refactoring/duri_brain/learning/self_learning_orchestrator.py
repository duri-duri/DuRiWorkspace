...
class AutoLearningScheduler:
    ...
    def _generate_improvement_proposals(self, learning_result: Dict[str, Any], 
                                      issues: List[str]) -> List[Any]:
        """개선 제안을 생성합니다."""
        from dataclasses import dataclass
    ...
    def _create_and_send_report(self, learning_result: Dict[str, Any], 
                               issues: List[str], 
                               proposals: List[Any]):
        """보고서를 생성하고 전송합니다."""
        try:
...
class PhaseTracker:
    ...
    def __init__(self):
        ...
        logger.info(f"진화 단계 추적기 초기화 완료 (현재 단계: {self.current_phase.value})")
...
class DuRiReporter:
    ...
    def create_report(self, learning_modules: List[LearningModule], 
                     issues: List[str], proposals: List[ImprovementProposal]) -> LearningReport:
        """학습 보고서를 생성합니다."""
        try:
            # 전체 진행률 계산
            total_progress = sum(module.progress for module in learning_modules) / len(learning_modules) if learning_modules else 0.0
...
class SelfLearningOrchestrator:
    ...
    def _initialize_learning_modules(self):
        """학습 모듈들을 초기화합니다."""
        modules = [
            LearningModule("SelfEvolutionAnalyzer", LearningStatus.IDLE, 0.0, datetime.now(), [], []),
            LearningModule("SmartLearningChecker", LearningStatus.IDLE, 0.0, datetime.now(), [], []),
            LearningModule("LearningLoopManager", LearningStatus.IDLE, 0.0, datetime.now(), [], []),
            LearningModule("FallbackHandler", LearningStatus.IDLE, 0.0, datetime.now(), [], []),
            LearningModule("PerformanceMonitor", LearningStatus.IDLE, 0.0, datetime.now(), [], [])
        ]
    ...
    def start_self_learning_system(self):
        ...
        try:
            ...
            # Phase 1.0 시작
            self.phase_tracker.advance_phase(LearningPhase.PHASE_1_0, "자율 학습 시스템 시작")
    ...
    def _generate_improvement_proposals(self, learning_result: Dict[str, Any], 
                                      issues: List[str]) -> List[ImprovementProposal]:
        """개선 제안을 생성합니다."""
        proposals = []
    ...
    def _create_and_send_report(self, learning_result: Dict[str, Any], 
        ...
        try:
            # 보고서 생성
            report = self.reporter.create_report(self.learning_modules, issues, proposals)
            ...
            if report:
                ...
                # 승인 대기 중인 제안이 있으면 알림
                if report.proposals_pending:
                    print(f"⏳ {len(report.proposals_pending)}개의 개선 제안이 아빠 승인을 기다리고 있습니다.")
