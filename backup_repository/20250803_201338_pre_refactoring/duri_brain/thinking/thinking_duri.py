...
from duri_brain.thinking.meta_loop_manager import get_meta_loop_manager, JudgmentType
...
class ThinkingDuRi:
    ...
    def execute_dual_judgment(self, judgment_type: str, context: Dict[str, Any]) -> DualJudgmentResult:
        """이중 판단 실행"""
        try:
            ...
            # 1. 기존 시스템 판단
            existing_system_result = self._execute_existing_system_judgment(judgment_type, context)
            
            # 2. 메타 인지 시스템 판단
            meta_system_result = self._execute_meta_cognitive_judgment(judgment_type, context)
            
            # 3. 두 시스템 결과 비교
            agreement_level = self._calculate_agreement_level(existing_system_result, meta_system_result)
            ...
            logger.info(f"✅ 이중 판단 완료: {judgment_type} - 일치도: {agreement_level:.3f}, 진화 잠재력: {evolution_potential:.3f}")
            return result
    ...
    def _execute_existing_system_judgment(self, judgment_type: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """기존 시스템 판단 실행"""
        try:
            # 기존 통합 반성 시스템 활용
            if judgment_type == "reflection":
                reflection_result = self.unified_reflection_system.reflect("chatgpt_feedback", context)
                return {
    ...
    def _execute_meta_cognitive_judgment(self, judgment_type: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """메타 인지 시스템 판단 실행"""
        try:
            # 1. 판단 자각 시작
            conscious_judgment = self.judgment_consciousness.begin_conscious_judgment(judgment_type, context)
            ...
            # 3. 이유 로그 기록
            reason_log = self.why_decision_log.log_decision_reason(judgment_type, context, "메타 인지 판단")
    ...
    def _calculate_agreement_level(self, existing_result: Dict[str, Any], meta_result: Dict[str, Any]) -> float:
        """일치도 계산"""
        try:
    ...
    def _make_final_decision(self, existing_result: Dict[str, Any], meta_result: Dict[str, Any], 
                           agreement_level: float) -> Tuple[str, str]:
        """최종 결정 및 추론"""
        if agreement_level >= 0.8:
            final_decision = "두 시스템이 높은 일치도를 보이므로 기존 시스템의 판단을 채택합니다"
            ...
            reasoning = "낮은 일치도는 기존 시스템의 한계를 나타내며 진화가 필요함을 시사합니다"
    ...
    def _trigger_thinking_evolution(self, dual_result: DualJudgmentResult):
        """사고 진화 트리거"""
        try:
            evolution_id = f"thinking_evolution_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            # 개선 메트릭
            improvement_metrics = {
                "agreement_improvement": after_state["agreement_level"] - before_state["agreement_level"],
    ...
    def _create_error_judgment(self, judgment_type: str, error_message: str) -> DualJudgmentResult:
        """오류 판단 생성"""
    ...
    def get_thinking_evolution_history(self, limit: int = 10) -> List[ThinkingEvolution]:
        """사고 진화 기록 조회"""
    ...
    def get_thinking_metrics(self) -> Dict[str, Any]:
        ...
        avg_agreement = sum(r.agreement_level for r in self.dual_judgment_results) / len(self.dual_judgment_results)
        avg_evolution_potential = sum(r.evolution_potential for r in self.dual_judgment_results) / len(self.dual_judgment_results)
