...
class ExecutionCentricFamilyInteractionSystem:
    ...
    def create_daily_question(self, question_text: str, target_member: FamilyMember,
                             question_type: str, expected_impact: str) -> DailyQuestion:
        """일일 질문 생성"""
    ...
    def record_family_response(self, question_id: str, response_content: str,
    ...
    def conduct_family_interaction(self, interaction_type: InteractionType,
                                  participants: List[FamilyMember], interaction_content: str,
                                  duRi_response: str, family_response: str,
                                  interaction_quality: InteractionQuality,
                                  feedback_type: FeedbackType) -> FamilyInteraction:
        """가족 상호작용 수행"""
    ...
    def run_mvp_test(self, test_type: str, test_description: str,
                     participants: List[FamilyMember], test_duration_minutes: int,
                     success_metrics: Dict[str, float], family_feedback: str,
                     improvement_suggestions: List[str]) -> MVPTestResult:
        """MVP 테스트 실행"""
        test_id = f"mvp_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        overall_success_rate = sum(success_metrics.values()) / len(success_metrics) if success_metrics else 0.0
    ...
    def collect_execution_feedback(self, feedback_type: FeedbackType,
                                  feedback_content: str, family_member: FamilyMember,
                                  interaction_id: str, improvement_suggestions: List[str]) -> ExecutionFeedback:
        """실행 피드백 수집"""
    ...
    def _calculate_feedback_confidence(self, feedback_type: FeedbackType,
                                     feedback_content: str,
                                     improvement_suggestions: List[str]) -> float:
        """피드백 신뢰도 계산"""
        base_confidence = 0.8
        ...
        confidence = base_confidence + type_adj + content_specificity * 0.1 + suggestion_specificity * 0.1
    ...
    def get_execution_statistics(self) -> Dict[str, Any]:
        ...
        # 응답률 계산
        responded_questions = sum(1 for q in self.daily_questions if q.response_received)
        response_rate = responded_questions / max(total_questions, 1)
        
        # 평균 피드백 점수
        avg_feedback_score = sum(q.feedback_score for q in self.daily_questions if q.feedback_score) / max(responded_questions, 1) if responded_questions > 0 else 0.0
        ...
        # 평균 성공률
        avg_success_rate = sum(t.overall_success_rate for t in self.mvp_test_results) / max(total_tests, 1)
    ...
    def export_execution_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'family_interactions': [asdict(interaction) for interaction in self.family_interactions],
            'mvp_test_results': [asdict(test) for test in self.mvp_test_results],
            'execution_feedbacks': [asdict(feedback) for feedback in self.execution_feedbacks],
...
def test_execution_centric_family_interaction_system():
    ...
    print(f"📊 통계: 질문 {stats['total_questions']}개, 상호작용 {stats['total_interactions']}개")
