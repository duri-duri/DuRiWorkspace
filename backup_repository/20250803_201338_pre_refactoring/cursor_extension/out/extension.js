...
const conversationCapture_1 = require("./conversationCapture");
const learningManager_1 = require("./learningManager");
const duriAPI_1 = require("./duriAPI");
function activate(context) {
    console.log('DuRi 통합 학습 확장 프로그램이 활성화되었습니다.');
    // DuRi API 클라이언트 초기화
    const duriAPI = new duriAPI_1.DuRiAPI();
    // 학습 매니저 초기화
    const learningManager = new learningManager_1.DuRiLearningManager(duriAPI);
    // 대화 캡처 시스템 초기화
    const conversationCapture = new conversationCapture_1.ConversationCapture(learningManager);
    // 명령어 등록
    ...
    // 컨텍스트에 명령어 추가
    context.subscriptions.push(startCaptureCommand, stopCaptureCommand, testUnifiedSystemCommand, getUnifiedStatisticsCommand, getUnifiedHistoryCommand);
}
