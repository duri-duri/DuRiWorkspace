...
async def test_unified_system():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(f"{base_url}/growth/status") as response:
                ...
                if data.get('status') == 'success':
                    ...
                    print(f"   ⚠️  과부하 상태: {'예' if bandwidth_status.get('overload_status', {}).get('is_overloaded', False) else '아니오'}")
                else:
        ...
        try:
            async with session.get(f"{base_url}/bandwidth/status") as response:
                ...
                if data.get('status') == 'success':
                    ...
                    print(f"   🔄 동시 처리 한계: {bandwidth_status.get('config', {}).get('max_concurrent_processing', 0)}")
                    ...
                    print(f"      - 강도 감소 권장: {'예' if recommendations.get('should_reduce_intensity', False) else '아니오'}")
                else:
        ...
        for i, stimulus in enumerate(newborn_stimuli, 1):
            try:
                async with session.post(f"{base_url}/growth/stimulus", json={
                    "user_input": stimulus,
                    "duri_response": ""
                }) as response:
                    data = await response.json()
        ...
        try:
            async with session.get(f"{base_url}/bandwidth/status") as response:
                ...
                if data.get('status') == 'success':
                    ...
                    print(f"      - 강도 감소 권장: {'⚠️' if recommendations.get('should_reduce_intensity', False) else '✅'}")
