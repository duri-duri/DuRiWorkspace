...
async def check_autonomous_status():
    ...
    try:
        ...
        try:
            ...
            # 활성 세션 확인
            active_learning = [s for s in unified_system.learning_sessions if s.status.value == "in_progress"]
            active_evolution = [s for s in unified_system.evolution_sessions if s.status.value == "in_progress"]
        ...
        try:
            if "unified_system" in locals() and (len(active_learning) > 0 or len(active_evolution) > 0):
