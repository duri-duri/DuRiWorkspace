...
def main():
    rows = load_rows()
    if not rows:
        # print("no league.jsonl yet"); return

    with OUT_CSV.open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["time","idx","valid_r2","test_r2","r2_gap","mse_ratio","nrmse_ratio","gate_pass","gate_reason"])
        for r in rows:
            m=r["metrics"]; g=m.get("gap_analysis",{})
            w.writerow([
    ...
    md.append("\n## 최근 10개")
    md.append("| time | idx | valid R² | test R² | r2_gap | mse_ratio | gate | reason |")
    md.append("|---|---:|---:|---:|---:|---:|:--:|:--|")
    for r in rows[-10:]:
        m=r["metrics"]; g=m.get("gap_analysis",{})
        md.append(f"| {r.get('time')} | {m['validation']['r2']:.4f} | {m['test']['r2']:.4f} | "
                  f"{g.get('r2_gap',float('nan')):.4f} | {g.get('mse_ratio',float('nan')):.3f} | "
                  f"{'PASS' if r.get('gate_pass') else 'FAIL'} | {r.get('gate_reason','')} |")
