...
python3 - <<'PY'
import yaml, os, sys, glob, time, json, re
...
# === Gate 2: Policy verify (whitelist/blacklist/size/token/time) ===
echo "[GATE] policy verification"
if ! bash "$SECURITY_CHECK" --policy "$POLICY" --plan "$PLAN_JSON"; then
  echo "[FAIL] policy verification"
  exit 3
fi
