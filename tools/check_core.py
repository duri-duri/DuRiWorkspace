import sys, json, yaml, argparse, collections, datetime
...
rels = fam["relations"]; assert len(rels) == 52

# parent_of graph
adj = collections.defaultdict(list); indeg = collections.Counter()
for r in rels:
    if r["type"] == "parent_of":
        adj[r["src"]].append(r["dst"]); indeg[r["dst"]] += 1
from collections import deque
deg = {u:0 for u in members}
for u, vs in adj.items():
    for v in vs: deg[v] += 1
q = deque([u for u,d in deg.items() if d==0]); seen=0
while q:
    u=q.popleft(); seen+=1
    for v in adj[u]:
        deg[v]-=1
...
for a,b in list(sib): assert (b,a) in sib
