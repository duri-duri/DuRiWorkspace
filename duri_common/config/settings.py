...
class DuRiSettings(BaseSettings):
    ...
    EVOLUTION_LOG_PATH: str = Field(
        default="/tmp/emotion_data/evolution_log.json", description="Evolution log file path"
    )
    ACTION_STATS_PATH: str = Field(
        default="/tmp/evolution_data/action_stats.json", description="Action stats file path"
    )
